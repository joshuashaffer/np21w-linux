...
void debugwriteseg(const OEMCHAR *fname, const descriptor_t *sd, UINT32 addr,
                   UINT32 size);
...
static const OEMCHAR *flagstr[16][2] = {{NULL, NULL},  // 0x8000
                                        {NULL, NULL},  // 0x4000
                                        {NULL, NULL},  // 0x2000
                                        {NULL, NULL},  // 0x1000
                                        {s_nv, s_ov},  // 0x0800
...
static const OEMCHAR str_register[] =
    OEMTEXT("EAX=%.8x  EBX=%.8x  ECX=%.8x  EDX=%.8x") OEMTEXT(CRLITERAL)
        OEMTEXT("ESP=%.8x  EBP=%.8x  ESI=%.8x  EDI=%.8x") OEMTEXT(CRLITERAL)
            OEMTEXT("DS=%.4x  ES=%.4x  SS=%.4x  CS=%.4x  ")
                OEMTEXT("EIP=%.8x  ") OEMTEXT(CRLITERAL);
static const OEMCHAR str_picstat[] = OEMTEXT("PIC0=%.2x:%.2x:%.2x")
    OEMTEXT(CRLITERAL) OEMTEXT("PIC1=%.2x:%.2x:%.2x") OEMTEXT(CRLITERAL)
...
const OEMCHAR *debugsub_regs(void) {
  ...
  OEMSPRINTF(work, str_register, CPU_EAX, CPU_EBX, CPU_ECX, CPU_EDX, CPU_ESP,
             CPU_EBP, CPU_ESI, CPU_EDI, CPU_DS, CPU_ES, CPU_SS, CPU_CS,
             CPU_EIP);
...
void debugsub_status(void) {
  ...
  if (fh != FILEH_INVALID) {
    ...
    OEMSPRINTF(work, str_picstat, pic.pi[0].imr, pic.pi[0].irr, pic.pi[0].isr,
  ...
  debugwriteseg(work, &CPU_STAT_SREG(CPU_SS_INDEX), CPU_ESP & 0xffff0000,
                0x10000);
  filenum++;
}
