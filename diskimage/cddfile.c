...
BOOL isCDImage(const OEMCHAR *fname) {
  ...
  if ((!file_cmpname(ext, str_cue)) || (!file_cmpname(ext, str_ccd)) ||
      (!file_cmpname(ext, str_cdm)) || (!file_cmpname(ext, str_mds)) ||
      (!file_cmpname(ext, str_nrg)) || (!file_cmpname(ext, str_iso))) {
    return TRUE;
  }
  return FALSE;
}
...
long issec(FILEH fh, _CDTRK *trk, UINT trks) {
  ...
  trk[trks - 1].end_sec =
      (UINT32)(trk[trks - 1].str_sec + (fsize / trk[trks - 1].sector_size));
...
REG8 sec2352_read_with_ecc(SXSIDEV sxsi, FILEPOS pos, UINT8 *buf, UINT size) {
  ...
  while (size) {
    ...
    if (calcCRC(bufdata, 2064) != LOADINTELDWORD(bufedc)) {
      // EDC Error
      // TODO: Check ECC
      // sxsi->cdflag_ecc = (sxsi->cdflag_ecc & ~CD_ECC_BITMASK) |
      // CD_ECC_RECOVERED; // ECC recovered
      sxsi->cdflag_ecc =
          (sxsi->cdflag_ecc & ~CD_ECC_BITMASK) | CD_ECC_ERROR; // ECC error
                                                               // return(0xd0);
    }
...
REG8 sec_read(SXSIDEV sxsi, FILEPOS pos, UINT8 *buf, UINT size) {
  ...
  while (size) {
    ...
    for (i = 0; i < cdinfo->trks; i++) {
      if (cdinfo->trk[i].str_sec <= (UINT32)pos &&
          (UINT32)pos <= cdinfo->trk[i].end_sec) {
