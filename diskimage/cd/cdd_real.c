...
REG8 sec2048_read_SPTI(SXSIDEV sxsi, FILEPOS pos, UINT8 *buf, UINT size) {
  ...
  if (1 || size != 2048) {
    ...
    if (sptd.info.DataTransferLength > sizeof(sptd.ucDataBuf))
      return 0xd0;
    ...
    // SetLastError(0);
    if (DeviceIoControl(
            fh, IOCTL_SCSI_PASS_THROUGH, &sptd,
            offsetof(struct sptdinfo, sense_buffer) + sizeof(sptd.sense_buffer),
            &sptd, offsetof(struct sptdinfo, ucDataBuf) + 2048 * size,
            &sptisize, FALSE)) {
...
BRESULT setsxsidev_SPTI(SXSIDEV sxsi, const OEMCHAR *path, const _CDTRK *trk,
                        UINT trks) {
...
UINT32 readcapacity_SPTI(FILEH fh) {
...

  {
    ...
    sptd.info.DataBufferOffset =
        offsetof(struct sptdreadcapacityinfo, ucDataBuf);
    // sptd.info.DataBuffer = ucDataBuf;
    sptd.info.SenseInfoLength = sizeof(sptd.sense_buffer);
    sptd.info.SenseInfoOffset =
        offsetof(struct sptdreadcapacityinfo, sense_buffer);
    sptd.info.TimeOutValue = 5;

    // memset(buf, 0, 2048 * size);
    // memset(ucDataBuf, 0, 16384);
    // SetLastError(0);
    if (DeviceIoControl(fh, IOCTL_SCSI_PASS_THROUGH, &sptd,
                        offsetof(struct sptdreadcapacityinfo, sense_buffer) +
                            sizeof(sptd.sense_buffer),
                        &sptd,
                        offsetof(struct sptdreadcapacityinfo, ucDataBuf) + 8,
                        &sptisize, FALSE)) {
      if (sptd.info.DataTransferLength != 0) {
...
BRESULT openrealcdd(SXSIDEV sxsi, const OEMCHAR *path) {
  ...
  DeviceIoControl(fh, IOCTL_CDROM_GET_DRIVE_GEOMETRY, NULL, 0, &dgCDROM,
                  sizeof(DISK_GEOMETRY), &dwNotUsed, NULL);
  ...
  totals = dgCDROM.SectorsPerTrack * dgCDROM.TracksPerCylinder *
  ...
  //	トラック情報を拾う
  DeviceIoControl(fh, IOCTL_CDROM_READ_TOC, NULL, 0, &tocCDROM,
                  sizeof(tocCDROM), &dwNotUsed, NULL);
...
//		}else{
//			trk[i].end_sector		=
//(msf2lba(LOADMOTOROLADWORD(lptocCDROM->Descriptors[i].Msf)) - 150 - 1);
//		}
//
//		trk[i].img_pregap_sec	= trk[i].pregap_sector;
//		trk[i].img_start_sec	= trk[i].start_sector;
