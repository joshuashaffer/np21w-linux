...
//	手抜きで拙いBE->LE変換マクロ
#define UINT8_FROM_BE(val) (val << 4) | (val >> 4)
#define UINT16_FROM_BE(val) (val << 8) | (val >> 8 & 0x00ff)
#define UINT32_FROM_BE(val)                                                    \
  (val << 24) | (val << 8 & 0x00ff0000) | (val >> 8 & 0x0000ff00) |            \
      (val >> 24 & 0x000000ff)
#define UINT64_FROM_BE(val)                                                    \
  (val << 56) | (val << 40 & 0x00ff000000000000) |                             \
...
static NRG_BlockIDs NRGBlockID[] = {
    {"CUEX", 0, 8},   {"CUES", 0, 8},   {"ETN2", 0, 32}, {"ETNF", 0, 20},
    {"DAOX", 22, 42}, {"DAOI", 22, 30}, {"CDTX", 0, 0},  {"SINF", 0, 0},
    {"MTYP", 0, 0},   {"END!", 0, 0},   {NULL, 0, 0}};
...
BRESULT opennrg(SXSIDEV sxsi, const OEMCHAR *fname) {
  ...
  if (!memcmp(sig, NRGBlockID[0].block_id, sizeof(sig)) ||
    ...
    for (i = 0; i < blk_size / sizeof(NRG_CUE_Block); i++) {
      ...
      if (file_read(fh, &NRG_CB, sizeof(NRG_CUE_Block)) !=
          sizeof(NRG_CUE_Block)) {
        goto opennrg_err1;
      }
  ...
  if (!memcmp(sig, NRGBlockID[4].block_id, sizeof(sig)) ||
    ...
    if (file_read(fh, &NRG_DH, sizeof(NRG_DAO_Header)) !=
    ...
    if (nrg_ver == 0) {
      for (i = 0; i < blk_size / sizeof(NRG_DAO_Block64); i++) {
        ...
        if (file_read(fh, &NRG_DB, sizeof(NRG_DAO_Block64)) !=
            sizeof(NRG_DAO_Block64)) {
          goto opennrg_err1;
        }
      ...
      for (i = 0; i < blk_size / sizeof(NRG_DAO_Block32); i++) {
        ...
        if (file_read(fh, &NRG_DB, sizeof(NRG_DAO_Block32)) !=
            sizeof(NRG_DAO_Block32)) {
          goto opennrg_err1;
        }
