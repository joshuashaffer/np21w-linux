  ...
  while (textfile_read(tfh, buf, NELEMENTS(buf)) == SUCCESS) {
    ...
    if ((argc >= 3) && (!milstr_cmp(argv[0], str_file))) { //	FILE
      ...
      if (index < NELEMENTS(trk)) {
        ...
        trk[index].pregap_sectors =
            curpregap + (trk[index].pos - trk[index].pos0);

        trk[index].img_pregap_sec =
            (trk[index].pos0 == 0) ? trk[index].pos : trk[index].pos0;
