...
static const UINT8 mds_sig[16] = {'M', 'E', 'D', 'I', 'A', ' ', 'D', 'E',
                                  'S', 'C', 'R', 'I', 'P', 'T', 'O', 'R'};
...
#define MDS_SUBCHAN_PW_INTERLEAVED                                             \
  0x08 //	96-byte PW subchannel, interleaved
...
typedef struct {
  UINT32 filename_offset;   /*	Start offset of image filename.
                             */
  UINT32 widechar_filename; /*	Seems to be set to 1 if widechar filename is
                               used	*/
  UINT32 __dummy1__;
  UINT32 __dummy2__;
...
BRESULT openmds(SXSIDEV sxsi, const OEMCHAR *fname) {
  ...
  for (i = 0; i < MDS_SB.num_all_blocks; i++) {
    ...
    //	MDS_TRACKMODE_MODE1
    //	のみ、認識対象
    if (MDS_TB.mode == MDS_TRACKMODE_AUDIO ||
        MDS_TB.mode == MDS_TRACKMODE_MODE1) {
  ...
  for (i = 0; i < index; i++) {
    if (ex_offset[i] != 0) {
      ...
      trk[i].end_offset =
          trk[i].start_offset + (trk[i].track_sectors * trk[i].sector_size);
    }
  }
