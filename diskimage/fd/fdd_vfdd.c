...
BRESULT fdd_set_vfdd(FDDFILE fdd, FDDFUNC fdd_fn, const OEMCHAR *fname,
  ...
  rsize =
      file_read(fh, &fdd->inf.vfdd.head, VFDD_HEADERSIZE); //	VFDDヘッダ読込
  file_close(fh);
  ...
  if (memcmp(fdd->inf.vfdd.head.verID, vfdd_verID_100, 8)) {
    if (memcmp(fdd->inf.vfdd.head.verID, vfdd_verID_101,
               8)) { //	1.01もとりあえず対象に(kaiF)
      return (FAILURE);
    }
  }
  ...
  for (i = 0; i < VFDD_TRKMAX * VFDD_SECMAX; i++) {
    if (sec_vfdd->C != 0xff) {
      fdd->inf.vfdd.ptr[(sec_vfdd->C << 1) + sec_vfdd->H][sec_vfdd->R - 1] =
...
BRESULT fdd_read_vfdd(FDDFILE fdd) {
  ...
  if (fdd->type == DISKTYPE_VFDD) {
    ...
    if (fdd->inf.vfdd.ptr[trk][sec] == 0xffffffff ||
        fdd->inf.vfdd.ptr[trk][sec] == 0x00000000) {
