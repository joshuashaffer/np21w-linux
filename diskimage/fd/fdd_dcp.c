...
static const __DCPINFO supportdcp[] = {
    {0x01,
     {162, 154, 8, 3, DISKTYPE_2HD, 0}}, //	01h	2HD- 8セクタ(1.25MB)
    {0x02,
     {162, 160, 15, 2, DISKTYPE_2HD, 0}}, //	02h	2HD-15セクタ(1.21MB)
    {0x03,
     {162, 160, 18, 2, DISKTYPE_2HD, 1}}, //	03h	2HQ-18セクタ(1.44MB)
    {0x04,
     {162, 160, 8, 2, DISKTYPE_2DD, 0}}, //	04h	2DD- 8セクタ( 640KB)
    {0x05,
     {162, 160, 9, 2, DISKTYPE_2DD, 0}}, //	05h	2DD- 9セクタ( 720KB)
    {0x08,
     {162, 154, 9, 3, DISKTYPE_2HD, 0}},        //	08h	2HD- 9セクタ(1.44MB)
  ...
  while (dcp < (supportdcp + NELEMENTS(supportdcp))) {
    if (fdd->inf.dcp.head.mediatype == dcp->mediatype) {
      ...
      for (i = 0; i < fdd->inf.xdf.tracks; i++) {
        if (fdd->inf.dcp.head.trackmap[i] == 0x01 ||
            fdd->inf.dcp.head.alltrackflg == 0x01) {
