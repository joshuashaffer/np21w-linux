...
static const UINT8 nfd_FileID_r0[15] = {'T', '9', '8', 'F', 'D', 'D', 'I', 'M',
                                        'A', 'G', 'E', '.', 'R', '0', 0x00};
static const UINT8 nfd_FileID_r1[15] = {'T', '9', '8', 'F', 'D', 'D', 'I', 'M',
                                        'A', 'G', 'E', '.', 'R', '1', 0x00};
...
BRESULT fdd_set_nfd(FDDFILE fdd, FDDFUNC fdd_fn, const OEMCHAR *fname, int ro) {
  ...
  rsize = file_read(fh, &fdd->inf.nfd.head,
                    NFD_HEADERSIZE); //	nfdヘッダ読込(とりあえずr0で)
  file_close(fh);
  ...
  if (memcmp(fdd->inf.nfd.head.r0.szFileID, nfd_FileID_r0, 15) == 0) {
    ...
    for (i = 0; i < NFD_TRKMAX; i++) {
      for (j = 0; j < NFD_SECMAX; j++) {
        TRACEOUT(("NFD(r0) C[%02x]:H[%02x]:R[%02x]:N[%02x]", sec_nfd->C,
                  sec_nfd->H, sec_nfd->R, sec_nfd->N));
        if (sec_nfd->C != 0xff) {
          TRACEOUT(
              ("\tSetOffset Trk[%03d]Sec[%02x] = Offset[%08x]", i, j, ptr));
    ...
    fdd_fn->formatinit =
        fdd_formatinit_nfd; /* 170107 to support format command */
    ...
    for (i = 0; i < NFD_TRKMAX1; i++) {
      ...
      /* 170107 modified to work on Windows 9x/2000 ... to */
      if ((file_seek(fh, LOADINTELDWORD(&(fdd->inf.nfd.head.r1.dwTrackHead[i])),
                     FSEEK_SET) !=
           LOADINTELDWORD(&(fdd->inf.nfd.head.r1.dwTrackHead[i]))) ||
          (file_read(fh, &trk_id, sizeof(NFD_TRACK_ID1)) !=
           sizeof(NFD_TRACK_ID1))) {
        file_close(fh);
        return (FAILURE);
      }
      trksize = 0;
...
BRESULT fdd_seeksector_nfd(FDDFILE fdd) {
  ...
  if ((CTRL_FDMEDIA != fdd->inf.xdf.disktype) ||
    ...
    TRACEOUT(("fdd_seek_nfd FAILURE CTRL_FDMEDIA[%02x], DISKTYPE[%02x]",
              CTRL_FDMEDIA, fdd->inf.xdf.disktype));
    TRACEOUT(("fdd_seek_nfd FAILURE fdc.rpm[%02x], fdd->rpm[%02x]",
              fdc.rpm[fdc.us], fdd->inf.xdf.rpm));
    TRACEOUT(("fdd_seek_nfd FAILURE fdc.treg[%02x], fdd->trk[%02x]",
              fdc.treg[fdc.us], (fdd->inf.xdf.tracks >> 1)));
    fddlasterror = 0xe0;
    return (FAILURE);
  }
...
BRESULT fdd_read_nfd(FDDFILE fdd) {
  ...
  if (fdc.N != fdd->inf.nfd.head.r0.si[trk][secR].N) {
    TRACEOUT(("NFD(r0) read FAILURE N not match FDC[%02x],DSK[%02x]", fdc.N,
              fdd->inf.nfd.head.r0.si[trk][secR].N));
    fddlasterror = 0xc0;
    return (FAILURE);
  }
  ...
  fdc.stat[fdc.us] = fdd->inf.nfd.head.r0.si[trk][secR].byST0 +
                     (fdd->inf.nfd.head.r0.si[trk][secR].byST1 * 256) +
                     (fdd->inf.nfd.head.r0.si[trk][secR].byST2 * 256 * 256);
  fddlasterror = fdd->inf.nfd.head.r0.si[trk][secR].byStatus;
  TRACEOUT(
      ("NFD(r0) read FDC Result Status[%02x],STS0[%02x],STS1[%02x],STS2[%02x]",
  ...
  TRACEOUT(
      ("NFD(r0) read C:%02x,H:%02x,R:%02x,N:%02x", fdc.C, fdc.H, fdc.R, fdc.N));
  TRACEOUT(("NFD(r0) read dump"));
  TRACEOUT(("\t%02x %02x %02x %02x %02x %02x %02x %02x", fdc.buf[0x00],
            fdc.buf[0x01], fdc.buf[0x02], fdc.buf[0x03], fdc.buf[0x04],
            fdc.buf[0x05], fdc.buf[0x06], fdc.buf[0x07]));
  //
  return (SUCCESS);
}
...
BRESULT fdd_seeksector_nfd1(FDDFILE fdd) {
  ...
  if ((CTRL_FDMEDIA != fdd->inf.xdf.disktype) ||
      (fdc.rpm[fdc.us] != fdd->inf.xdf.rpm) ||
      (fdc.treg[fdc.us] >= (fdd->inf.xdf.tracks >> 1))) {
    TRACEOUT(("fdd_seek_nfd1 FAILURE CTRL_FDMEDIA[%02x], DISKTYPE[%02x]",
              CTRL_FDMEDIA, fdd->inf.xdf.disktype));
    TRACEOUT(("fdd_seek_nfd1 FAILURE fdc.rpm[%02x], fdd->rpm[%02x]",
              fdc.rpm[fdc.us], fdd->inf.xdf.rpm));
    TRACEOUT(("fdd_seek_nfd1 FAILURE fdc.treg[%02x], fdd->trk[%02x]",
              fdc.treg[fdc.us], (fdd->inf.xdf.tracks >> 1)));
    fddlasterror = 0xe0;
    return (FAILURE);
  }
  ...
  trk = (fdc.treg[fdc.us] << 1) + fdc.hd;
  file_seek(hdl, LOADINTELDWORD(&(fdd->inf.nfd.head.r1.dwTrackHead[trk])),
            FSEEK_SET);
...
BRESULT fdd_read_nfd1(FDDFILE fdd) {
  ...
  for (i = 0; i < LOADINTELWORD(&(trk_id.wSector)); i++) {
    ...
    TRACEOUT(("NFD(r1) read NOR trk[%03d]:        "
              "C[%02x]:H[%02x]:R[%02x]:N[%02x]:MFM[%02x]:DDAM[%02x]:Retry[%02x]"
              ":PDA[%02x]",
              trk, sec_idx.C, sec_idx.H, sec_idx.R, sec_idx.N, sec_idx.flMFM,
              sec_idx.flDDAM, sec_idx.byRetry, sec_idx.byPDA));
    if (sec_idx.R == fdc.R) {
      secR = i;
  ...
  if (fdc.N != sec_id.N) {
    BOOL flg;
    TRACEOUT(
        ("NFD(r1) read failure ... N not match : fdc.n:[%02x],sec_id.N:[%02x]",
         fdc.N, sec_id.N));
#if 1
    flg = FALSE;
    for (i = 0; i < LOADINTELWORD(&(trk_id.wDiag)); i++) {
      file_read(hdl, &dia_id, sizeof(NFD_DIAG_ID1));
      TRACEOUT(("NFD(r1) read DIA "
                "trk[%03d]:Cmd[%02x]:C[%02x]:H[%02x]:R[%02x]:N[%02x]:Len[%08x]:"
                "Retry[%02x]",
                trk, dia_id.Cmd, dia_id.C, dia_id.H, dia_id.R, dia_id.N,
                dia_id.dwDataLen, dia_id.byRetry));
      if (fdc.N == dia_id.N) {
  ...
  if (fdd->type == DISKTYPE_NFD) {
    if (fdc.N == sec_id.N) {
      ...
      seekp =
          fdd->inf.nfd.ptr[trk][sec] + fdd->inf.nfd.trksize[trk] + ex_offset2;
    }
  ...
  fdc.stat[fdc.us] =
      sec_id.bySTS0 | (sec_id.bySTS1 << 8) | (sec_id.bySTS2 << 16);
  fddlasterror = sec_id.byStatus;
  fdc.bufcnt = secsize;
  TRACEOUT(
      ("NFD(r1) read FDC Result Status[%02x],STS0[%02x],STS1[%02x],STS2[%02x]",
       sec_id.byStatus, sec_id.bySTS0, sec_id.bySTS1, sec_id.bySTS2));
  TRACEOUT(
      ("NFD(r1) read C:%02x,H:%02x,R:%02x,N:%02x", fdc.C, fdc.H, fdc.R, fdc.N));
