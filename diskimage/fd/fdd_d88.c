...
static D88SEC searchsector_d88(BOOL check) { // ver0.29
  ...
  for (sec = 0; sec < 40;) {
    ...
    if ((((D88SEC)p)->c == fdc.C) && (((D88SEC)p)->h == fdc.H) &&
        (((D88SEC)p)->r == fdc.R) && (((D88SEC)p)->n == fdc.N) &&
        (!rpmcheck((D88SEC)p))) {
...
BRESULT fdd_set_d88(FDDFILE fdd, FDDFUNC fdd_fn, const OEMCHAR *fname, int ro) {
  ...
  fdd->protect =
      ((attr & 1) || (fdd->inf.d88.head.protect & 0x10) || (ro)) ? TRUE : FALSE;
...
BRESULT fdd_seeksector_d88(FDDFILE fdd) {
  ...
  if (trkseek(fdd, (fdc.treg[fdc.us] << 1) + fdc.hd)) {
    TRACEOUT(("D88 seeksector FAILURE trkseek[%03d]",
              (fdc.treg[fdc.us] << 1) + fdc.hd));
    return (FAILURE);
  }
  if (!searchsector_d88(FALSE)) {
    TRACEOUT(("D88 seeksector FAILURE searchsector[%03d]",
              (fdc.treg[fdc.us] << 1) + fdc.hd));
    return (FAILURE);
  }
  return (SUCCESS);
...
BRESULT fdd_read_d88(FDDFILE fdd) {
  ...
  if (trkseek(fdd, (fdc.treg[fdc.us] << 1) + fdc.hd)) {
    TRACEOUT(
        ("D88 read FAILURE trkseek[%03d]", (fdc.treg[fdc.us] << 1) + fdc.hd));
    fddlasterror = 0xe0;
    return (FAILURE);
  }
  p = searchsector_d88(TRUE);
  if (!p) {
    TRACEOUT(("D88 read FAILURE searchsector_d88[%03d]",
              (fdc.treg[fdc.us] << 1) + fdc.hd));
    fddlasterror = 0xc0;
    return (FAILURE);
  }
  ...
  TRACEOUT(
      ("D88 read C:%02x,H:%02x,R:%02x,N:%02x", fdc.C, fdc.H, fdc.R, fdc.N));
...
static int fileappend(FILEH hdl, FDDFILE fdd, UINT32 ptr, long last,
                      long apsize) {
...
static void endoftrack(UINT fmtsize, UINT8 sectors) {
  ...
  lastpointer = (long)file_getsize(
      hdl); /*	lastpointer = file_seek(hdl, 0, FSEEK_END);	*/
...
BOOL fdd_isformating_d88(FDDFILE fdd) { return (formating); }
#else
...
BOOL fdd_isformating_d88(void) { return (formating); }
