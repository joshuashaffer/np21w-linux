...
BRESULT fdd_seeksector_common(FDDFILE fdd) {
  ...
  if ((CTRL_FDMEDIA != fdd->inf.xdf.disktype) ||
    ...
    TRACEOUT(("fdd_seek_common FAILURE CTRL_FDMEDIA[%02x], DISKTYPE[%02x]",
              CTRL_FDMEDIA, fdd->inf.xdf.disktype));
    TRACEOUT(("fdd_seek_common FAILURE fdc.rpm[%02x], fdd->rpm[%02x]",
              fdc.rpm[fdc.us], fdd->inf.xdf.rpm));
    TRACEOUT(("fdd_seek_common FAILURE fdc.treg[%02x], fdd->trk[%02x]",
              fdc.treg[fdc.us], (fdd->inf.xdf.tracks >> 1)));
    fddlasterror = 0xe0;
    return (FAILURE);
  }
  if ((!fdc.R) || (fdc.R > fdd->inf.xdf.sectors)) {
    TRACEOUT(("fdd_seek_common FAILURE fdc.R[%02x], Secters[%02x]", fdc.R,
              fdd->inf.xdf.sectors));
    fddlasterror = 0xc0;
    return (FAILURE);
  }
...
void fddfile_initialize(void) { ZeroMemory(fddfile, sizeof(fddfile)); }
