...
static const UINT8 bytemask_u0[64] = // dir:right by startbit + (len-1)*8
    {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01, 0xc0, 0x60, 0x30,
     0x18, 0x0c, 0x06, 0x03, 0x01, 0xe0, 0x70, 0x38, 0x1c, 0x0e, 0x07,
     0x03, 0x01, 0xf0, 0x78, 0x3c, 0x1e, 0x0f, 0x07, 0x03, 0x01, 0xf8,
     0x7c, 0x3e, 0x1f, 0x0f, 0x07, 0x03, 0x01, 0xfc, 0x7e, 0x3f, 0x1f,
     0x0f, 0x07, 0x03, 0x01, 0xfe, 0x7f, 0x3f, 0x1f, 0x0f, 0x07, 0x03,
     0x01, 0xff, 0x7f, 0x3f, 0x1f, 0x0f, 0x07, 0x03, 0x01};
...
static const UINT8 bytemask_d0[64] = // dir:left by startbit + (len-1)*8
    {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x03, 0x06, 0x0c,
     0x18, 0x30, 0x60, 0xc0, 0x80, 0x07, 0x0e, 0x1c, 0x38, 0x70, 0xe0,
     0xc0, 0x80, 0x0f, 0x1e, 0x3c, 0x78, 0xf0, 0xe0, 0xc0, 0x80, 0x1f,
     0x3e, 0x7c, 0xf8, 0xf0, 0xe0, 0xc0, 0x80, 0x3f, 0x7e, 0xfc, 0xf8,
     0xf0, 0xe0, 0xc0, 0x80, 0x7f, 0xfe, 0xfc, 0xf8, 0xf0, 0xe0, 0xc0,
     0x80, 0xff, 0xfe, 0xfc, 0xf8, 0xf0, 0xe0, 0xc0, 0x80};
...
static void MEMCALL egcsftb_upn_sub(UINT ext) {
  ...
  if (egc.dstbit) {
    if ((egc.dstbit + egc.remain) >= 8) {
      ...
      egc.srcmask._b[ext] = bytemask_u0[egc.dstbit + (egc.remain - 1) * 8];
      egc.remain = 0;
      egc.dstbit = 0;
    }
  } else {
...
static void MEMCALL egcsftb_dnn_sub(UINT ext) {
  ...
  if (egc.dstbit) {
    if ((egc.dstbit + egc.remain) >= 8) {
      ...
      egc.srcmask._b[ext] = bytemask_d0[egc.dstbit + (egc.remain - 1) * 8];
      egc.remain = 0;
      egc.dstbit = 0;
    }
  } else {
...
static void MEMCALL egcsftb_upr_sub(UINT ext) {
  ...
  if (egc.dstbit) {
    ...
    egc_src._b[0][ext] =
        (egc.outptr[0] << egc.sft8bitl) | (egc.outptr[1] >> egc.sft8bitr);
    egc_src._b[1][ext] =
        (egc.outptr[4] << egc.sft8bitl) | (egc.outptr[5] >> egc.sft8bitr);
    egc_src._b[2][ext] =
        (egc.outptr[8] << egc.sft8bitl) | (egc.outptr[9] >> egc.sft8bitr);
    egc_src._b[3][ext] =
        (egc.outptr[12] << egc.sft8bitl) | (egc.outptr[13] >> egc.sft8bitr);
    egc.outptr++;
  }
}
...
static void MEMCALL egcsftb_dnr_sub(UINT ext) {
  ...
  if (egc.dstbit) {
    ...
    egc.outptr--;
    egc_src._b[0][ext] =
        (egc.outptr[1] >> egc.sft8bitl) | (egc.outptr[0] << egc.sft8bitr);
    egc_src._b[1][ext] =
        (egc.outptr[5] >> egc.sft8bitl) | (egc.outptr[4] << egc.sft8bitr);
    egc_src._b[2][ext] =
        (egc.outptr[9] >> egc.sft8bitl) | (egc.outptr[8] << egc.sft8bitr);
    egc_src._b[3][ext] =
        (egc.outptr[13] >> egc.sft8bitl) | (egc.outptr[12] << egc.sft8bitr);
  }
}
...
static const EGCSFTB egcsftb[6] = {egcsftb_upn0, egcsftb_dnn0, egcsftb_upr0,
                                   egcsftb_dnr0, egcsftb_upl0, egcsftb_dnl0};

static const EGCSFTW egcsftw[6] = {egcsftw_upn0, egcsftw_dnn0, egcsftw_upr0,
                                   egcsftw_dnr0, egcsftw_upl0, egcsftw_dnl0};
...
static const UINT8 data_00[8] = {0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00};
static const UINT8 data_ff[8] = {0xff, 0xff, 0xff, 0xff,
                                 0xff, 0xff, 0xff, 0xff};
...
static const OPEFN opefn[256] = {
    ...
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_nd, ope_xx,
    ...
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx,
    ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_xx, ope_f0, ope_xx, ope_xx,
...
REG16 MEMCALL egc_readword(UINT32 addr) {
  ...
  if (!(egc.ope & 0x2000)) {
    ...
    temp_2 = 0;
    if (ad > 3)
      temp_2 = mem[ad - 3 + planead[pl]];
    if (ad > 2)
      temp_1 = mem[ad - 2 + planead[pl]];
    if (ad > 1)
      temp0 = mem[ad - 1 + planead[pl]];
  ...
  }
  {
    ...
    fg1 = fg2 = fg4 = fg8 = 0;
    if (!(egc.access & 1))
      fg1 = (egc.fg & 1) | (egc.fg & 1) << 1 | (egc.fg & 1) << 2 |
            (egc.fg & 1) << 3 | (egc.fg & 1) << 4 | (egc.fg & 1) << 5 |
            (egc.fg & 1) << 6 | (egc.fg & 1) << 7;
    if (!(egc.access & 1))
      fg2 = (egc.fg & 2) | (egc.fg & 2) << 1 | (egc.fg & 2) << 2 |
            (egc.fg & 2) << 3 | (egc.fg & 2) << 4 | (egc.fg & 2) << 5 |
            (egc.fg & 2) << 6 | (egc.fg & 2) << 7;
    if (!(egc.access & 1))
      fg4 = (egc.fg & 4) | (egc.fg & 4) << 1 | (egc.fg & 4) << 2 |
            (egc.fg & 4) << 3 | (egc.fg & 4) << 4 | (egc.fg & 4) << 5 |
            (egc.fg & 4) << 6 | (egc.fg & 4) << 7;
    if (!(egc.access & 1))
      fg8 = (egc.fg & 8) | (egc.fg & 8) << 1 | (egc.fg & 8) << 2 |
            (egc.fg & 8) << 3 | (egc.fg & 8) << 4 | (egc.fg & 8) << 5 |
            (egc.fg & 8) << 6 | (egc.fg & 8) << 7;
