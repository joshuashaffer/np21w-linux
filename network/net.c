...
#define NET_BUFLEN                                                             \
  (10 *                                                                        \
   1024) // バッファ1つの長さ（XXX:
         // パケットサイズの最大値にしないと無駄。もっと言えば可変長で大きな1つのバッファに入れるべき？）
#define NET_ARYLEN (128) // バッファの数

NP2NET np2net;

static TCHAR *GetNetWorkDeviceGuid(CONST TCHAR *, TCHAR *,
                                   DWORD); // TAPデバイス名からGUIDを取得する
...
#define ETHERDATALEN_MIN 46
  ...
  if (!WriteFile(hTap, pSendBuf, len, &dwWriteLen, &np2net_write_ovl)) {
    ...
    if (err == ERROR_IO_PENDING) {
      if (WaitForSingleObject(np2net_write_hEvent, 3000) !=
          WAIT_TIMEOUT) { // 完了待ち
...
static void np2net_updateHighSpeedMode() {
  ...
  int timediff;
  if (np2net_pmm &&
      np2net_membuf_readpos != (np2net_membuf_writepos + 1) % NET_ARYLEN) {
  ...
  while (!np2net_hThreadexit) {
    if (np2net.recieve_packet != np2net_default_recieve_packet) {
      if (np2net_membuf_readpos != np2net_membuf_writepos) {
        doWriteTap(np2net_hTap, (UCHAR *)(np2net_membuf[np2net_membuf_readpos]),
                   np2net_membuflen[np2net_membuf_readpos]);
      ...
      if (err == ERROR_IO_PENDING) {
        ...
        if (WaitForSingleObject(hEvent, 3000) != WAIT_TIMEOUT) { // 受信完了待ち
          ...
          if (dwLen > 0) {
            ...
            np2net.recieve_packet((UINT8 *)np2net_Buf,
                                  dwLen); // 受信できたので通知する
...
static int np2net_openTAP(const TCHAR *tapname) {
  ...
  // TAP デバイスを開く
  np2net_hTap = CreateFile(szDevicePath, GENERIC_READ | GENERIC_WRITE, 0, 0,
                           OPEN_EXISTING,
  ...
  // TAP デバイスをアクティブに
  status = TRUE;
  if (!DeviceIoControl(np2net_hTap, TAP_IOCTL_SET_MEDIA_STATUS, &status,
                       sizeof(status), &status, sizeof(status), &dwLen, NULL)) {
  ...
  np2net_hThreadR =
      (HANDLE)_beginthreadex(NULL, 0, np2net_ThreadFuncR, NULL, 0, &dwID);
  np2net_hThreadW =
      (HANDLE)_beginthreadex(NULL, 0, np2net_ThreadFuncW, NULL, 0, &dwID);
...
void np2net_init(void) {
  ...
  np2net_write_ovl.hEvent = np2net_write_hEvent =
...
static TCHAR *GetNetWorkDeviceGuid(CONST TCHAR *pDisplayName, TCHAR *pszBuf,
                                   DWORD cbBuf) {
...
#define BUFSZ 256
  ...
  // が ネットワークデバイス名（ユニーク）の格納されたエントリであり、 {id2}
  // がこのデバイスの GUID である
  ...
  while (bDone != TRUE) { // {id1} を列挙するループ
    ...
    dwSize = BUFSZ;
    nResult = RegEnumKeyEx(hKey1, dwIdx1++, pKeyName1, &dwSize, NULL, NULL,
                           NULL, &ft);
    ...
    nResult = RegOpenKeyEx(HKEY_LOCAL_MACHINE, pKeyName2, 0, KEY_READ, &hKey2);
    if (nResult != ERROR_SUCCESS) {
      continue;
    }
    dwIdx2 = 0;
    while (1) { // {id2} を列挙するループ
      dwSize = BUFSZ;
      nResult = RegEnumKeyEx(hKey2, dwIdx2++, pKeyName3, &dwSize, NULL, NULL,
                             NULL, &ft);
      if (nResult == ERROR_NO_MORE_ITEMS) {
        break;
      }
      ...
      _stprintf(pKeyName4, _T("%s\\%s\\%s"), pKeyName2, pKeyName3,
                _T("Connection"));
      nResult =
          RegOpenKeyEx(HKEY_LOCAL_MACHINE, pKeyName4, 0, KEY_READ, &hKey3);
      if (nResult != ERROR_SUCCESS) {
        continue;
      }
      ...
      nResult = RegQueryValueEx(hKey3, _T("Name"), 0, &dwType, (LPBYTE)szData,
                                &dwSize);
