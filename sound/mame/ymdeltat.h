...
typedef struct deltat_adpcm_state { /* AT: rearranged and tigntened structure */
  ...
  UINT8 control2;  /* control reg: SAMPLE, DA/AD, RAM TYPE (x8bit / x1bit),
                      ROM/RAM */
  ...
  UINT8 status_change_EOS_bit; /* 1 on End Of Sample (record/playback/cycle time
                                  of AD/DA converting has passed)*/
  UINT8 status_change_BRDY_bit; /* 1 after recording 2 datas (2x4bits) or after
                                   reading/writing 1 data */
  UINT8 status_change_ZERO_bit; /* 1 if silence lasts for more than 290
                                   miliseconds on ADPCM recording */
