...
const INT32 ym_deltat_decode_tableB2[16] = {57, 57, 57, 57, 77, 102, 128, 153,
...
void YM_DELTAT_ADPCM_Write(YM_DELTAT *DELTAT, int r, int v) {
  ...
  case 0x00:
    ...
    */
    ...
    {
      DELTAT->now_addr = DELTAT->start << 1;
      DELTAT->memread = 2; /* two dummy reads needed before accesing external
                              memory via register $08*/
...
*/
  ...
  case 0x0a: /* DELTA-N H */
    ...
    DELTAT->step =
        (UINT32)((double)(DELTAT->delta /* *(1<<(YM_DELTAT_SHIFT-16)) */) *
                 (DELTAT->freqbase));
...
void YM_DELTAT_ADPCM_Reset(YM_DELTAT *DELTAT, int pan, int emulation_mode) {
  ...
  DELTAT->start = 0;
  DELTAT->end = 0;
  DELTAT->limit = ~0; /* this way YM2610 and Y8950 (both of which don't have
                         limit address reg) will still work */
  DELTAT->volume = 0;
  ...
  DELTAT->portstate =
      (emulation_mode == YM_DELTAT_EMULATION_MODE_YM2610) ? 0x20 : 0;
  DELTAT->control2 = (emulation_mode == YM_DELTAT_EMULATION_MODE_YM2610)
                         ? 0x01
                         : 0; /* default setting depends on the emulation mode.
                                 MSX demo called "facdemo_4" doesn't setup
                                 control2 register at all and still works */
...
#ifdef __STATE_H__
  state_save_register_UINT8(statename, num, "DeltaT.portstate",
                            &DELTAT->portstate, 1);
  state_save_register_UINT32(statename, num, "DeltaT.address",
                             &DELTAT->now_addr, 1);
  state_save_register_UINT32(statename, num, "DeltaT.step", &DELTAT->now_step,
                             1);
  state_save_register_INT32(statename, num, "DeltaT.acc", &DELTAT->acc, 1);
  state_save_register_INT32(statename, num, "DeltaT.prev_acc",
                            &DELTAT->prev_acc, 1);
  state_save_register_INT32(statename, num, "DeltaT.adpcmd", &DELTAT->adpcmd,
                            1);
  state_save_register_INT32(statename, num, "DeltaT.adpcml", &DELTAT->adpcml,
                            1);
#endif
}
...
INLINE static void YM_DELTAT_synthesis_from_external_memory(YM_DELTAT *DELTAT) {
  UINT32 step;
  int data;
...
INLINE static void YM_DELTAT_synthesis_from_CPU_memory(YM_DELTAT *DELTAT) {
  UINT32 step;
  int data;
