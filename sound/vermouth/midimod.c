...
enum { CFG_DIR = 0, CFG_SOURCE, CFG_DEFAULT, CFG_BANK, CFG_DRUM };
...
static const OEMCHAR *cfgstr[] = {str_dir, str_source, str_default, str_bank,
                                  str_drumset};
...
BRESULT VERMOUTHCL midimod_getfile(MIDIMOD mod, const OEMCHAR *filename,
  ...
  if ((filename == NULL) || (filename[0] == '\0') || (path == NULL) ||
      (size == 0)) {
    goto fpgf_exit;
  }
  p = mod->pathlist;
...
void VERMOUTHCL midimod_lock(MIDIMOD mod) { mod->lockcount++; }
