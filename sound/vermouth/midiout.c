...
static const int gaintbl[24 + 1] = {16,  19,  22,  26,  32,  38,  45,  53,  64,
                                    76,  90,  107, 128, 152, 181, 215, 256, 304,
                                    362, 430, 512, 608, 724, 861, 1024};
...
static void VERMOUTHCL key_pressure(MIDIHDL hdl, CHANNEL ch, int key, int vel) {
...
static void VERMOUTHCL ctrlchange(MIDIHDL hdl, CHANNEL ch, int ctrl, int val) {
...
static void VERMOUTHCL pitchbendor(MIDIHDL hdl, CHANNEL ch, int val1,
  ...
  if (1) {
    ...
    if (!val1) {
      ...
      val1 *= ch->pitchsens;
      ch->pitchfactor =
          bendhtbl[(val1 >> (6 + 7)) + 24] * bendltbl[(val1 >> 7) & 0x3f];
    }
    v = VOICEHDLPTR(hdl);
    vterm = VOICEHDLEND(hdl);
    do {
