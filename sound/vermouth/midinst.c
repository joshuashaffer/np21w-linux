...
static SAMPLE VERMOUTHCL upsamp(SAMPLE dst, SAMPLE src, int count, int mrate) {
...
static INSTRUMENT VERMOUTHCL inst_create(MIDIMOD mod, const _TONECFG *cfg) {
  ...
  if ((_file_read(fh, &head, sizeof(head)) != sizeof(head)) &&
      ...
      (memcmp(head.id, sig_ID000002, 10)) && (head.instruments != 0) &&
      (head.instruments != 1) && (head.layers != 0) && (head.layers != 1)) {
    goto li_err2;
  }
  ...
  do {
    ...
    if (cfg->flag & TONECFG_NOLOOP) {
      wave.mode &=
          ~(MODE_SUSTAIN | MODE_LOOPING | MODE_PINGPONG | MODE_REVERSE);
    }
    if (cfg->flag & TONECFG_NOENV) {
      ...
      if (wave.mode & (MODE_LOOPING | MODE_PINGPONG | MODE_REVERSE)) {
        if ((!(wave.mode & MODE_SUSTAIN)) ||
            (!memcmp(wave.env, str_question6, 6)) || (wave.envpos[5] >= 100)) {
          wave.mode &= ~MODE_ENVELOPE;
        }
      } else {
    ...
    for (i = 0; i < 6; i++) {
      ...
      layer->envratetbl[i] =
          ((((wave.env[i] & 0x3f) << sft) * 44100 / mod->samprate) * ENV_RATE)
          << (3 + 1);
      layer->envpostbl[i] = wave.envpos[i] << (7 + 9);
    }
    if ((wave.tre_rate != 0) && (wave.tre_depth != 0)) {
      layer->tremolo_step =
          ((ENV_RATE * wave.tre_rate) << (SINENT_BIT + TRERATE_SHIFT)) /
    ...
    if ((wave.vib_rate != 0) && (wave.vib_depth != 0)) {
      layer->vibrate_rate =
          (VIBRATE_TUNE * mod->samprate) / (wave.vib_rate << VIBRATE_SHIFT);
      if (wave.vib_sweep) {
        layer->vibrate_sweep =
...
int VERMOUTHCL inst_bankloadex(MIDIMOD mod, UINT bank, FNMIDIOUTLAEXCB cb,
