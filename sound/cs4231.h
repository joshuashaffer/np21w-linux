...
typedef struct {
  UINT
      bufsize; // サウンド再生用の循環バッファサイズ。データのread/writeは4byte単位（16bitステレオの1サンプル単位）で行うこと
  UINT bufdatas; // = (bufwpos-bufpos)&CS4231_BUFMASK
  UINT
      bufpos; // バッファの読み取り位置。bufwposと一致してもよいが追い越してはいけない
  UINT
      bufwpos; // バッファの書き込み位置。周回遅れのbufposに追いついてはいけない（一致も不可）
  UINT32 pos12;
  UINT32 step12;
...
#define SDC                                                                    \
  (1 << 2) // bit2 Single DMA Channel 0 Dual 1 Single 逆と思ってたので修正すべし
#define CAL0                                                                   \
  (1 << 3) // bit3 Calibration 0 No Calibration 1 Converter calibration
...
// cs4231.reg.errorstatus(11)0xb  Error Status and Initialization (I11, Read
// Only)
...
#define PI                                                                     \
  (1 << 4) // bit4 Playback Interrupt pending from Playback DMA count registers
#define CI                                                                     \
  (1 << 5) // bit5 Capture Interrupt pending from record DMA count registers
           // when SDC=1 non-functional
#define TI (1 << 6) // bit6 Timer Interrupt pending from timer count registers
// PI,CI,TI bits are reset by writing a "0" to the particular interrupt bit or
// by writing any value to the Status register
