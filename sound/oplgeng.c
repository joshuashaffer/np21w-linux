...
static void calcratechannel(OPLGEN oplgen, OPLCH *ch) {
  ...
  if (envout >= 0) {
    if (ch->feedback) {
      ...
      opout = ch->op1fb;
      ch->op1fb =
          SLOTOUT(ch->slot[0].sintable, envout,
                  ch->slot[0].freq_cnt + ((ch->op1fb >> ch->feedback)
                                          << (FREQ_BITS - (TL_BITS - 2))));
  ...
  if (envout >= 0) {
    *ch->connect2 +=
        SLOTOUT(ch->slot[1].sintable, envout,
                ch->slot[1].freq_cnt +
                    (oplgen->feedback2 << (FREQ_BITS - (TL_BITS - 2))));
  }
}
...
static UINT calcraterhythm(OPLGEN oplgen) {
  ...
  if (ch7->playing & 2) {
    ...
    if (ch7->algorithm == 0) {
      ...
      if (envout >= 0) {
        if (ch7->feedback) {
          ...
          opout = ch7->op1fb;
          ch7->op1fb =
              SLOTOUT(ch7->slot[0].sintable, envout,
                      ch7->slot[0].freq_cnt + ((ch7->op1fb >> ch7->feedback)
                                               << (FREQ_BITS - (TL_BITS - 2))));
    ...
    if (envout >= 0) {
      *ch7->connect2 += SLOTOUT(ch7->slot[1].sintable, envout,
                                ch7->slot[1].freq_cnt +
                                    (opout << (FREQ_BITS - (TL_BITS - 2))));
    }
    playing++;
  }
  ...
  on |= (ch9->slot[1].freq_cnt ^ (ch9->slot[1].freq_cnt << 2)) &
        (1 << (FREQ_BITS - 5));
  ...
  if (ch8->playing & 2) {
    ...
    if (envout >= 0) {
      freq = (ch8->slot[0].freq_cnt & (1 << (FREQ_BITS - 2))) ? DEG2FREQ(180)
                                                              : DEG2FREQ(90);
      if (oplgen->noise & 1) {
        freq += DEG2FREQ(90);
      }
  ...
  if (ch9->playing & 1) {
    ...
    if (envout >= 0) {
      *ch9->connect2 +=
          SLOTOUT(ch9->slot[0].sintable, envout, ch9->slot[0].freq_cnt);
    }
    playing++;
  }
