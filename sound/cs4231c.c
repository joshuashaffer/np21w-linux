...
static const UINT8 cs4231cnt64[8] = {3072 / 64,  //  8000/ 5510
                                     1536 / 64,  // 16000/11025
                                     896 / 64,   // 27420/18900
                                     768 / 64,   // 32000/22050
                                     448 / 64,   // 54840/37800
...
void cs4231_initialize(UINT rate) { cs4231cfg.rate = rate; }
...
// 廃止：cs4231g.cで調整
void cs4231_setvol(UINT vol) { (void)vol; }
  ...
  if (item->flag & NEVENT_SETEVENT) {
    if (cs4231.dmach != 0xff) {
      ...
      if (!w31play || !(cs4231.reg.featurestatus & (PI | TI | CI))) {
        if (cs4231.bufsize * cs4231_playcountshift[cs4231.reg.datafmt >> 4] /
                    4 -
                4 >
            cs4231.bufdatas) {
          rem = min(
              cs4231.bufsize - 4 - cs4231.bufdatas,
              CS4231_MAXDMAREADBYTES); //読み取り単位は16bitステレオの1サンプル分(4byte)にしておかないと雑音化する
          ...
          cs4231.bufwpos = (cs4231.bufwpos + r) &
                           CS4231_BUFMASK; // バッファ書き込み位置を更新
          cs4231.bufdatas += r; // バッファ内の有効なデータ数を更新 =
                                // (bufwpos-bufpos)&CS4231_BUFMASK
        }
      }
      // NEVENTをセット
      if (cs4231cfg.rate) {
        ...
        playcountsmp_Ictl += ((CS4231_BUFREADSMP - (int)r) /
                              cs4231_playcountshift[cs4231.reg.datafmt >> 4]) /
                             2;
        if (playcountsmp_Ictl < 1)
          playcountsmp_Ictl = 1;
        if (playcountsmp_Ictl > CS4231_MAXDMAREADBYTES)
          playcountsmp_Ictl = CS4231_MAXDMAREADBYTES;
        // int playcountsmp = min(playcountsmpmax, r /
        // cs4231_playcountshift[cs4231.reg.datafmt >> 4])-4; if(playcountsmp <
        // CS4231_MINDMAREADBYTES) 	playcountsmp = CS4231_MINDMAREADBYTES*2;

        // playcountsmp = min(max(r, CS4231_MAXDMAREADBYTES/4) /
        // cs4231_playcountshift[cs4231.reg.datafmt >> 4], playcountsmp) / 2;
        ...
        // cs4231cfg.rate / 10); nevent_setbyms(NEVENT_CS4231, neventms,
        // cs4231_dma, NEVENT_RELATIVE);
        nevent_set(NEVENT_CS4231,
                   pccore.realclock / cs4231cfg.rate * playcountsmp_Ictl,
                   cs4231_dma, NEVENT_RELATIVE);
      }
...
REG8 DMACCALL cs4231dmafunc(REG8 func) {
  ...
  case DMAEXT_START:
    if (cs4231cfg.rate) {
      int playcount =
          (cs4231.reg.playcount[1] | (cs4231.reg.playcount[0] << 8)) *
          cs4231_playcountshift[cs4231.reg.datafmt >>
                                4]; // PI割り込みを発生させるサンプル数(Playback
                                    // Base register) * サンプルあたりのバイト数
      // DMA読み取り数カウンタを初期化
      cs4231.totalsample = 0;
      ...
      // nevent_setbyms(NEVENT_CS4231, CS4231_MAXDMAREADBYTES * 1000 /
      // cs4231cfg.rate, cs4231_dma, NEVENT_ABSOLUTE); cnt = pccore.realclock /
      // cs4231cfg.rate * 512; nevent_set(NEVENT_CS4231, cnt, cs4231_dma,
      // NEVENT_ABSOLUTE);
...
void cs4231_control(UINT idx, REG8 dat) {
  ...
  case CS4231REG_IRQSTAT:
    // バッファオーバーラン・アンダーランや割り込みの状態を表すレジスタ　Alternate
    // Feature Status (I24) 0をセットしたビットだけ消す（1の場合はそのまま）
  ...
  while (size > 0) {
    ...
    if (leng) {
      ...
      if (cs4231.totalsample + (SINT32)leng > playcount) {
        // DMA再生サンプル数カウンタ(Playback DMA count
        // register)がPI割り込みを発生させるサンプル数(Playback Base
        // register)を超えないように調整
      ...
      if (!(dmach->mode & 0x20)) { // dir +
        ...
        for (i = 0; i < leng; i++) {
          ...
          if (addr > dmach->lastaddr) {
            addr =
                dmach
                    ->startaddr; // DMA読み取りアドレスがアドレス範囲の最後に到達したら最初に戻す
          }
          offset =
              (offset + 1) &
              CS4231_BUFMASK; // DMAデータ読み取りバッファの書き込み位置を進める（＆最後に到達したら最初に戻す）
        }

        // XXX:
        // 再生位置調整（Win9x,Win2000再生ノイズ対策用・とりあえず+方向だけ）
            ...
            if (!w31play) {
              ...
              if (addr > dmach->lastaddr) {
                addr =
                    dmach->startaddr +
                    (addr - dmach->lastaddr -
                     1); // DMA読み取りアドレスがアドレス範囲の最後に到達したら最初に戻す
              }
        ...
        for (i = 0; i < leng; i++) {
          ...
          if (addr < dmach->startaddr) {
            addr =
                dmach
                    ->lastaddr; // DMA読み取りアドレスがアドレス範囲の最初に到達したら最後に戻す
          }
          offset =
              (offset + 1) &
              CS4231_BUFMASK; // DMAデータ読み取りバッファの書き込み位置を進める（＆最後に到達したら最初に戻す）
        }
        playcount_adjustcounter =
            (playcount_adjustcounter + leng) % PLAYCOUNT_ADJUST_VALUE;
        dmach->adrs.d = addr;
      }
      ...
      // DMA再生バイト数カウンタ(Playback DMA count
      // register)がPI割り込みを発生させるバイト数になったらPI割り込みを発生させる
      if (cs4231.totalsample >= playcount) {
        ...
        if ((cs4231.reg.pinctrl & IEN) && (cs4231.dmairq != 0xff)) {
          ...
          if (calpenflag != 2) {
            ...
            cs4231.reg.featurestatus |=
                PI; // PI(Playback Interrupt)ビットをセット
