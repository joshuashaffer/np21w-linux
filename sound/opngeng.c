...
static SINT32 SLOTOUT(SINT32 e, SINT32 c) {
  ...
  return (opncfg.sintable[c] * opncfg.envtable[e]) >>
         (sinshift[c] + envshift[e]);
}
...
static void calcratechannel(OPNGEN opngen, OPNCH *ch) {
  ...
  if (envout >= 0) {
    if (ch->feedback) {
      /* with self feed back */
      opout = ch->op1fb;
      ch->op1fb = SLOTOUT(envout, ch->slot[0].freq_cnt +
  ...
  if (envout >= 0) {
    *ch->connect2 +=
        SLOTOUT(envout, ch->slot[1].freq_cnt +
                            (opngen->feedback2 << (FREQ_BITS - (TL_BITS - 2))));
  }
  /* SLOT 3 */
  CALCENV(envout, ch, 2);
  if (envout >= 0) {
    *ch->connect3 +=
        SLOTOUT(envout, ch->slot[2].freq_cnt +
                            (opngen->feedback3 << (FREQ_BITS - (TL_BITS - 2))));
  }
  /* SLOT 4 */
  CALCENV(envout, ch, 3);
  if (envout >= 0) {
    *ch->connect4 +=
        SLOTOUT(envout, ch->slot[3].freq_cnt +
                            (opngen->feedback4 << (FREQ_BITS - (TL_BITS - 2))));
  }
}
