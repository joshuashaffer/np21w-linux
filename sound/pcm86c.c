...
static const UINT clk25_128[] = {0x00001bde, 0x00002527, 0x000037bb,
                                 0x00004a4e, 0x00006f75, 0x0000949c,
                                 0x0000df5f, 0x00012938};
static const UINT clk20_128[] = {0x000016a4, 0x00001e30, 0x00002d48,
                                 0x00003c60, 0x00005a8f, 0x000078bf,
                                 0x0000b57d, 0x0000f17d};
...
void SOUNDCALL pcm86gen_checkbuf(PCM86 pcm86, UINT nCount) {
  ...
  UINT32 bufunder_threshold =
      pcm86->fifosize * smpsize[(pcm86->dactrl >> 4) & 0x7] / 4;
  // UINT32 newtime;
  ...
  // XXX:
  // Windowsでフリーズする問題の暫定対症療法（ある程度時間が経った小さいバッファを捨てる）
  ...
  if (bufs < smpsize[(pcm86->dactrl >> 4) & 0x7]) /* 処理落ちてる… */
  {
    ...
    if (bufundercounter >= bufunder_threshold) {
      if (pcm86->virbuf < pcm86->fifosize) {
        ...
        // bufundercounter = 0;
        // TRACEOUT(("buf: %d, (FIFOSIZE: %d) FORCE IRQ", pcm86->virbuf,
        // pcm86->fifosize));
      } else {
        pcm86_setnextintr();
        // TRACEOUT(("buf: %d, (FIFOSIZE: %d) WARNING", pcm86->virbuf,
        // pcm86->fifosize));
      }
    } else {
      if (pcm86->virbuf < pcm86->fifosize) {
  ...
  // bufundertime += bufundertime_interval * ((newtime - bufundertime) /
  // bufundertime_interval);
}
