...
static void SOUNDCALL pcm8m(CS4231 cs, SINT32 *pcm, UINT count) {
  ...
  do {
    ...
    fract = pos12 & ((1 << 12) - 1);
    ptr1 = cs->buffer + ((cs->bufpos + (samppos << 0) + 0) & CS4231_BUFMASK);
    ptr2 = cs->buffer + ((cs->bufpos + (samppos << 0) + 1) & CS4231_BUFMASK);
    samp1 = (ptr1[0] - 0x80) << 8;
    samp2 = (ptr2[0] - 0x80) << 8;
...
static void SOUNDCALL pcm8s(CS4231 cs, SINT32 *pcm, UINT count) {
  ...
  do {
    ...
    fract = pos12 & ((1 << 12) - 1);
    ptr1 = cs->buffer + ((cs->bufpos + (samppos << 1) + 0) & CS4231_BUFMASK);
    ptr2 = cs->buffer + ((cs->bufpos + (samppos << 1) + 2) & CS4231_BUFMASK);
    samp1 = (ptr1[0] - 0x80) << 8;
    samp2 = (ptr2[0] - 0x80) << 8;
    samp1 += ((samp2 - samp1) * fract) >> 12;
    pcm[0] += (samp1 * cs4231_DACvolume_L * np2cfg.vol_pcm) >> 15;
...
static void SOUNDCALL pcm16s(CS4231 cs, SINT32 *pcm, UINT count) {
  ...
  do {
    ...
    fract = pos12 & ((1 << 12) - 1);
    ptr1 = cs->buffer + ((cs->bufpos + (samppos << 2) + 0) & CS4231_BUFMASK);
    ptr2 = cs->buffer + ((cs->bufpos + (samppos << 2) + 4) & CS4231_BUFMASK);
    samp1 = (((SINT8)ptr1[0]) << 8) + ptr1[1];
    samp2 = (((SINT8)ptr2[0]) << 8) + ptr2[1];
    samp1 += ((samp2 - samp1) * fract) >> 12;
    pcm[0] += (samp1 * cs4231_DACvolume_L * np2cfg.vol_pcm) >> 15;
...
static const CS4231FN cs4231fn[16] = {pcm8m, // 0: 8bit PCM
                                      pcm8s,
                                      nomake, // 1: u-Law
                                      nomake,
  ...
  if (((cs->reg.iface & 1) || bufdelaycounter > 0) && (count)) {
    ...
    if (cs4231_DACvolumereg_L != cs->reg.dac_l) {
      ...
      if (cs->reg.dac_l & 0x80) { // DAC L Mute
        ...
        cs4231_DACvolume_L =
            (int)(pow(10, -1.5 * ((cs4231_DACvolumereg_L)&0x3f) / 20.0) *
                  1024); // DAC L Volume (1bit毎に-1.5dB)
      }
    }
    if (cs4231_DACvolumereg_R != cs->reg.dac_r) {
      ...
      if (cs->reg.dac_r & 0x80) { // DAC R Mute
        ...
        cs4231_DACvolume_R =
            (int)(pow(10, -1.5 * ((cs4231_DACvolumereg_R)&0x3f) / 20.0) *
                  1024); // DAC R Volume (1bit毎に-1.5dB)
      }
    }
