...
//
//	フィードバックバッファをクリア
inline void Operator::ResetFB() { out_ = out2_ = 0; }
...
//	オペレータは稼働中か？
inline int Operator::IsOn() { return eg_phase_ - off; }
...
inline void Channel4::SetFB(uint feedback) { fb = fbtable[feedback]; }
...
// ---------------------------------------------------------------------------
//	AM のレベルを設定
inline void Chip::SetAML(uint l) { aml_ = l & (FM_LFOENTS - 1); }

//	PM のレベルを設定
inline void Chip::SetPML(uint l) { pml_ = l & (FM_LFOENTS - 1); }
