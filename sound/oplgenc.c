...
void oplgen_initialize(UINT rate) {
  ...
  for (i = 0; i < SIN_ENT; i++) {
    ...
    if ((3 / (8 * SIN_ENT) < i) && (i < 4 / (8 * SIN_ENT)))
      oplcfg.sintable[4][i] = 0;
    else if (i > 7 / (8 * SIN_ENT))
      oplcfg.sintable[4][i] = 0;
    else
      oplcfg.sintable[4][i] = (SINT32)pom;
  }
  for (i = 0; i < SIN_ENT; i++) {
    if ((3 / (8 * SIN_ENT) < i) && (i < 4 / (8 * SIN_ENT))) {
    ...
    } else /* keyoff */
    {
      if (slot->env_mode > EM_RELEASE) {
        ...
        if (!(slot->env_cnt & EC_DECAY)) {
          slot->env_cnt =
              (oplcfg.envcurve[slot->env_cnt >> ENV_BITS] << ENV_BITS) +
              EC_DECAY;
        }
        slot->env_end = EC_OFF;
        slot->env_inc = slot->env_inc_release;
...
static void rhythmon(OPLGEN oplgen, REG8 value) {
  ...
  keyon(oplgen, &oplgen->oplch[7],
        ((value & 0x01) ? 1 : 0) | ((value & 0x08) ? 2 : 0));
  keyon(oplgen, &oplgen->oplch[8],
        ((value & 0x04) ? 1 : 0) | ((value & 0x02) ? 2 : 0));
}
