...
typedef struct hax_capabilityinfo {
  ...
  UINT16
      winfo; // wstatusで使用可の場合：HAXMで使用できる機能のフラグ、使用不可の場合：使用不可の理由フラグ
...
typedef struct hax_alloc_ram_info {
  ...
  UINT32 pad;  // 無視
  UINT64 va; // 登録したいホストのメモリアドレス。4KB境界でないと駄目。vaはVirtual
             // Addressの略？
} HAX_ALLOC_RAM_INFO;

typedef struct hax_ramblock_info {
  UINT64 start_va; // 登録したいホストのメモリアドレス。4KB境界でないと駄目。vaはVirtual
                   // Addressの略？
...
typedef struct hax_set_ram_info {
  UINT64 pa_start; // 割当先のゲスト物理アドレス(Guest Physical Address;
                   // GPA)。既に登録されている場合は上書き。4KB境界でないと駄目
  UINT32 size; // 割当するサイズ
  UINT8
      flags; // 0:通常のRAM、HAX_RAM_INFO_ROM:ROM領域、HAX_RAM_INFO_INVALID:割り当て解除（vaは0にする）
  UINT8 pad[3]; // 無視
  UINT64
      va; // 割当したいホストのメモリアドレス(Host Virtual Address;
          // HVA)。先にi386haxfunc_allocRAMで登録しておかないと駄目。4KB境界でないと駄目
} HAX_SET_RAM_INFO;
...
typedef struct hax_tunnel {
  ...
  union {
    struct {
      ...
      UINT8
          _df; // _countが2以上の場合、メモリを読み取る順番。0はインクリメント、1はデクリメント（I/Oポート番号はそのまま）
    ...
    struct {
      UINT64
          gla; // ゲストリニアアドレスの略？もう使われていないようなので分からない
    } mmio;
    struct {
...
typedef struct hax_fastmmio {
  ...
  UINT8 direction; // READ=0、WRITE=1、READ&WRITE=2
                   // (I/Oポートの時と何故か逆なので注意。参考:
                   // I/OポートではREAD=1, WRITE=0)
...
UINT8 i386haxfunc_getversion(
    HAX_MODULE_VERSION *version); // HAXMのバージョンを取得する
UINT8 i386haxfunc_getcapability(
    HAX_CAPINFO *cap); // HAXMで使用可能な機能を取得する
UINT8 i386haxfunc_setmemlimit(
    HAX_SET_MEMLIMIT *
        memlimit); // HAXMのメモリ容量制限をセットする（最近の版では意味なし？）
UINT8 i386haxfunc_createVM(
    UINT32 *vm_id); // HAXMの仮想マシンを作成して仮想マシンIDを返す

// HAXM VM IOCTL
UINT8 i386haxfunc_notifyQEMUversion(
    HAX_QEMU_VERSION
        *hax_qemu_ver); // QEMUバージョンの通知（Fast
                        // MMIOの挙動が変わる？通知しなくても問題はなさそう）
UINT8 i386haxfunc_VCPUcreate(UINT32 vcpu_id); // HAXM仮想マシンに指定したIDの仮想CPUを作成する（CPU
                                              // IDは0〜15で指定）
UINT8 i386haxfunc_allocRAM(
    HAX_ALLOC_RAM_INFO
        *allocraminfo); // HAXM仮想マシンで使用するホストのメモリ範囲を登録
UINT8 i386haxfunc_addRAMblock(
    HAX_RAMBLOCK_INFO
        *ramblkinfo); // HAXM仮想マシンで使用するホストのメモリ範囲を登録
UINT8 i386haxfunc_setRAM(
    HAX_SET_RAM_INFO *
        setraminfo); // HAXM仮想マシンの物理アドレスにホストのメモリを割り当て（メモリは事前にi386haxfunc_allocRAMで登録が必要）。未割り当てのアドレスはMMIO扱いになる。

// HAXM VCPU IOCTL
UINT8 i386haxfunc_vcpu_run(); // 仮想マシン実行開始。I/Oポートアクセス・割り込み発生等で実行継続できなくなったら実行中断する
UINT8 i386haxfunc_vcpu_setup_tunnel(
    HAX_TUNNEL_INFO *
        info); // 仮想マシンとデータをやりとりするためのメモリ領域(tunnel)を確保
UINT8 i386haxfunc_vcpu_setMSRs(
    HAX_MSR_DATA *inbuf,
    HAX_MSR_DATA
        *outbuf); // モデル固有レジスタ（Model Specific Register; MSR）を設定
UINT8 i386haxfunc_vcpu_getMSRs(
    HAX_MSR_DATA *outbuf); // モデル固有レジスタを取得
...
UINT8 i386haxfunc_vcpu_interrupt(
    UINT32 vector); // 指定したベクタの割り込みを発生させる
UINT8 i386haxfunc_vcpu_kickoff(); // 謎
UINT8 i386haxfunc_vcpu_debug(
    HAX_DEBUG *inbuf); // デバッグの有効/無効やデバッグ方法を設定する
