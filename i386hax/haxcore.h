...
typedef struct {
  ...
  UINT8
      irq_req[256]; // 割り込み待機バッファ。大気中の割り込みベクタが格納される
...
void i386hax_vm_allocmemoryex(UINT8 *vramptr,
                              UINT32 size); // メモリ領域を登録（汎用）

// ゲスト物理アドレス(Guest Physical Address; GPA)にホストの仮想アドレス(Host
// Virtual Address; HVA)を割り当て
