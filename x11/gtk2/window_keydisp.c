...
static UINT32 kdwinpal[KEYDISP_PALS] = {0x00000000, 0xffffffff, 0xf9ff0000};
...
static void kdispwin_window_destroy(GtkWidget *w, gpointer p) {
 ...
 * signal
 */
static gint kdispwin_expose(GtkWidget *w, GdkEventExpose *ev) {
 ...
 */
static UINT8 getpal8(CMNPALFN *self, UINT num) {
...
static void setkeydispmode(UINT8 mode) { keydisp_setmode(mode); }
...
void kdispwin_create(void) {
  ...
  gtk_widget_show(da);
  g_signal_connect(G_OBJECT(da), "expose_event", G_CALLBACK(kdispwin_expose),
                   NULL);
