...
const char *cmmidi_mdlname[] = {"MT-32",      "CM-32L",     "CM-64", "CM-300",
                                "CM-500(LA)", "CM-500(GS)", "SC-55", "SC-88",
                                "LA",         "GM",         "GS",    "XG"};
...
static const UINT8 midictrltbl[] = {
    0,  1,  5,  7, 10, 11, 64, 65, 66, 67, 84, 91, 93,
    94,            // for SC-88
    71, 72, 73, 74 // for XG
};
...
static int getmidiout(const char *midiout) {
  int hmidiout = -1;
...
static int getmidiin(const char *midiin) {
  int hmidiin = -1;
...
static INLINE void midi_write(CMMIDI midi, const UINT8 *cmd, UINT cnt) {
  struct timeval ct;
  int ds, du;
  int rv;
  UINT i;
...
static void midiout_device(CMMIDI midi, UINT32 msg, UINT cnt) {
  UINT8 buf[3];
  UINT i;
  ...
  for (i = 0; i < 16; i++, mch++) {
    ...
    for (j = 0; j < sizeof(midictrltbl) / sizeof(UINT8); j++) {
      if (mch->ctrl[j + 1] != 0xff) {
        (*midi->outfn)(midi,
                       MIDIOUTS(0xb0 + i, midictrltbl[j], mch->ctrl[j + 1]), 3);
      }
    }
    if (mch->prog != 0xff) {
