...
static void inirdsnddrv(const char *src, INITBL *ini) {

  *(UINT8 *)ini->value = snddrv_drv2num(src);
}

static void inirdinterp(const char *src, INITBL *ini) {
...
static const char *iniwrinterp(UINT8 interp) {
...
void ini_write(const char *path, const char *title, INITBL *tbl, UINT count,
  ...
  while (p < pterm) {
    if (!(p->itemtype & INIFLAG_RO) || read_iniread_flag(p)) {
      ...
      case INITYPE_SNDDRV:
        g_snprintf(work, sizeof(work), "%s",
                   snddrv_num2drv(*(UINT8 *)p->value));
        break;
...
static int calc_index(const INITBL *p) {
  UINT offset;
  UINT idx;
