...
CGimic::CGimic(UINT nIndex)
    : m_nRef(1), m_nIndex(nIndex), m_nChipType(CHIP_UNKNOWN), m_nQueIndex(0),
      m_nQueCount(0), m_gimic2(this), m_chip3(this) {}
 ...
 * @return C86CTL_ERR
 */
C86CtlErr CGimic::Transaction(const void *lpOutput, int cbOutput, void *lpInput,
                              int cbInput) {
  if (!m_usb.IsOpened()) {
    return C86CTL_ERR_NODEVICE;
  }
...
C86CtlErr CGimic::GetInfo(UINT8 cParam, Devinfo *pInfo) {
  ...
  sData[2] = cParam;
  const C86CtlErr ret =
      Transaction(sData, sizeof(sData), pInfo, sizeof(*pInfo));
 ...
 */
CGimic::Gimic2::Gimic2(CGimic *pDevice) : m_pDevice(pDevice) {}

/**
 * Get the instance of the device
 * @return The instance
 */
inline CGimic *CGimic::Gimic2::GetDevice() { return m_pDevice; }

/**
 * Increments the reference count
 * @return The new reference count
 */
size_t CGimic::Gimic2::AddRef() { return GetDevice()->AddRef(); }

/**
 * Decrements the reference count
 * @return The new reference count
 */
size_t CGimic::Gimic2::Release() { return GetDevice()->Release(); }
 ...
 * @return C86CTL_ERR
 */
C86CtlErr CGimic::Gimic2::getFWVer(UINT *pnMajor, UINT *pnMinor, UINT *pnRev,
                                   UINT *pnBuild) {
  static const UINT8 sData[2] = {0xfd, 0x92};
  UINT8 sRecv[16];
  const C86CtlErr ret =
      GetDevice()->Transaction(sData, sizeof(sData), sRecv, sizeof(sRecv));
...
C86CtlErr CGimic::Gimic2::getSSGVolume(UINT8 *pcVolume) {
  ...
  return GetDevice()->Transaction(sData, sizeof(sData), pcVolume,
                                  sizeof(*pcVolume));
}
...
C86CtlErr CGimic::Gimic2::setPLLClock(UINT nClock) {
  ...
  if ((nChipType != CHIP_OPNA) && (nChipType != CHIP_OPM) &&
      (nChipType != CHIP_OPL3)) {
    return C86CTL_ERR_UNSUPPORTED;
  }
...
C86CtlErr CGimic::Gimic2::getPLLClock(UINT *pnClock) {
  ...
  if ((ret == C86CTL_ERR_NONE) && (pnClock != nullptr)) {
    *pnClock =
        (sRecv[0] << 0) | (sRecv[1] << 8) | (sRecv[2] << 16) | (sRecv[3] << 24);
  }
  return ret;
}
 ...
 * @param[in] pDevice The instance of the device
 */
CGimic::Chip3::Chip3(CGimic *pDevice) : m_pDevice(pDevice) {}

/**
 * Get the instance of the device
 * @return The instance
 */
inline CGimic *CGimic::Chip3::GetDevice() { return m_pDevice; }

/**
 * Increments the reference count
 * @return The new reference count
 */
size_t CGimic::Chip3::AddRef() { return GetDevice()->AddRef(); }

/**
 * Decrements the reference count
 * @return The new reference count
 */
size_t CGimic::Chip3::Release() { return GetDevice()->Release(); }
