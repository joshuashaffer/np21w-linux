 ...
 */
CSoundChip::CSoundChip(CSoundInterface *pInterface,
                       const SCCI_SOUND_CHIP_INFO &info)
    : m_pInterface(pInterface), m_info(info) {}
...
bool CSoundChip::IsMatch(SC_CHIP_TYPE iSoundChipType, UINT dClock) const {
  ...
  for (UINT i = 0; i < 2; i++) {
    if ((m_info.iCompatibleSoundChip[i] == iSoundChipType) &&
        (m_info.dCompatibleClock[i] == dClock)) {
      return true;
    }
  }
  return false;
 ...
 */
const SCCI_SOUND_CHIP_INFO *CSoundChip::getSoundChipInfo() { return &m_info; }

/**
 * Gets sound chip type
 * @return The type of the chip
 */
SC_CHIP_TYPE CSoundChip::getSoundChipType() { return m_info.iSoundChip; }
