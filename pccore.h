...
enum tagSoundId {
  ...
  SOUNDID_PC_9801_86_WSS_SB16 =
      0x42, /*!< PC-9801-86 + Mate-X PCM(B460) + Sound Blaster 16 */
  ...
  SOUNDID_PC_9801_86_118_SB16 =
      0x46, /*!< PC-9801-86 + PC-9801-118(B460) + Sound Blaster 16 */
...
struct tagNP2Config {
  ...
  UINT32
      iderwait; // IDE読み取りの割り込み遅延時間(clock)。  np21w ver0.86 rev19
  UINT32
      idewwait; // IDE書き込みの割り込み遅延時間(clock)。  np21w ver0.86 rev18
  UINT32 idemwait; // IDE BIOSがある場合の割り込み遅延最小値  np21w ver0.86
                   // rev26 廃止
  UINT8 savecdfile;
  UINT8 useasynccd;
  ...
  UINT8 fpu_type; // FPU種類（0=Berkeley SoftFloat, 1=DOSBox FPU, 2=DOSBox
                  // FPU+INT64）
  ...
  UINT8 useram_d; // EPSONでなくてもD0000h-DFFFFhをRAMに（ただしIDE BIOS
                  // D8000h-DBFFFhは駄目）
...
enum { COREEVENT_SHUT = 0, COREEVENT_RESET = 1, COREEVENT_EXIT = 2 };
