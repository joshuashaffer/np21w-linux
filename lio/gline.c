...
static void gbox(const _GLIO *lio, const LINEPT *lp, UINT8 *tile, UINT leng) {
  ...
  if ((x1 > lio->draw.x2) || (x2 < lio->draw.x1) || (y1 > lio->draw.y2) ||
      (y2 < lio->draw.y1)) {
    return;
  }
...
REG8 lio_gline(GLIO lio) {
  ...
  TRACEOUT(
      ("lio_gline %d,%d-%d,%d [%d]", lp.x1, lp.y1, lp.x2, lp.y2, dat.type));
  ...
  if (dat.type < 2) {
    if (dat.sw) {
      pat = (GDCPATREVERSE(dat.style[0]) << 8) + GDCPATREVERSE(dat.style[1]);
    }
    lp.pal = dat.pal;
    if (dat.type == 0) {
      gline(lio, &lp, pat);
    ...
    if (dat.sw == 2) {
      ...
      MEMR_READS(LOADINTELWORD(dat.seg), LOADINTELWORD(dat.off), tile, leng);
    }
    if (dat.sw != 1) {
      lp.pal = dat.pal;
