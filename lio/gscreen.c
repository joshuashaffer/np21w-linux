...
REG8 lio_gscreen(GLIO lio) {
  ...
  if (disp == 0xff) {
    ...
    if ((plane > planemax) && (plane != (1 << colorbit))) {
      goto gscreen_err5;
    }
...
gscreen_err5:
  TRACEOUT(("screen error! %d %d %d %d", dat.mode, dat.sw, dat.act, dat.disp));
...
REG8 lio_gcolor2(GLIO lio) {
  ...
  if (!lio->palmode) {
    ...
    gdc_setanalogpal(dat.pal, offsetof(RGB32, p.b), (UINT8)(dat.color1 & 0x0f));
    gdc_setanalogpal(dat.pal, offsetof(RGB32, p.r), (UINT8)(dat.color1 >> 4));
    gdc_setanalogpal(dat.pal, offsetof(RGB32, p.g), (UINT8)(dat.color2 & 0x0f));
  }
  MEMR_WRITES(CPU_DS, 0x0620, &lio->work, sizeof(lio->work));
  return (LIO_SUCCESS);
