...
static REG8 putsub(GLIO lio, const LIOPUT *lput) {
  ...
  if ((lput->x < lio->draw.x1) || (lput->y < lio->draw.y1) ||
...
REG8 lio_gget(GLIO lio) {
  ...
  if ((x < lio->draw.x1) || (y < lio->draw.y1) || (x2 > lio->draw.x2) ||
      (y2 > lio->draw.y2)) {
    return (LIO_ILLEGALFUNC);
  }
  x2 = x2 - x + 1;
...
REG8 lio_gput2(GLIO lio) {
  ...
  if (!pat) {
    ...
    MEMR_WRITES(lput.seg, lput.off,
                mem + (LIO_SEGMENT << 4) + LIO_FONT + ((pat - 1) << 4), 0x10);
    size = 0x0102;
  }
