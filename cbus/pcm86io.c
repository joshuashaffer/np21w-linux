...
static const UINT8 s_irqtable[8] = {0xff, 0xff, 0xff, 0xff,
                                    0x03, 0x0a, 0x0d, 0x0c};

// for Q-Vision Wave Star (XXX:
// この辺のポートを操作中にResumeやStateSaveを使うとやばい)
...
static void IOOUTCALL pcm86_oa462(UINT port, REG8 val) {
  if (wavestar_a462_seq_index < sizeof(wavestar_a462_seq) &&
      val == wavestar_a462_seq[wavestar_a462_seq_index]) {
...
static void IOOUTCALL pcm86_oa464(UINT port, REG8 val) {
  if (wavestar_a462_seq_index == sizeof(wavestar_a462_seq)) {
    if (val == 0x04) {
      cs4231.devvolume[0xfe] =
          1; // XXX: 暫定でフラグ用に借りる（本来は変数を追加すべき）
      wavestar_a464_value = 0x0c;
    } else {
      cs4231.devvolume[0xfe] =
          0; // XXX: 暫定でフラグ用に借りる（本来は変数を追加すべき）
