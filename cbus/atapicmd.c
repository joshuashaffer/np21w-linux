...
#ifdef YUIDEBUG
    // うちのドライブの奴 NECCDは Product Level 3.00以上で
    // modesense10のコードがちげー
    0x05,             // CD-ROM
...
static void senddata(IDEDRV drv, UINT size, UINT limit) {
  ...
  drv->status |=
      IDESTAT_DRQ |
      IDESTAT_DSC; // XXX: set Drive Seek Complete bit np21w ver0.86 rev29
  drv->error = 0;
...
static void cmddone(IDEDRV drv) {
  ...
  drv->status |=
      IDESTAT_DRDY |
      IDESTAT_DSC; // XXX: set Drive Seek Complete bit np21w ver0.86 rev13
  drv->error = 0;
  ATAPI_SET_SENSE_KEY(drv, ATAPI_SK_NO_SENSE);
  drv->asc = ATAPI_ASC_NO_ADDITIONAL_SENSE_INFORMATION;
...
static void senderror(IDEDRV drv) {
  ...
  // drv->sc = IDEINTR_IO;
  drv->sc =
      IDEINTR_IO | IDEINTR_CD; // set Command or Data bit np21w ver0.86 rev38
...
#define MEDIA_CHANGE_WAIT                                                      \
  6 // Waitを入れないとWinNT系で正しくメディア交換出来ない
...
void atapicmd_a0(IDEDRV drv) {
  ...
  case 0x00: // test unit ready
    ...
    if (!(drv->media & IDEIO_MEDIA_LOADED)) {
      ...
      if (drv->sxsidrv == cdchange_drv &&
          g_nevent.item[NEVENT_CDWAIT].clock > 0) {
        if (mediachangeflag == MEDIA_CHANGE_WAIT) {
          nevent_set(
              NEVENT_CDWAIT, 1, cdchange_timeoutproc,
              NEVENT_ABSOLUTE); // OS側がCDを催促しているようなので更に急いで交換
        } else if (mediachangeflag == 0) {
          nevent_setbyms(
              NEVENT_CDWAIT, 1000, cdchange_timeoutproc,
              NEVENT_ABSOLUTE); // OS側がCDが無いと認識したようなので急いで交換
        }
      }
      if (mediachangeflag < MEDIA_CHANGE_WAIT)
        mediachangeflag++;
  ...
  case 0x03: // request sense
    ...
    drv->buf[13] = (UINT8)((drv->asc >> 8) &
                           0xff); // Additional Sense Code Qualifier (Optional)
  ...
  case 0x28: // read(10)
    ...
    lba = (drv->buf[2] << 24) + (drv->buf[3] << 16) + (drv->buf[4] << 8) +
          drv->buf[5];
...
void atapi_cmd_traycmd_eject_threadfunc(void *vdParam) {
  ...
  HANDLE handle;
  DWORD dwRet = 0;
  handle = CreateFile(np2cfg.idecd[(int)vdParam], GENERIC_READ | GENERIC_WRITE,
                      FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING,
                      FILE_ATTRIBUTE_NORMAL, 0);
  if (handle != INVALID_HANDLE_VALUE) {
    if (DeviceIoControl(handle, FSCTL_LOCK_VOLUME, 0, 0, 0, 0, &dwRet, 0)) {
      if (DeviceIoControl(handle, FSCTL_DISMOUNT_VOLUME, 0, 0, 0, 0, &dwRet,
                          0)) {
        DeviceIoControl(handle, IOCTL_STORAGE_EJECT_MEDIA, 0, 0, 0, 0, &dwRet,
                        0);
      }
    }
    CloseHandle(handle);
...
void atapi_cmd_traycmd_close_threadfunc(void *vdParam) {
  ...
  HANDLE handle;
  DWORD dwRet = 0;
  handle = CreateFile(np2cfg.idecd[(int)vdParam], GENERIC_READ,
...
static void atapi_cmd_start_stop_unit(IDEDRV drv) {
  ...
  case 2: // Eject the Disc if possible
    ...
    if (np2cfg.allowcdtraycmd &&
        _tcsnicmp(np2cfg.idecd[sxsi->drv], OEMTEXT("\\\\.\\"), 4) == 0) {
#if defined(_WINDOWS)
...
void atapi_dataread_threadfunc_part(IDEDRV drv) {
  ...
  if (np2cfg.usecdecc &&
      (sxsi->cdflag_ecc & CD_ECC_BITMASK) == CD_ECC_RECOVERED) {
...
static const UINT8 chgval_pagecode_2a[PC_2A_SIZE] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x02, 0xc2,
    0x00, 0x02, 0x00, 0x00, 0x02, 0xc2, 0x00, 0x00, 0x00, 0x00,
};
...
static const UINT8 defval_pagecode_2a[PC_2A_SIZE] = {
    ...
    //	0x00, 0x00, 0x00, 0x00,
    //#else
    0x2a, 0x12, 0x00, 0x00, 0x71, 0x65, 0x29, 0x07, 0x02, 0xc2,
    0x00, 0xff, 0x00, 0x80, 0x02, 0xc2, 0x00, 0x00, 0x00, 0x00,
    //#endif
};
...
static void atapi_cmd_mode_sense(IDEDRV drv) {
  ...
  leng = (drv->buf[7] << 8) + drv->buf[8];
  pctrl =
      ((drv->buf[2] >> 6) & 3) & ~0x2; // 0: current, 1: changeable, 2: default
  ...
  case 0x0f: {
    ...
    // note: When byte04 bit1 is set, neccdd.sys will set bit10 in device status
    // call. (bit10 is "reserved" in the Microsoft's reference) for drive
    // specific applications? or simply a bug?)
  }
    cnt += PC_0F_SIZE;
    if (cnt > leng) {
      goto length_exceeded;
    ...
    drv->damsfbcd =
        1; // XXX: NEC
           // CD-ROMコマンドを飛ばしてきたらBCDモードにする･･･（暫定neccdd.sys判定）
    break;
#endif
  case 0x3f:
    /*FALLTHROUGH*/
...
static void atapi_cmd_readtoc(IDEDRV drv) {
...
#endif
  TRACEOUT(
      ("ATAPI CMD: read TOC : time=%d fmt=%d leng=%d", time, format, leng));
  TRACEOUT(("\t[%02x %02x %02x %02x %02x %02x %02x %02x]", drv->buf[0x00],
            drv->buf[0x01], drv->buf[0x02], drv->buf[0x03], drv->buf[0x04],
            drv->buf[0x05], drv->buf[0x06], drv->buf[0x07]));
  TRACEOUT(("\t[%02x %02x %02x %02x %02x %02x %02x %02x]", drv->buf[0x08],
            drv->buf[0x09], drv->buf[0x0a], drv->buf[0x0b], drv->buf[0x0c],
            drv->buf[0x0d], drv->buf[0x0e], drv->buf[0x0f]));
#endif
  ...
  case 0: // track info
    ...
    strack = min(max(1U, strack),
                 (tracks + 1)); // special case: 0 = 1sttrack, 0xaa = leadout
...
static void atapi_cmd_playaudio(IDEDRV drv) {
  ...
  pos = (drv->buf[2] << 24) | (drv->buf[3] << 16) | (drv->buf[4] << 8) |
        drv->buf[5];
...
static void atapi_cmd_seek(IDEDRV drv, UINT32 lba) {
  ...
  sxsi = sxsi_getptr(drv->sxsidrv);
  trk = sxsicd_gettrk(sxsi, &tracks);
  TRACEOUT(("atapicmd: seek LBA=%d NSEC=%d", lba,
            trk[tracks - 1].pos + trk[tracks - 1].sectors));
...
static void atapi_cmd_mechanismstatus(IDEDRV drv) {
  ...
  // drv->buf[6] = 0x00;
  // drv->buf[7] = 0x04;
  // drv->buf[8] = (sxsi->flag & SXSIFLAG_READY) ? 0x80 : 0x00; // XXX:
  // CD挿入状態とか入れてやるべき？ drv->buf[9] = 0x00; drv->buf[10] = 0x00;
  // drv->buf[11] = 0x00;
  // senddata(drv, 12, 12);
  sendabort(drv);
}
...
void atapi_initialize(void) {
  ...
  if (!atapi_thread_initialized) {
    if (DuplicateHandle(GetCurrentProcess(), GetCurrentThread(),
                        GetCurrentProcess(), &atapi_thread_main, 0, FALSE,
                        DUPLICATE_SAME_ACCESS)) {
      atapi_thread_initialized = 1;
      atapi_thread = (HANDLE)_beginthreadex(NULL, 0, atapi_dataread_threadfunc,
                                            NULL, CREATE_SUSPENDED, &dwID);
    } else {
