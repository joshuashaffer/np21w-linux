...
void boardspb_reset(const NP2CFG *pConfig, int opnaidx) {
  ...
  opna_reset(&g_opna[opna_idx], OPNA_MODE_2608 | OPNA_HAS_TIMER |
                                    OPNA_HAS_ADPCM | OPNA_HAS_VR | OPNA_S98);
  if (opna_idx == 1) {
    opna_timer(&g_opna[opna_idx], (pConfig->spbopt & 0xc0) | 0x10,
               NEVENT_FMTIMER2A, NEVENT_FMTIMER2B);
  } else {
    opna_timer(&g_opna[opna_idx], (pConfig->spbopt & 0xc0) | 0x10,
...
void boardspr_reset(const NP2CFG *pConfig) {
  ...
  opna_reset(&g_opna[0], OPNA_MODE_2608 | OPNA_HAS_TIMER | OPNA_HAS_ADPCM |
                             OPNA_HAS_VR | OPNA_S98);
  opna_timer(&g_opna[0], (pConfig->spbopt & 0xc0) | 0x10, NEVENT_FMTIMERA,
             NEVENT_FMTIMERB);
