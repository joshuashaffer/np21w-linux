  ...
  g_sb16.mixregexp[MIXER_MASTER_LEFT] = g_sb16.mixreg[MIXER_MASTER_LEFT] =
      g_sb16.mixregexp[MIXER_MASTER_RIGHT] = g_sb16.mixreg[MIXER_MASTER_RIGHT] =
          g_sb16.mixregexp[MIXER_VOC_LEFT] = g_sb16.mixreg[MIXER_VOC_LEFT] =
              g_sb16.mixregexp[MIXER_VOC_RIGHT] =
                  g_sb16.mixreg[MIXER_VOC_RIGHT] =
                      g_sb16.mixregexp[MIXER_MIDI_LEFT] =
                          g_sb16.mixreg[MIXER_MIDI_LEFT] =
                              g_sb16.mixregexp[MIXER_MIDI_RIGHT] =
  ...
  g_sb16.mixregexp[MIXER_TREBLE_LEFT] = g_sb16.mixreg[MIXER_TREBLE_LEFT] =
      g_sb16.mixregexp[MIXER_TREBLE_RIGHT] = g_sb16.mixreg[MIXER_TREBLE_RIGHT] =
          g_sb16.mixregexp[MIXER_BASS_LEFT] = g_sb16.mixreg[MIXER_BASS_LEFT] =
              g_sb16.mixregexp[MIXER_BASS_RIGHT] =
...
static void IOOUTCALL sb16_o2500(UINT port, REG8 dat) {
  ...
  if (g_sb16.mixsel >= MIXER_VOL_START && g_sb16.mixsel <= MIXER_VOL_END) {
    ...
    g_sb16.mixregexp[g_sb16.mixsel] =
        (int)(pow(((dat >> 3) & 0x1f) / 32.0, 2) *
              255); //(int)(pow(dat / 255.0, 4) * 255);
  ...
  case 0x26: // MIDI volume(old)
    g_sb16.mixregexp[MIXER_MIDI_LEFT] = g_sb16.mixreg[MIXER_MIDI_LEFT] =
        (dat & 0x0f) << 4;
    g_sb16.mixregexp[MIXER_MIDI_RIGHT] = g_sb16.mixreg[MIXER_MIDI_RIGHT] =
        (dat & 0xf0);
    break;
  case 0x28: // CD volume(old)
    g_sb16.mixregexp[MIXER_CD_LEFT] = g_sb16.mixreg[MIXER_CD_LEFT] =
        (dat & 0x0f) << 4;
    g_sb16.mixregexp[MIXER_CD_RIGHT] = g_sb16.mixreg[MIXER_CD_RIGHT] =
        (dat & 0xf0);
    break;
  case 0x2e: // Line volume(old)
    g_sb16.mixregexp[MIXER_LINE_LEFT] = g_sb16.mixreg[MIXER_LINE_LEFT] =
        (dat & 0x0f) << 4;
    g_sb16.mixregexp[MIXER_LINE_RIGHT] = g_sb16.mixreg[MIXER_LINE_RIGHT] =
        (dat & 0xff);
...
static REG8 IOINPCALL sb16_i2400(UINT port) { return g_sb16.mixsel; }
static REG8 IOINPCALL sb16_i2500(UINT port) {
  // printf("mixer port read %x
  ...
  switch (g_sb16.mixsel) {
  case 0x04:
    return (g_sb16.mixreg[MIXER_VOC_LEFT] + g_sb16.mixreg[MIXER_VOC_RIGHT])
           << 1;
  case 0x22:
    return (g_sb16.mixreg[MIXER_MASTER_LEFT] +
            g_sb16.mixreg[MIXER_MASTER_RIGHT])
           << 1;
  case 0x26:
    return (g_sb16.mixreg[MIXER_MIDI_LEFT] + g_sb16.mixreg[MIXER_MIDI_RIGHT])
           << 1;
  case 0x28: // CD volume(old)
    return (g_sb16.mixreg[MIXER_CD_LEFT] + g_sb16.mixreg[MIXER_CD_RIGHT]) << 1;
  case 0x2e: // Line volume(old)
    return (g_sb16.mixreg[MIXER_LINE_LEFT] + g_sb16.mixreg[MIXER_LINE_RIGHT])
           << 1;
  case 0x0a: // Mic volume(old)
  ...
  case 0x82: // Irq pending(98には不要)　diagnose用　他よくわからず
    if (g_sb16.mixreg[0x82] == 0x41)
      return 0x1;
    if (g_sb16.mixreg[0x82] == 0x42)
      return 0x2;
    if (g_sb16.mixreg[0x82] == 0x43)
      return 0x3;
    else
      return 0x40;
  default:
...
void ct1745io_reset(void) { ct1745_mixer_reset(); }

void ct1745io_bind(void) {
  iocore_attachout(0x2400 + g_sb16.base,
                   sb16_o2400); /* Mixer Chip Register Address Port */
  iocore_attachout(0x2500 + g_sb16.base, sb16_o2500); /* Mixer Chip Data Port */
  iocore_attachinp(0x2400 + g_sb16.base,
                   sb16_i2400); /* Mixer Chip Register Address Port */
