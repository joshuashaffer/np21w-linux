...
static REG8 IOINPCALL wss_i881e(UINT port) {
  if (g_nSoundID == SOUNDID_MATE_X_PCM ||
      g_nSoundID == SOUNDID_PC_9801_86_WSS || g_nSoundID == SOUNDID_WSS_SB16 ||
...
REG8 ymf701;
static void IOOUTCALL wss_o548e(UINT port, REG8 dat) { ymf701 = dat; }
static REG8 IOINPCALL wss_i548e(UINT port) { return (ymf701); }
...
static REG8 IOINPCALL gameport_i1480(UINT port) {
  ...
  if (CPU_REMCLOCK > 0) {
    clockdiff = CPU_MSR_TSC -
                CPU_REMCLOCK * pccore.maxmultiple / pccore.multiple -
                gameport_tsc;
  } else {
    clockdiff = CPU_MSR_TSC - gameport_tsc;
  }
...
static void IOOUTCALL ym_o1488(UINT port,
                               REG8 dat) // FM Music Register Address Port
{
...
ym_o148a(UINT port, REG8 dat) // Advanced FM Music Register Address	 Port
{
  g_opl3[G_OPL3_INDEX].s.addrh = dat;
  (void)port;
}
static void IOOUTCALL ym_o148b(UINT port,
                               REG8 dat) // Advanced FM Music Data Port
{
  opl3_writeExtendedRegister(&g_opl3[G_OPL3_INDEX],
                             g_opl3[G_OPL3_INDEX].s.addrh, dat);
  (void)port;
...
static REG8 IOINPCALL csctrl_i148f(UINT port) {
  TRACEOUT(("%x read", port));
  (void)port;
  // if(sound118 == 0)	return(0xf0);//PC-9801-118は3だけどYMFは0xff
  // 2000はこれだけじゃまだダメ
  if (sound118 == 0)
    return (0xf3); // PC-9801-118は3だけどYMFは0xff 2000はこれだけじゃまだダメ
  if (sound118 == 0x05) {
    if (control118 == 4)
      return 4;
    if (control118 == 0x0c)
  ...
  if (sound118 == 0x04)
    return (0x00);
  if (sound118 == 0x21) {
    switch (10 /*mpu98.irqnum2*/) { // MIDI割り込み 00:IRQ10　01:IRQ6 02:IRQ5
                                    // 03:IRQ3
    case 10:
      return 0;
    case 6:
  ...
  if (sound118 == 0xff)
    return (
        0x05); // bit0 MIDI割り込みあり bit1:Cb Na7 bit2:Mate-X bit3:A-Mate Ce2
  else
    return (0xff);
}
...
static REG8 IOINPCALL csctrl_i486(UINT port) { return (0); }
...
static void SOUNDCALL opl3gen_getpcm2(void *opl3, SINT32 *pcm, UINT count) {
  ...
  // Canbe/ValueStarミキサー FMボリューム(0～31) bit7:1=mute, bit6-5:reserved,
  // bit4-0:volume(00000b max, 11111b min)
  ...
  for (i = 0; i < count; i++) {
    ...
    outbuf[0] +=
        ((((SINT32)s1l << 1) * oplfm_volume * oplfm_softvolume_L) >> 10);
    outbuf[1] +=
        ((((SINT32)s1r << 1) * oplfm_volume * oplfm_softvolume_R) >> 10);
    outbuf += 2;
  }
}
#endif
...
void board118_reset(const NP2CFG *pConfig) {
  ...
  if (g_nSoundID == SOUNDID_PC_9801_86_WSS ||
      g_nSoundID == SOUNDID_PC_9801_86_118 || g_nSoundID == SOUNDID_WAVESTAR ||
      g_nSoundID == SOUNDID_PC_9801_86_WSS_SB16 ||
  ...
  if (g_nSoundID == SOUNDID_PC_9801_86_WSS ||
      g_nSoundID == SOUNDID_MATE_X_PCM || g_nSoundID == SOUNDID_WSS_SB16 ||
      g_nSoundID == SOUNDID_PC_9801_86_WSS_SB16) {
    // OPNAタイマーをセットしない
    ...
    if (g_nSoundID == SOUNDID_PC_9801_86_118 ||
      ...
      UINT8 nIrq86 = (np2cfg.snd86opt & 0x10) | ((np2cfg.snd86opt & 0x4) << 5) |
...
void board118_bind(void) {
  ...
  if (g_nSoundID == SOUNDID_PC_9801_86_WSS ||
      g_nSoundID == SOUNDID_MATE_X_PCM || g_nSoundID == SOUNDID_WAVESTAR ||
