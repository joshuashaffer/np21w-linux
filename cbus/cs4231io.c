...
void cs4231io_reset(void) {
  ...
  */
  if (g_nSoundID == SOUNDID_PC_9801_86_WSS ||
    ...
    // UINT8 irq86table[4] = {0x03, 0x0d, 0x0a, 0x0c};
    // UINT8 nIrq86 = (np2cfg.snd86opt & 0x10) | ((np2cfg.snd86opt & 0x4) << 5)
    // | ((np2cfg.snd86opt & 0x8) << 3); UINT8 irq86 = irq86table[nIrq86 >> 6];
  ...
  if (g_nSoundID == SOUNDID_PC_9801_86_WSS ||
      ...
      g_nSoundID == SOUNDID_PC_9801_86_118_SB16) {
    cs4231.port[1] =
        0xb460; // Sound ID I/O port (A460hは86音源が使うのでB460hに変更)
  } else {
...
void cs4231io_bind(void) {
  ...
  sound_streamregist(
      &cs4231, (SOUNDCB)cs4231_getpcm); // CS4231用 オーディオ再生ストリーム
  ...
  if (!(g_nSoundID == SOUNDID_PC_9801_86_WSS ||
    ...
    //		iocore_attachinp(0x52ef,
    srnf_i52ef);//f40等を読み書きしたあとここを読んでエラー
                    iocore_attachinp(0x56ef,
    srnf_i56ef);//2番めに読まれて割り込み等の設定？　4番めに2回読まれ直す
                    iocore_attachinp(0x57ef, srnf_i57ef);//5番めに読まれる
                    iocore_attachinp(0x59ef,
    srnf_i59ef);//3番めに読まれて何か調査 ３と４でとりあえず通る
...
void cs4231io_unbind(void) {
  ...
  if (!(g_nSoundID == SOUNDID_PC_9801_86_WSS ||
    ...
    iocore_detachinp(0x52ef);//f40等を読み書きしたあとここを読んでエラー
                    ...
                    iocore_detachinp(0x59ef);//3番めに読まれて何か調査
    ３と４でとりあえず通る
...
void IOOUTCALL cs4231io0_w8(UINT port, REG8 value) {
  ...
  case 0x00: // PCM音源の割り込みアドレス設定
    ...
    if (cs4231.dmach != 0xff) {
      if ((cs4231.adrs >> 2) & 1) {
        if (cs4231.dmach == 0)
          dmac_attach(DMADEV_NONE, 1);
        else
          dmac_attach(DMADEV_NONE, 0);
      }
      dmac_attach(DMADEV_CS4231, cs4231.dmach); // CS4231のDMAチャネルを割り当て
#if 0
  ...
  case 0x04: // Index Address Register (R0) INIT MCE TRD IA4 IA3 IA2 IA1 IA0
    if (!(cs4231.index & MCE) && (value & MCE) &&
        (cs4231.reg.iface & (CAL0 | CAL1)))
      acicounter = 1;
    if (!(cs4231.index & MCE))
      cs4231.intflag |= (PRDY | CRDY);
  ...
  case 0x06: // Status Register (R2, Read Only) CU/L CL/R CRDY SER PU/L PL/R
             // PRDY INT
  ...
  case 0x07: // Capture I/O Data Register (R3, Read Only) CD7 CD6 CD5 CD4 CD3
             // CD2 CD1 CD0
...
REG8 IOINPCALL cs4231io0_r8(UINT port) {
  ...
  case 0x05: // Indexed Data Register (R1) ID7 ID6 ID5 ID4 ID3 ID2 ID1 ID0
  {
    ...
    case 0x0b: // Error Status and Initialization (I11, Read Only) COR PUR ACI
               // DRS ORR1 ORR0 ORL1 ORL0
      if (acicounter) {
        TRACEOUT(("acicounter"));
        acicounter -= 1;
