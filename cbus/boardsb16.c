...
static UINT8 seq[] = {
    0x60, 0x80, 0xff,
    0x21}; // XXX: Win2kドライバのチェックを通すためだけの暫定シーケンス
...
static void IOOUTCALL sb16_o0400(UINT port, REG8 dat) { port = dat; }
static void IOOUTCALL sb16_o0500(UINT port, REG8 dat) { port = dat; }
static void IOOUTCALL sb16_o0600(UINT port, REG8 dat) { port = dat; }
static void IOOUTCALL sb16_o0700(UINT port, REG8 dat) { port = dat; }
...
static void IOOUTCALL sb16_o2100(UINT port, REG8 dat) {
  ...
  opl3_writeRegister(&g_opl3[G_OPL3_INDEX], g_opl3[G_OPL3_INDEX].s.addrl,
                     dat); // Key Display用
#endif
...
static void IOOUTCALL sb16_o2300(UINT port, REG8 dat) {
  ...
  opl3_writeExtendedRegister(&g_opl3[G_OPL3_INDEX],
...
static REG8 IOINPCALL sb16_i0400(UINT port) { return 0xff; }
static REG8 IOINPCALL sb16_i0500(UINT port) { return 0; }
static REG8 IOINPCALL sb16_i0600(UINT port) { return 0; }
static REG8 IOINPCALL sb16_i0700(UINT port) { return 0; }
...
static void IOOUTCALL gameport_o4d2(UINT port, REG8 dat) {
  ...
  if (CPU_REMCLOCK > 0) {
    gameport_tsc = CPU_MSR_TSC -
                   (UINT64)CPU_REMCLOCK * pccore.maxmultiple / pccore.multiple;
  } else {
    gameport_tsc = CPU_MSR_TSC;
  }
...
static REG8 IOINPCALL gameport_i4d2(UINT port) {
...
#if defined(SUPPORT_IA32_HAXM)
  if (gameport_useqpc) {
    // li.QuadPart = li.QuadPart * GAMEPORT_JOYCOUNTER_TMPCLK /
    // gameport_qpf.QuadPart;
    clockdiff =
        (unsigned long long)(li.QuadPart -
                             gameport_tsc); // * GAMEPORT_JOYCOUNTER_TMPCLK /
                                            // gameport_qpf.QuadPart;
  ...
  gameport_clkmax =
      pccore.baseclock * pccore.maxmultiple / 1430; // とりあえず0.7msで･･･
#if !defined(SUPPORT_IA32_HAXM)
  if (gameport_tsccounter == 0 || !np2cfg.consttsc) {
    gameport_clkmax = gameport_clkmax * pccore.maxmultiple /
                      pccore.multiple; // CPUクロック依存の場合のfix
  }
#endif
#else
  gameport_clkmax = 32;
...
#endif
    outbuf[0] += (SINT32)(((s1l << 1) * oplfm_volume * midivolL / 255 *
                           (SINT32)g_sb16.mixregexp[MIXER_MASTER_LEFT] / 255) >>
                          6);
    outbuf[1] +=
        (SINT32)(((s1r << 1) * oplfm_volume * midivolR / 255 *
                  (SINT32)g_sb16.mixregexp[MIXER_MASTER_RIGHT] / 255) >>
                 6);
    outbuf += 2;
  }
}
...
void boardsb16_bind(void) {
  ...
  iocore_attachout(0x2000 + g_sb16.base,
                   sb16_o2000); /* FM Music Register Address Port */
  iocore_attachout(0x2100 + g_sb16.base, sb16_o2100); /* FM Music Data Port */
  iocore_attachout(0x2200 + g_sb16.base,
                   sb16_o2200); /* Advanced FM Music Register Address Port */
  iocore_attachout(0x2300 + g_sb16.base,
                   sb16_o2300); /* Advanced FM Music Data Port */
  iocore_attachout(0x2800 + g_sb16.base,
                   sb16_o2800); /* FM Music Register Port */
  ...
  iocore_attachinp(0x2200 + g_sb16.base,
                   sb16_i2200); /* Advanced FM Music Status Port */
  ...
  iocore_attachout(0xC800 + g_sb16.base,
                   sb16_o2000); /* FM Music Register Address Port */
  ...
  iocore_attachout(0xCA00 + g_sb16.base,
                   sb16_o2200); /* Advanced FM Music Register Address Port */
  iocore_attachinp(0xCA00 + g_sb16.base,
                   sb16_i2200); /* Advanced FM Music Status Port */
  iocore_attachout(0xCB00 + g_sb16.base,
                   sb16_o2300); /* Advanced FM Music Data Port */
...
void boardsb16_unbind(void) {
  ...
  iocore_detachout(0x2200 +
                   g_sb16.base); /* Advanced FM Music Register Address Port */
