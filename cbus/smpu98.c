...
enum { MIDIE_STEP = 0x01, MIDIE_EVENT = 0x02, MIDIE_DATA = 0x04 };
...
static const UINT8 shortmsgleng[0x10] = {0, 0, 0, 0, 0, 0, 0, 0,
                                         3, 3, 3, 3, 2, 2, 3, 1};

static const UINT8 hclk_step1[4][4] = {
...
static void smpu98_writeimm_r(UINT16 dat) {
  if (smpu98.native_immbuf_r_len < SMPU_IMMBUF_R_LEN) {
    ...
    if (smpu98.native_immbuf_r_len >= SMPU_IMMBUF_R_LEN) {
      // smpu98.native_status |= SMPU_STATUS_IRR; // XXX:
      // これで良いのか分からない
    }
...
static void sendmpushortmsg(const UINT8 *dat, UINT count) {
  ...
  if (smpu98.mode == SMPUMODE_NATIVE) {
    for (i = 0; i < count; i++) {
      cm_smpu98[smpu98.native_portnum]->write(cm_smpu98[smpu98.native_portnum],
                                              dat[i]);
    }
  } else {
    for (i = 0; i < count; i++) {
        ...
        if (!tr->step) {
          if ((tr->datas) && (tr->remain == 0)) {
            ...
            if (tr->data[0] == MIDI_STOP) {
              ...
              if (smpu98.portBready && !smpu98.muteB)
                cm_smpu98[1]->write(cm_smpu98[1], MIDI_STOP);
            ...
            for (i = 0; i < tr->datas; i++) {
              ...
              if (smpu98.portBready && !smpu98.muteB)
                cm_smpu98[1]->write(cm_smpu98[1], tr->data[i]);
            }
            tr->datas = 0;
          }
...
static REG8 mpucmd_md(REG8 cmd) { // 00-2F: Mode
  ...
  case 2: // Start Play
    ...
    if (!nevent_iswork(NEVENT_MIDIINT)) {
      nevent_set(NEVENT_MIDIINT, smpu98.stepclock, smpu98_midiint,
                 NEVENT_ABSOLUTE);
    }
    break;
  }
...
static const MPUCMDFN mpucmds[0x100] = {
    ...
    mpucmd_xx, mpucmd_md, mpucmd_md, mpucmd_md, mpucmd_xx, mpucmd_md,
    mpucmd_md, mpucmd_md, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx,
    ...
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx,
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_3f,

    mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr, // 40
    mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr,
    mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr, mpucmd_sr,
    ...
    mpucmd_mm, mpucmd_mm, mpucmd_8x, mpucmd_8x, mpucmd_8x, mpucmd_8x,
    mpucmd_8x, mpucmd_8x, mpucmd_8x, mpucmd_8x, mpucmd_8x, mpucmd_8x,

    mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x, // 90
    mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x,
    mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x, mpucmd_9x,

    mpucmd_rq, mpucmd_rq, mpucmd_rq, mpucmd_rq, // a0
    mpucmd_rq, mpucmd_rq, mpucmd_rq, mpucmd_rq, mpucmd_xx, mpucmd_xx,
    mpucmd_xx, mpucmd_rq, mpucmd_rq, mpucmd_rq, mpucmd_xx, mpucmd_rq,

    mpucmd_xx, mpucmd_b1, mpucmd_xx, mpucmd_xx, // b0
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_b8, mpucmd_xx,
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx,

    mpucmd_xx, mpucmd_xx, mpucmd_tb, mpucmd_tb, // c0
    mpucmd_tb, mpucmd_tb, mpucmd_tb, mpucmd_tb, mpucmd_tb, mpucmd_xx,
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx,

    mpucmd_ws, mpucmd_ws, mpucmd_ws, mpucmd_ws, // d0
    mpucmd_ws, mpucmd_ws, mpucmd_ws, mpucmd_ws, mpucmd_xx, mpucmd_xx,
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_df,

    mpucmd_fo, mpucmd_fo, mpucmd_fo, mpucmd_xx, // e0
    mpucmd_fo, mpucmd_xx, mpucmd_fo, mpucmd_fo, mpucmd_xx, mpucmd_xx,
    mpucmd_xx, mpucmd_xx, mpucmd_fo, mpucmd_fo, mpucmd_fo, mpucmd_fo,

    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, // f0
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx,
    mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_xx, mpucmd_fe, mpucmd_ff};
...
void IOOUTCALL smpu98_o0(UINT port, REG8 dat) {
  ...
  if (cm_smpu98[0]->connect != COMCONNECT_OFF) {
    if (smpu98.mode == SMPUMODE_UART) {
      ...
      if (smpu98.portBready && !smpu98.muteB)
        cm_smpu98[1]->write(cm_smpu98[1], (UINT8)dat);
...
void IOOUTCALL smpu98_o4(UINT port, UINT16 dat) {
  ...
  if (cm_smpu98[0]->connect != COMCONNECT_OFF &&
      cm_smpu98[1]->connect != COMCONNECT_OFF) {
  ...
  if (cm_smpu98[0]->connect != COMCONNECT_OFF &&
    ...
    if ((smpu98.native_status & SMPU_STATUS_SRR) == 0) {
      if (smpu98.native_seqbuf_w_len < SMPU_SEQBUF_W_LEN) {
        ...
        if (smpu98.native_seqbuf_w_len >= SMPU_SEQBUF_W_LEN) {
          // smpu98.native_status |= SMPU_STATUS_SRR; // XXX:
          // これで良いのか分からない
        }
      }
    }
      ...
      while (smpu98.native_immbuf_w_pos < smpu98.native_immbuf_w_len) {
        ...
        if (msg & 0x80) {
          ...
          if (msg == 0xf7) {
            ...
            sent += cm_smpu98[smpu98.native_portnum]->write(
                cm_smpu98[smpu98.native_portnum], (w1 >> 8) & 0xff);
            ...
            if (smpu98.native_lastmsg == 0xf0) {
              // タイムスタンプとかを間に挟めるらしい。Continue
              // Exclusiveが送られてきたらExclusive送信再開
            }
          } else {
            // chを入れて送る
            smpu98.native_linenum = (w1 & 0xff);
            smpu98.native_portnum = (smpu98.native_linenum > 0xf ? 1 : 0);
            sent += cm_smpu98[smpu98.native_portnum]->write(
                cm_smpu98[smpu98.native_portnum],
                (w1 >> 8) & 0xf0 | (smpu98.native_linenum & 0xf));
            smpu98.native_runningmsg = msg;
          }
        } else {
          msg = smpu98.native_lastmsg; // 直近のメッセージの続き
          if (!(msg == 0xf7) && !(0xf0 <= msg && msg <= 0xff || msg == 0xef) &&
              (msg & 0x0f)) {
            // とりあえず無視
          } else {
            switch (msg) {
            ...
            default:
              ...
              sent += cm_smpu98[smpu98.native_portnum]->write(
                  cm_smpu98[smpu98.native_portnum], (w1 >> 8) & 0xff);
              sent += cm_smpu98[smpu98.native_portnum]->write(
                  cm_smpu98[smpu98.native_portnum], (w1)&0xff);
              if ((w1 & 0xff) == 0xf7) {
                smpu98.native_lastmsg = smpu98.native_runningmsg;
              }
            }
...
UINT16 IOINPCALL smpu98_ia(UINT port) {
  ...
  if (cm_smpu98[0]->connect != COMCONNECT_OFF &&
      cm_smpu98[1]->connect != COMCONNECT_OFF) {
    ret = smpu98.native_status & ~(SMPU_INTMASK_ALL << 9) |
          (smpu98.native_intmask << 9);
    // if ((smpu98.r.cnt == 0) && (smpu98.intreq == 0)) {
    //	ret |= MIDIIN_AVAIL;
    //}
    // TRACEOUT(("smpu98 inp %.4x %.2x", port, ret));
...
void(IOOUTCALL *smpu98_io16outfunc[])(UINT port, UINT16 dat) = {
    NULL,      NULL, NULL,      NULL, smpu98_o4, NULL, smpu98_o6, NULL,
    smpu98_o8, NULL, smpu98_oa, NULL, NULL,      NULL, NULL,      NULL,
};
UINT16(IOINPCALL *smpu98_io16inpfunc[])
(UINT port) = {
    NULL,      NULL, NULL,      NULL, smpu98_i4, NULL, smpu98_i6, NULL,
    smpu98_i8, NULL, smpu98_ia, NULL, NULL,      NULL, NULL,      NULL,
};
  ...
  if (smpu98.native_immread_phase[port] == 0) {
    ...
    if (dat & 0x80) {
      ...
      if (dat == 0xf7) {
        ...
        smpu98.native_immread_portbuf[port] |=
            (port == 0 ? 0x00 : 0x10) | (dat & 0x0f); // ch→ラインナンバー変換
        smpu98_writeimm_r(smpu98.native_immread_portbuf[port]);
      }
    } else {
      UINT8 oldmsg =
          smpu98.native_immread_lastmsg[port]; // 直近のメッセージの続き
      smpu98.native_immread_portbuf[port] = ((UINT16)dat) << 8;
      switch (oldmsg) {
      case 0xc0:
      case 0xd0:
...
void smpu98_callback(void) {
  ...
  if (smpu98.mode == SMPUMODE_NATIVE) {
    if (cm_smpu98[0]) {
      while ((SMPU_IMMBUF_R_LEN < smpu98.native_immbuf_r_len) &&
    ...
    if (cm_smpu98[1]) {
      while ((SMPU_IMMBUF_R_LEN < smpu98.native_immbuf_r_len) &&
             (cm_smpu98[1]->read(cm_smpu98[1], &data))) {
        smpu98_callback_port(data, 0);
      }
    }
  } else {
