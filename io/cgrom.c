...
static void IOOUTCALL cgrom_oa9(UINT port, REG8 dat) {
  ...
  if ((cr->code & 0x007e) == 0x0056) {
    fontrom[((cr->code & 0x7f7f) << 4) + cr->lr + (cr->line & 0x0f)] =
        (UINT8)dat;
...
static const IOOUT cgromoa1[8] = {cgrom_oa1, cgrom_oa3, cgrom_oa5, NULL,
                                  cgrom_oa9, NULL,      NULL,      NULL};

static const IOINP cgromia1[8] = {NULL,      NULL, NULL, NULL,
