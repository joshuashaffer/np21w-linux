...
enum { FDC_DMACH2HD = 2, FDC_DMACH2DD = 3 };

static const UINT8 FDCCMD_TABLE[32] = {0, 0, 8, 2, 1, 8, 8, 1, 0, 8, 1,
                                       0, 8, 5, 0, 2, 0, 8, 0, 0, 0, 0,
                                       0, 0, 0, 8, 0, 0, 0, 8, 0, 0};
...
static void fdc_interruptreset(void) { fdc.intreq = FALSE; }

static BOOL fdc_isfdcinterrupt(void) { return (fdc.intreq); }
...
static void FDC_WriteData(void) { // cmd: 05
  ...
  case FDCEVENT_CMDRECV:
    ...
    fdc.stat[fdc.us] = (fdc.hd << 2) | fdc.us;
    fdc.treg[fdc.us] =
        fdc.C; /* 170101 ST modified to work on Windows 9x/2000 */
    if (FDC_DriveCheck(TRUE)) {
      fdc.event = FDCEVENT_BUFRECV;
      fdc.bufcnt = 128 << fdc.N;
      fdc.bufp = 0;
...
static REG8 IOINPCALL fdc_i4be(UINT port) {
  ...
  (void)port;
  TRACEOUT(("fdc in %.2x %.2x [%.4x:%.4x]", port,
            (fdc.rpm[(fdc.reg144 >> 5) & 3] | 0xf0), CPU_CS, CPU_IP));
  return (fdc.rpm[(fdc.reg144 >> 5) & 3] | 0xfe);
}

// ---- I/F

static const IOOUT fdco90[4] = {NULL, fdc_o92, fdc_o94, NULL};
static const IOINP fdci90[4] = {fdc_i90, fdc_i92, fdc_i94, NULL};
