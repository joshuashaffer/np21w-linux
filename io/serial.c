...
void keyboard_callback(NEVENTITEM item) {
  ...
  if (item->flag & NEVENT_SETEVENT) {
    if ((keybrd.ctrls) || (keybrd.buffers)) {
      ...
      pic_setirq(1);
      nevent_set(NEVENT_KEYBOARD, keybrd.xferclock, keyboard_callback,
                 NEVENT_RELATIVE);
    }
  }
}
...
static const IOOUT keybrdo41[2] = {keyboard_o41, keyboard_o43};

static const IOINP keybrdi41[2] = {keyboard_i41, keyboard_i43};
...
void keyboard_changeclock(void) { keybrd.xferclock = pccore.realclock / 1920; }
...
void rs232c_construct(void) { cm_rs232c = NULL; }
...
void rs232c_callback(void) {
  ...
  if ((bufused > 0 && rs232c_removecounter == 0) ||
      bufused == RS232C_BUFFER - 1) {
    rs232c_buf_rpos =
        (rs232c_buf_rpos + 1) & RS232C_BUFFER_MASK; // 一番古いものを捨てる
  }
  if ((cm_rs232c) &&
...
#endif
  if (rs232c_buf_rpos != rs232c_buf_wpos) {
    rs232c_buf_rpos = (rs232c_buf_rpos + 1) &
                      RS232C_BUFFER_MASK; // バッファ読み取り位置を1進める
  }
#if defined(SUPPORT_RS232C_FIFO)
  if (port == 0x130) {
...
static REG8 IOINPCALL rs232c_i132(UINT port) {
  ...
  ret = rs232c.result;
  ret = (ret & ~0xf7) | ((rs232c.result << 1) & 0x6) |
        ((rs232c.result >> 2) & 0x1);
...
#if defined(SUPPORT_RS232C_FIFO)
static REG8 IOINPCALL rs232c_i134(UINT port) { return (0x00); }
...
static const IOOUT rs232co30[2] = {rs232c_o30, rs232c_o32};

static const IOINP rs232ci30[2] = {rs232c_i30, rs232c_i32};
