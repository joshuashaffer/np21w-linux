...
void pcidev_cbusbridge_cfgreg_w(UINT32 devNumber, UINT8 funcNumber,
                                UINT8 cfgregOffset, UINT8 sizeinbytes,
                                UINT32 value) {}

void pcidev_cbusbridge_reset(const NP2CFG *pConfig) { (void)pConfig; }
...
void pcidev_cbusbridge_bind(void) {
  ...
  // pcidev.devices[devid].header.classcode[0] = 0x00; //
  // レジスタレベルプログラミングインタフェース
  // pcidev.devices[devid].header.classcode[1] = 0x80; // サブクラスコード
  // pcidev.devices[devid].header.classcode[2] = 0x06; // ベースクラスコード
  // pcidev.devices[devid].header.cachelinesize = 0;
  // pcidev.devices[devid].header.latencytimer = 0x0;
