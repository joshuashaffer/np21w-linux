...
enum { IOFUNC_SYS = 0x01, IOFUNC_SND = 0x02, IOFUNC_EXT = 0x04 };
...
static void detachout(IOFUNC iof, UINT port) { iof->ioout[port] = &defout8; }
...
static void detachinp(IOFUNC iof, UINT port) { iof->ioinp[port] = &definp8; }
...
void iocore_attachcmnoutex(UINT port, UINT mask, const IOOUT *func,
                           UINT funcs) {
...
void iocore_attachsysoutex(UINT port, UINT mask, const IOOUT *func,
                           UINT funcs) {
...
void iocore_attachcmninpex(UINT port, UINT mask, const IOINP *func,
                           UINT funcs) {
...
void iocore_attachsysinpex(UINT port, UINT mask, const IOINP *func,
                           UINT funcs) {
...
BRESULT iocore_build(void) {
  ...
  for (i = 0; i < 256; i++) {
    ...
    if (!(i &
          (0x0c |
           (np2cfg.sysiomsk >>
            8)))) { // PC-9821用 追加マスク(0x0c -> 0xff) np21w ver0.86 rev37
#else
    if (!(i & 0x0c)) {
#endif
      ioc->base[i] = sys;
...
void IOOUTCALL iocore_out16(UINT port, REG16 dat) {
  ...
  if (np2clvga.enabled && cirrusvga_opaque) {
    if ((np2clvga.gd54xxtype & CIRRUS_98ID_WABMASK) == CIRRUS_98ID_WAB ||
        (np2clvga.gd54xxtype & CIRRUS_98ID_GA98NBMASK) ==
