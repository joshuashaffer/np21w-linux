...
static const _PICITEM def_master = {
    {0x11, 0x08, 0x80, 0x1d}, 0x7d, 0, 0, 0, 0, 0};

static const _PICITEM def_slave = {
    {0x11, 0x10, 0x07, 0x09}, 0x71, 0, 0, 0, 0, 0};
...
void pic_irq(void) { // ver0.78
  ...
  if (p->pi[0].icw[2] & bit) { // スレーヴ
    ...
    if (!(p->pi[1].isr & bit)) {
      ...
      p->pi[1].isr |= bit;
      p->pi[1].irr &= ~bit;
      //			TRACEOUT(("hardware-int %.2x", (p->pi[1].icw[1]
      //& 0xf8) | num));
      CPU_INTERRUPT((REG8)((p->pi[1].icw[1] & 0xf8) | num), 0);
    }
  } else if (!(p->pi[0].isr & bit)) { // マスター
...
static const IOOUT pico00[2] = {pic_o00, pic_o02};

static const IOINP pici00[2] = {pic_i00, pic_i02};
#else
static const IOOUT pico00[4] = {pic_o00, pic_o02, NULL, NULL};

static const IOINP pici00[4] = {pic_i00, pic_i02, NULL, NULL};
#endif
