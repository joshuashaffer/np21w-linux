...
// 現状はConfiguration Mechanism
// #1対応でバス番号0のみ。bios1a.cにエミュレーションPCI BIOSがあります。
...
#define SETCFGREG_B_MASK(reg, ofs, value, mask)                                \
  PCI_SETCFGREG_B_MASK(reg, ofs, value, mask)
#define SETCFGREG_W_MASK(reg, ofs, value, mask)                                \
  PCI_SETCFGREG_W_MASK(reg, ofs, value, mask)
#define SETCFGREG_D_MASK(reg, ofs, value, mask)                                \
  PCI_SETCFGREG_D_MASK(reg, ofs, value, mask)
...
void pcidev_pcmc_cfgreg_w(UINT32 devNumber, UINT8 funcNumber,
                          UINT8 cfgregOffset, UINT8 sizeinbytes, UINT32 value) {
...
static void IOOUTCALL pci_o0cf9(UINT port, REG8 dat) { pcidev.reg_trc = dat; }
static void IOOUTCALL pci_o0cfa(UINT port, REG8 dat) { pcidev.reg_fwd = dat; }
static void IOOUTCALL pci_o0cfb(UINT port, REG8 dat) { pcidev.reg_cms = dat; }

static REG8 IOINPCALL pci_i0cf8(UINT port) { return pcidev.reg_cse; }
static REG8 IOINPCALL pci_i0cf9(UINT port) { return pcidev.reg_trc; }
static REG8 IOINPCALL pci_i0cfa(UINT port) { return pcidev.reg_fwd; }
static REG8 IOINPCALL pci_i0cfb(UINT port) { return pcidev.reg_cms; }
...
void IOOUTCALL pcidev_w8_0xcfc(UINT port, UINT8 value) {
  ...
  if (pcidev.reg32_caddr & 0x80000000) {
    ...
    if (!pcidev.enable && idselSelect != 0)
      return;
    if (busNumber == 0) {
      if (pcidev.devices[idselSelect].enable /* && funcNumber==0*/) {
        UINT32 mask =
            GETCFGREG_B(pcidev.devices[idselSelect].cfgreg8rom, cfgregOffset);
        SETCFGREG_B_MASK(pcidev.devices[idselSelect].cfgreg8, cfgregOffset,
                         value, mask);
        if (pcidev.devices[idselSelect].regwfn) {
          (*pcidev.devices[idselSelect].regwfn)(idselSelect, funcNumber,
                                                cfgregOffset, 1, value);
        }
      }
    }
...
void IOOUTCALL pcidev_w16_0xcfc(UINT port, UINT16 value) {
  ...
  if (pcidev.reg32_caddr & 0x80000000) {
    ...
    if (busNumber == 0) {
      if (pcidev.devices[idselSelect].enable /* && funcNumber==0*/) {
        UINT32 mask =
            GETCFGREG_W(pcidev.devices[idselSelect].cfgreg8rom, cfgregOffset);
        SETCFGREG_W_MASK(pcidev.devices[idselSelect].cfgreg8, cfgregOffset,
                         value, mask);
        if (pcidev.devices[idselSelect].regwfn) {
          (*pcidev.devices[idselSelect].regwfn)(idselSelect, funcNumber,
                                                cfgregOffset, 2, value);
        }
      }
    }
    if (idselSelect == 0 && cfgregOffset == 0x64) {
  ...
  if (port == 0xcf8) {
    ...
    if (pcidev.reg32_caddr & 0x80000000) {
      ...
      if (busNumber == 0) {
        if (pcidev.devices[idselSelect].enable /* && funcNumber==0*/) {
          UINT32 mask =
              GETCFGREG_D(pcidev.devices[idselSelect].cfgreg8rom, cfgregOffset);
          SETCFGREG_D_MASK(pcidev.devices[idselSelect].cfgreg8, cfgregOffset,
                           value, mask);
          if (pcidev.devices[idselSelect].regwfn) {
            (*pcidev.devices[idselSelect].regwfn)(idselSelect, funcNumber,
                                                  cfgregOffset, 4, value);
          }
        }
      }
      if (idselSelect == 0 && cfgregOffset == 0x64) {
...
static REG8 IOINPCALL pci_i063c(UINT port) { return pcidev.membankd8; }
...
static REG8 IOINPCALL pci_i18f0(UINT port) { return pcidev.unkreg_bank1; }
...
static REG8 IOINPCALL pnp_iReg(UINT port) { return 0xff; }
static void IOOUTCALL pnp_o259(UINT port, REG8 dat) {

  pnp_addr = dat;
  (void)port;
}
static REG8 IOINPCALL pnp_i259(UINT port) { return pnp_addr; }
...
void pcidev_reset(const NP2CFG *pConfig) {
  ...
  for (i = 0; i < PCI_DEVICES_MAX; i++) {
    ...
    pcidev.devices[i].regwfn =
        NULL; // Configuration register書き込み時に呼ばれる
  ...
  if (pcidev.enable) {
    ...
    pcidev.devices[devid].header.classcode[0] =
    ...
    pcidev.devices[devid].cfgreg8[0x53] =
        0x00; // ホスト読取り／書込みバッファ制御(HBC)
    pcidev.devices[devid].cfgreg8[0x54] =
        0x00; // PCI読取り／書込みバッファ制御(PBC)
    pcidev.devices[devid].cfgreg8[0x55] =
        0x00; // 2次キャッシュ制御拡張レジスタ(SCCE)
    ...
    pcidev.devices[devid].cfgreg8[0x60] =
        0x10; // DRAMロー境界レジスタ(DRB) ROW#0
    ...
    pcidev.devices[devid].cfgreg8[0x64] =
        0x20; // ????? pcidev.devices[0].cfgreg8[0x63]; // ROW#0〜4
    pcidev.devices[devid].cfgreg8[0x65] =
        0x20; // ????? pcidev.devices[0].cfgreg8[0x65]; // ROW#0〜5
    ...
    SETCFGREG_W(pcidev.devices[devid].cfgreg8, 0x78,
                0x0000); // メモリ空間ギャップ(MSG)
    SETCFGREG_D(pcidev.devices[devid].cfgreg8, 0x7C,
                0x00000000); // フレームバッファ領域(FBR)
...
void pcidev_updateRoutingTable() {
  ...
  for (i = 1; i < PCI_DEVICES_MAX; i++) { // 0は除く
    if (pcidev.devices[i].enable && !pcidev.devices[i].skipirqtbl) {
      ...
      pcidev.biosdata.data[pcidev.biosdata.datacount].devicenumber =
          (i << 3); // デバイス番号
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intA =
          (pcidev.devices[i].header.interruptpin == 1
               ? pcidev.devices[i].header.interruptline
               : 0); // このデバイスのINT#A(PCI仕様上はデバイス毎に独立)の接続先。0=接続無し,
                     // 1=PIRQ#0, 2=PIRQ#1, 3=PIRQ#2, 4=PIRQ#3,
                     // 5以上:その他接続（同じ番号同士が接続）
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intA =
          allpciirq; // このデバイスのINT#Aで使用できるIRQ(PIRQではない)のマップ。bit0=IRQ0,
                     // bit1=IRQ1, ... , bit15=IRQ15に対応
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intB =
          (pcidev.devices[i].header.interruptpin == 2
               ? pcidev.devices[i].header.interruptline
               : 0); // このデバイスのINT#B(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intB =
          allpciirq; // このデバイスのINT#Bで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intC =
          (pcidev.devices[i].header.interruptpin == 3
               ? pcidev.devices[i].header.interruptline
               : 0); // このデバイスのINT#C(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intC =
          allpciirq; // このデバイスのINT#Cで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intD =
          (pcidev.devices[i].header.interruptpin == 4
               ? pcidev.devices[i].header.interruptline
               : 0); // このデバイスのINT#D(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intD =
          allpciirq; // このデバイスのINT#Dで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].slot =
          pcidev.devices[i].slot; // PCIスロット番号（0はオンボード）
      pcidev.allirqbitmap |=
          pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intA |
          pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intB |
          pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intC |
          pcidev.biosdata.data[pcidev.biosdata.datacount]
              .irqmap4intD; // PCIで使用できる全IRQのマップ？ bit0=IRQ0,
                            // bit1=IRQ1, ... , bit15=IRQ15に対応
      ...
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intA =
          ((i - 12) & 0x3) +
          1; // このデバイスのINT#A(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intA =
          allpciirq; // このデバイスのINT#Aで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intB =
          ((i - 12 + 1) & 0x3) +
          1; // このデバイスのINT#B(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intB =
          allpciirq; // このデバイスのINT#Bで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intC =
          ((i - 12 + 2) & 0x3) +
          1; // このデバイスのINT#C(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intC =
          allpciirq; // このデバイスのINT#Cで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].link4intD =
          ((i - 12 + 3) & 0x3) +
          1; // このデバイスのINT#D(PCI仕様上はデバイス毎に独立)の接続先。
      pcidev.biosdata.data[pcidev.biosdata.datacount].irqmap4intD =
          allpciirq; // このデバイスのINT#Dで使用できるIRQ(PIRQではない)のマップ。
      pcidev.biosdata.data[pcidev.biosdata.datacount].slot =
          (i - 12) + 1; // PCIスロット番号（0はオンボード）
      pcidev.biosdata.datacount++;
    }
  }
}
...
void pcidev_updateBIOS32data() {
  UINT16 j;
  UINT8 checksum;
  UINT8 pcibios_BIOS32SD[16] = {0x5F, 0x33, 0x32, 0x5F, 0x00, 0x00, 0xFF, 0xFF,
                                0x00, 0x01, 0x0,  0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  /*
   ...
   ofs=A 1byte BIOS32 Service
   Directory全体のチェックサム（全バイトの和が0になるようにする） ofs=B 5byte
   予約（0にする）
  */
  ...
  if (pcidev.bios32svcdir != 0) {
    if (pcidev.bios32entrypoint != 0 && pcidev.usebios32) {
      ...
      // メモリに書き込む
      CopyMemory(mem + pcidev.bios32svcdir, pcibios_BIOS32SD,
                 sizeof(pcibios_BIOS32SD));
    } else {
      // BIOS32を潰す
