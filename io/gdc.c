...
// 31kHzの時の動作クロックが不明…
static const GDCCLK gdcclk[] = {{14318180 / 8, 112 - 8, 112 + 8, 200, 300},
...
static const UINT8 defsyncm15[8] = {0x10, 0x4e, 0x07, 0x25,
                                    0x0d, 0x0f, 0xc8, 0x94};
static const UINT8 defsyncs15[8] = {0x06, 0x26, 0x03, 0x11,
                                    0x86, 0x0f, 0xc8, 0x94};

static const UINT8 defsyncm24[8] = {0x10, 0x4e, 0x07, 0x25,
                                    0x07, 0x07, 0x90, 0x65};
static const UINT8 defsyncs24[8] = {0x06, 0x26, 0x03, 0x11,
                                    0x83, 0x07, 0x90, 0x65};
...
static const UINT16 defanapal[16] = {0x000, 0x007, 0x070, 0x077, 0x700, 0x707,
                                     0x770, 0x777, 0x444, 0x00f, 0x0f0, 0x0ff,
...
void gdc_analogext(BOOL extend) {
  ...
  if (extend) {
...
#if defined(SUPPORT_IA32_HAXM)
    i386hax_vm_setmemoryarea(vramex + ((vramop.mio1[0] & 15) << 15), 0xA8000,
                             0x8000);
    i386hax_vm_setmemoryarea(vramex + ((vramop.mio1[2] & 15) << 15), 0xB0000,
                             0x8000);
#endif
  } else {
    gdc.analog &= ~(1 << (GDCANALOG_256));
    vramop.operate &= ~(1 << VOPBIT_VGA);
#if defined(SUPPORT_IA32_HAXM)
    i386hax_vm_removememoryarea(vramex + ((vramop.mio1[0] & 15) << 15), 0xA8000,
                                0x8000);
    i386hax_vm_removememoryarea(vramex + ((vramop.mio1[2] & 15) << 15), 0xB0000,
                                0x8000);
#endif
  }
  gdcs.palchange = GDCSCRN_REDRAW;
...
static const IOOUT gdco60[8] = {gdc_o60, gdc_o62, gdc_o64, NULL,
                                gdc_o68, gdc_o6a, NULL,    gdc_o6e};

static const IOOUT gdcoa0[8] = {gdc_oa0, gdc_oa2, gdc_oa4, gdc_oa6,
                                gdc_oa8, gdc_oaa, gdc_oac, gdc_oae};

static const IOINP gdci60[8] = {gdc_i60, gdc_i62, NULL, NULL,
                                gdc_i68, gdc_i6a, NULL, NULL};

#if defined(SUPPORT_PC9821)
static const IOINP gdcia0[8] = {gdc_ia0, gdc_ia2, gdc_ia4, gdc_ia6,
