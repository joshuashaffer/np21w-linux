...
void rs232ctimer(NEVENTITEM item) {
  ...
  if (item->flag & NEVENT_SETEVENT) {
    ...
    if (rs232cfifo.vfast & 0x80) {
      ...
      nevent_set(NEVENT_RS232C, pccore.realclock * 8 / speed, rs232ctimer,
                 NEVENT_RELATIVE);
    } else
#endif
        if ((pitch->ctrl & 0x0c) == 0x04) {
...
void pit_setflag(PITCH pitch, REG8 value) {
  ...
  if (value & PIT_CTRL_RL) {
    ...
    pitch->flag &=
        ~(PIT_FLAG_R | PIT_FLAG_W | PIT_FLAG_L | PIT_FLAG_S | PIT_FLAG_C);
  } else { // latch
...
static const IOOUT pito71[4] = {pit_o71, pit_o73, pit_o75, pit_o77};

static const IOINP piti71[4] = {pit_i71, pit_i71, pit_i71, NULL};
