...
#define PCI_SETCFGREG_B_MASK(reg, ofs, value, mask)                            \
  (PCI_GETCFGREG_B(reg, ofs) =                                                 \
       (PCI_GETCFGREG_B(reg, ofs) & mask) | (value & ~mask))
#define PCI_SETCFGREG_W_MASK(reg, ofs, value, mask)                            \
  (PCI_GETCFGREG_W(reg, ofs) =                                                 \
       (PCI_GETCFGREG_W(reg, ofs) & mask) | (value & ~mask))
#define PCI_SETCFGREG_D_MASK(reg, ofs, value, mask)                            \
  (PCI_GETCFGREG_D(reg, ofs) =                                                 \
       (PCI_GETCFGREG_D(reg, ofs) & mask) | (value & ~mask))

// コンフィギュレーションレジスタ変更時に呼ばれる。
typedef void (*PCIREGWCB)(UINT32 devNumber, UINT8 funcNumber,
