...
#endif
  dmach->adrs.b[DMA32_HIGH + DMA16_HIGH] =
      0; // 25bitより上はここでは転送できない→0E05
  bank = dat;
  if (dmach->bound != 3) {
    dmach->startaddr = dmach->adrs.d;
...
static void IOOUTCALL dmac_o2b(UINT port, REG8 dat) { (void)port; }

// ---- I/F

static const IOOUT dmaco00[16] = {dmac_o01, dmac_o03, dmac_o01, dmac_o03,
                                  dmac_o01, dmac_o03, dmac_o01, dmac_o03,
                                  NULL,     dmac_o13, dmac_o15, dmac_o17,
                                  dmac_o19, dmac_o1b, dmac_o1d, dmac_o1f};

static const IOINP dmaci00[16] = {dmac_i01, dmac_i03, dmac_i01, dmac_i03,
                                  dmac_i01, dmac_i03, dmac_i01, dmac_i03,
                                  dmac_i11, NULL,     NULL,     NULL,
                                  NULL,     NULL,     NULL,     NULL};

static const IOOUT dmaco21[8] = {dmac_o21, dmac_o21, dmac_o21, dmac_o21,
...
static void dmacset(REG8 channel) {
  ...
  switch (dmadev) {
  case 0:
    TRACEOUT(("dmac set %d - dummy", channel));
    break;
  case 1:
    TRACEOUT(("dmac set %d - 2HD", channel));
    break;
  case 2:
    TRACEOUT(("dmac set %d - 2DD", channel));
    break;
  case 3:
    TRACEOUT(("dmac set %d - SASI", channel));
    break;
  case 4:
    TRACEOUT(("dmac set %d - SCSI", channel));
    break;
  case 5:
    TRACEOUT(("dmac set %d - cs4231p", channel));
    break;
#if defined(SUPPORT_SOUND_SB16)
  case 6:
    TRACEOUT(("dmac set %d - CT1741", channel));
    break;
#endif
  }
