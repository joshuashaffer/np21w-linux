...
static const MSYSITEM s_exit[] = {
    {str_sysr, NULL, 0, MENU_GRAY},           {str_sysm, NULL, 0, MENU_GRAY},
...
#endif
    {str_sysx, NULL, 0, MENU_GRAY},           {NULL, NULL, 0, MENU_SEPARATOR},
...
#endif
    {NULL, NULL, SID_CLOSE, MENUS_CLOSE | MENU_DELETED}};
...
static void bitemdraw(VRAMHDL vram, MENUHDL menu, int flag) {
  ...
  if (menutype == 0) {
    ...
    if (!(menu->flag & MENU_GRAY)) {
      ...
      vrammix_text(vram, font, menu->string, menucolor[MVC_GRAYTEXT2], &pt,
                   NULL);
    ...
    if (flag) {
      menuvram_box(vram, &menu->rct, MVC2(MVC_SHADOW, MVC_HILIGHT),
                   (flag == 2));
    }
  }
}
  ...
  while (menu) {
    if (!(menu->flag & (MENU_DISABLE | MENU_SEPARATOR))) {
      ...
      if (menutype == MENUS_POPUP) {
        ...
        menu->rct.top = mrect.top + ((MENUSYS_CYCAPTION - MENUSYS_CYCLOSE) / 2);
...
static void citemdraw2(VRAMHDL vram, MENUHDL menu, UINT mvc, int pos) {
  ...
  if (menu->flag & MENU_CHECKED) {
    pt.x = menu->rct.left + MENUSYS_SXITEM + pos, pt.y = menu->rct.top + pos;
...
static BRESULT childopn(MENUSYS *sys, int depth, int pos) {
  ...
  while (menu) {
    if (!(menu->flag & MENU_DISABLE)) {
      ...
      if (menu->flag & MENU_SEPARATOR) {
        if (height >
            (menubase.height - MENUSYS_CYSEP - (MENU_FBORDER + MENU_BORDER))) {
          break;
        }
        ...
        if (height >
            (menubase.height - MENUSYS_CYITEM - (MENU_FBORDER + MENU_BORDER))) {
          break;
        }
  ...
  if (dir == 1) {
    if ((parent.top < height) || (parent.bottom < (menubase.height - height))) {
      parent.top = parent.bottom;
    } else {
      parent.top -= height;
    }
  } else if (dir >= 2) {
    if ((parent.left < width) || (parent.right < (menubase.width - width))) {
...
static void defcmd(MENUID id) { (void)id; }
...
void menusys_moving(int x, int y, int btn) {
  ...
  if (cur.menu != NULL) {
    ...
    if (!(cur.menu->flag & MENU_GRAY)) {
      if (btn == 1) {
        if ((!sys->opened) && (cur.depth == 0) && (cur.menu->child != NULL)) {
          wndclose(sys, 1);
...
void menusys_setstyle(UINT16 style) { menusys.style = style; }
