...
static const UINT8 __pat[64] = {
    0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00, 0x11, 0x00, 0x44,
    0x00, 0x55, 0x00, 0x44, 0x00, 0x55, 0x00, 0x55, 0x00, 0x55, 0x22,
    0x55, 0x00, 0x55, 0x22, 0x55, 0x88, 0x55, 0xaa, 0x55, 0x88, 0x55,
    0xaa, 0x55, 0xaa, 0x77, 0xaa, 0x55, 0xaa, 0x77, 0xaa, 0xdd, 0xaa,
    0xff, 0xaa, 0xdd, 0xaa, 0xff, 0xaa, 0xff, 0xaa, 0xff, 0xbb, 0xff,
    0xaa, 0xff, 0xbb, 0xff, 0xee, 0xff, 0xff, 0xff, 0xee};
...
static void res3put16(VRAMHDL vram, const MENURES2 *res, RESPUT *rp, UINT mvc) {
...
static void captionbar16(VRAMHDL vram, const RECT_T *rect, UINT32 color1,
                         UINT32 color2) {
...
static void res3put24(VRAMHDL vram, const MENURES2 *res, RESPUT *rp, UINT mvc) {
...
static void captionbar24(VRAMHDL vram, const RECT_T *rect, UINT32 color1,
                         UINT32 color2) {
...
void menuvram_linex(VRAMHDL vram, int posx, int posy, int term, UINT mvc) {
  ...
  if ((vram == NULL) || (posy < 0) || (posy >= vram->height)) {
    goto mvlx_end;
  }
  if (posx < 0) {
    posx = 0;
...
void menuvram_liney(VRAMHDL vram, int posx, int posy, int term, UINT mvc) {
  ...
  if ((vram == NULL) || (posx < 0) || (posx >= vram->width)) {
    goto mvly_end;
  }
  if (posy < 0) {
    posy = 0;
...
void menuvram_base(VRAMHDL vram) {
  ...
  menuvram_box2(vram, &rct, MVC4(MVC_LIGHT, MVC_DARK, MVC_HILIGHT, MVC_SHADOW));
}
...
void menuvram_caption(VRAMHDL vram, const RECT_T *rect, UINT16 icon,
                      const OEMCHAR *caption) {
...
static void putbtn(VRAMHDL vram, const RECT_T *rect, const MENURES2 *res,
                   BOOL focus) {
  RECT_T rct;
  POINT_T pt;
  UINT mvc4;
