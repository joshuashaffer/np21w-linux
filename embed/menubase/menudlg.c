...
static void resattachicon(MENUDLG dlg, DLGPRM prm, UINT16 icon, int width,
                          int height) {
...
static void dlg_text(MENUDLG dlg, DLGHDL hdl, const POINT_T *pt,
                     const RECT_T *rect) {
  ...
  if (string) {
    if (!(hdl->flag & MENU_GRAY)) {
      ...
      vrammix_text(dlg->vram, hdl->c.dt.font, string, menucolor[MVC_GRAYTEXT2],
                   &p, rect);
      color = MVC_GRAYTEXT1;
    }
    vrammix_text(dlg->vram, hdl->c.dt.font, string, menucolor[color], &fp,
                 rect);
  }
}
...
static BRESULT dlgbase_create(MENUDLG dlg, DLGHDL hdl, const void *arg) {
  ...
  rct.right =
      hdl->rect.right - hdl->rect.left - ((MENU_FBORDER + MENU_BORDER) * 2);
...
static void dlglist_drawitem(DLGHDL hdl, DLGPRM prm, int focus, POINT_T *pt,
                             RECT_T *rct) {
...
static BOOL dlglist_setex(MENUDLG dlg, DLGHDL hdl, const ITEMEXPRM *arg) {
  ...
  resattachicon(dlg, dp, arg->icon, hdl->c.dl.fontsize, hdl->c.dl.fontsize);
...
static void dlgslider_paint(MENUDLG dlg, DLGHDL hdl) {
  ...
  if (!(hdl->flag & MSS_VERT)) {
    ...
    rct.r.top = hdl->rect.top + ptr + (hdl->c.ds.sldv / 2) - (MENU_LINE * 2);
    ...
    rct.r.left = hdl->rect.left + ptr + (hdl->c.ds.sldh / 2) - (MENU_LINE * 2);
...
static void dlgtablist_paint(MENUDLG dlg, DLGHDL hdl) {
  ...
  tabey = rct.top + hdl->c.dtl.fontsize + MENUDLG_SYTAB + MENUDLG_TYTAB +
          MENUDLG_EYTAB;
  rct.top = tabey;
  ...
  while (prm) {
    if (cnt) {
      ...
      pt.x += MENU_LINE;
      menuvram_liney(dst, pt.x, pt.y + MENU_LINE, pt.y + (MENU_LINE * 2),
                     MVC_HILIGHT);
      ...
      menuvram_liney(dst, lx, pt.y + MENU_LINE, pt.y + (MENU_LINE * 2),
                     MVC_DARK);
      ...
      pt.x += MENUDLG_TXTAB;
      pt.y += MENUDLG_TYTAB;
      vrammix_text(dst, hdl->c.dtl.font, prm->str, menucolor[MVC_TEXT], &pt,
                   NULL);
    }
    cnt--;
...
static void dlgtablist_onclick(MENUDLG dlg, DLGHDL hdl, int x, int y) {
  ...
  if (y <
      (hdl->c.dtl.fontsize + MENUDLG_SYTAB + MENUDLG_TYTAB + MENUDLG_EYTAB)) {
    pos = 0;
    prm = hdl->prm;
    while (prm) {
      x -= (MENU_LINE * 4);
...
static void dlgedit_paint(MENUDLG dlg, DLGHDL hdl) {
  ...
  vram_filldat(dlg->vram, &rct,
               menucolor[(hdl->flag & MENU_GRAY) ? MVC_STATIC : MVC_HILIGHT]);
  if (hdl->prm == NULL) {
    goto dged_exit;
  }
  string = hdl->prm->str;
  ...
  vrammix_text(dlg->vram, hdl->c.dt.font, string, menucolor[MVC_TEXT], &pt,
               &rct);
...
static BRESULT dlgicon_create(MENUDLG dlg, DLGHDL hdl, const void *arg) {
  ...
  resattachicon(dlg, hdl->prm, (UINT16)(long)arg,
                hdl->rect.right - hdl->rect.left,
                hdl->rect.bottom - hdl->rect.top);
  return (SUCCESS);
}
...
static void dlgline_paint(MENUDLG dlg, DLGHDL hdl) {
  ...
  if (!(hdl->flag & MSL_VERT)) {
    menuvram_linex(dlg->vram, hdl->rect.left, hdl->rect.top, hdl->rect.right,
                   MVC_SHADOW);
    menuvram_linex(dlg->vram, hdl->rect.left, hdl->rect.top + MENU_LINE,
                   hdl->rect.right, MVC_HILIGHT);
  } else {
    menuvram_liney(dlg->vram, hdl->rect.left, hdl->rect.top, hdl->rect.bottom,
                   MVC_SHADOW);
...
BRESULT menudlg_create(int width, int height, const OEMCHAR *str,
  ...
  height +=
      ((MENU_FBORDER + MENU_BORDER) * 2) + MENUDLG_CYCAPTION + MENUDLG_BORDER;
  mb = &menubase;
  dlg->font = mb->font;
  ...
  if (menudlg_append(DLGTYPE_BASE, SID_CAPTION, 0, str, 0, 0, width, height) !=
      SUCCESS) {
    goto mdcre_err;
  }
  ...
  dlg->sy = (MENU_FBORDER + MENU_BORDER) + (MENUDLG_CYCAPTION + MENUDLG_BORDER);
  if (proc == NULL) {
    proc = defproc;
  }
  dlg->proc = proc;
...
BRESULT menudlg_appends(const MENUPRM *res, int count) {
  ...
  r = SUCCESS;
  while (count--) {
    r |= menudlg_append(res->type, res->id, res->flg, res->arg, res->posx,
                        res->posy, res->width, res->height);
    res++;
  }
  return (r);
}
...
BRESULT menudlg_append(int type, MENUID id, MENUFLG flg, const void *arg,
  ...
  dhdl.prmcnt = 0;
  dhdl.val = 0;
  if (((UINT)type >= NELEMENTS(dlgcre)) || (dlgcre[type](dlg, &dhdl, arg))) {
    goto mda_err;
  }
  drawlock(TRUE);
...
static BOOL mddph_cb(void *vpItem, void *vpArg) {
  ...
  hdl = (DLGHDL)vpItem;
  mdcb = (MDCB4 *)vpArg;
  if ((hdl->page == mdcb->page) && ((hdl->flag ^ mdcb->flag) & MENU_DISABLE)) {
    hdl->flag ^= MENU_DISABLE;
  }
  return (FALSE);
}
