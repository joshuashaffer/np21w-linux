...
static const MENUPRM res_scr0[] = {
    {DLGTYPE_TABLIST, DID_TAB, 0, NULL, 5, 3, 270, 144},
    {DLGTYPE_BUTTON, DID_OK, MENU_TABSTOP, mstr_ok, 152, 150, 58, 15},
    {DLGTYPE_BUTTON, DID_CANCEL, MENU_TABSTOP, mstr_cancel, 214, 150, 58, 15}};

static const MENUPRM res_scr1[] = {
    {DLGTYPE_CHECK, DID_LCD, MENU_TABSTOP, str_lcd, 18, 28, 240, 11},
    {DLGTYPE_CHECK, DID_LCDX, MENU_TABSTOP, str_lcdx, 32, 43, 226, 11},
    {DLGTYPE_CHECK, DID_SKIPLINE, MENU_GRAY | MENU_TABSTOP, str_skipline, 18,
     62, 240, 11},
    {DLGTYPE_LTEXT, DID_STATIC, MENU_GRAY, str_skiplght, 32, 77, 32, 11},
    {DLGTYPE_SLIDER, DID_SKIPLIGHT, MENU_GRAY | MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 255), 66, 77, 112, 11},
    {DLGTYPE_RTEXT, DID_LIGHTSTR, MENU_GRAY, NULL, 186, 77, 20, 11}};

static const MENUPRM res_scr2[] = {
    {DLGTYPE_FRAME, DID_STATIC, 0, str_gdc, 18, 26, 153, 32},
    {DLGTYPE_RADIO, DID_GDC7220, MENU_TABSTOP, str_gdc0, 32, 40, 64, 11},
    {DLGTYPE_RADIO, DID_GDC72020, 0, str_gdc1, 96, 40, 64, 11},
    {DLGTYPE_FRAME, DID_STATIC, 0, str_grcg, 18, 64, 196, 32},
    {DLGTYPE_RADIO, DID_GRCGNON, MENU_TABSTOP, str_grcg0, 32, 78, 44, 11},
    {DLGTYPE_RADIO, DID_GRCG, 0, str_grcg1, 76, 78, 44, 11},
    {DLGTYPE_RADIO, DID_GRCG2, 0, str_grcg2, 120, 78, 48, 11},
    {DLGTYPE_RADIO, DID_EGC, 0, str_grcg3, 168, 78, 40, 11},
    {DLGTYPE_CHECK, DID_PC980124, MENU_TABSTOP, str_pc980124, 28, 104, 224,
     11}};

static const MENUPRM res_scr3[] = {
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_tram, 18, 28, 42, 11},
    {DLGTYPE_SLIDER, DID_TRAMWAIT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 48), 60, 28, 96, 11},
    {DLGTYPE_RTEXT, DID_TRAMSTR, 0, NULL, 168, 28, 22, 11},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_clock, 192, 28, 32, 11},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_vram, 18, 46, 42, 11},
    {DLGTYPE_SLIDER, DID_VRAMWAIT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 48), 60, 46, 96, 11},
    {DLGTYPE_RTEXT, DID_VRAMSTR, 0, NULL, 168, 46, 22, 11},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_clock, 192, 46, 32, 11},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_grcg1, 18, 64, 42, 11},
    {DLGTYPE_SLIDER, DID_GRCGWAIT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 48), 60, 64, 96, 11},
    {DLGTYPE_RTEXT, DID_GRCGSTR, 0, NULL, 168, 64, 22, 11},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_clock, 192, 64, 32, 11},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_realpal, 18, 84, 192, 11},
    {DLGTYPE_SLIDER, DID_REALPAL, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 64), 60, 98, 96, 11},
    {DLGTYPE_RTEXT, DID_REALPALSTR, 0, NULL, 168, 98, 22, 11}};
#else
static const MENUPRM res_scr0[] = {
    {DLGTYPE_TABLIST, DID_TAB, 0, NULL, 7, 6, 379, 196},
    {DLGTYPE_BUTTON, DID_OK, MENU_TABSTOP, mstr_ok, 204, 208, 88, 21},
    {DLGTYPE_BUTTON, DID_CANCEL, MENU_TABSTOP, mstr_cancel, 299, 208, 88, 21}};

static const MENUPRM res_scr1[] = {
    {DLGTYPE_CHECK, DID_LCD, MENU_TABSTOP, str_lcd, 25, 40, 358, 13},
    {DLGTYPE_CHECK, DID_LCDX, MENU_TABSTOP, str_lcdx, 46, 64, 337, 13},
    {DLGTYPE_CHECK, DID_SKIPLINE, MENU_TABSTOP, str_skipline, 25, 88, 358, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_skiplght, 46, 109, 40, 13},
    {DLGTYPE_SLIDER, DID_SKIPLIGHT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 255), 92, 110, 160, 11},
    {DLGTYPE_RTEXT, DID_LIGHTSTR, 0, NULL, 264, 109, 26, 13}};

static const MENUPRM res_scr2[] = {
    {DLGTYPE_FRAME, DID_STATIC, 0, str_gdc, 25, 39, 214, 42},
    {DLGTYPE_RADIO, DID_GDC7220, MENU_TABSTOP, str_gdc0, 47, 58, 90, 13},
    {DLGTYPE_RADIO, DID_GDC72020, 0, str_gdc1, 138, 58, 90, 13},
    {DLGTYPE_FRAME, DID_STATIC, 0, str_grcg, 25, 90, 273, 42},
    {DLGTYPE_RADIO, DID_GRCGNON, MENU_TABSTOP, str_grcg0, 47, 109, 60, 13},
    {DLGTYPE_RADIO, DID_GRCG, 0, str_grcg1, 107, 109, 60, 13},
    {DLGTYPE_RADIO, DID_GRCG2, 0, str_grcg2, 167, 109, 69, 13},
    {DLGTYPE_RADIO, DID_EGC, 0, str_grcg3, 236, 109, 60, 13},
    {DLGTYPE_CHECK, DID_PC980124, MENU_TABSTOP, str_pc980124, 39, 148, 224,
     13}};

static const MENUPRM res_scr3[] = {
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_tram, 24, 41, 60, 13},
    {DLGTYPE_SLIDER, DID_TRAMWAIT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 48), 86, 42, 130, 11},
    {DLGTYPE_RTEXT, DID_TRAMSTR, 0, NULL, 234, 41, 28, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_clock, 268, 41, 48, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_vram, 24, 65, 60, 13},
    {DLGTYPE_SLIDER, DID_VRAMWAIT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 48), 86, 66, 130, 11},
    {DLGTYPE_RTEXT, DID_VRAMSTR, 0, NULL, 234, 65, 28, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_clock, 268, 65, 48, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_grcg1, 24, 89, 60, 13},
    {DLGTYPE_SLIDER, DID_GRCGWAIT, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 48), 86, 90, 130, 11},
    {DLGTYPE_RTEXT, DID_GRCGSTR, 0, NULL, 234, 89, 28, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_clock, 268, 89, 48, 13},
    {DLGTYPE_LTEXT, DID_STATIC, 0, str_realpal, 24, 116, 224, 13},
    {DLGTYPE_SLIDER, DID_REALPAL, MSS_BOTH | MENU_TABSTOP,
     (void *)SLIDERPOS(0, 64), 86, 137, 130, 11},
    {DLGTYPE_RTEXT, DID_REALPALSTR, 0, NULL, 234, 136, 28, 13}};
#endif
...
int dlgscr_cmd(int msg, MENUID id, long param) {
  ...
  case DLGMSG_COMMAND:
    ...
    case DID_REALPAL:
      setintstr(DID_REALPALSTR, menudlg_getval(DID_REALPAL) - 32);
      break;
    }
    break;
