...
VRAMHDL vram_create(int width, int height, BOOL alpha, int bpp) {
  ...
  size = width * height;
  xalign = (bpp + 7) >> 3;
  if ((width <= 0) || (size <= 0) || (size > 0x1000000) || (xalign <= 0) ||
      (xalign > 4)) {
    return (NULL);
  }
...
VRAMHDL vram_resize(VRAMHDL base, int width, int height, int bpp) {
  ...
  (*rsz->func)(rsz, resize_gettype(bpp, base->bpp), ret->ptr, ret->yalign,
               base->ptr, base->yalign);
  if (base->alpha) {
    (*rsz->func)(rsz, RSZFN_8BPP, ret->alpha, ret->width, base->alpha,
                 base->width);
  }
  _MFREE(rsz);
  return (ret);
...
BRESULT vram_cliprect(RECT_T *clip, const VRAMHDL vram, const RECT_T *rct) {
  ...
  if ((rct->bottom <= 0) || (rct->right <= 0) || (rct->left >= vram->width) ||
      (rct->top >= vram->height)) {
    return (FAILURE);
  }
