  ...
  do {
    if (*(UINT16 *)(sdraw->dirty + y)) {
      for (x = 0; x < sdraw->width; x += 2) {
        ...
        work += np2_pal16[p[x + 0 + SURFACE_WIDTH] + q[x + 0 + SURFACE_WIDTH] +
                          NP2PAL_GRPH];
        work += np2_pal16[p[x + 1 + SURFACE_WIDTH] + q[x + 1 + SURFACE_WIDTH] +
                          NP2PAL_GRPH];
        work &= 0x07e0f81f << 2;
    ...
    if (*(UINT16 *)(sdraw->dirty + y)) {
      for (x = 0; x < sdraw->width; x += 2) {
        ...
        if (q[x + 0 + SURFACE_WIDTH] & 0xf0) {
          work += np2_pal16[(q[x + 0 + SURFACE_WIDTH] >> 4) + NP2PAL_TEXT];
        } else {
          work += np2_pal16[p[x + 0] + NP2PAL_GRPH];
        }
        if (q[x + 1 + SURFACE_WIDTH] & 0xf0) {
          work += np2_pal16[(q[x + 1 + SURFACE_WIDTH] >> 4) + NP2PAL_TEXT];
        } else {
          work += np2_pal16[p[x + 1] + NP2PAL_GRPH];
        }
        work &= 0x07e0f81f << 2;
...
static const SDRAWFN qvga16p[] = {
    qvga16p_0,  qvga16p_1,  qvga16p_1,  qvga16p_2,  qvga16p_0,  qvga16p_1,
    qvga16p_gi, qvga16p_2i, qvga16p_0,  qvga16p_1,  qvga16p_gi, qvga16p_2i,
#if defined(SUPPORT_CRT15KHZ)
