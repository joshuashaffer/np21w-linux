...
void maketext_reset(void) { ZeroMemory(&tramflag, sizeof(tramflag)); }
      ...
      if (new_flag) {
        ...
        for (x = 0; x < TEXTXMAX; x++) { // width80
          ...
          if (kanji2nd) {
            ...
            gaiji1st = 0;
            if (gdc.mode1 & 8) {
              bitmap[x] = 0x80000 + (mem[0xa0000 + edi * 2] << 4);
              if ((curx[x] & TXTATR_BG) && (gdc.mode1 & 1)) {
                bitmap[x] += 0x1000;
              }
            } else {
              bitmap[x] = 0x82000 + (mem[0xa0000 + edi * 2] << 4);
            ...
            if ((kc == 0x56) || (kc == 0x57)) {
              tramflag.gaiji = 1;
              if ((gaiji1st) && (bitmap[x] == (lastbitp & (~15)))) {
                curx[x - 1] |= 0x80;
              }
              bitmap[x] += gaiji1st;
              gaiji1st ^= 0x800;
      ...
      if (new_flag) {
        ...
        if ((line_effect & TXTATR_UL) &&
          ...
          for (x = 0; x < (TEXTXMAX - 1); x++) {
            if (curx[x] & TXTATR_UL) {
              *(UINT32 *)(q + 0) = text_table[(color[x] & 0x70) + 0x0f];
              *(UINT32 *)(q + 4) = text_table[(color[x + 1] & 0x70) + 0x0f];
            }
            q += 8;
          }
          if (curx[TEXTXMAX - 1] & TXTATR_UL) {
            *(UINT32 *)q = text_table[(color[TEXTXMAX - 1] & 0x70) + 0x0f];
          }
          q += 4;
        }
      ...
      if (new_flag) {
        ...
        if ((nowline >= 0) && (nowline < TEXT_CL)) {
          ...
          for (x = 0; x < (TEXTXMAX / 2); x++) {
            ...
            data = fontrom[bitmap[x] + (fntline & 0x0f)];
            *(UINT32 *)(q + 0) = text_tblx2[color[x] + (data >> 4)][0];
            *(UINT32 *)(q + 4) = text_tblx2[color[x] + (data >> 4)][1];
            *(UINT32 *)(q + 8) = text_tblx2[color[x] + (data & 0xf)][0];
            *(UINT32 *)(q + 12) = text_tblx2[color[x] + (data & 0xf)][1];
            q += 16;
          }
        } else {
          // width40
        ...
        if ((line_effect & TXTATR_UL) &&
          ...
          for (x = 0; x < ((TEXTXMAX / 2) - 1); x++) {
            if (curx[x] & 8) {
              *(UINT32 *)(q + 0) = text_table[(color[x] & 0x70) + 0x0f];
              *(UINT32 *)(q + 4) = text_table[(color[x] & 0x70) + 0x0f];
              *(UINT32 *)(q + 8) = text_table[(color[x] & 0x70) + 0x0f];
              *(UINT32 *)(q + 12) = text_table[(color[x + 1] & 0x70) + 0x0f];
            }
            q += 16;
          }
          if (curx[(TEXTXMAX / 2) - 1] & TXTATR_UL) {
            *(UINT32 *)(q + 0) =
                text_table[(color[TEXTXMAX - 1] & 0x70) + 0x0f];
            *(UINT32 *)(q + 4) =
                text_table[(color[TEXTXMAX - 1] & 0x70) + 0x0f];
            *(UINT32 *)(q + 8) =
                text_table[(color[TEXTXMAX - 1] & 0x70) + 0x0f];
