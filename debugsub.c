...
static const OEMCHAR str_register[] =
    OEMTEXT("AX=%.4x  BX=%.4x  CX=%.4x  DX=%.4x  ")
        OEMTEXT("SP=%.4x  BP=%.4x  SI=%.4x  DI=%.4x") OEMTEXT(CRLITERAL)
            OEMTEXT("DS=%.4x  ES=%.4x  SS=%.4x  CS=%.4x  ")
                OEMTEXT("IP=%.4x   ");
static const OEMCHAR str_picstat[] =
    OEMTEXT(CRLITERAL) OEMTEXT("PIC0=%.2x:%.2x:%.2x") OEMTEXT(CRLITERAL)
...
const OEMCHAR *debugsub_regs(void) {
  ...
  OEMSPRINTF(work, str_register, CPU_AX, CPU_BX, CPU_CX, CPU_DX, CPU_SP, CPU_BP,
             CPU_SI, CPU_DI, CPU_DS, CPU_ES, CPU_SS, CPU_CS, CPU_IP);
