...
enum { TFMODE_READ = 0x01, TFMODE_WRITE = 0x02 };
...
static TEXTFILEH registfile(_FILEH fh, UINT buffersize, const UINT8 *hdr,
                            UINT hdrsize) {
...
#if defined(SUPPORT_TEXTCNV)
  if (tf->fromoem != NULL) {
    leng = (tf->fromoem)(tf->cnvbuf, tf->cnvbufsize / tf->width, buffer, leng);
    buf = tf->cnvbuf;
  } else {
    buf = buffer;
  }
