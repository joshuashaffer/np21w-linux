...
enum { RESIZE_HDIF = 0x01, RESIZE_VDIF = 0x02 };
...
static void aaout16(RSZEX hdl, UINT8 *dst) {
  ...
  do {
    ...
    col +=
        (tmp >> (MXBITS + MYBITS - G16SFT)) & (((1 << G16BIT) - 1) << G16SFT);
    tmp = buf[2] + (buf[2] >> (8 - R16BIT));
    col +=
        (tmp >> (MXBITS + MYBITS - R16SFT)) & (((1 << R16BIT) - 1) << R16SFT);
    *(UINT16 *)dst = (UINT16)col;
    dst += 2;
    buf += 3;
  } while (--rem);
...
static const AAMIX fnaamix[RSZFNMAX] = {aamix8, aamix16, aamix24, aamix24,
                                        aamix16};
static const AAOUT fnaaout[RSZFNMAX] = {aaout8, aaout16, aaout24, aaout16by24,
                                        aaout24by16};
