 ...
 * @return The start of the string
 */
static OEMCHAR *ParseLine(const OEMCHAR *lpString, UINT *lpcchString,
                          OEMCHAR **lppData, UINT *lpcchData) {
  UINT cchString;
  UINT nIndex;
  ...
  if ((cchString >= 2) && (lpString[0] == '[') &&
      (lpString[cchString - 1] == ']')) {
    lpString++;
    cchString -= 2;
  } else {
 ...
 * @param[in] lpParam An application-defined value to be passed to the callback
 * function.
 ...
 */
static BRESULT SearchKey(PFILEH hdl, PFPOS *pfp, const OEMCHAR *lpAppName,
  ...
  while (cchProfile > 0) {
    nIndex = 0;
    while ((nIndex < cchProfile) && (lpProfile[nIndex] != '\r') &&
           (lpProfile[nIndex] != '\n')) {
      nIndex++;
    }
    lpLeft = lpProfile;
    ...
    if (lpLeft) {
      if (lpRight == NULL) {
        ...
        if ((cchLeft == ret.cchAppName) &&
            (!milstr_memcmp(lpLeft, lpAppName))) {
          ret.apphit = 1;
        }
      } else if ((ret.apphit) && (cchLeft == ret.cchKeyName) &&
                 (!milstr_memcmp(lpLeft, lpKeyName))) {
...
static PFILEH registfile(FILEH fh) {
  ...
  cbHeader = 0;
  nWidth = 1;
  if ((nReadSize >= 3) && (szHeader[0] == 0xef) && (szHeader[1] == 0xbb) &&
      (szHeader[2] == 0xbf)) {
    // UTF-8
    cbHeader = 3;
  } else if ((nReadSize >= 2) && (szHeader[0] == 0xff) &&
             (szHeader[1] == 0xfe)) {
    // UCSLE
    cbHeader = 2;
    nWidth = 2;
    ...
    goto rf_err1;
#endif
  } else if ((nReadSize >= 2) && (szHeader[0] == 0xfe) &&
             (szHeader[1] == 0xff)) {
    // UCS2BE
    cbHeader = 2;
    nWidth = 2;
...
void profile_close(PFILEH hdl) {
  ...
  if (hdl->nFlags & PFILEH_MODIFY) {
    ...
    if (inf.fromoem) {
      cchBuffer2 =
          (inf.fromoem)(NULL, 0, (const OEMCHAR *)lpBuffer1, cchBuffer1);
      lpBuffer2 = _MALLOC(cchBuffer2 * inf.width, "profile tmp");
      if (lpBuffer2 == NULL) {
        goto wf_err1;
      }
      (inf.fromoem)(lpBuffer2, cchBuffer2, (const OEMCHAR *)lpBuffer1,
                    cchBuffer1);
      _MFREE(lpBuffer1);
      lpBuffer1 = lpBuffer2;
 ...
 * @param[in] cchBuffer The size of the buffer pointed to by the lpBuffer
    ...
    if ((lpKeyName != NULL) && (lpData == NULL)) {
      if (lpBuffer) {
        ...
        if (cchRemain >= (cchKeyName + 1)) {
          memcpy(lpBuffer + cchWritten, lpKeyName,
                 cchKeyName * sizeof(*lpBuffer));
 ...
 * @param[in] lpKeyName The name of the key whose associated string is to be
 * retrieved
 * @param[in] lpDefault A default string
 * @param[out] lpReturnedString A pointer to the buffer that receives the
 * retrieved string
 * @param[in] nSize The size of the buffer pointed to by the lpReturnedString
 * parameter, in characters
 * @param[in] hdl The handle of profiler
 * @retval SUCCESS If the function succeeds
 * @retval FAILIURE If the function fails
...
BRESULT profile_read(const OEMCHAR *lpAppName, const OEMCHAR *lpKeyName,
  ...
  if ((SearchKey(hdl, &pfp, lpAppName, lpKeyName) != SUCCESS) ||
      (pfp.lpString == NULL)) {
    if (lpDefault == NULL) {
      lpDefault = str_null;
    }
 ...
 * @param[in] lpAppName The name of the section to which the string will be
 * copied
...
BRESULT profile_write(const OEMCHAR *lpAppName, const OEMCHAR *lpKeyName,
  ...
  if ((hdl == NULL) || (hdl->nFlags & PFILEH_READONLY) || (lpString == NULL) ||
      (SearchKey(hdl, &pfp, lpAppName, lpKeyName) != SUCCESS)) {
    return FAILURE;
  }
 ...
 * @param[in] cbBin The count of the bit
 */
static void binget(OEMCHAR *lpString, int cchString, const UINT8 *lpBin,
                   UINT cbBin) {
  UINT i;
  OEMCHAR tmp[8];
...
void profile_iniread(const OEMCHAR *lpPath, const OEMCHAR *lpApp,
  ...
  while (p < pterm) {
    if (profile_read(lpApp, p->item, NULL, work, NELEMENTS(work), pfh) ==
        SUCCESS) {
