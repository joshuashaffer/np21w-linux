...
static ZIPCAT *getcatnext(ZCATHDL *zch) {
  ...
  ret = (ZIPCAT *)zch->ptr;
  if ((ret->sig[0] != 0x50) || (ret->sig[1] != 0x4b) || (ret->sig[2] != 0x01) ||
      (ret->sig[3] != 0x02)) {
    //		TRACEOUT(("hdr error"));
    goto gdn_err;
  }
...
static UINT method8read(METHOD8 *m8, void *buffer, UINT size) {
  ...
  while (size) {
    ...
    if (m8->stream.avail_in == 0) {
      ...
      //			TRACEOUT(("read buf %d [%.2x %.2x %.2x
      //%.2x...]", r, m8->src[0], m8->src[1], m8->src[2], m8->src[3]));
      m8->srcpos += r;
...
static ARCFH openzipfile(ARCH arch, const ZIPCAT *cat) {
  ...
  if ((zd.sig[0] != 0x50) || (zd.sig[1] != 0x4b) || (zd.sig[2] != 0x03) ||
      (zd.sig[3] != 0x04)) {
    goto ozf_err;
  }
