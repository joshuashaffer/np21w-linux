  ...
  for (i = from; i < to; i++) {
    ...
    for (j = 0x21; j < 0x7f; j++) {
      ...
      if ((i >= 0x01) && (i < 0x08)) { // 2121〜277e
        p = src + 0x00000 + ((((i - 0x01) * 0x5e) + (j - 0x21)) * 0x20);
      } else if ((i >= 0x10) && (i < 0x30)) { // 3021〜5f7e
        p = src + 0x05e00 + ((((i - 0x10) * 0x5e) + (j - 0x21)) * 0x20);
      } else if ((i >= 0x30) && (i < 0x54)) { // 5021〜737e
        p = src + 0x1d600 + ((((i - 0x30) * 0x5e) + (j - 0x21)) * 0x20);
      } else if ((i == 0x54) && (j < 0x25)) { // 7421〜7424
        p = src + 0x1d600 + ((((0x54 - 0x30) * 0x5e) + (j - 0x21)) * 0x20);
      }
      if (p) { // 規格内コードならば
        // コピーする
        for (k = 0; k < 16; k++) {
