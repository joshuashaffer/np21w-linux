...
void cmndraw_fill(const CMNVRAM *vram, int x, int y, int cx, int cy,
                  CMNPAL fg) {
...
void cmndraw_setfg(const CMNVRAM *vram, const UINT8 *src, int x, int y,
                   CMNPAL fg) {
...
void cmndraw_setpat(const CMNVRAM *vram, const UINT8 *src, int x, int y,
                    CMNPAL bg, CMNPAL fg) {
...
void cmndraw_bmp16(CMNVRAM *vram, const void *ptr, CMNPALCNV cnv, UINT flag) {
  ...
  if ((vram == NULL) || (cnv == NULL) ||
      (cmndraw_bmp4inf(&bmp, ptr) != SUCCESS) || (vram->width < bmp.width) ||
      (vram->height < bmp.height)) {
    return;
  }
