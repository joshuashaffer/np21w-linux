...
static void setjumperxex(const DIPBMP *dipbmp, int ofsx, int ofsy, int x, int y,
                         UINT8 c) {
...
static void setjumperyex(const DIPBMP *dipbmp, int ofsx, int ofsy, int x, int y,
                         UINT8 c) {
...
UINT8 *dipswbmp_getsnd118(UINT16 snd118io, UINT8 snd118dma, UINT8 snd118irqf,
                          UINT8 snd118irqp, UINT8 snd118irqm, UINT8 snd118rom) {
  ...
  if (ret) {
    ...
    // PCM INT (#8,#9)=(OFF,OFF):INT5(IRQ12), (OFF,ON):INT1(IRQ5),
    // (ON,OFF):INT41(IRQ10), (ON,ON):INT0(IRQ3)
    switch (snd118irqp) {
    case 12:
      jmpflag[0] = 0;
      jmpflag[1] = 0;
    ...
    // FM INT (#5,#6)=(OFF,OFF):INT5(IRQ12), (OFF,ON):INT41(IRQ10),
    // (ON,OFF):INT6(IRQ13), (ON,ON):INT0(IRQ3)
    switch (snd118irqf) {
    case 12:
      jmpflag[0] = 0;
      jmpflag[1] = 0;
    ...
    setjumperyex(&dipbmp, 18, 9, 8, snd118rom ? 0 : 1,
                 15); // #4  ON:Sound BIOS Disable, OFF:Sound BIOS Enable

    // (#2,#3)=(OFF,OFF):Normal, (OFF,ON):CS4231&OPNA Disable, (ON,OFF):CS4231
    // Disable, (ON,ON):禁止
    setjumperyex(&dipbmp, 18, 9, 9, 0, 15);  // #3
    setjumperyex(&dipbmp, 18, 9, 10, 0, 15); // #2

    setjumperyex(&dipbmp, 18, 9, 11, 0,
                 15); // #1  ON:PnP Enable, OFF:PnP Disable
  }
  return (ret);
}
