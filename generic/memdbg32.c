...
static const RGB32 md32pal[MEMDBG32_PALS] = {
    RGB32D(0x33, 0x33, 0x33), RGB32D(0x00, 0x00, 0x00),
    RGB32D(0xff, 0xaa, 0x00), RGB32D(0xff, 0x00, 0x00),
    RGB32D(0x11, 0x88, 0x11), RGB32D(0x00, 0xff, 0x00),
...
void memdbg32_initialize(void) {
  ...
  memdbg32.width =
      (MEMDBG32_BLOCKW * MEMDBG32_DATAPERLINE) + MEMDBG32_LEFTMARGIN;
  ...
  if (CPU_STAT_PAGING) {
    ...
    for (i = 0; i < pdmax; i++) {
      ...
      for (j = 0; j < 1024; j++) {
        ...
        if ((pte & CPU_PTE_PRESENT) && (pte < 0x1000000 / 16 * MEMDBG32_MAXMEM /
                                                  128 * MEMDBG32_DATAPERLINE)) {
    ...
    if ((CPU_STAT_PM) && (pccore.extmem)) {
      FillMemory(
          use + 256,
          min(MEMDBG32_DATAPERLINE * 2 * pccore.extmem, sizeof(use) - 256),
          MEMDBG32_PALPM);
    }
  }
  for (i = 0; i < MEMDBG32_MAXMEM * 2; i++) {
    for (j = 0; j < MEMDBG32_DATAPERLINE; j++) {
      cmndraw_fill(vram, MEMDBG32_LEFTMARGIN + j * MEMDBG32_BLOCKW,
                   i * MEMDBG32_BLOCKH, MEMDBG32_BLOCKW - 1,
                   MEMDBG32_BLOCKH - 1,
