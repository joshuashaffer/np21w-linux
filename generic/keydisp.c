...
typedef struct TagNotePattern
    NOTEPATTERN; /*!< The define of the note's pattern */
...
static void opnakeyoff(KEYDISP *keydisp, OPNACTL *k, UINT nChannel) {
  delaysetevent(keydisp, (REG8)(k->cChannelNum + nChannel),
                k->ch[nChannel].cLastNote);
}
...
static void opnakeyon(KEYDISP *keydisp, OPNACTL *k, UINT nChannelNum) {
  ...
  delaysetevent(keydisp, (REG8)(k->cChannelNum + nChannelNum),
                (REG8)(k->ch[nChannelNum].cLastNote | 0x80));
}
...
void keydisp_bindopna(const UINT8 *pcRegister, UINT nChannels,
  ...
  if (((s_keydisp.keymax + nChannels) <= KEYDISP_CHMAX) &&
      (s_keydisp.opnamax < NELEMENTS(s_keydisp.opnactl))) {
    ...
    for (i = 0; i < NELEMENTS(k->wFNumber); i++) {
      k->wFNumber[i] =
          (UINT16)(440.0 * pow(2.0, (((double)i - 9.5) / 12.0) + 17.0) * 72.0 /
                   (double)nBaseClock);
    }
    s_keydisp.opnamax++;
...
void keydisp_bindpsg(const UINT8 *pcRegister, UINT nBaseClock) {
  ...
  if (((s_keydisp.keymax + 3) <= KEYDISP_CHMAX) &&
      (s_keydisp.psgmax < NELEMENTS(s_keydisp.psgctl))) {
    k = &s_keydisp.psgctl[s_keydisp.psgmax];
    k->cChannelNum = s_keydisp.keymax;
    k->pcRegister = pcRegister;
    for (i = 0; i < NELEMENTS(k->wTone); i++) {
      k->wTone[i] = (UINT16)((double)nBaseClock / 32.0 /
                             (440.0 * pow(2.0, ((double)i - 9.5) / 12.0)));
    }
    s_keydisp.psgmax++;
    s_keydisp.keymax += 3;
  }
 ...
 * bind
 */
void keydisp_bindopl3(const UINT8 *pcRegister, UINT nChannels,
                      UINT nBaseClock) {
  OPL3CTL *k;
  UINT i;

  if (((s_keydisp.keymax + nChannels) <= KEYDISP_CHMAX) &&
      (s_keydisp.opl3max < NELEMENTS(s_keydisp.opl3ctl))) {
    ...
    for (i = 0; i < NELEMENTS(k->wFNumber); i++) {
      k->wFNumber[i] =
          (UINT16)(440.0 * pow(2.0, (((double)i - 9.5) / 12.0) + 16.0) * 72.0 /
                   (double)nBaseClock);
    }
    s_keydisp.opl3max++;
    s_keydisp.keymax += nChannels;
...
static BOOL draw1key(CMNVRAM *vram, KDCHANNEL *kdch, UINT n) {
  ...
  case 8:
    if (pal != KEYDISP_LEVEL_MAX) {
      fg.pal8 =
          s_constData.pal8[(pPattern->nType) ? KEYDISP_PALBG : KEYDISP_PALFG];
...
void keydisp_setpal(CMNPALFN *palfn) {
  ...
  if (palfn->get32) {
    for (i = 0; i < KEYDISP_PALS; i++) {
      ...
      cmndraw_makegrad(s_constData.pal32[0], KEYDISP_LEVEL,
                       pal32[KEYDISP_PALFG], pal32[KEYDISP_PALHIT]);
      cmndraw_makegrad(s_constData.pal32[1], KEYDISP_LEVEL,
                       pal32[KEYDISP_PALBG], pal32[KEYDISP_PALHIT]);
    }
    if (palfn->cnv16) {
      for (i = 0; i < KEYDISP_LEVEL; i++) {
        s_constData.pal16[0][i] =
            (*palfn->cnv16)(palfn, s_constData.pal32[0][i]);
        s_constData.pal16[1][i] =
            (*palfn->cnv16)(palfn, s_constData.pal32[1][i]);
      }
    }
  }
 ...
 * Set Resize Flag
 */
void keydisp_setresizeflag(void) { s_keydisp.dispflag |= KEYDISP_FLAGSIZING; }
#endif
