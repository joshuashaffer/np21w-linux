...
static const HDRVFILE hdd_volume = {
    {'_', 'H', 'O', 'S', 'T', 'D', 'R', 'I', 'V', 'E', '_'},
    0,
    0,
    0x08,
    {0},
...
static void fetch_if4dos(void) {
  ...
  TRACEOUT(
      ("hostdrv:dosver = %d.%.2d", if4dos.dosver_major, if4dos.dosver_minor));
  TRACEOUT(
      ("hostdrv.sda = %.4x:%.4x", hostdrv.stat.sda_seg, hostdrv.stat.sda_off));
}
...
static void fetch_sda_currcds(SDACDS sc) {
  ...
  if (hostdrv.stat.dosver_major == 3) {
    MEMR_READS(hostdrv.stat.sda_seg, hostdrv.stat.sda_off, &sc->ver3.sda,
               sizeof(sc->ver3.sda));
    ...
    MEMR_READS(hostdrv.stat.sda_seg, hostdrv.stat.sda_off, &sc->ver4.sda,
               sizeof(sc->ver4.sda));
...
static void store_sda_currcds(SDACDS sc) {
  ...
  if (hostdrv.stat.dosver_major == 3) {
    MEMR_WRITES(hostdrv.stat.sda_seg, hostdrv.stat.sda_off, &sc->ver3.sda,
                sizeof(sc->ver3.sda));
    off = LOADINTELWORD(sc->ver3.sda.cdsptr.off);
    seg = LOADINTELWORD(sc->ver3.sda.cdsptr.seg);
    MEMR_WRITES(seg, off, &sc->ver3.cds, sizeof(sc->ver3.cds));
  } else {
    MEMR_WRITES(hostdrv.stat.sda_seg, hostdrv.stat.sda_off, &sc->ver4.sda,
                sizeof(sc->ver4.sda));
    off = LOADINTELWORD(sc->ver4.sda.cdsptr.off);
    seg = LOADINTELWORD(sc->ver4.sda.cdsptr.seg);
...
static void get_fileattr(INTRST intrst) {
  ...
  if (strcmp(intrst->fcbname_ptr, "???????????") ||
      intrst->filename_ptr[0]) { // XXX: Win用特例
    if ((is_wildcards(intrst->fcbname_ptr)) ||
        (hostdrvs_getrealpath(&hdp, intrst->filename_ptr) != ERR_NOERROR)) {
      fail(intrst, ERR_FILENOTFOUND);
      return;
...
static void rename_file(INTRST intrst) {
  ...
  lst = NULL;
  do {
    TRACEOUT(("rename_file: %s -> %s", intrst->filename_ptr,
              intrst->filename_ptr_2));
...
static void open_file(INTRST intrst) {
  ...
  do {
    ...
    TRACEOUT(("open_file: %s -> %s %d", intrst->filename_ptr, hdp.szPath,
              sft.open_mode[0] & 7));
...
static void create_file(INTRST intrst) {
  ...
  do {
    ...
    TRACEOUT(("create_file: %s -> %s %d", intrst->filename_ptr, hdp.szPath,
              sft.open_mode[0] & 7));
...
static void find_first(INTRST intrst) {
  ...
  if (*intrst->srch_attr_ptr == 0x08) /* ボリュームラベル */
  {
    ...
    if (hostdrvs_getrealdir(&hdp, fcbname, intrst->filename_ptr) !=
        ERR_NOERROR) {
      fail(intrst, ERR_PATHNOTFOUND);
      return;
    }
    ...
    hostdrv.flist =
        hostdrvs_getpathlist(&hdp, intrst->fcbname_ptr, *intrst->srch_attr_ptr);
    hostdrv.stat.flistpos = 0;
...
static void do_redir(INTRST intrst) {
  ...
  case 0x5f02:
    ...
    tmp[1] = ':';
    tmp[2] = '\0';
    MEMR_WRITES(LOADINTELWORD(intrst->r.w.ds), LOADINTELWORD(intrst->r.w.si),
                tmp, 3);
    MEMR_WRITES(LOADINTELWORD(intrst->r.w.es), LOADINTELWORD(intrst->r.w.di),
...
static const HDINTRFN intr_func[] = {
    ...
    NULL,          rename_file,                                         /* 11 */
    NULL,          delete_file,                                         /* 13 */
    NULL,          NULL,           open_file,                           /* 16 */
    create_file,                                                        /* 17 */
    NULL,          NULL,           NULL,      find_first,               /* 1B */
    find_next,                                                          /* 1C */
    NULL,          do_redir,       NULL,      NULL,       seek_fromend, /* 21 */
...
void hostdrv_mount(const void *arg1, long arg2) {
  ...
  if ((np2cfg.hdrvroot[0] == '\0') || (!np2cfg.hdrvenable) ||
      (hostdrv.stat.is_mount)) {
