/**
 * @file	tgui9680.c
 * @brief	Implementation of the Trident TGUI9680 from PCem
 * (vid_tgui9440.c)
 */
...
/*TGUI9400CXi has extended write modes, controlled by extended GDC registers :
  ...
  enabled. If the relevant bit is clear in the write mask then the data is not
  written.
...
void tgui_out(uint16_t addr, uint8_t val, void *p) {
  ...
  case 0x3CF:
    if (tgui->type == TGUI_9400CXI && svga->gdcaddr >= 16 &&
        svga->gdcaddr < 32) {
  ...
  case 0x3D5:
    ...
    //                if (svga->crtcreg != 0xE && svga->crtcreg != 0xF)
    //                pclog("CRTC R%02X = %02X\n", svga->crtcreg, val);
    if (old != val) {
    ...
    case 0x47:
      if (tgui->type >= TGUI_9440) {
        ...
        svga->hwcursor.addr = (svga->crtc[0x44] << 10) |
                              ((svga->crtc[0x45] & 0x7) << 18) |
                              (svga->hwcursor.yoff * 8);
      }
      break;
  ...
  case 0x3D8:
    ...
    if (svga->gdcreg[0xf] & 4) {
      ...
      //                                pclog("SVGAWBANK 3D8 %08X
      //                                %04X:%04X\n",svgawbank,CS,pc);
      if (!(svga->gdcreg[0xf] & 1)) {
        svga->read_bank = (val & 0x1f) * 65536;
        //                                        pclog("SVGARBANK 3D8 %08X
        //                                        %04X:%04X\n",svgarbank,CS,pc);
      }
    }
    return;
...
uint8_t tgui_in(uint16_t addr, void *p) {
  ...
  case 0x3C5:
    if ((svga->seqaddr & 0xf) == 0xb) {
      //                        printf("Read Trident ID %04X:%04X
    ...
    if ((svga->seqaddr & 0xf) == 0xc) {
      //                        printf("Read Trident Power Up 1 %04X:%04X
      //                        %04X\n",CS,pc,readmemw(ss,SP)); return 0x20; /*2
      //                        DRAM banks*/
    }
...
void tgui_recalctimings(svga_t *svga) {
  ...
  if (tgui->oldctrl2 &
      0x10) /*I'm not convinced this is the right register for this function*/
    svga->lowres = 0;
  ...
  if (tgui->type >= TGUI_9440) {
    if (svga->miscout & 8)
      svga->clock = (cpuclock * (double)(1ull << 32)) /
                    (((tgui->clock_n + 8) * 14318180.0) /
                     ((tgui->clock_m + 2) * (1 << tgui->clock_k)));
    ...
    switch (((svga->miscout >> 2) & 3) | ((tgui->newctrl2 << 2) & 4) |
            ((tgui->newctrl2 >> 3) & 8)) {
    case 0x02:
...
void tgui_recalcmapping(tgui_t *tgui) {
  ...
  if (tgui->type == TGUI_9400CXI) {
    if (tgui->ext_gdc_regs[0] & EXT_CTRL_LATCH_COPY) {
      mem_mapping_set_handler(&tgui->linear_mapping, tgui_ext_linear_read, NULL,
                              NULL, tgui_ext_linear_write,
                              tgui_ext_linear_writew, tgui_ext_linear_writel);
      mem_mapping_set_handler(&svga->mapping, tgui_ext_read, NULL, NULL,
                              tgui_ext_write, tgui_ext_writew, tgui_ext_writel);
    } else if (tgui->ext_gdc_regs[0] & EXT_CTRL_MONO_EXPANSION) {
      mem_mapping_set_handler(&tgui->linear_mapping, svga_read_linear,
                              svga_readw_linear, svga_readl_linear,
                              tgui_ext_linear_write, tgui_ext_linear_writew,
                              tgui_ext_linear_writel);
      mem_mapping_set_handler(&svga->mapping, svga_read, svga_readw, svga_readl,
                              tgui_ext_write, tgui_ext_writew, tgui_ext_writel);
    } else {
      mem_mapping_set_handler(&tgui->linear_mapping, svga_read_linear,
                              svga_readw_linear, svga_readl_linear,
...
void tgui_hwcursor_draw(svga_t *svga, int displine) {
  ...
  dat[1] = (svga->vram[svga->hwcursor_latch.addr + 4] << 24) |
           (svga->vram[svga->hwcursor_latch.addr + 5] << 16) |
           (svga->vram[svga->hwcursor_latch.addr + 6] << 8) |
           svga->vram[svga->hwcursor_latch.addr + 7];
  for (xx = 0; xx < 32; xx++) {
    if (offset >= svga->hwcursor_latch.x) {
      if (!(dat[0] & 0x80000000))
        ((uint32_t *)buffer32->line[displine])[offset + 32] =
            (dat[1] & 0x80000000) ? 0xffffff : 0;
      else if (dat[1] & 0x80000000)
        ((uint32_t *)buffer32->line[displine])[offset + 32] ^= 0xffffff;
      //                        pclog("Plot %i, %i (%i %i) %04X %04X\n", offset,
      //                        displine, x+xx, svga_hwcursor_on, dat[0],
      //                        dat[1]);
    }
...
static void *tgui_init(char *bios_fn, int type, int mem_size) {
  ...
  rom_init(&tgui->bios_rom, bios_fn, 0xc0000, 0x8000, 0x7fff, 0,
           MEM_MAPPING_EXTERNAL);

  svga_init(&tgui->svga, tgui, tgui->vram_size, tgui_recalctimings, tgui_in,
            tgui_out, tgui_hwcursor_draw, NULL);

  mem_mapping_add(&tgui->linear_mapping, 0, 0, svga_read_linear,
                  svga_readw_linear, svga_readl_linear, tgui_accel_write_fb_b,
                  tgui_accel_write_fb_w, tgui_accel_write_fb_l, NULL, 0,
                  &tgui->svga);
  mem_mapping_add(&tgui->accel_mapping, 0xbc000, 0x4000, tgui_accel_read,
                  tgui_accel_read_w, tgui_accel_read_l, tgui_accel_write,
                  tgui_accel_write_w, tgui_accel_write_l, NULL, 0, tgui);
  mem_mapping_disable(&tgui->accel_mapping);

  io_sethandler(0x03c0, 0x0020, tgui_in, NULL, NULL, tgui_out, NULL, NULL,
                tgui);
  if (tgui->type >= TGUI_9440)
    io_sethandler(0x43c8, 0x0002, tgui_in, NULL, NULL, tgui_out, NULL, NULL,
                  tgui);
...
static void tgui_ext_linear_write(uint32_t addr, uint8_t val, void *p) {
  ...
  case 3:
    for (c = 7; c >= 0; c--) {
      if (mask & (1 << c))
        *(uint8_t *)&svga->vram[addr] =
            (val & (1 << c)) ? fg[(c & 1) ^ 1] : bg[(c & 1) ^ 1];
      addr++;
    }
    break;
...
#define READ(addr, dat)                                                        \
  if (tgui->accel.bpp == 0)                                                    \
    dat = svga->vram[addr & 0x1fffff];                                         \
  else                                                                         \
    dat = vram_w[addr & 0xfffff];
...
void tgui_accel_command(int count, uint32_t cpu_dat, tgui_t *tgui) {
  ...
  uint16_t trans_col = (tgui->accel.flags & TGUI_TRANSREV) ? tgui->accel.fg_col
                                                           : tgui->accel.bg_col;
  ...
  if (tgui->accel.flags & TGUI_SOLIDFILL) {
    ...
    for (y = 0; y < 8; y++) {
      for (x = 0; x < 8; x++) {
        tgui->accel.tgui_pattern[y][x] = (tgui->accel.pattern[y] & (1 << x))
                                             ? tgui->accel.fg_col
                                             : tgui->accel.bg_col;
      }
    }
  ...
  /*	for (y = 0; y < 8; y++)
          {
                  if (count == -1) pclog("Pattern %i : %02X %02X %02X %02X %02X
     %02X %02X %02X\n", y, tgui->accel.tgui_pattern[y][0],
     tgui->accel.tgui_pattern[y][1], tgui->accel.tgui_pattern[y][2],
     tgui->accel.tgui_pattern[y][3], tgui->accel.tgui_pattern[y][4],
     tgui->accel.tgui_pattern[y][5], tgui->accel.tgui_pattern[y][6],
  ...
  case TGUI_BITBLT:
    ...
    if (count == -1) {
      ...
      tgui->accel.dst = tgui->accel.dst_old =
          tgui->accel.dst_x + (tgui->accel.dst_y * tgui->accel.pitch);
    ...
    case TGUI_SRCCPU:
      ...
      while (count) {
        ...
        pat_dat =
            tgui->accel
                .tgui_pattern[tgui->accel.pat_y & 7][tgui->accel.pat_x & 7];
...
static void tgui_accel_write_fifo_fb_b(tgui_t *tgui, uint32_t addr,
                                       uint8_t val) {
  tgui_accel_command(8, val << 24, tgui);
}
static void tgui_accel_write_fifo_fb_w(tgui_t *tgui, uint32_t addr,
                                       uint16_t val) {
  tgui_accel_command(16, (((val & 0xff00) >> 8) | ((val & 0x00ff) << 8)) << 16,
                     tgui);
}
static void tgui_accel_write_fifo_fb_l(tgui_t *tgui, uint32_t addr,
                                       uint32_t val) {
  ...
  while (1) {
    ...
    while (!FIFO_EMPTY) {
      ...
      case FIFO_WRITE_FB_BYTE:
        tgui_accel_write_fifo_fb_b(tgui, fifo->addr_type & FIFO_ADDR,
                                   fifo->val);
        break;
      case FIFO_WRITE_FB_WORD:
        tgui_accel_write_fifo_fb_w(tgui, fifo->addr_type & FIFO_ADDR,
                                   fifo->val);
        break;
      case FIFO_WRITE_FB_LONG:
        tgui_accel_write_fifo_fb_l(tgui, fifo->addr_type & FIFO_ADDR,
                                   fifo->val);
        break;
      }
...
static inline void wake_fifo_thread(tgui_t *tgui) {
  thread_set_event(
      tgui->wake_fifo_thread); /*Wake up FIFO thread if moving from idle*/
}
...
static void tgui_queue(tgui_t *tgui, uint32_t addr, uint32_t val,
                       uint32_t type) {
  ...
  if (FIFO_FULL) {
    ...
    if (FIFO_FULL) {
      thread_wait_event(tgui->fifo_not_full_event,
                        -1); /*Wait for room in ringbuffer*/
    }
  }
...
uint32_t tgui_accel_read_l(uint32_t addr, void *p) {
  ...
  return tgui_accel_read_w(addr, tgui) |
         (tgui_accel_read_w(addr + 2, tgui) << 16);
}
