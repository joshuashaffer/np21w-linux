...
CSpfmLight::CSpfmLight(CSoundInterfaceManager *pManager,
                       const std::oemstring &deviceName, UINT nDelay)
    : CSoundInterface(pManager, deviceName), m_nDelay(nDelay),
      m_bReseted(false), m_nQueIndex(0), m_nQueCount(0) {}
...
bool CSpfmLight::Initialize() {
  ...
  if (Write(query, sizeof(query), nUntil) == sizeof(query)) {
    ...
    bResult = (Read(buffer, sizeof(buffer), nUntil) == sizeof(buffer)) &&
              (buffer[0] == 'L') && (buffer[1] == 'T');
  }
  m_ttyGuard.Leave();
 ...
 * Deinitialize
 */
void CSpfmLight::Deinitialize() { m_serial.Close(); }
 ...
 * @retval true yes
 */
bool CSpfmLight::isSupportLowLevelApi() { return true; }
 ...
 */
ssize_t CSpfmLight::Read(unsigned char *lpBuffer, ssize_t cbBuffer,
                         UINT nUntil) {
  ssize_t nRead = m_serial.Read(lpBuffer, cbBuffer);
  if (nRead == -1) {
    return -1;
  }
  while ((nRead < cbBuffer) &&
         ((static_cast<SINT>(nUntil) - static_cast<SINT>(GETTICK())) > 0)) {
    Delay(1000);
 ...
 */
ssize_t CSpfmLight::Write(const unsigned char *lpBuffer, ssize_t cbBuffer,
                          UINT nUntil) {
  ssize_t nWritten = m_serial.Write(lpBuffer, cbBuffer);
  if (nWritten == -1) {
    return -1;
  }
  while ((nWritten < cbBuffer) &&
         ((static_cast<SINT>(nUntil) - static_cast<SINT>(GETTICK())) > 0)) {
    Delay(1000);
 ...
 * @param[in] info The information
 */
CSpfmLight::Chip::Chip(CSoundInterface *pInterface,
                       const SCCI_SOUND_CHIP_INFO &info)
 ...
 * @retval false If failed
 */
bool CSpfmLight::Chip::init() { return m_pInterface->reset(); }
