...
CSoundInterface::CSoundInterface(CSoundInterfaceManager *pManager,
                                 const std::oemstring &deviceName)
    : m_nRef(0), m_pManager(pManager) {
  memset(&m_info, 0, sizeof(m_info));

  milstr_ncpy(m_info.cInterfaceName, deviceName.c_str(),
              NELEMENTS(m_info.cInterfaceName));
...
void CSoundInterface::ReleaseAllChips() {
  for (std::map<UINT, CSoundChip *>::iterator it = m_chips.begin();
       it != m_chips.end(); ++it) {
    it->second->Release();
  }
}
 ...
 */
SoundChip *CSoundInterface::GetSoundChip(SC_CHIP_TYPE iSoundChipType,
                                         UINT dClock) {
  for (std::map<UINT, CSoundChip *>::iterator it = m_chips.begin();
       it != m_chips.end(); ++it) {
