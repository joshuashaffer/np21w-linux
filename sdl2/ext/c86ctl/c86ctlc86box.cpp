...
CC86Box::CC86Box(UINT nIndex)
    : m_nRef(1), m_nIndex(nIndex), m_nQueIndex(0), m_nQueCount(0) {}
...
C86CtlErr CC86Box::initialize() {
  ...
  for (UINT i = 0; i < 2; i++) {
    ...
    printf("Found %s in C86BOX-Slot#%c / chips: %d\n",
           GetBoardName(static_cast<CBUS_BOARD_TYPE>(info.type)), 'A' + i,
           info.nchips);
...
C86CtlErr CC86Box::deinitialize() {
  ...
  for (std::vector<Chip3 *>::iterator it = m_chips.begin(); it != m_chips.end();
       ++it) {
    delete *it;
  }
  m_chips.clear();
 ...
 * @return The chips
 */
size_t CC86Box::getNumberOfChip() { return m_chips.size(); }
 ...
 * @return C86CTL_ERR
 */
C86CtlErr CC86Box::Transaction(const void *lpOutput, int cbOutput,
                               void *lpInput, int cbInput) {
  if (!m_usb.IsOpened()) {
    return C86CTL_ERR_NODEVICE;
  }
...
void CC86Box::Out(UINT nDevId, UINT nAddr, UINT8 cData) {
  ...
  m_que[(m_nQueIndex + m_nQueCount) % NELEMENTS(m_que)] =
      (nDevId << 17) | ((nAddr & 0x1ff) << 8) | cData;
  m_nQueCount++;
...
CC86Box::Chip3::Chip3(CC86Box *pC86Box, UINT nDevId, ChipType nChipType)
    : m_pC86Box(pC86Box), m_nDevId(nDevId), m_nChipType(nChipType) {}

/**
 * Get the instance of the device
 * @return The instance
 */
inline CC86Box *CC86Box::Chip3::GetDevice() { return m_pC86Box; }

/**
 * Increments the reference count
 * @return The new reference count
 */
size_t CC86Box::Chip3::AddRef() { return GetDevice()->AddRef(); }

/**
 * Decrements the reference count
 * @return The new reference count
 */
size_t CC86Box::Chip3::Release() { return GetDevice()->Release(); }
