...
short file_delete(const char *path) { return (remove(path)); }
...
static BRESULT setflist(WIN32_FIND_DATA *w32fd, FLINFO *fli) {
  ...
  codecnv_sjistoutf8(fli->path, sizeof(fli->path), w32fd->cFileName, (UINT)-1);
#else
...
void file_listclose(FLISTH hdl) { FindClose(hdl); }
#else
FLISTH file_list1st(const char *dir, FLINFO *fli) {
...
void file_listclose(FLISTH hdl) { closedir((DIR *)hdl); }
#endif
