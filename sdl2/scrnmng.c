...
static BRESULT calcdrawrect(SDL_Surface *surface, DRAWRECT *dr, VRAMHDL s,
                            const RECT_T *rt) {
...
BRESULT scrnmng_create(int width, int height) {
  ...
  s_sdlWindow = SDL_CreateWindow(app_name, SDL_WINDOWPOS_UNDEFINED,
  ...
  s_surface = SDL_CreateRGBSurface(SDL_SWSURFACE, width, height, 16, 0xf800,
                                   0x07e0, 0x001f, 0);
...
void scrnmng_destroy(void) { scrnmng.enable = FALSE; }
...
void scrnmng_setwidth(int posx, int width) { scrnstat.width = width; }

void scrnmng_setheight(int posy, int height) { scrnstat.height = height; }
...
void scrnmng_leavemenu(void) { VRAM_RELEASE(scrnmng.vram); }
