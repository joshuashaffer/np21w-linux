 ...
 * コンストラクタ
 */
CUsbDev::CUsbDev() : m_device(NULL), m_interface(NULL) {}
...
bool CUsbDev::Open(unsigned int vid, unsigned int pid, unsigned int nIndex) {
  ...
  CFDictionarySetValue(
      matchingDict, CFSTR(kUSBVendorName),
      CFNumberCreate(kCFAllocatorDefault, kCFNumberSInt32Type, &usbVendor));
  CFDictionarySetValue(
      matchingDict, CFSTR(kUSBProductName),
  ...
  while (1 /*EVER*/) {
    ...
    for (int pipeRef = 1; pipeRef <= interfaceNumEndpoints; pipeRef++) {
      ...
      UInt8 interval;
      kr = (*interface)
               ->GetPipeProperties(interface, pipeRef, &direction, &number,
...
int CUsbDev::WriteBulk(const void *lpBuffer, int cbBuffer) {
  ...
  IOReturn kr = (*m_interface)
                    ->WritePipe(m_interface, 1, const_cast<void *>(lpBuffer),
                                static_cast<UInt32>(cbBuffer));
  if (kr != kIOReturnSuccess) {
...
int CUsbDev::ReadBulk(void *lpBuffer, int cbBuffer) {
  ...
  IOReturn kr =
      (*m_interface)->ReadPipe(m_interface, 2, lpBuffer, &numBytesRead);
  if (kr != kIOReturnSuccess) {
