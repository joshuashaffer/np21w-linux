...
DEFINE_GUID(GUID_DEVINTERFACE_WINUSB_GIMIC, 0x63275336, 0x530b, 0x4069, 0x92,
            0xb6, 0x5f, 0x8a, 0xe3, 0x46, 0x54, 0x62);

// デバイスドライバのinf内で定義したGUID
// (WinUSB.sys使用デバイスに対する識別子）
// {b0320d09-0791-4c3f-a741-9ef97c8885d6}
DEFINE_GUID(GUID_DEVINTERFACE_WINUSB_C86BOX, 0xb0320d09, 0x0791, 0x4c3f, 0xa7,
            0x41, 0x9e, 0xf9, 0x7c, 0x88, 0x85, 0xd6);

/**
 * コンストラクタ
 */
CUsbDev::CUsbDev()
    : m_hDev(INVALID_HANDLE_VALUE), m_hWinUsb(NULL), m_cOutPipeId(0),
      m_cInPipeId(0) {}

/**
 * デストラクタ
 */
CUsbDev::~CUsbDev() { Close(); }
...
bool CUsbDev::Open(const GUID &InterfaceGuid) {
  ...
  LPTSTR lpDevicePath =
      GetDevicePath(InterfaceGuid, szDevicePath, _countof(szDevicePath));
 ...
 * @return パス
 */
LPTSTR CUsbDev::GetDevicePath(const GUID &InterfaceGuid, LPTSTR lpDevicePath,
                              int cchDevicePath) {
  ...
  for (DWORD i = 0; lpRet == NULL; i++) {
    ...
    if (!::SetupDiEnumDeviceInterfaces(hDeviceInfo, NULL, &InterfaceGuid, i,
                                       &interfaceData)) {
      break;
    }

    ULONG nRequiredLength = 0;
    ::SetupDiGetDeviceInterfaceDetail(hDeviceInfo, &interfaceData, NULL, 0,
                                      &nRequiredLength, NULL);

    PSP_DEVICE_INTERFACE_DETAIL_DATA pDetailData =
        static_cast<PSP_DEVICE_INTERFACE_DETAIL_DATA>(
            ::LocalAlloc(LMEM_FIXED, nRequiredLength));
    if (pDetailData == NULL) {
      continue;
    }
    ...
    if (::SetupDiGetDeviceInterfaceDetail(hDeviceInfo, &interfaceData,
                                          pDetailData, nLength,
                                          &nRequiredLength, NULL)) {
...
bool CUsbDev::OpenDevice(LPCTSTR lpDevicePath) {
  HANDLE hDev = ::CreateFile(lpDevicePath, GENERIC_READ | GENERIC_WRITE, 0,
  ...
  if (cOutPipeId != 0) {
    ULONG nTimeOut = 500;
    ::WinUsb_SetPipePolicy(hWinUsb, cOutPipeId, PIPE_TRANSFER_TIMEOUT,
                           sizeof(nTimeOut), &nTimeOut);
    ::WinUsb_FlushPipe(hWinUsb, cOutPipeId);
  }
  if (cInPipeId != 0) {
    ULONG nTimeOut = 500;
    ::WinUsb_SetPipePolicy(hWinUsb, cInPipeId, PIPE_TRANSFER_TIMEOUT,
                           sizeof(nTimeOut), &nTimeOut);
...
int CUsbDev::CtrlXfer(int nType, int nRequest, int nValue, int nIndex,
  ...
  if (!::WinUsb_ControlTransfer(m_hWinUsb, setup, static_cast<PUCHAR>(lpBuffer),
                                cbBuffer, &nTransferred, NULL)) {
    return -1;
  }
...
int CUsbDev::WriteBulk(const void *lpBuffer, int cbBuffer) {
  ...
  DWORD dwLength = 0;
  while (!::WinUsb_WritePipe(m_hWinUsb, m_cOutPipeId,
                             static_cast<PUCHAR>(const_cast<void *>(lpBuffer)),
                             static_cast<ULONG>(cbBuffer), &dwLength, 0)) {
...
int CUsbDev::ReadBulk(void *lpBuffer, int cbBuffer) {
  ...
  DWORD dwLength = 0;
  if (!::WinUsb_ReadPipe(m_hWinUsb, m_cInPipeId, static_cast<PUCHAR>(lpBuffer),
                         static_cast<ULONG>(cbBuffer), &dwLength, 0)) {
    Close();
    return -1;
  }
