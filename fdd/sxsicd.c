...
BRESULT sxsicd_open(SXSIDEV sxsi, const OEMCHAR *fname) {
  ...
  return (openiso(
      sxsi, fname)); //	知らない拡張子なら、とりあえずISOとして開いてみる
}
...
BRESULT sxsicd_readraw(SXSIDEV sxsi, FILEPOS pos, void *buf) {
  ...
  // XXX: 事前に判定して記録しておくべき･･･
  isPhysicalCD = (cdinfo->path[0] == '\\' && cdinfo->path[1] == '\\' &&
                  cdinfo->path[2] == '.' && cdinfo->path[3] == '\\');
  ...
  if (isPhysicalCD) {
    ...
    if (!DeviceIoControl(fh, IOCTL_CDROM_RAW_READ, &rawReadInfo,
                         sizeof(RAW_READ_INFO), buf, 2352, &BytesReturned, 0)) {
      return (FAILURE);
    }
...
static BRESULT openimg(SXSIDEV sxsi, const OEMCHAR *path, const _CDTRK *trk,
                       UINT trks) {
