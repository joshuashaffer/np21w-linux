...
enum { SXSIMEDIA_DATA = 0x10, SXSIMEDIA_AUDIO = 0x20 };

#ifdef SUPPORT_KAI_IMAGES

// WinDDKの構造体の名前と被っちゃったので TRACK -> TRACKTYPE に変更 np21w
// ver0.86 rev33
...
typedef struct {
  ...
  UINT32 pos; //	トラックのイメージファイル内での開始セクタ位置
              ...
              //		CCD:
              //		CDM:
              //		MDS:((MDS_TrackBlock.min * 60) +
              //MDS_TrackBlock.sec) * 75 + MDS_TrackBlock.frame
  //	--------
  UINT32 pos0; //	CUEシートの"INDEX
               //00"等で指定されたPREGAPのイメージ内での開始セクタ位置 		ISO:0
               //		CUE:INDEX 0
               //		CCD:
               //		CDM:
               //		MDS:0
  ...
  //	--------
  UINT16
      sector_size; //	トラックのセクタサイズ
                   //		ISO:2048 or 2352 or
                   //2448(ファイルサイズを割って余りの出ない数値)
  ...
  UINT64 pregap_offset; //	イメージファイル上のトラックのPREGAPのoffset
                        ...
                        //		MDS:start_offset
  UINT64
      start_offset; //	イメージファイル上のトラック開始位置のoffset
                    //	※PREGAPが無い場合やPREGAPの実体が無い場合は
