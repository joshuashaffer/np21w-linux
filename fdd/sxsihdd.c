...
const SASIHDD sasihdd[7] = {{33, 4, 153},  // 5MB
                            {33, 4, 310},  // 10MB
                            {33, 6, 310},  // 15MB
                            {33, 8, 310},  // 20MB
...
static void hdd_close(SXSIDEV sxsi) { file_close((FILEH)sxsi->hdl); }
...
BRESULT sxsihdd_open(SXSIDEV sxsi, const OEMCHAR *fname) {
  ...
  // フォーマット確認〜
  if ((surfaces == 0) || (surfaces >= 256) || (cylinders == 0) ||
      (cylinders >= 65536) || (sectors == 0) || (sectors >= 256) ||
