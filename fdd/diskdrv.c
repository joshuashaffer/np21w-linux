  ...
  for (drv = 0x00; drv < 0x04; drv++) {
    ...
    if (np2cfg.idetype[drv] == SXSIDEV_HDD) {
      if (sxsi_devopen(drv, np2cfg.sasihdd[drv & 0x0f]) != SUCCESS) {
        ...
        if (np2cfg.sasihdd[drv & 0x0f] != NULL &&
            np2cfg.sasihdd[drv & 0x0f][0] != '\0') {
      ...
      if (sxsicd_open(sxsi, np2cfg.idecd[drv & 0x0f]) == SUCCESS) {
        file_cpyname(sxsi->fname, np2cfg.idecd[drv & 0x0f],
                     NELEMENTS(sxsi->fname));
 ...
 * @param[in] readonly The flags of file
 */
void diskdrv_setfddex(REG8 drv, const OEMCHAR *fname, UINT ftype,
                      int readonly) {
  if ((drv < 4) && (fdc.equip & (1 << drv))) {
    fdd_eject(drv);
    diskdrv_delay[drv] = 0;
...
void diskdrv_callback(void) {
  ...
  for (drv = 0; drv < 4; drv++) {
    if (diskdrv_delay[drv]) {
      diskdrv_delay[drv]--;
      if (!diskdrv_delay[drv]) {
        diskdrv_readyfddex(drv, diskdrv_fname[drv], diskdrv_ftype[drv],
                           diskdrv_ro[drv]);
        diskdrv_fname[drv][0] = '\0';
      }
    }
