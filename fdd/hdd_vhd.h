...
static const UINT8 CookieVPCVHDFooter[8] = {'c', 'o', 'n', 'e',
                                            'c', 't', 'i', 'x'};
static const UINT8 CookieVPCVHDDDH[8] = {'c', 'x', 's', 'p',
                                         'a', 'r', 's', 'e'};
...
BRESULT sxsihdd_vpcvhd_mount(SXSIDEV sxsi, FILEH fh) {
  ...
  if (vhdlen == (VPCVHD_FPOS)-1 ||
      (vhdlen >= 0 && vhdlen < (VPCVHD_FPOS)footerlen))
    return (FAILURE);
  ...
  if (memcmp(footer.Cookie, CookieVPCVHDFooter, sizeof(footer.Cookie)) != 0) {
    CopyMemory(&(footer.Cookie[0]), &(footer.Cookie[1]),
               sizeof(footer.Cookie) - 1);
    file_read(fh, &(footer.Cookie[sizeof(footer.Cookie) - 1]), 1);
    --footerlen;
  }
  if (memcmp(footer.Cookie, CookieVPCVHDFooter, sizeof(footer.Cookie)) != 0)
    return (FAILURE);
  readlen = file_read(fh, (UINT8 *)(&footer) + sizeof(footer.Cookie),
                      (UINT)(footerlen - sizeof(footer.Cookie)));
  ...
  if (mychecksum != checksum) {
    TRACEOUT(
        ("vpc_vhd: bad checksum (file=%x, calc=%x)", checksum, mychecksum));
    return (FAILURE);
  }
  ...
  /* QEMU hack */
  if (!memcmp(footer.CreatorApplication, CookieVPCQEMUCreator,
              sizeof(footer.CreatorApplication)) &&
      (totals * 512U + footerlen > (UINT64)vhdlen)) {
    --cylinders;
    totals = (UINT64)sectors * surfaces * cylinders;
  }
