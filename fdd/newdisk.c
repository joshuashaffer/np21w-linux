...
#if defined(OSLANG_UTF8) || defined(OSLANG_UCS2)
  oemtext_oemtosjis((char *)d88head.fd_name, sizeof(d88head.fd_name), label,
                    (UINT)-1);
#else
...
static BRESULT writehddiplex2(FILEH fh, UINT ssize, FILELEN tsize, int blank,
                              int *progress, int *cancel) {
...
static BRESULT writehddiplex(FILEH fh, UINT ssize, FILELEN tsize, int *progress,
                             int *cancel) {
...
static void hddsize2CHS(UINT hddsizeMB, UINT32 *C, UINT16 *H, UINT16 *S,
                        UINT16 *SS) {
...
void newdisk_nhd_ex_CHS(const OEMCHAR *fname, UINT32 C, UINT16 H, UINT16 S,
                        UINT16 SS, int blank, int *progress, int *cancel) {
...
void newdisk_nhd_ex(const OEMCHAR *fname, UINT hddsize, int blank,
                    int *progress, int *cancel) {
...
void newdisk_vpcvhd_ex_CHS(const OEMCHAR *fname, UINT32 C, UINT16 H, UINT16 S,
                           UINT16 SS, int dynamic, int blank, int *progress,
                           int *cancel) {
  ...
  if (dynamic) {
    ...
    STOREMOTOROLAQWORD(vpcvhd_dh.TableOffset,
                       (UINT64)(sizeof(VPCVHDFOOTER) + sizeof(VPCVHDDDH)));
    ...
    r |= (file_write(fh, &vpcvhd, sizeof(vpcvhd)) == sizeof(vpcvhd)) ? SUCCESS
                                                                     : FAILURE;
    r |= (file_write(fh, &vpcvhd_dh, sizeof(vpcvhd_dh)) == sizeof(vpcvhd_dh))
             ? SUCCESS
             : FAILURE;
...
void newdisk_vpcvhd_ex(const OEMCHAR *fname, UINT hddsize, int dynamic,
                       int blank, int *progress, int *cancel) {
