...
static void nc_close(SXSIDEV sxsi) { (void)sxsi; }

static void nc_destroy(SXSIDEV sxsi) { (void)sxsi; }
...
BRESULT sxsi_devopen(REG8 drv, const OEMCHAR *fname) {
  ...
  case SXSIDEV_CDROM:
    ...
    if ((fname == NULL) || (fname[0] == '\0')) {
      ...
      if ((sxsi->flag & SXSIFLAG_READY) &&
          (_tcsnicmp(sxsi->fname, OEMTEXT("\\\\.\\"), 4) != 0 ||
           _tcsicmp(sxsi->fname, np2cfg.idecd[drv & 0x0f]) == 0)) {
        // いったん取り出す
        if (sxsi->flag & SXSIFLAG_FILEOPENED) {
          ideio_notify(sxsi->drv, 0);
          (*sxsi->close)(sxsi);
        ...
        cdchange_drv = drv;
        file_cpyname(sxsi->fname, _T("\0\0\0\0"), 1);
        file_cpyname(np2cfg.idecd[drv & 0x0f], _T("\0\0\0\0"),
                     NELEMENTS(cdchange_fname));
