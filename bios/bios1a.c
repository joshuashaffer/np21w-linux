...
void bios0x1a_pci_part(int is32bit) {
  ...
  case 0x02: // FIND PCI DEVICE
    ...
    for (i = 0; i < PCI_DEVICES_MAX; i++) {
      if (pcidev.devices[i].enable) {
        if (pcidev.devices[i].header.deviceID == CPU_CX &&
            pcidev.devices[i].header.vendorID == CPU_DX) {
          break;
        }
      }
    }
    if (i < PCI_DEVICES_MAX) {
      ...
      CPU_BL = (i << 3) |
               (0); // device/function number (bits 7-3 device, bits 2-0 func)
    } else {
      // 発見できず
  ...
  case 0x03: // FIND PCI CLASS CODE
    ...
    for (i = 0; i < PCI_DEVICES_MAX; i++) {
      if (pcidev.devices[i].enable) {
        if ((*((UINT32 *)pcidev.devices[i].header.classcode) & 0xffffff) ==
            (CPU_ECX & 0xffffff)) {
          if (idx == 0)
            break;
          idx--;
  ...
  case 0x09: // READ CONFIGURATION WORD
    ...
    if (CPU_BH == 0 /* && funcnum==0*/ && pcidev.devices[devnum].enable) {
      if (CPU_DI <= 0xff) {
        ...
        CPU_CX = *((
            UINT16
                *)(pcidev.devices[devnum].cfgreg8 +
                   CPU_DI)); // XXX:
                             // 2の倍数のレジスタ番号でなくても読めちゃうけどまあいいかー
      } else {
        CPU_AH = PCIBIOS_STATUS_BAD_PCI_REG_NUMBER;
        CPU_FLAGL |= C_FLAG;
        CPU_CX = 0xffff;
  ...
  case 0x0C: // WRITE CONFIGURATION WORD
    ...
    if (CPU_BH == 0 /* && funcnum==0*/ && pcidev.devices[devnum].enable) {
      if (CPU_DI <= 0xff) {
        CPU_AH = PCIBIOS_STATUS_SUCCESSFUL;
        CPU_FLAGL &= ~C_FLAG;
        *((UINT16 *)(pcidev.devices[devnum].cfgreg8 + CPU_DI)) =
            CPU_CX; // XXX:
                    // 2の倍数のレジスタ番号でなくても書けちゃうけどまあいいかー
      } else {
        CPU_AH = PCIBIOS_STATUS_BAD_PCI_REG_NUMBER;
        CPU_FLAGL |= C_FLAG;
      }
  ...
  case 0x0D: // WRITE CONFIGURATION DWORD
    ...
    if (CPU_BH == 0 /* && funcnum==0*/ && pcidev.devices[devnum].enable) {
      if (CPU_DI <= 0xff) {
        CPU_AH = PCIBIOS_STATUS_SUCCESSFUL;
        CPU_FLAGL &= ~C_FLAG;
        *((UINT32 *)(pcidev.devices[devnum].cfgreg8 + CPU_DI)) =
            CPU_ECX; // XXX:
                     // 4の倍数のレジスタ番号でなくても書けちゃうけどまあいいかー
      } else {
        CPU_AH = PCIBIOS_STATUS_BAD_PCI_REG_NUMBER;
        CPU_FLAGL |= C_FLAG;
      }
  ...
  case 0x0E: // GET IRQ ROUTING INFORMATION
    if (CPU_BX == 0x0000) {
      ...
      if (is32bit) {
        ...
        dataAddress = (UINT32)MEMR_READ16(CPU_ES, CPU_EDI + 2) |
                      (((UINT32)MEMR_READ16(CPU_ES, CPU_EDI + 4)) << 16);
        if (dataSize <
            pcidev.biosdata.datacount * sizeof(_PCIPNP_IRQTBL_ENTRY)) {
          MEMR_WRITE16(CPU_ES, CPU_EDI,
                       pcidev.biosdata.datacount *
                           sizeof(_PCIPNP_IRQTBL_ENTRY));
          ...
          MEMR_WRITES((dataAddress >> 16) & 0xffff, dataAddress & 0xffff,
                      pcidev.biosdata.data, dataSize);
        ...
        dataAddress = (UINT32)MEMR_READ16(CPU_ES, CPU_DI + 2) |
                      (((UINT32)MEMR_READ16(CPU_ES, CPU_DI + 4)) << 16);
        if (dataSize <
            pcidev.biosdata.datacount * sizeof(_PCIPNP_IRQTBL_ENTRY)) {
          MEMR_WRITE16(CPU_ES, CPU_DI,
                       pcidev.biosdata.datacount *
                           sizeof(_PCIPNP_IRQTBL_ENTRY));
          CPU_AH = PCIBIOS_STATUS_BUFFER_TOO_SMALL;
          CPU_FLAGL |= C_FLAG;
...
void bios0x1a_pci(void) {
  ...
  if (pcidev.enable) {
    ...
    // XXX: np2
    // BIOSがDXレジスタをPUSH/POPしてしまうので、DXレジスタの内容をスタックから強引に拾ってくる
...
#if !defined(SUPPORT_IA32_HAXM)
    // XXX: np2
    // BIOSがDXレジスタをPUSH/POPしてしまうので、DXレジスタの内容をスタックに強引に書き込む
...
void bios0x1a_pcipnp(void) {
  ...
  if (pcidev.enable) {
    ...
    case 0x00: // Intel Plug-and-Play AUTO-CONFIGURATION - INSTALLATION CHECK
      ...
      break;
    case 0x06: // Intel Plug-and-Play AUTO-CONFIGURATION - GET PCI IRQ ROUTING
               // TABLE
      if (CPU_BX == 0x0000) {
        UINT16 dataSize = 0;
