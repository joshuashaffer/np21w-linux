...
static const UINT8 msw_default[8] = {0x48, 0x05, 0x04, 0x00,
                                     0x01, 0x00, 0x00, 0x6e};
...
static void bios_updatehookinst(UINT8 *mem, UINT32 updatesize) {
  ...
  if (bioshookinfo.hookinst == HOOKINST_DEFAULT)
    return; // 書き換え不要
  ...
  for (i = 0; i < updatesize - 1; i++) {
    if (*mem == HOOKINST_DEFAULT) {
      // 次の命令がNOP, STI, RET,
      // IRETっぽければ書き換え（間違って書き換えるのを回避）
      if (*(mem + 1) == 0x90 || *(mem + 1) == 0xFB || *(mem + 1) == 0xC2 ||
          *(mem + 1) == 0xC3 || *(mem + 1) == 0xCB || *(mem + 1) == 0xCA ||
          *(mem + 1) == 0xCF || *(mem + 1) == 0xE8 || *(mem + 1) == 0xE9 ||
          *(mem + 1) == 0xEE) {
...
static void bios_reinitbyswitch(void) {
  ...
  if (pccore.hddif & PCHDD_IDE) {
    ...
    mem[0xF8E80 + 0x0010] = (sxsi_getdevtype(3) != SXSIDEV_NC ? 0x8 : 0x0) |
                            (sxsi_getdevtype(2) != SXSIDEV_NC ? 0x4 : 0x0) |
                            (sxsi_getdevtype(1) != SXSIDEV_NC ? 0x2 : 0x0) |
                            (sxsi_getdevtype(0) != SXSIDEV_NC ? 0x1 : 0x0);

    // WinNT4.0でHDDが認識するようになる。Win9xもBIOS
    // I/Oエミュレーションで対応。　ideio.cのideio_basereset()も参照のこと
    mem[0x05bb] = (sxsi_getdevtype(3) == SXSIDEV_HDD ? 0x8 : 0x0) |
                  (sxsi_getdevtype(2) == SXSIDEV_HDD ? 0x4 : 0x0) |
                  (sxsi_getdevtype(1) == SXSIDEV_HDD ? 0x2 : 0x0) |
                  (sxsi_getdevtype(0) == SXSIDEV_HDD
                       ? 0x1
                       : 0x0); // XXX: 未使用って書いてあったので勝手に借りる
    if (compmode) {
    ...
    if (np2cfg.winntfix) {
      // WinNT3.50で必要
      if (sxsi_getdevtype(1) == SXSIDEV_NC &&
          sxsi_getdevtype(3) == SXSIDEV_NC) {
        mem[0x0457] = (sxsi_getdevtype(2) == SXSIDEV_HDD ? 0x42 : 0x07) |
                      (sxsi_getdevtype(0) == SXSIDEV_HDD
                           ? 0x90
                           : 0x38); // 0xd2; // 接続なしは111でないと駄目
  ...
  mem[0xF8E80 + 0x0011] =
      mem[0xF8E80 + 0x0011] &
      ~0x20; // 0x20のビットがONだとWin2000でマウスがカクカクする？
  if (np2cfg.modelnum)
    mem[0xF8E80 + 0x003F] = np2cfg.modelnum; // PC-9821 Model Number
...
static void bios_vectorset(void) {
  ...
  for (i = 0; i < 0x20; i++) {
    *(UINT16 *)(mem + (i * 4)) =
        *(UINT16 *)(mem + BIOS_BASE + BIOS_TABLE + (i * 2));
...
void bios_initialize(void) {
...
#if defined(SUPPORT_IA32_HAXM)
  if (np2hax.enable) {
    bioshookinfo.hookinst =
        0xCC; // 0xF4;//;0xCC;//HOOKINST_DEFAULT; //
              // BIOSフックに使う命令（デフォルトはNOP命令をフック）
  } else
#endif
  {
    bioshookinfo.hookinst =
        HOOKINST_DEFAULT; // BIOSフックに使う命令（デフォルトはNOP命令をフック）
  }
#endif
  ...
  if (np2cfg.memchkmx) { // メモリカウント最大値変更
    mem[ITF_ADRS + 6057] = mem[ITF_ADRS + 6061] =
        (UINT8)max((int)np2cfg.memchkmx - 14, 1); // XXX: 場所決め打ち
  } else {
    ...
    if (np2cfg.EXTMEM >= 256) { // 大容量メモリカウント
      ...
      mem[ITF_ADRS + 6067] = mem[ITF_ADRS + 6068] =
          BIOS_HOOKINST; // call	WAITVSYNC を NOP化
      mem[ITF_ADRS + 6055] =
          0x81; // CMP r/m16, imm8 を CMP r/m16, imm16 に変える
      STOREINTELWORD(
          (mem + ITF_ADRS + 6057),
          (MEMORY_MAXSIZE - 14)); // cmp　bx, (EXTMEMORYMAX - 16)の部分をいじる
      STOREINTELWORD(
          (mem + ITF_ADRS + 6061 + 1),
          (MEMORY_MAXSIZE -
           14)); // mov　bx, (EXTMEMORYMAX -
                 // 16)の部分をいじる（1byteずらしたのでオフセット注意）
    }
#endif
  }
  if (np2cfg.sbeeplen || np2cfg.sbeepadj) { // ピポ音長さ変更
    UINT16 beeplen =
        (np2cfg.sbeeplen ? np2cfg.sbeeplen
                         : mem[ITF_ADRS + 5553]); // XXX: 場所決め打ち
  ...
  if (i == 0x10000) {
    ...
    for (i = 0; i < 0x400; i += 0x10) {
      ...
      if (emptyflag) {
        ...
        TRACEOUT(("Allocate BIOS32 Service Directory at 0x%.5x",
                  pcidev.bios32svcdir));
...
void biosioemu_push16_read(UINT16 port) {
  ...
  if (biosioemu.count < BIOSIOEMU_DATA_MAX) {
    biosioemu.data[biosioemu.count].flag =
        BIOSIOEMU_FLAG_READ | BIOSIOEMU_FLAG_MB;
    biosioemu.data[biosioemu.count].port = port;
    biosioemu.data[biosioemu.count].data = 0;
    biosioemu.count++;
  }
...
//	TRACEOUT(("biosfunc(%x)", adrs));
  ...
  case BIOS_BASE + BIOSOFST_INIT: // ブート
    ...
    bios_vectorset();
    bios_screeninit();
    if (((pccore.model & PCMODELMASK) >= PCMODEL_VX) && (pccore.sound & 0x7e)) {
