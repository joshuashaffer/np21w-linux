...
static const CRTDATA crtdata[7] = {{0x09, 0x1f, 0x08, 0x08},  // 200-20
...
static REG8 bios0x18_30(REG8 rate, REG8 scrn) {
  ...
  if (CPU_STAT_PM && CPU_STAT_VM86 && biosioemu.enable) {
    biosioemu_enq8(
        0x6a, 0x40 | ((gdc.display & (1 << GDCDISP_PLAZMA))
                          ? 1
                          : 0)); // gdcs.textdisp |= GDCSCRN_EXT; の代わり・・・
    biosioemu_enq8(0x6a,
                   0x82 | (gdc.clock & 1)); // gdcs.grphdisp |= GDCSCRN_EXT;
    biosioemu_enq8(0x62, CMD_STOP);         // gdcs.textdisp &= ~GDCSCRN_ENABLE;
                                            // pcstat.screenupdate |= 2;
  } else
#endif
  {
...
static void bios0x18_47(void) {
  ...
  csrw += (GBSX1 & 0xf) << 20;
  GBMDOTI =
      (GDCPATREVERSE(ucw.GBMDOTI[0]) << 8) + GDCPATREVERSE(ucw.GBMDOTI[1]);
  if ((CPU_CH & 0x30) == 0x30) {
    ope = (ucw.GBON_PTN & 1) ? GDCOPE_SET : GDCOPE_CLEAR;
...
static void bios0x18_4d(REG8 mode) {
  ...
  if ((mem[0x45c] & 0x40) && ((mem[MEMB_CRT_BIOS] & 3) == 2)) {
    if (mode == 0) {
      gdc_analogext(FALSE);
