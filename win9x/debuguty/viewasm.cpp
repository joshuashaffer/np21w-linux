...
CDebugUtyAsm::CDebugUtyAsm(CDebugUtyView *lpView)
    : CDebugUtyItem(lpView, IDM_VIEWMODEASM), m_nSegment(0), m_nOffset(0) {}
...
bool CDebugUtyAsm::Lock() {
  ...
  m_mem.Update();
  m_mem.Read(m_nSegment << 4, &m_buffer.at(0),
 ...
 */
bool CDebugUtyAsm::IsLocked() { return (!m_buffer.empty()); }
 ...
 */
void CDebugUtyAsm::ReadMemory(UINT nOffset, unsigned char *lpBuffer,
                              UINT cbBuffer) const {
  while (cbBuffer) {
