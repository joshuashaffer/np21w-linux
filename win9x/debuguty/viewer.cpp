...
void CDebugUtyView::Initialize(HINSTANCE hInstance) {
  ...
  np2vc.cbClsExtra = 0;
  np2vc.cbWndExtra = 0;
  np2vc.hInstance = sm_hInstance; // hInstance; // XXX:
                                  // 言語リソースを使うとデバッグユーティリティが表示できなくなるのをごまかす
                                  // np21w ver0.86 rev13
...
void CDebugUtyView::New() {
  for (size_t i = 0; i < _countof(g_np2view); i++) {
    ...
    if (view->CreateEx(0, s_szViewClass, NULL, WS_OVERLAPPEDWINDOW | WS_VSCROLL,
...
CDebugUtyView::CDebugUtyView()
    : m_bActive(false), m_nVPos(0), m_nVLines(0), m_nVPage(0), m_nVMultiple(1),
      m_lpItem(NULL) {}

/**
 * デストラクタ
 */
CDebugUtyView::~CDebugUtyView() { DetachDebugView(); }
...
LRESULT CDebugUtyView::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_SIZE:
    OnSize(static_cast<UINT>(wParam), GET_X_LPARAM(lParam),
           GET_Y_LPARAM(lParam));
    break;
...
void CDebugUtyView::OnPaint() {
  ...
  HFONT hFont = ::CreateFont(16, 0, 0, 0, 0, 0, 0, 0, SHIFTJIS_CHARSET,
...
/**
 * フレームワークは、ユーザーがウィンドウに垂直スクロール
 * バーをクリックすると、このメンバー関数を呼び出します
...
/**
 * The framework calls this member function when a menu modal loop has been
 * entered
 * @param[in] bIsTrackPopupMenu Specifies whether the menu involved is a popup
 * menu
 */
void CDebugUtyView::OnEnterMenuLoop(BOOL bIsTrackPopupMenu) {
  ...
  ::CheckMenuItem(hMenu, IDM_VIEWMODELOCK,
                  MF_BYCOMMAND | MFCHECK(m_lpItem->IsLocked()));

  const UINT nID = m_lpItem->GetID();
#if 1
  ::CheckMenuRadioItem(hMenu, IDM_VIEWMODEREG, IDM_VIEWMODESND, nID,
                       MF_BYCOMMAND);
#else
  ::CheckMenuItem(hMenu, IDM_VIEWMODEREG,
                  MF_BYCOMMAND | MFCHECK(nID == IDM_VIEWMODEREG));
  ::CheckMenuItem(hMenu, IDM_VIEWMODESEG,
                  MF_BYCOMMAND | MFCHECK(nID == IDM_VIEWMODESEG));
  ::CheckMenuItem(hMenu, IDM_VIEWMODE1MB,
                  MF_BYCOMMAND | MFCHECK(nID == IDM_VIEWMODE1MB));
  ::CheckMenuItem(hMenu, IDM_VIEWMODEASM,
                  MF_BYCOMMAND | MFCHECK(nID == IDM_VIEWMODEASM));
  ::CheckMenuItem(hMenu, IDM_VIEWMODESND,
                  MF_BYCOMMAND | MFCHECK(nID == IDM_VIEWMODESND));
#endif
...
/**
 * フレームワークは CWnd
 * のオブジェクトがアクティブ化または非アクティブ化されたときにこのメンバー関数が呼び出されます
 * @param[in] nState
 * アクティブになっているか非アクティブになっているかを指定します
 * @param[in] hwndOther アクティブまたは非アクティブになるウィンドウ ハンドル
 * @param[in] bMinimized
 * アクティブまたは非アクティブになるウィンドウの最小化の状態を指定します
 */
 ...
 * destroyed.
 */
void CDebugUtyView::PostNcDestroy() { delete this; }
 ...
 */
void CDebugUtyView::SetSegmentItem(HMENU hMenu, int nId, LPCTSTR lpSegment,
                                   UINT nSegment) {
  TCHAR szString[32];
