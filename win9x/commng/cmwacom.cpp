...
LRESULT CALLBACK tabletWndProc(HWND hWnd, UINT msg, WPARAM wParam,
                               LPARAM lParam) {
  if (g_cmwacom) {
    switch (msg) {
    case WT_PACKET:
 ...
 */
CComWacom::CComWacom()
    : CComBase(COMCONNECT_TABLET), m_hwndMain(NULL), m_hTab(NULL), m_hMgr(NULL),
      m_cmdbuf_pos(0), m_sBuffer_wpos(0), m_sBuffer_rpos(0), m_wait(0),
      m_skipmouseevent(0), m_skiptabletevent(0), m_exclusivemode(false),
      m_nccontrol(false), m_lastdatalen(0), m_sendlastdata(false) {
...
void CComWacom::GetConfig(CMWACOM_CONFIG *cfg) { *cfg = m_config; }
void CComWacom::SetConfig(CMWACOM_CONFIG cfg) { m_config = cfg; }
...
void CComWacom::InitializeTabletDevice() {
  ...
  lcMine.lcOptions |=
      CXO_MESSAGES | CXO_MARGIN; /* Wintabﾒｯｾｰｼﾞが渡されるようにする */
  ...
  m_hTab = gpWTOpenA(m_hwndMain, &lcMine,
                     GetForegroundWindow() == m_hwndMain ? TRUE : FALSE);
  if (!m_hTab) {
    return;
  }
...
HCTX CComWacom::GetHTab() { return m_hTab; }
bool CComWacom::HandlePacketMessage(HWND hWnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam) {
  PACKET tPckt;
  ...
  if (gpWTPacket((HCTX)LOWORD(lParam), (UINT)wParam, (LPSTR)&tPckt)) {
    ...
    if (m_rawPressureMax - m_rawPressureMin > 0) {
      if (m_rawPressure <= m_rawPressureMin) {
        ...
        m_pressure = (double)(m_rawPressure - m_rawPressureMin) /
                     (m_rawPressureMax - m_rawPressureMin);
      }
    } else {
      m_pressure = 0.0;
    }
    ...
    if (m_rawTangentPressureMax > 0) {
      if (m_rawTangentPressure < m_rawTangentPressureMin) {
        ...
        m_tangentPressure =
            (double)(m_rawTangentPressure - m_rawTangentPressureMin) /
            (m_rawTangentPressureMax - m_rawTangentPressureMin);
      }
    } else {
      m_tangentPressure = 0.0;
    }
    ...
    if (m_wait == 0) {
      ...
      UINT16 pktX, pktY;
      SINT32 tablet_resX =
          TABLET_RAWMAX_X * m_config.resolution_w / TABLET_BASERASOLUTION;
      SINT32 tablet_resY =
          TABLET_RAWMAX_Y * m_config.resolution_h / TABLET_BASERASOLUTION;
      char buf[32];
      ...
      if (!m_config.disablepressure) {
        if (m_pressure < 0.5) {
          m_rawButtons &= ~0x1;
          pktPressure = pktPressure =
              (UINT32)((m_pressure) / (1.0 - 0.5) * 255);
        } else {
          pktPressure = pktPressure =
              (UINT32)((m_pressure - 0.5) / (1.0 - 0.5) * 255);
        }
      }
      if (m_exclusivemode) {
        // 排他モード（マウスキャプチャ中）
        ...
        if (m_fixedaspect) {
          ...
          if (tablet_resX * (m_maxY - m_minY) >
              tablet_resY * (m_maxX - m_minX)) {
            pktXtmp = (m_rawX * tablet_resX / (m_maxX - m_minX));
            pktYtmp = tablet_resY - (m_rawY * tablet_resX /
                                     (m_maxX - m_minX)); // 左下原点ですってよ
          } else {
            pktXtmp = (m_rawX * tablet_resY / (m_maxY - m_minY));
            pktYtmp = tablet_resY - (m_rawY * tablet_resY /
                                     (m_maxY - m_minY)); // 左下原点ですってよ
          }
        } else {
          pktXtmp = (m_rawX * tablet_resX / (m_maxX - m_minX));
          pktYtmp = tablet_resY - (m_rawY * tablet_resY /
        ...
        pktXtmp = ((m_rawX - pt.x) * tablet_resX /
                   (rectClient.right - rectClient.left));
        pktYtmp = (((m_maxY - m_rawY) - pt.y) * tablet_resY /
                   (rectClient.bottom - rectClient.top));
        // pktXtmp = m_rawX - pt.x;
        // pktYtmp = m_rawY - pt.y;
        if (pktXtmp < 0 || pktYtmp < 0 || pktXtmp > tablet_resX ||
            pktYtmp > tablet_resY) {
          // 範囲外は移動のみ可
      ...
      if (pktYtmp < 0)
        pktYtmp = 0;
      if (pktXtmp > tablet_resX)
        pktXtmp = tablet_resX;
      if (pktYtmp > tablet_resY)
        pktYtmp = tablet_resY;
      ...
      if (m_config.disablepressure && m_config.csvmode) {
        ...
        if (m_config.disablepressure) {
          ...
          // 通常モード
          buf[0] = (proximityflag ? 0xA0 : 0xE0) |
                   ((m_rawButtons & ~0x1) ? 0x8 : 0) | ((pktX >> 14) & 0x3) |
                   ((((pktPressure)&0) << 2));
          buf[1] = ((pktX >> 7) & 0x7f);
          buf[2] = (pktX & 0x7f);
          buf[3] = ((m_rawButtons & 0xf) << 3) | ((pktY >> 14) & 0x3) |
                   ((((pktPressure >> 1) & 0) << 2));
          buf[4] = ((pktY >> 7) & 0x7f);
          buf[5] = (pktY & 0x7f);
          buf[6] = (((m_rawButtons & ~0x4) ? 0x00 : 0x40)) |
                   ((pktPressure >> 2) & 0x3f);
        }
...
bool CComWacom::HandleMouseMoveMessage(HWND hWnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam) {
  if (!m_config.enable)
    return false;
...
bool CComWacom::HandleMouseUpMessage(HWND hWnd, UINT msg, WPARAM wParam,
                                     LPARAM lParam) {
  if (!m_config.enable)
    return false;
...
bool CComWacom::HandleMouseDownMessage(HWND hWnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam) {
  if (!m_config.enable)
    return false;
  if (!m_exclusivemode && m_skipmouseevent > 0) {
...
bool CComWacom::SkipMouseEvent() { return (m_skipmouseevent > 0); }
...
void CComWacom::SetNCControl(bool enable) { m_nccontrol = enable; }
  ...
  if (m_hTab) {
    if (cData == '\r' ||
      ...
      if (strcmp(m_cmdbuf, "#") == 0) {
        ...
        // I'm Fine!
        char data[] =
            "KT-0405-R00 V1.3-2 95/04/28 by WACOM\r\nFINE\r\nI AM FINE.\r\n";
      ...
      m_cmdbuf_pos++;
      if (m_cmdbuf_pos >= 2 && strncmp(&m_cmdbuf[m_cmdbuf_pos - 2], "~#", 2) ==
                                   0) {  // 例外的に即応答
        m_sBuffer_rpos = m_sBuffer_wpos; //バッファ消す
...
INTPTR CComWacom::Message(UINT nMessage, INTPTR nParam) {
  ...
  case COMMSG_SETFLAG: {
    ...
    if ((flag) && (flag->size == sizeof(_COMFLAG) + sizeof(m_config)) &&
  ...
  case COMMSG_GETFLAG: {
    COMFLAG flag =
        (COMFLAG)_MALLOC(sizeof(_COMFLAG) + sizeof(m_config), "PENTAB FLAG");
    if (flag) {
