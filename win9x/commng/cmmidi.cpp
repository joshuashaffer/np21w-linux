...
LPCTSTR cmmidi_mdlname[12] = {
    TEXT("MT-32"),      TEXT("CM-32L"),     TEXT("CM-64"), TEXT("CM-300"),
    TEXT("CM-500(LA)"), TEXT("CM-500(GS)"), TEXT("SC-55"), TEXT("SC-88"),
    TEXT("LA"),         TEXT("GM"),         TEXT("GS"),    TEXT("XG")};
...
static const UINT8 EXCV_MTRESET[] = {0xf0, 0x41, 0x10, 0x16, 0x12, 0x7f,
                                     0x00, 0x00, 0x00, 0x01, 0xf7};
static const UINT8 EXCV_GMRESET[] = {0xf0, 0x7e, 0x7f, 0x09, 0x01, 0xf7};
static const UINT8 EXCV_GM2RESET[] = {0xf0, 0x7e, 0x7f, 0x09, 0x03, 0xf7};
static const UINT8 EXCV_GSRESET[] = {0xf0, 0x41, 0x10, 0x42, 0x12, 0x40,
 ...
 * @return インスタンス
 */
CComMidi *CComMidi::CreateInstance(LPCTSTR lpMidiOut, LPCTSTR lpMidiIn,
                                   LPCTSTR lpModule) {
 ...
 */
CComMidi::CComMidi()
    : CComBase(COMCONNECT_MIDI), m_pMidiIn(NULL), m_pMidiOut(NULL),
      m_nModule(MIDI_OTHER), m_nMidiCtrl(MIDICTRL_READY), m_nIndex(0),
      m_nRecvSize(0), m_cLastData(0), m_bMimpiDef(false) {
 ...
 * @retval false 失敗
 */
bool CComMidi::Initialize(LPCTSTR lpMidiOut, LPCTSTR lpMidiIn,
                          LPCTSTR lpModule) {
#if defined(VERMOUTH_LIB)
 ...
 * @return ステータス
 */
UINT8 CComMidi::GetStat() { return 0x00; }
...
INTPTR CComMidi::Message(UINT nMessage, INTPTR nParam) {
  ...
  case COMMSG_SETFLAG: {
    ...
    if ((flag) && (flag->size == sizeof(_COMFLAG) + sizeof(m_midich)) &&
  ...
  case COMMSG_GETFLAG: {
    COMFLAG flag =
        (COMFLAG)_MALLOC(sizeof(_COMFLAG) + sizeof(m_midich), "MIDI FLAG");
    if (flag) {
