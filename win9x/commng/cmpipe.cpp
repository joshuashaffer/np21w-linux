 ...
 */
CComPipe::CComPipe()
    : CComBase(COMCONNECT_SERIAL), m_hSerial(INVALID_HANDLE_VALUE),
      m_isserver(false), m_lastdata(0), m_lastdatafail(0) {}
...
bool CComPipe::Initialize(LPCTSTR pipename, LPCTSTR servername) {
  ...
  if (_tcslen(pipename) + (servername != NULL ? _tcslen(servername) : 0) >
      MAX_PATH - 16) {
    // too long pipe name
    return false;
  }
  if (_tcschr(pipename, '\\') != NULL ||
      (servername != NULL && _tcschr(servername, '\\') != NULL)) {
  ...
  // クライアント接続をトライ
  m_hSerial = CreateFile(szName, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                         OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (m_hSerial == INVALID_HANDLE_VALUE) {
    // サーバーで再トライ
    m_hSerial =
        CreateNamedPipe(szName, PIPE_ACCESS_DUPLEX,
                        PIPE_TYPE_BYTE | PIPE_NOWAIT, 1, 1024, 1024, 500, NULL);
    if (m_hSerial == INVALID_HANDLE_VALUE) {
      return false;
    }
    m_isserver = true;
    // ConnectNamedPipe(m_hSerial, NULL); // With a nonblocking-wait handle, the
    // connect operation returns zero immediately, and the GetLastError function
    // returns ERROR_PIPE_LISTENING. とのことなので常時0
  }
...
UINT CComPipe::WriteRetry() {
  ...
  DWORD dwWrittenSize;
  if (m_lastdatafail) {
    if (GetTickCount() - m_lastdatatime > 1000)
      return 1; // バッファデータが減りそうにないならあきらめる（毎秒1byte(8bit)は流石にあり得ない）
    if (m_hSerial == INVALID_HANDLE_VALUE) {
      return 0;
    }
    ret =
        (::WriteFile(m_hSerial, &m_lastdata, 1, &dwWrittenSize, NULL)) ? 1 : 0;
    if (dwWrittenSize == 0) {
      return 0;
    }
    m_lastdatafail = 0;
 ...
 * @return ステータス
 */
UINT8 CComPipe::GetStat() { return 0x20; }
