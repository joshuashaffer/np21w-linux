...
bool CComSerial::Initialize(UINT nPort, UINT8 cParam, UINT32 nSpeed,
  ...
  m_hSerial = CreateFile(szName, GENERIC_READ | GENERIC_WRITE, 0, 0,
...
INTPTR CComSerial::Message(UINT nMessage, INTPTR nParam) {
  ...
  case COMMSG_CHANGEMODE:
    if (!m_fixedspeed) {
      bool changed = false;
      UINT8 newmode =
          *(reinterpret_cast<UINT8 *>(nParam)); // I/O 32h モードセットのデータ
      BYTE stopbits_value[] = {ONESTOPBIT, ONESTOPBIT, ONE5STOPBITS,
                               TWOSTOPBITS};
      ...
      if (changed) {
        ::PurgeComm(m_hSerial, PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR |
                                   PURGE_RXCLEAR);
        ::SetCommState(m_hSerial, &dcb);
      }
      break;
