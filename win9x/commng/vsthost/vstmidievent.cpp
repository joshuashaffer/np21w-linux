 ...
 * コンストラクタ
 */
CVstMidiEvent::CVstMidiEvent() : m_nEvents(0) {}
 ...
 */
void CVstMidiEvent::LongMessage(UINT nTick, const void *lpMessage,
                                UINT cbMessage) {
 ...
 */
void CVstMidiEvent::Add(const VstEvent *pEvent, const void *lpMessage,
                        UINT cbMessage) {
  m_event.insert(m_event.end(), reinterpret_cast<const unsigned char *>(pEvent),
                 reinterpret_cast<const unsigned char *>(pEvent) +
                     pEvent->byteSize);
  if ((lpMessage) && (cbMessage)) {
    m_event.insert(m_event.end(), static_cast<const unsigned char *>(lpMessage),
                   static_cast<const unsigned char *>(lpMessage) + cbMessage);
    // needs padding?
  }
  m_nEvents++;
}
...
const VstEvents *CVstMidiEvent::GetEvents() {
  ...
  for (UINT i = 0; i < m_nEvents; i++) {
    ...
    if (pEvent->type == kVstSysExType) {
      VstMidiSysexEvent *pSysexEvent =
          reinterpret_cast<VstMidiSysexEvent *>(pEvent);
