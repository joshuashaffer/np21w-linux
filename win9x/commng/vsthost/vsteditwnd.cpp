...
void CVstEditWnd::OnIdle() {
  for (std::map<HWND, CVstEditWnd *>::iterator it = sm_wndMap.begin();
       it != sm_wndMap.end(); ++it) {
 ...
 * コンストラクタ
 */
CVstEditWnd::CVstEditWnd() : m_hWnd(NULL), m_pEffect(NULL) {}
 ...
 * @retval false 失敗
 */
bool CVstEditWnd::Create(CVstEffect *pEffect, LPCTSTR lpszWindowName,
                         DWORD dwStyle, LONG x, LONG y) {
  if (pEffect == NULL) {
    return false;
  }
  ERect *lpRect = NULL;
  ...
  HWND hWnd = ::CreateWindow(s_szClassName, lpszWindowName, dwStyle, x, y,
                             nWidth, nHeight, NULL, NULL, sm_hInstance, this);
  if (hWnd == NULL) {
    return false;
  }
 ...
 */
LRESULT CALLBACK CVstEditWnd::WndProc(HWND hWnd, UINT message, WPARAM wParam,
                                      LPARAM lParam) {
  ...
  if (message == WM_CREATE) {
    ...
    pWnd = reinterpret_cast<CVstEditWnd *>(
...
bool CVstEditWnd::OnResize(int nWidth, int nHeight) {
  ...
  for (UINT i = 0; i < 2; i++) {
    ...
    const LONG w = nWidth + (rectWindow.right - rectWindow.left) -
                   (rectClient.right - rectClient.left);
    const LONG h = nHeight + (rectWindow.bottom - rectWindow.top) -
 ...
 * @retval false 失敗
 */
bool CVstEditWnd::OnUpdateDisplay() { return (m_hWnd != NULL); }
