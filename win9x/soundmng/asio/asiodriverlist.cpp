...
void AsioDriverList::EnumerateDrivers() {
  clear();
  CRegKey asio;
  if (asio.Open(HKEY_LOCAL_MACHINE, TEXT("software\\asio"),
                KEY_ENUMERATE_SUB_KEYS) == ERROR_SUCCESS) {
    for (DWORD dwIndex = 0;; dwIndex++) {
      TCHAR szKeyName[MAX_PATH];
      if (::RegEnumKey(asio, dwIndex, szKeyName, _countof(szKeyName)) !=
          ERROR_SUCCESS) {
        break;
      }
      ...
      if (drv.QueryValue(info.szDriverName, TEXT("description"), &nSize) !=
          ERROR_SUCCESS) {
 ...
 * @retval false 失敗
 */
bool AsioDriverList::FindDrvPath(LPCTSTR lpClsId, LPTSTR lpDllPath,
                                 UINT cchDllPath) {
...
IASIO *AsioDriverList::OpenDriver(LPCTSTR lpDriverName) {
  ...
  if (it != end()) {
    VOID *pAsio = NULL;
    if (::CoCreateInstance(it->clsid, 0, CLSCTX_INPROC_SERVER, it->clsid,
                           &pAsio) == S_OK) {
