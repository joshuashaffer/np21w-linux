...
void CSoundDeviceWasapi::Initialize() {
  ...
  if (SUCCEEDED(::CoCreateInstance(CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL,
                                   ...
                                   reinterpret_cast<PVOID *>(&pEnumerator)))) {
      ...
      if (SUCCEEDED(pDevices->GetCount(&nNumDevices))) {
        for (UINT i = 0; i < nNumDevices; i++) {
          ...
          if (SUCCEEDED(pDevice->OpenPropertyStore(STGM_READ, &pPropStore))) {
            ...
            USES_CONVERSION;
            ::lstrcpyn(dev.szDevice, W2CT(varName.pwszVal),
                       _countof(dev.szDevice));
...
void CSoundDeviceWasapi::Deinitialize() {
  if (!sm_devices.empty()) {
    for (std::vector<WasapiDevice>::iterator it = sm_devices.begin();
...
void CSoundDeviceWasapi::EnumerateDevices(std::vector<LPCTSTR> &devices) {
  for (std::vector<WasapiDevice>::const_iterator it = sm_devices.begin();
       it != sm_devices.end(); ++it) {
...
CSoundDeviceWasapi::CSoundDeviceWasapi()
    : m_pEnumerator(NULL), m_pDevice(NULL), m_pAudioClient(NULL), m_pwfx(NULL),
      m_pRenderClient(NULL), m_nBufferSize(0), m_mastervolume(100) {
 ...
 * デストラクタ
 */
CSoundDeviceWasapi::~CSoundDeviceWasapi() { Close(); }
...
bool CSoundDeviceWasapi::Open(LPCTSTR lpDevice, HWND hWnd) {
  if (FAILED(::CoCreateInstance(CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL,
                                IID_IMMDeviceEnumerator,
                                reinterpret_cast<PVOID *>(&m_pEnumerator)))) {
    return false;
  }
  ...
  if ((lpDevice) && (lpDevice[0] != '\0')) {
    ...
    hr = m_pEnumerator->GetDefaultAudioEndpoint(eRender, eMultimedia,
                                                &m_pDevice);
  }
 ...
 * @return バッファ サイズ
 */
UINT CSoundDeviceWasapi::CreateStream(UINT nSamplingRate, UINT nChannels,
                                      UINT nBufferSize) {
  do {
    if (m_pDevice == NULL) {
      break;
    ...
    if (FAILED(
            m_pDevice->Activate(IID_IAudioClient, CLSCTX_ALL, NULL,
                                reinterpret_cast<PVOID *>(&m_pAudioClient)))) {
      break;
    }
    ...
    if (FAILED(m_pAudioClient->IsFormatSupported(AUDCLNT_SHAREMODE_EXCLUSIVE,
                                                 m_pwfx, NULL))) {
    ...
    if (nBufferSize == 0) {
      ...
      hnsRequestedDuration = static_cast<REFERENCE_TIME>(
          ((nBufferSize * 10000000.0) / nSamplingRate) + 0.5);
    }
    HRESULT hr = m_pAudioClient->Initialize(
        AUDCLNT_SHAREMODE_EXCLUSIVE,
        AUDCLNT_STREAMFLAGS_EVENTCALLBACK | AUDCLNT_STREAMFLAGS_NOPERSIST,
        hnsRequestedDuration, hnsRequestedDuration, m_pwfx, NULL);
    if (hr == AUDCLNT_ERR(0x019) /* AUDCLNT_E_BUFFER_SIZE_NOT_ALIGNED */) {
      ...
      hnsRequestedDuration = static_cast<REFERENCE_TIME>(
          ((nCurrentBufferSize * 10000000.0) / nSamplingRate) + 0.5);
    ...
    if (FAILED(m_pAudioClient->GetService(
            IID_IAudioRenderClient,
            reinterpret_cast<PVOID *>(&m_pRenderClient)))) {
      break;
    }
...
bool CSoundDeviceWasapi::Task() {
  ...
  switch (
      ::WaitForMultipleObjects(_countof(m_hEvents), m_hEvents, 0, INFINITE)) {
...
void CSoundDeviceWasapi::FillStream() {
  if (m_pRenderClient) {
    ...
    if (SUCCEEDED(hr)) {
      ...
      if (m_pSoundData) {
        nStreamLength = m_pSoundData->Get16(reinterpret_cast<short *>(pData),
                                            m_nBufferSize);
      }
      if (nStreamLength != m_nBufferSize) {
        ZeroMemory(pData + (nStreamLength * m_pwfx->nBlockAlign),
                   (m_nBufferSize - nStreamLength) * m_pwfx->nBlockAlign);
      }
