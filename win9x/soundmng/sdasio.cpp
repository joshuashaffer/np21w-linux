 ...
 * 初期化
 */
void CSoundDeviceAsio::Initialize() { sm_asioDriverList.EnumerateDrivers(); }
...
void CSoundDeviceAsio::EnumerateDevices(std::vector<LPCTSTR> &devices) {
  for (AsioDriverList::const_iterator it = sm_asioDriverList.begin();
       it != sm_asioDriverList.end(); ++it) {
...
CSoundDeviceAsio::CSoundDeviceAsio()
    : m_pAsioDriver(NULL), m_nBufferLength(0) {}

/**
 * デストラクタ
 */
CSoundDeviceAsio::~CSoundDeviceAsio() { Close(); }
 ...
 * @return バッファ サイズ
 */
UINT CSoundDeviceAsio::CreateStream(UINT nSamplingRate, UINT nChannels,
                                    UINT nBufferSize) {
  if (m_pAsioDriver == NULL) {
    return 0;
  }
  ...
  do {
    ...
    long preferredSize;
    long granularity;
    if (m_pAsioDriver->getBufferSize(&minSize, &maxSize, &preferredSize,
                                     &granularity) != ASE_OK) {
      break;
    }
    ...
    m_pAsioDriver->createBuffers(&m_bufferInfo.at(0),
                                 static_cast<long>(m_bufferInfo.size()),
                                 preferredSize, &m_callback);
 ...
 */
void CSoundDeviceAsio::cBufferSwitch(long doubleBufferIndex,
 ...
 */
long CSoundDeviceAsio::cAsioMessage(long selector, long value, void *message,
                                    double *opt) {
  return 0;
}
/**
 ...
 */
ASIOTime *CSoundDeviceAsio::cBufferSwitchTimeInfo(ASIOTime *params,
 ...
 * @param[in] directProcess immediately start processing
 */
void CSoundDeviceAsio::BufferSwitch(long doubleBufferIndex,
                                    ASIOBool directProcess) {
  ...
  for (std::vector<ASIOBufferInfo>::iterator it = m_bufferInfo.begin();
       it != m_bufferInfo.end(); ++it) {
    ...
    case ASIOSTInt16LSB: {
      ...
      if (nStreamLength != m_nBufferLength) {
        memset(lpOutput + nStreamLength, 0,
               (m_nBufferLength - nStreamLength) * sizeof(short));
      }
    } break;
    ...
    case ASIOSTInt24LSB: {
      ...
      if (nStreamLength != m_nBufferLength) {
        memset(lpOutput + nStreamLength * 3, 0,
               (m_nBufferLength - nStreamLength) * sizeof(char) * 3);
      }
    }
    ...
    case ASIOSTInt32LSB: {
      ...
      if (nStreamLength != m_nBufferLength) {
        memset(lpOutput + nStreamLength * 2, 0,
               (m_nBufferLength - nStreamLength) * sizeof(short) * 2);
      }
    } break;
