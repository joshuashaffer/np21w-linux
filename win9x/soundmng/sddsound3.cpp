 ...
 * @retval TRUE 継続
 */
BOOL CALLBACK CSoundDeviceDSound3::EnumCallback(LPGUID lpGuid,
...
void CSoundDeviceDSound3::EnumerateDevices(std::vector<LPCTSTR> &devices) {
  for (std::vector<DSound3Device>::const_iterator it = sm_devices.begin();
       it != sm_devices.end(); ++it) {
...
CSoundDeviceDSound3::CSoundDeviceDSound3()
    : m_lpDSound(NULL), m_lpDSStream(NULL), m_nChannels(0), m_nBufferSize(0),
      m_dwHalfBufferSize(0), m_mastervolume(100) {
 ...
 * デストラクタ
 */
CSoundDeviceDSound3::~CSoundDeviceDSound3() { Close(); }
...
bool CSoundDeviceDSound3::Open(LPCTSTR lpDevice, HWND hWnd) {
  ...
  if ((lpDevice) && (lpDevice[0] != '\0')) {
    ...
    while ((it != sm_devices.end()) &&
           (::lstrcmpi(lpDevice, it->szDevice) != 0)) {
      ++it;
    }
 ...
 * @return バッファ サイズ
 */
UINT CSoundDeviceDSound3::CreateStream(UINT nSamplingRate, UINT nChannels,
                                       UINT nBufferSize) {
  if (m_lpDSound == NULL) {
    return 0;
  }
  ...
  dsbdesc.dwFlags = DSBCAPS_CTRLPAN |
                    (s_mastervol_available ? DSBCAPS_CTRLVOLUME : 0) |
  ...
  if (FAILED(m_lpDSStream->QueryInterface(
          IID_IDirectSoundNotify, reinterpret_cast<LPVOID *>(&pNotify)))) {
    DestroyStream();
    return 0;
  }
...
void CSoundDeviceDSound3::ResetStream() {
  if (m_lpDSStream) {
    ...
    LPVOID lpBlock2;
    DWORD cbBlock2;
    if (SUCCEEDED(m_lpDSStream->Lock(0, m_dwHalfBufferSize * 2, &lpBlock1,
                                     &cbBlock1, &lpBlock2, &cbBlock2, 0))) {
...
bool CSoundDeviceDSound3::Task() {
  ...
  case WAIT_OBJECT_0 + 0:
    if (m_lpDSStream) {
      ...
      if (SUCCEEDED(m_lpDSStream->GetCurrentPosition(&dwCurrentPlayCursor,
                                                     &dwCurrentWriteCursor))) {
        const DWORD dwPos = (dwCurrentPlayCursor >= m_dwHalfBufferSize)
                                ? 0
                                : m_dwHalfBufferSize;
        FillStream(dwPos);
      }
...
void CSoundDeviceDSound3::FillStream(DWORD dwPosition) {
  ...
  LPVOID lpBlock2;
  DWORD cbBlock2;
  HRESULT hr = m_lpDSStream->Lock(dwPosition, m_dwHalfBufferSize, &lpBlock1,
                                  &cbBlock1, &lpBlock2, &cbBlock2, 0);
  if (hr == DSERR_BUFFERLOST) {
    m_lpDSStream->Restore();
    hr = m_lpDSStream->Lock(dwPosition, m_dwHalfBufferSize, &lpBlock1,
                            &cbBlock1, &lpBlock2, &cbBlock2, 0);
  }
  if (SUCCEEDED(hr)) {
    ...
    if (m_pSoundData) {
      nStreamLength =
          m_pSoundData->Get16(static_cast<SINT16 *>(lpBlock1), m_nBufferSize);
    }
    if (nStreamLength != m_nBufferSize) {
      ZeroMemory(static_cast<short *>(lpBlock1) + nStreamLength * m_nChannels,
                 (m_nBufferSize - nStreamLength) * m_nChannels * sizeof(short));
    }
    m_lpDSStream->Unlock(lpBlock1, cbBlock1, lpBlock2, cbBlock2);
  }
}
...
void CSoundDeviceDSound3::DestroyAllPCM() {
  for (std::map<UINT, LPDIRECTSOUNDBUFFER>::iterator it = m_pcm.begin();
       it != m_pcm.begin(); ++it) {
    LPDIRECTSOUNDBUFFER lpDSBuffer = it->second;
    lpDSBuffer->Stop();
    lpDSBuffer->Release();
...
LPDIRECTSOUNDBUFFER CSoundDeviceDSound3::CreateWaveBuffer(LPCTSTR lpFilename) {
  ...
  do {
    ...
    if ((riff.riff != MAKEFOURCC('R', 'I', 'F', 'F')) ||
        (riff.nFileType != MAKEFOURCC('W', 'A', 'V', 'E'))) {
      break;
    }
    ...
    LPVOID lpBlock2;
    DWORD cbBlock2;
    hr = lpDSBuffer->Lock(0, chunk.nSize, &lpBlock1, &cbBlock1, &lpBlock2,
                          &cbBlock2, 0);
    if (hr == DSERR_BUFFERLOST) {
      lpDSBuffer->Restore();
      hr = lpDSBuffer->Lock(0, chunk.nSize, &lpBlock1, &cbBlock1, &lpBlock2,
...
void CSoundDeviceDSound3::SetPCMVolume(UINT nNum, int nVolume) {
  if (s_mastervol_available) {
    ...
    if (it != m_pcm.end()) {
      ...
      // lpDSBuffer->SetVolume((((DSBVOLUME_MAX - DSBVOLUME_MIN) * nVolume) /
      // 100) + DSBVOLUME_MIN);
    }
  }
}
