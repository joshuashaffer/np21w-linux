 ...
 * @file	sdbase.h
 * @brief	サウンド
 * デバイス基底クラスの宣言およびインターフェイスの定義をします
 */
...
public:
   ...
   * コンストラクタ
   */
  CSoundDeviceBase() : m_pSoundData(NULL) {}
   ...
   */
  void SetStreamData(ISoundData *pSoundData) { m_pSoundData = pSoundData; }
   ...
   * @retval false 失敗
   */
  virtual bool LoadPCM(UINT nNum, LPCTSTR lpFilename) { return false; }
   ...
   * @retval false 失敗
   */
  virtual bool PlayPCM(UINT nNum, BOOL bLoop) { return false; }
