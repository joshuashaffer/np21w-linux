...
enum { TCTL_STATIC = 0, TCTL_BUTTON = 1, TCTL_DDLIST = 2 };
...
static const UINT8 fddlist[FDDLIST_DRV] = {IDC_TOOLFDD1LIST, IDC_TOOLFDD2LIST};

static const DISKACC diskacc[3] = {{IDC_TOOLFDD1ACC, &toolwin.m_fddaccess[0]},
 ...
 * @return インスタンス
 */
CToolWnd *CToolWnd::GetInstance() { return &toolwin; }
...
static HBITMAP skinload(const OEMCHAR *path) {
  ...
  if (toolskin.main[0]) {
    ...
    ret = static_cast<HBITMAP>(
        ::LoadImage(NULL, fname, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE));
  }
  if (ret == NULL) {
#ifdef SUPPORT_PC9821
    HINSTANCE hInstance =
        CWndProc::FindResourceHandle(TEXT("NP21TOOL"), RT_BITMAP);
    ret = ::LoadBitmap(hInstance, TEXT("NP21TOOL"));
#else
    HINSTANCE hInstance =
        CWndProc::FindResourceHandle(TEXT("NP2TOOL"), RT_BITMAP);
...
void CToolWnd::CreateSubItems() {
  m_hfont =
      ::CreateFont(toolskin.fontsize, 0, 0, 0, 0, 0, 0, 0, SHIFTJIS_CHARSET,
                   OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                   FIXED_PITCH, toolskin.font);
  HDC hdc = ::GetDC(NULL);
  ...
  for (UINT i = 0; i < IDC_MAXITEMS; i++) {
    ...
    if ((cls) && (p->width > 0) && (p->height > 0)) {
      sub = CreateWindow(cls, p->text, WS_CHILD | WS_VISIBLE | style, p->posx,
                         p->posy, p->width, p->height, m_hWnd,
                         (HMENU)(i + IDC_BASE), CWndProc::GetInstanceHandle(),
                         NULL);
    }
    m_sub[i] = sub;
    m_subproc[i] = NULL;
  ...
  for (UINT i = 0; i < IDC_MAXITEMS; i++) {
    ...
    if (sub) {
      if (m_subproc[i]) {
        SetWindowLongPtr(
            sub, GWLP_WNDPROC,
            (LONG_PTR)m_subproc[i]); // 一応戻しておいてあげましょう
      }
      m_subproc[i] = NULL;
...
void CToolWnd::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  if (m_hbmp) {
    HDC hmdc = ::CreateCompatibleDC(hdc);
    ::SelectObject(hmdc, m_hbmp);
    ::BitBlt(hdc, 0, 0, btn.right, btn.bottom, hmdc, rect.left - pt.x,
             rect.top - pt.y, SRCCOPY);
    ::DeleteDC(hmdc);
  }
...
static void setSkinMruMenu(HMENU hMenu) {
  ...
  for (i = 0; i < uCount; i++) {
    ...
    AppendMenu(hmenuSub, MF_STRING + uFlag, IDM_TOOL_SKINMRU + j,
               pcszMruList[j]);
  }
}
...
void CToolWnd::ChangeSkin() {
  ...
  if (p[0]) {
    ...
    file_cpyname(s_toolwndcfg.skinmru[0], p,
                 NELEMENTS(s_toolwndcfg.skinmru[0]));
  }
...
LRESULT CToolWnd::WindowProc(UINT nMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_COMMAND:
    ...
    case IDM_TOOL_SKINSEL: {
      ...
      CFileDlg dlg(TRUE, rExt.c_str(), s_toolwndcfg.skin,
                   OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, rFilter.c_str(),
                   m_hWnd);
      ...
      if (r) {
        file_cpyname(s_toolwndcfg.skin, dlg.GetPathName(),
                     _countof(s_toolwndcfg.skin));
        ChangeSkin();
      }
    } break;
    ...
    case IDM_TOOL_SKINMRU + 2:
    case IDM_TOOL_SKINMRU + 3:
      file_cpyname(s_toolwndcfg.skin,
                   s_toolwndcfg.skinmru[LOWORD(wParam) - IDM_TOOL_SKINMRU],
                   NELEMENTS(s_toolwndcfg.skin));
      ChangeSkin();
      break;
  ...
  case WM_DRAWITEM:
    OnDrawItem(static_cast<int>(wParam),
...
void CToolWnd::Create() {
  ...
  if (!CSubWndBase::Create(IDS_CAPTION_TOOL, WS_SYSMENU | WS_MINIMIZEBOX,
                           s_toolwndcfg.posx, s_toolwndcfg.posy, bmp.bmWidth,
                           bmp.bmHeight, NULL, NULL)) {
    np2oscfg.toolwin = 0;
    sysmng_update(SYS_UPDATEOSCFG);
    return;
  }
...
void toolwin_hddaccess(UINT8 drv) { setdiskacc(2, 10); }
...
int gettoolwndini(PFTBL **ptoolwndini) {
  ...
  if (fdlistlen == 0)
    fdlistlen = FDDLIST_DEFAULT;
  if (fdlistlen > FDDLIST_MAX)
    fdlistlen = FDDLIST_MAX;

  toolwndini =
      (PFTBL *)malloc(sizeof(s_toolwndini) + 2 * fdlistlen * sizeof(PFTBL));

  slistlen = _countof(s_toolwndini);
  memcpy(toolwndini, s_toolwndini, sizeof(s_toolwndini));
  for (int j = 0; j < 2; j++) {
    for (UINT i = 0; i < fdlistlen; i++) {
      OEMSPRINTF(toolwndini[slistlen + j * fdlistlen + i].item,
                 OEMTEXT("FD%dNAME%X"), j + 1, i);
      toolwndini[slistlen + j * fdlistlen + i].itemtype = PFTYPE_STR;
      toolwndini[slistlen + j * fdlistlen + i].value =
          s_toolwndcfg.fdd[j].name[i];
