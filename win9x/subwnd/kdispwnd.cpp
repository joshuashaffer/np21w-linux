 ...
 * モード
 */
enum { KDISPCFG_FM = 0x00, KDISPCFG_MIDI = 0x80 };
...
//! パレット
static const UINT32 s_kdisppal[KEYDISP_PALS] = {0x00000000, 0xffffffff,
                                                0xf9ff0000};

/**
 * 初期化
 */
void CKeyDisplayWnd::Initialize() { keydisp_initialize(); }
...
void CKeyDisplayWnd::Create() {
  ...
  HINSTANCE hInstance =
      FindResourceHandle(MAKEINTRESOURCE(IDR_KEYDISP), RT_MENU);
  HMENU hMenu = ::LoadMenu(hInstance, MAKEINTRESOURCE(IDR_KEYDISP));
  if (!CSubWndBase::Create(IDS_CAPTION_KEYDISP,
                           WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION |
                               WS_MINIMIZEBOX,
                           s_kdispcfg.posx, s_kdispcfg.posy, KEYDISP_WIDTH,
                           KEYDISP_HEIGHT, NULL, hMenu)) {
    np2oscfg.keydisp = 0;
