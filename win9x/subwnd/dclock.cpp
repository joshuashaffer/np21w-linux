 ...
 * コンストラクタ
 */
DispClock::DispClock() { ZeroMemory(this, sizeof(*this)); }
 ...
 * 再描画
 */
void DispClock::Redraw() { m_cDirty = 0x3f; }
...
void DispClock::Draw8(void *lpBuffer, int nYAlign) const {
  ...
  for (UINT i = 0; i < 4; i++) {
    ...
    for (UINT j = 0; j < 3; j++) {
      ...
      p += DCLOCK_YALIGN;
      lpBuffer = reinterpret_cast<void *>(reinterpret_cast<INTPTR>(lpBuffer) +
                                          nYAlign);
    }
  }
}
  ...
  for (UINT i = 0; i < 4; i++) {
    ...
    for (UINT j = 0; j < 3; j++) {
      for (UINT x = 0; x < (8 * DCLOCK_YALIGN); x++) {
        (static_cast<UINT16 *>(lpBuffer))[x] =
            (p[x >> 3] & (0x80 >> (x & 7))) ? pal : 0;
      }
      p += DCLOCK_YALIGN;
      lpBuffer = reinterpret_cast<void *>(reinterpret_cast<INTPTR>(lpBuffer) +
                                          nYAlign);
