...
DD2Surface::DD2Surface()
    : m_hWnd(NULL), m_pDDraw(NULL), m_pDDraw2(NULL), m_pPrimarySurface(NULL),
      m_pBackSurface(NULL), m_pClipper(NULL), m_pPalette(NULL), m_r16b(0),
      m_l16r(0), m_l16g(0) {
  m_pal16.d = 0;
...
bool DD2Surface::Create(HWND hWnd, int nWidth, int nHeight) {
  ...
  do {
    ...
    m_pDDraw->QueryInterface(IID_IDirectDraw2,
                             reinterpret_cast<LPVOID *>(&m_pDDraw2));
    ...
    if (ddpf.dwRGBBitCount == 8) {
      ...
      for (m_r16b = 8; (m_r16b) && (ddpf.dwBBitMask & bit);
           m_r16b--, bit <<= 1) {
        m_pal16.p.b >>= 1;
        m_pal16.p.b |= 0x80;
      }
      for (m_l16r = 0, bit = 1; (bit) && (!(ddpf.dwRBitMask & bit));
           m_l16r++, bit <<= 1) {
      }
      ...
      for (m_l16g = 0, bit = 1; (bit) && (!(ddpf.dwGBitMask & bit));
           m_l16g++, bit <<= 1) {
      }
...
void DD2Surface::Blt(const POINT *pt, const RECT *lpRect) {
  if (m_pBackSurface) {
    ...
    if (m_pPrimarySurface->Blt(&scrn, m_pBackSurface,
                               const_cast<LPRECT>(lpRect), DDBLT_WAIT,
...
UINT16 DD2Surface::GetPalette16(RGB32 pal) const {
  pal.d &= m_pal16.d;
  return (static_cast<UINT>(pal.p.g) << m_l16g) |
         (static_cast<UINT>(pal.p.r) << m_l16r) | (pal.p.b >> m_r16b);
}
