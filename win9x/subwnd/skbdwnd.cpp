 ...
 * 初期化
 */
void CSoftKeyboardWnd::Initialize() { softkbd_initialize(); }

/**
 * 解放
 */
void CSoftKeyboardWnd::Deinitialize() { softkbd_deinitialize(); }

/**
 * コンストラクタ
 */
CSoftKeyboardWnd::CSoftKeyboardWnd() : m_nWidth(0), m_nHeight(0) {}
...
void CSoftKeyboardWnd::Create() {
  ...
  if (!CSubWndBase::Create(
          IDS_CAPTION_SOFTKEY,
          WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX,
          s_skbdcfg.posx, s_skbdcfg.posy, m_nWidth, m_nHeight, NULL, NULL)) {
    return;
  }
 ...
 * @param[in] bpp 色数
 */
void CSoftKeyboardWnd::skpalcnv(CMNPAL *dst, const RGB32 *src, UINT pals,
                                UINT bpp) {
  switch (bpp) {
  ...
  case 16:
    for (UINT i = 0; i < pals; i++) {
      dst[i].pal16 =
          CSoftKeyboardWnd::GetInstance()->m_dd2.GetPalette16(src[i]);
    }
    break;
#endif
