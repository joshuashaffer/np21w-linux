 ...
 * 初期化
 */
void CMemDebugWnd::Initialize() { memdbg32_initialize(); }
 ...
 * コンストラクタ
 */
CMemDebugWnd::CMemDebugWnd() : m_nWidth(0), m_nHeight(0) {}
...
void CMemDebugWnd::Create() {
  ...
  if (!CSubWndBase::Create(
          s_mdbgapp, WS_OVERLAPPED | WS_SYSMENU | WS_CAPTION | WS_MINIMIZEBOX,
          s_mdbgcfg.posx, s_mdbgcfg.posy, m_nWidth, m_nHeight, NULL, NULL)) {
    return;
  }
 ...
 * @param[in] bpp 色数
 */
void CMemDebugWnd::mdpalcnv(CMNPAL *dst, const RGB32 *src, UINT pals,
                            UINT bpp) {
  switch (bpp) {
