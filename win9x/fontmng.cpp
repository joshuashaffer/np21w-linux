...
static const OEMCHAR *deffont[4] = {deffontface, deffontface2, edeffontface,
                                    edeffontface2};
...
void *fontmng_create(int size, UINT type, const OEMCHAR *fontface) {
  ...
  hdc = GetDC(NULL);
  ret->hBitmap =
      CreateDIBSection(hdc, bi, DIB_RGB_COLORS, (void **)&ret->image, NULL, 0);
  ...
  ret->hfont =
      CreateFont(size, 0, FW_DONTCARE, FW_DONTCARE, weight, FALSE, FALSE, FALSE,
...
static void getlength1(FNTMNG fhdl, FNTDAT fdat, const OEMCHAR *string,
                       int length) {
