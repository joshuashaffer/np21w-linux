...
public:
  ...
  HWND Detach();
  //	BOOL Create(LPCTSTR lpszWindowName, DWORD dwStyle, int x, int y, int
  //nWidth, int nHeight, HWND hwndParent, HMENU nIDorHMenu); 	BOOL
  //DestroyWindow();
 ...
 */
inline CWndBase::CWndBase(HWND hWnd) : m_hWnd(hWnd) {}
 ...
 * @param[in] hWnd ウィンドウ ハンドル
 */
inline void CWndBase::Attach(HWND hWnd) { m_hWnd = hWnd; }
 ...
 * @return HWND
 */
inline CWndBase::operator HWND() const { return m_hWnd; }
...
/**
 * ウィンドウ スタイルを変更します
 * @param[in] dwRemove スタイルのリビジョン中に削除するウィンドウ
 * スタイルを指定します
 * @param[in] dwAdd スタイルのリビジョン中に追加するウィンドウ
 * スタイルを指定します
 ...
 * @return メッセージの処理の結果
 */
inline LRESULT CWndBase::SendMessage(UINT message, WPARAM wParam,
                                     LPARAM lParam) {
...
/**
 * メッセージをウィンドウのメッセージ
 * キューに置き、対応するウィンドウがメッセージを処理するのを待たずに返されます
 ...
 * @param[in] lpString
 * 新しいウィンドウタイトルまたはコントロールのテキストとして使われる、NULL
 * で終わる文字列へのポインタを指定します
 * @retval TRUE 成功
 * @retval FALSE 失敗
 */
 ...
 * @param[in] lpszStringBuf
 * バッファへのポインタを指定します。このバッファにテキストが格納されます
 * @param[in] nMaxCount
 * バッファにコピーする文字の最大数を指定します。テキストのこのサイズを超える部分は、切り捨てられます。NULL
 * 文字も数に含められます
 * @return コピーされた文字列の文字数が返ります (終端の NULL
 * 文字は含められません)
 */
 ...
 * @param[in] bRedraw メッセージを処理した直後にウィンドウを再描画する場合は
 * TRUE
 */
 ...
 */
inline HMENU CWndBase::GetMenu() const { return ::GetMenu(m_hWnd); }
 ...
 * @retval FALSE 失敗
 */
inline BOOL CWndBase::DrawMenuBar() { return ::DrawMenuBar(m_hWnd); }

/**
 * 指定されたウィンドウに割り当てられているシステム
 * メニューのハンドルを取得します
 ...
 * @
 */
inline void CWndBase::MoveWindow(int x, int y, int nWidth, int nHeight,
                                 BOOL bRepaint) {
 ...
 * @param[in,out] lpPoint 変換対象のクライアント座標を保持している、1 個の
 * 構造体へのポインタを指定します
 * @retval TRUE 成功
 * @retval FALSE 失敗
 */
 ...
 * @param[in] hWndTo
 * 変換後の点を保持する（変換先）ウィンドウのハンドルを指定します
 * @param[in,out] lpPoint 変換対象の点の座標を保持している 構造体からなる 1
 * つの配列へのポインタを指定します
 * @param[in] nCount lpPoint パラメータで、複数の POINT 構造体からなる 1
 * つの配列へのポインタを指定した場合、配列内の POINT 構造体の数を指定します
 * @return 関数が成功すると、各点の移動距離を示す 32 ビット値が返ります
 */
inline int CWndBase::MapWindowPoints(HWND hWndTo, LPPOINT lpPoint,
                                     UINT nCount) const {
...
/**
 * 指定されたウィンドウの更新リージョンが空ではない場合、ウィンドウへ
 * メッセージを送信し、そのウィンドウのクライアント領域を更新します
 * @retval TRUE 成功
 * @retval FALSE 失敗
 */
inline BOOL CWndBase::UpdateWindow() { return ::UpdateWindow(m_hWnd); }

/**
 * 指定されたウィンドウのすべてを更新リージョンにします
 * @param[in] bErase
 * 更新リージョンを処理するときに、更新リージョン内の背景を消去するかどうかを指定します
 * @retval TRUE 成功
 * @retval FALSE 失敗
 */
 ...
 * @param[in] lpRect
 * 更新リージョンへ追加したい長方形のクライアント座標を保持する1個の構造体へのポインタを指定します
 * @param[in] bErase
 * 更新リージョンを処理するときに、更新リージョン内の背景を消去するかどうかを指定します
 * @retval TRUE 成功
 * @retval FALSE 失敗
 ...
 */
inline HWND CWndBase::SetFocus() { return ::SetFocus(m_hWnd); }
 ...
 * @param[in] bSigned
 * テキストを符号付きとして扱って符号付きの値を返すかどうかを指定します
 * @return コントロールテキストに相当する整数値が返ります
 */
inline UINT CWndBase::GetDlgItemInt(int nID, BOOL *lpTrans,
                                    BOOL bSigned) const {
 ...
 * @param[in] nID
 * 取得したいタイトルまたはテキストを保持しているコントロールの識別子を指定します
 * @param[out] lpStr
 * タイトルまたはテキストを受け取るバッファへのポインタを指定します
 * @param[in] nMaxCount lpStr
 * パラメータが指すバッファへコピーされる文字列の最大の長さを TCHAR
 * 単位で指定します
 * @return バッファへコピーされた文字列の長さ（ 終端の NULL を含まない）が TCHAR
 * 単位で返ります
 */
inline UINT CWndBase::GetDlgItemText(int nID, LPTSTR lpStr,
                                     int nMaxCount) const {
 ...
 */
inline LRESULT CWndBase::SendDlgItemMessage(int nID, UINT message,
 ...
 * @param[in] lpszString コントロールへコピーしたいテキストを保持する、NULL
 * で終わる文字列へのポインタを指定します
 * @retval TRUE 成功
 * @retval FALSE 失敗
 */
 ...
 * @param[in] lpScrollInfo
 * 設定するべき情報を保持している、1個の構造体へのポインタを指定します
 * @param[in] bRedraw スクロールバーを再描画するかどうかを指定します
 * @return スクロールバーの現在のスクロール位置が返ります
 */
inline int CWndBase::SetScrollInfo(int nBar, LPSCROLLINFO lpScrollInfo,
                                   BOOL bRedraw) {
 ...
 * @return 指定したウィンドウハンドルを持つウィンドウが存在している場合は、0
 * 以外の値が返ります
 */
inline BOOL CWndBase::IsWindow() const { return ::IsWindow(m_hWnd); }
