...
std::tstring LoadTString(UINT uID) {
  ...
  do {
    HINSTANCE hInstance = CWndProc::FindResourceHandle(
        MAKEINTRESOURCE((uID >> 4) + 1), RT_STRING);
    HRSRC hRsrc =
        ::FindResource(hInstance, MAKEINTRESOURCE((uID >> 4) + 1), RT_STRING);
    if (hRsrc == NULL) {
      break;
    }
    ...
    UINT nSize = ::WideCharToMultiByte(CP_ACP, 0, pRes + dwPos, nLength, NULL,
                                       0, NULL, NULL);
    if (nSize) {
      ret.resize(nSize);
      nSize = ::WideCharToMultiByte(CP_ACP, 0, pRes + dwPos, nLength,
                                    &ret.at(0), nSize, NULL, NULL);
      ret.resize(nSize);
    }
