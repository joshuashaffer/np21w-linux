...
/**
 * コンストラクタ
 * @param[in] lpszTemplateName
 * このページのテンプレートの名前を含む文字列へのポインター
 * @param[in] nIDCaption このページのタブに設定される名前の ID
 */
 ...
 * @return 0
 */
UINT CALLBACK CPropPageProc::PropPageCallback(HWND hWnd, UINT message,
                                              LPPROPSHEETPAGE pPropPage) {
  switch (message) {
  case PSPCB_CREATE:
...
/**
 * このメンバー関数は、フレームワークによって
 * OnKillActiveフレームワークがを呼び出した直後にユーザーが[OK]を選択するか、更新時に呼び出されます
 ...
 */
void CPropPageProc::OnReset() { OnCancel(); }
 ...
 * コンストラクタ
 */
CPropSheetProc::CPropSheetProc() { CommonConstruct(NULL, 0); }
 ...
 */
CPropSheetProc::CPropSheetProc(UINT nIDCaption, HWND hwndParent,
                               UINT iSelectPage) {
 ...
 * @param[in] iSelectPage スタート ページ
 */
CPropSheetProc::CPropSheetProc(LPCTSTR pszCaption, HWND hwndParent,
                               UINT iSelectPage) {
 ...
 * @param[in] pPage ページ
 */
void CPropSheetProc::AddPage(CPropPageProc *pPage) { m_pages.push_back(pPage); }
