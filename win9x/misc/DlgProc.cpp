 ...
 * コンストラクタ
 */
CDlgProc::CDlgProc() : m_lpszTemplateName(NULL), m_hwndParent(NULL) {}

/**
 * コンストラクタ
 * @param[in] nIDTemplate ダイアログ ボックス テンプレートのリソース id
 * 番号を指定します
...
/**
 * モーダル
 * @return ダイアログ ボックスを閉じるために使用される、CDialog::EndDialog
 * のメンバー関数に渡された nResult のパラメーター値を指定する int の値
 */
INT_PTR CDlgProc::DoModal() {
  ...
  const INT_PTR nRet =
      ::DialogBox(hInstance, m_lpszTemplateName, m_hwndParent, DlgProc);
 ...
 */
#if defined(_WIN64)
INT_PTR CALLBACK CDlgProc::DlgProc(HWND hWnd, UINT message, WPARAM wParam,
                                   LPARAM lParam)
#else  // defined(_WIN64)
BOOL CALLBACK CDlgProc::DlgProc(HWND hWnd, UINT message, WPARAM wParam,
                                LPARAM lParam)
...
LRESULT CDlgProc::WindowProc(UINT nMsg, WPARAM wParam, LPARAM lParam) {
  if (nMsg == WM_COMMAND) {
    ...
    POINT pt;
    pt.x = rectparent.left -
           (rect2.left - rect.left); //((rectparent.right - rectparent.left) -
                                     //(rect.right - rect.left)) / 2;
    pt.y = rectparent.top -
           (rect2.top - rect.top); //((rectparent.bottom - rectparent.top) -
                                   //(rect.bottom - rect.top)) / 2;
    MoveWindow(pt.x, pt.y, (rect.right - rect.left), (rect.bottom - rect.top),
               TRUE);
  }
 ...
 * @retval FALSE 既に設定済
 */
BOOL CDlgProc::OnInitDialog() { return TRUE; }

/**
 * ユーザーが OK のボタン (IDOK ID がのボタン) をクリックすると呼び出されます
 */
void CDlgProc::OnOK() { EndDialog(IDOK); }

/**
 * フレームワークは、ユーザーが [キャンセル]
 * をクリックするか、モーダルまたはモードレス ダイアログ ボックスの Esc
 * キーを押したときにこのメソッドを呼び出します
 */
void CDlgProc::OnCancel() { EndDialog(IDCANCEL); }
 ...
 */
CFileDlg::CFileDlg(BOOL bOpenFileDialog, LPCTSTR lpszDefExt,
  ...
  if (lpszFilter != NULL) {
    ...
    for (std::tstring::iterator it = m_strFilter.begin();
         it != m_strFilter.end(); ++it) {
#if !defined(_UNICODE)
