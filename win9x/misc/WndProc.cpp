...
void CWndProc::Initialize(HINSTANCE hInstance) {
  ...
  sm_hHookOldCbtFilter =
      ::SetWindowsHookEx(WH_CBT, CbtFilterHook, NULL, sm_dwThreadId);
...
HINSTANCE CWndProc::FindResourceHandle(LPCTSTR lpszName, LPCTSTR lpszType) {
  ...
  if ((hInst != GetInstanceHandle()) &&
      (::FindResource(hInst, lpszName, lpszType) != NULL)) {
    return hInst;
  }
 ...
 * コンストラクタ
 */
CWndProc::CWndProc() : CWndBase(NULL), m_pfnSuper(NULL) {}

/**
 * デストラクタ
 */
CWndProc::~CWndProc() { DestroyWindow(); }

/**
 * ウィンドウのハンドルが指定されている場合、CWndProc
 * オブジェクトへのポインターを返します
...
/**
 * ウィンドウを動的サブクラス化し、CWnd
 * オブジェクトに結び付けるためにこのメンバー関数を呼び出します
 * @param[in] hWnd ウィンドウ ハンドル
 * @retval TRUE 成功
 * @retval FALSE 失敗
 */
BOOL CWndProc::SubclassWindow(HWND hWnd) {
  ...
  WNDPROC oldWndProc = reinterpret_cast<WNDPROC>(::SetWindowLongPtr(
      hWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(newWndProc)));
...
/**
 * WndProc に元の値を設定して CWnd オブジェクトから HWND
 * で識別されるウィンドウを切り離すために、このメンバー関数を呼び出します
 ...
 * @retval FALSE 失敗
 */
BOOL CWndProc::CreateEx(DWORD dwExStyle, LPCTSTR lpszClassName,
  ...
  HWND hWnd = ::CreateWindowEx(
      cs.dwExStyle, cs.lpszClass, cs.lpszName, cs.style, cs.x, cs.y, cs.cx,
...
/**
 * CWnd オブジェクトに結び付けられた Windows
 * のウィンドウが作成される前に、フレームワークから呼び出されます
 * @param[in,out] cs CREATESTRUCT の構造
 * @retval TRUE 継続
 */
BOOL CWndProc::PreCreateWindow(CREATESTRUCT &cs) { return TRUE; }
 ...
 * @return リザルト コード
 */
LRESULT CALLBACK CWndProc::CbtFilterHook(int nCode, WPARAM wParam,
                                         LPARAM lParam) {
 ...
 * @return ウィンドウが破棄された場合は 0 以外を返します。それ以外の場合は 0
 * を返します
 */
 ...
 */
LRESULT CALLBACK CWndProc::WndProc(HWND hWnd, UINT message, WPARAM wParam,
                                   LPARAM lParam) {
 ...
 */
BOOL CWndProc::OnCommand(WPARAM wParam, LPARAM lParam) { return FALSE; }
 ...
 * @param[in] lParam 通知コードと追加情報を含む通知メッセージ (NMHDR)
 * の構造体へのポインター
 * @param[out] pResult メッセージが処理されたとき結果を格納するコードする
 * LRESULT の変数へのポインター
...
/**
 * Windows
 * のウィンドウが破棄されるときに非クライアント領域が破棄されると、最後に呼び出されたメンバー関数は、フレームワークによって呼び出されます
 * @param[in] wParam パラメタ
 * @param[in] lParam パラメタ
 */
