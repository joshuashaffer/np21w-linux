...
public:
   ...
   */
  void CDlgProc::EndDialog(int nResult) { ::EndDialog(m_hWnd, nResult); }
...
#if defined(_WIN64)
  static INT_PTR CALLBACK DlgProc(HWND hWnd, UINT message, WPARAM wParam,
                                  LPARAM lParam);
#else  // defined(_WIN64)
  static BOOL CALLBACK DlgProc(HWND hWnd, UINT message, WPARAM wParam,
                               LPARAM lParam);
...
public:
  ...
  int AddString(LPCTSTR lpszString) {
    return static_cast<int>(::SendMessage(
        m_hWnd, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(lpszString)));
  }

  /**
   * コンボ ボックスのリスト
   * ボックス部分の項目数を取得するには、このメンバー関数を呼び出します
   * @return 項目の数
   */
  int GetCount() const {
  ...
  /**
   * コンボ
   * ボックスのどの項目が選択されたかを判定するためにこのメンバー関数を呼び出します
   * @return コンボ ボックスのリスト
   * ボックスで現在選択されている項目のインデックス
   */
  int GetCurSel() const {
  ...
  /**
   * 指定したコンボ ボックスの項目に関連付けられたアプリケーションに用意された
   * 32 ビット値を取得します
  ...
  DWORD_PTR GetItemData(int nIndex) const {
    return static_cast<DWORD_PTR>(
        ::SendMessage(m_hWnd, CB_GETITEMDATA, static_cast<WPARAM>(nIndex), 0));
  }

  /**
   * コンボ ボックスのリスト
   * ボックスで指定されているプレフィックスを含む最初の文字列を検索します
  ...
  int FindString(int nStartAfter, LPCTSTR lpszString) const {
    return static_cast<int>(
        ::SendMessage(m_hWnd, CB_FINDSTRING, nStartAfter,
                      reinterpret_cast<LPARAM>(lpszString)));
  }
  ...
  int FindStringExact(int nStartAfter, LPCTSTR lpszString) const {
    return static_cast<int>(
        ::SendMessage(m_hWnd, CB_FINDSTRINGEXACT, nStartAfter,
                      reinterpret_cast<LPARAM>(lpszString)));
  }
  ...
  int InsertString(int nIndex, LPCTSTR lpszString) {
    return static_cast<int>(
        ::SendMessage(m_hWnd, CB_INSERTSTRING, static_cast<WPARAM>(nIndex),
                      reinterpret_cast<LPARAM>(lpszString)));
  }

  /**
   * コンボ ボックスのリスト ボックスとエディット
   ...
   * @param[in] nIndex
   * 項目に始まるインデックスを設定するためのメソッドが含まれます
   * @param[in] dwItemData 新しい値を項目に関連付けるに含まれています
   * @return エラーの時は CB_ERR
   */
  int SetItemData(int nIndex, DWORD_PTR dwItemData) {
    return static_cast<int>(::SendMessage(m_hWnd, CB_SETITEMDATA,
                                          static_cast<WPARAM>(nIndex),
                                          static_cast<LPARAM>(dwItemData)));
  }
  ...
  int SetCurSel(int nSelect) {
    return static_cast<int>(
        ::SendMessage(m_hWnd, CB_SETCURSEL, static_cast<WPARAM>(nSelect), 0));
  }
};
...
public:
   ...
   */
  void SetPos(int nPos) { ::SendMessage(m_hWnd, TBM_SETPOS, TRUE, nPos); }
};
...
public:
  CFileDlg(BOOL bOpenFileDialog, LPCTSTR lpszDefExt = NULL,
           ...
           LPCTSTR lpszFilter = NULL, HWND hParentWnd = NULL);
  int DoModal();
   ...
   */
  LPCTSTR GetPathName() const { return m_ofn.lpstrFile; }
