 ...
 * @return インスタンス
 */
inline CTraceWnd *CTraceWnd::GetInstance() { return &sm_instance; }

/**
 * コンストラクタ
 */
CTraceWnd::CTraceWnd()
    : m_nFlags(0), m_tfh(NULL), m_hBrush(NULL), m_hFont(NULL) {}
...
void CTraceWnd::Initialize() {
  ...
  if (!CreateEx(WS_EX_CONTROLPARENT, s_szClassName, s_szTitle,
                WS_OVERLAPPEDWINDOW, tracecfg.posx, tracecfg.posy,
                tracecfg.width, tracecfg.height, NULL, NULL)) {
    return;
  }
  ShowWindow(SW_SHOW);
  UpdateWindow();
 ...
 * @retval false 無効
 */
inline bool CTraceWnd::IsTrace() const { return ((m_nFlags & 1) != 0); }
 ...
 * @retval false 無効
 */
inline bool CTraceWnd::IsVerbose() const { return ((m_nFlags & 2) != 0); }
...
void CTraceWnd::AddString(LPCTSTR lpString) {
  if ((m_nFlags & 4) && (m_wndView.IsWindow())) {
    UINT nStart = 0;
    UINT nEnd = 0;
    m_wndView.SendMessage(EM_GETSEL, reinterpret_cast<WPARAM>(&nStart),
                          reinterpret_cast<LPARAM>(&nEnd));

    const UINT nLength = m_wndView.GetWindowTextLength();
    m_wndView.SendMessage(EM_SETSEL, static_cast<WPARAM>(nLength),
                          static_cast<LPARAM>(nLength));

    m_wndView.SendMessage(EM_REPLACESEL, FALSE,
    ...
    if (nLength > 20000) {
      TCHAR temp[1] = {0};
      int llength =
          m_wndView.SendMessage(EM_LINELENGTH, static_cast<WPARAM>(1), NULL);
      m_wndView.SendMessage(EM_SETSEL, static_cast<WPARAM>(0),
                            static_cast<LPARAM>(nLength));
      m_wndView.SendMessage(EM_REPLACESEL, FALSE,
                            reinterpret_cast<LPARAM>(temp));
    }

    m_wndView.SendMessage(EM_SETSEL, static_cast<WPARAM>(nStart),
                          static_cast<LPARAM>(nEnd));
    ...
    // m_wndView.SendMessage(EM_REPLACESEL, FALSE,
    // reinterpret_cast<LPARAM>(lpString)); m_wndView.SendMessage(EM_REPLACESEL,
    // FALSE, reinterpret_cast<LPARAM>(crlf)); m_wndView.SendMessage(EM_SETSEL,
    // static_cast<WPARAM>(nStart), static_cast<LPARAM>(nEnd));
  }
  if (m_tfh != NULL) {
...
/**
 * フレームワークは、Windows のウィンドウは [作成] または CreateEx
 * のメンバー関数を呼び出すことによって作成されたアプリケーションが必要とすると、このメンバー関数を呼び出します
 * @param[in] lpCreateStruct
 * 作成されたオブジェクトに関する情報が含まれています。
 * @retval 0 成功
 * @retval -1 失敗
 */
int CTraceWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  RECT rc;
  GetClientRect(&rc);
  if (m_wndView.CreateEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), NULL,
                         WS_CHILD | WS_VISIBLE | ES_READONLY | ES_LEFT |
                             ES_MULTILINE | WS_VSCROLL | ES_AUTOVSCROLL,
                         0, 0, rc.right, rc.bottom, m_hWnd, NULL)) {
    m_hFont = ::CreateFont(VIEW_SIZE, 0, 0, 0, 0, 0, 0, 0, SHIFTJIS_CHARSET,
                           OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                           DEFAULT_QUALITY, FIXED_PITCH, s_szViewFont);
    if (m_hFont) {
...
/**
 * フレームワークは、ユーザーがコントロール
 * メニューからコマンドを選択したとき、または最大化または最小化ボタンを選択すると、このメンバー関数を呼び出します
...
void CTraceWnd::OnEnterMenuLoop(BOOL bIsTrackPopupMenu) {
  ...
  ::CheckMenuItem(hMenu, IDM_TRACE_TRACE,
                  (m_nFlags & 1) ? MF_CHECKED : MF_UNCHECKED);
  ::CheckMenuItem(hMenu, IDM_TRACE_VERBOSE,
                  (m_nFlags & 2) ? MF_CHECKED : MF_UNCHECKED);
  ::CheckMenuItem(hMenu, IDM_TRACE_ENABLE,
                  (m_nFlags & 4) ? MF_CHECKED : MF_UNCHECKED);
  ::CheckMenuItem(hMenu, IDM_TRACE_FILEOUT,
                  (m_tfh != NULL) ? MF_CHECKED : MF_UNCHECKED);
}

// ----

void trace_init(void) { CTraceWnd::GetInstance()->Initialize(); }

void trace_term(void) { CTraceWnd::GetInstance()->Deinitialize(); }
...
void trace_char(char c) { CTraceWnd::GetInstance()->AddChar(c); }
#endif
