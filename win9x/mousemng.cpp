...
typedef HRESULT(WINAPI *FN_DIRECTINPUT8CREATE)(HINSTANCE hinst, DWORD dwVersion,
...
BRESULT mousemng_checkdinput8() {
  ...
  fndi8create =
      (FN_DIRECTINPUT8CREATE)GetProcAddress(hModuleDI8, "DirectInput8Create");
  if (!fndi8create) {
    goto scre_err2;
  }
  if (FAILED(fndi8create(GetModuleHandle(NULL), DIRECTINPUT_VERSION,
                         IID_IDirectInput8, (LPVOID *)&test_dinput, NULL))) {
    goto scre_err2;
  }
...
UINT8 mousemng_supportrawinput() { return (dinput && diRawMouse); }
...
static void initDirectInput() {
  ...
  if (fndi8create && !dinput) {
    // hr = DirectInputCreateEx(GetModuleHandle(NULL), DIRECTINPUT_VERSION,
    // IID_IDirectInput7, (void**)&dinput, NULL); hr =
    // DirectInput8Create(GetModuleHandle(NULL), DIRECTINPUT_VERSION,
    // IID_IDirectInput8, (LPVOID*)&dinput, NULL); // 関数名変えてやがった( ﾟдﾟ)
    hr = fndi8create(GetModuleHandle(NULL), DIRECTINPUT_VERSION,
    ...
    if (!FAILED(hr)) {
      ...
      if (!FAILED(hr)) {
        ...
        if (!FAILED(hr)) {
          // 協調レベル設定
          hr = diRawMouse->SetCooperativeLevel(
              g_hWndMain, DISCL_NONEXCLUSIVE | DISCL_FOREGROUND);
        }
        if (!FAILED(hr)) {
          // デバイス設定
          DIPROPDWORD diprop;
...
static void mousecapture(BOOL capture) {
  ...
  if (np2oscfg.rawmouse) {
    if (mousemng_checkdinput8() != SUCCESS) {
      np2oscfg.rawmouse = 0;
      MessageBox(g_hWndMain, _T("Failed to initialize DirectInput8."),
                 _T("DirectInput Error"), MB_OK | MB_ICONEXCLAMATION);
    }
  }
...
void mousemng_destroy(void) { destroyDirectInput(); }
...
BOOL mousemng_buttonevent(UINT event) {
  ...
  if (!mousemng.flag ||
      (np2oscfg.mouse_nc /* && !scrnmng_isfullscreen()*/ && mousemng.flag)) {
    switch (event) {
