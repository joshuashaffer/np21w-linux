...
enum { SNAPDOTPULL = 12, SNAPDOTREL = 16 };
typedef HRESULT(WINAPI *pfnDwmIsCompositionEnabled)(BOOL *pfEnabled);
typedef HRESULT(WINAPI *pfnDwmGetWindowAttribute)(HWND hwnd, DWORD dwAttribute,
                                                  PVOID pvAttribute,
                                                  DWORD cbAttribute);
...
BOOL winloc_InitDwmFunc() {
  ...
  if (noDWM) {
    ...
    if (!hDwmModule) {
      ...
      F_DwmIsCompositionEnabled = (pfnDwmIsCompositionEnabled)GetProcAddress(
          hDwmModule, "DwmIsCompositionEnabled");
      F_DwmGetWindowAttribute = (pfnDwmGetWindowAttribute)GetProcAddress(
          hDwmModule, "DwmGetWindowAttribute");
...
BOOL winloc_GetWindowRect(HWND hwnd, LPRECT lpRect) {
  if (F_DwmGetWindowAttribute) {
    ...
    if (SUCCEEDED(hr) && dwmenable) {
      // DWM環境
      hr = F_DwmGetWindowAttribute(hwnd, DWMWA_EXTENDED_FRAME_BOUNDS, lpRect,
                                   sizeof(RECT));
      if (SUCCEEDED(hr)) {
        return 1;
      } else {
...
void winloc_movingstart(WINLOC *wl) { ZeroMemory(wl, sizeof(WINLOC)); }
...
static BOOL gravityx(WINLOCEX wle, RECT *rect) {
  ...
  for (i = 0; i < wle->count; i++, wnd++) {
    ...
    if (!wnd->connect) {
      if ((rect->bottom >= wnd->rect.top) && (rect->top <= wnd->rect.bottom)) {
  ...
  for (i = 0; i < wle->count; i++, wnd++) {
    if (wnd->connect) {
      d = wnd->rect.left + (rect->left - wle->rect.left) - workrc.left;
      if ((d < SNAPDOTPULL) && (d > -SNAPDOTPULL)) {
        break;
      }
      d = wnd->rect.right + (rect->right - wle->rect.right) - workrc.right;
      if ((d < SNAPDOTPULL) && (d > -SNAPDOTPULL)) {
        break;
      }
    }
...
static BOOL gravityy(WINLOCEX wle, RECT *rect) {
  ...
  for (i = 0; i < wle->count; i++, wnd++) {
    if (!wnd->connect) {
      if ((rect->right >= wnd->rect.left) && (rect->left <= wnd->rect.right)) {
  ...
  for (i = 0; i < wle->count; i++, wnd++) {
    if (wnd->connect) {
      d = wnd->rect.top + (rect->top - wle->rect.top) - workrc.top;
      if ((d < SNAPDOTPULL) && (d > -SNAPDOTPULL)) {
        break;
      }
      d = wnd->rect.bottom + (rect->bottom - wle->rect.bottom) - workrc.bottom;
      if ((d < SNAPDOTPULL) && (d > -SNAPDOTPULL)) {
        break;
      }
    }
