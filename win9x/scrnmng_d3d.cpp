...
static void clearoutofrect(const RECT *target, const RECT *base) {
  ...
  if (rectd3dc) {
    dev->Clear(rectd3dc, rectd3d, D3DCLEAR_TARGET, D3DCOLOR_XRGB(0, 0, 0), 0.0f,
               0);
...
static void update_backbuffer2size() {
  if (current_d3d_imode == D3D_IMODE_PIXEL ||
      current_d3d_imode == D3D_IMODE_PIXEL2 ||
      current_d3d_imode == D3D_IMODE_PIXEL3) {
    ...
    if (backbufheight < 480)
      backbufheight = UINT_MAX;
    while (d3d.backsurf2width * 2 < (int)backbufwidth &&
           d3d.backsurf2height * 2 < (int)backbufheight) {
    ...
    if (d3d.backsurf2width / 2 == d3d.d3dparam.BackBufferWidth ||
        d3d.backsurf2width / 2 + 2 == d3d.d3dparam.BackBufferWidth ||
        d3d.d3ddev->CreateRenderTarget(
            d3d.backsurf2width + 2, d3d.backsurf2height + 2,
            d3d.d3dparam.BackBufferFormat, D3DMULTISAMPLE_NONE, 0, FALSE,
            &d3d.backsurf2, NULL) != D3D_OK) {
      d3d.backsurf2width /= 2;
      d3d.backsurf2height /= 2;
      d3d.backsurf2mul--;
      if (d3d.backsurf2mul <= 0 ||
          d3d.d3ddev->CreateRenderTarget(
              d3d.backsurf2width + 2, d3d.backsurf2height + 2,
              d3d.d3dparam.BackBufferFormat, D3DMULTISAMPLE_NONE, 0, FALSE,
              &d3d.backsurf2, NULL) != D3D_OK) {
  ...
  if (d3d.d3ddev->TestCooperativeLevel() == D3DERR_DEVICENOTRESET) {
    ...
    if (r == D3D_OK) {
      ...
      if (g_scrnmode & SCRNMODE_FULLSCREEN) {
        d3d.d3ddev->CreateOffscreenPlainSurface(
            bufwidth, bufheight, d3d.d3dparam.BackBufferFormat, D3DPOOL_DEFAULT,
            &d3d.backsurf, NULL);
        // d3d.d3ddev->CreateRenderTarget(bufwidth, bufheight,
        // d3d.d3dparam.BackBufferFormat, D3DMULTISAMPLE_NONE, 0, FALSE,
        // &d3d.backsurf, NULL);
#ifdef SUPPORT_WAB
        d3d.d3ddev->CreateOffscreenPlainSurface(
            wabwidth, wabheight, d3d.d3dparam.BackBufferFormat, D3DPOOL_DEFAULT,
            &d3d.wabsurf, NULL);
#endif

#if defined(SUPPORT_DCLOCK)
        d3d.d3ddev->CreateOffscreenPlainSurface(
            DCLOCK_WIDTH, DCLOCK_HEIGHT, d3d.d3dparam.BackBufferFormat,
            D3DPOOL_DEFAULT, &d3d.clocksurf, NULL);
#endif
      } else {
        d3d.d3ddev->CreateOffscreenPlainSurface(
            bufwidth, bufheight, d3d.d3dparam.BackBufferFormat, D3DPOOL_DEFAULT,
      ...
      d3d.d3ddev->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO,
                                &d3d.d3dbacksurf);
      // scrnmngD3D_destroy();
...
static BOOL CALLBACK monitorInfoEnumProc(HMONITOR hMonitor, HDC hdcMonitor,
  ...
  if (distx < (w1 + w2) / 2 && disty < (h1 + h2) / 2) {
    int areasize =
        (MIN(ex1, ex2) - MAX(sx1, sx2)) * (MIN(ey1, ey2) - MAX(sy1, sy2));
...
BRESULT scrnmngD3D_check() {
  ...
  fnd3dcreate9 =
      (TEST_DIRECT3DCREATE9)GetProcAddress(hModule, "Direct3DCreate9");
  if (!fnd3dcreate9) {
    goto scre_err2;
  }
  ...
  if (test_d3d->GetAdapterIdentifier(D3DADAPTER_DEFAULT, 0, &identifier) ==
      D3D_OK) {
  ...
  if (test_d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, g_hWndMain,
                             D3DCREATE_HARDWARE_VERTEXPROCESSING,
                             &test_d3dparam, &test_d3ddev) != D3D_OK) {
    if (test_d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, g_hWndMain,
                               D3DCREATE_SOFTWARE_VERTEXPROCESSING,
                               &test_d3dparam, &test_d3ddev) != D3D_OK) {
      if (test_d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_REF, g_hWndMain,
                                 D3DCREATE_SOFTWARE_VERTEXPROCESSING,
                                 &test_d3dparam, &test_d3ddev) != D3D_OK) {
        goto scre_err3;
      }
...
BRESULT scrnmngD3D_create(UINT8 scrnmode) {
  ...
  d3d.d3dparam.Windowed =
      ((scrnmode & SCRNMODE_FULLSCREEN) && np2oscfg.d3d_exclusive) ? FALSE
                                                                   : TRUE;
  ...
  d3d.d3dparam.Flags =
      (scrnmode & SCRNMODE_FULLSCREEN) ? D3DPRESENTFLAG_LOCKABLE_BACKBUFFER : 0;
  bitcolor = 32;
  ...
  if (scrnmode & SCRNMODE_FULLSCREEN) {
    ...
    if (!np2oscfg.d3d_exclusive) {
      ...
      fscrnmod |=
          FSCRNMOD_SAMERES |
          FSCRNMOD_SAMEBPP; // 排他モードを使わないとき、解像度変更しないことにする（できなくはないが面倒なことこの上ない）
      GetWindowRect(g_hWndMain,
                    &mondata.np2windowrect); // 猫ウィンドウの位置とサイズを取得
      EnumDisplayMonitors(NULL, NULL, (MONITORENUMPROC)monitorInfoEnumProc,
                          (LPARAM)&mondata); // 猫のいるモニターを探す
      if (!mondata.found) {
      ...
      MoveWindow(g_hWndMain, mondata.monitorrect.left, mondata.monitorrect.top,
                 mondata.monitorrect.right - mondata.monitorrect.left,
                 mondata.monitorrect.bottom - mondata.monitorrect.top,
                 TRUE); // ウィンドウサイズを全画面に変える
    }
    ...
    for (k = 0; k < CREATEDEVICE_RETRY_MAX; k++) {
      if (d3d.d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, g_hWndMain,
                                D3DCREATE_HARDWARE_VERTEXPROCESSING,
                                &d3d.d3dparam, &d3d.d3ddev) != D3D_OK) {
        if (d3d.d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL,
                                  g_hWndMain,
                                  D3DCREATE_SOFTWARE_VERTEXPROCESSING,
                                  &d3d.d3dparam, &d3d.d3ddev) != D3D_OK) {
          if (d3d.d3d->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_REF,
                                    g_hWndMain,
                                    D3DCREATE_SOFTWARE_VERTEXPROCESSING,
                                    &d3d.d3dparam, &d3d.d3ddev) != D3D_OK) {
            Sleep(1000);
...
#ifdef SUPPORT_WAB
    d3d.d3ddev->CreateOffscreenPlainSurface(
        width, height, d3d.d3dparam.BackBufferFormat, D3DPOOL_DEFAULT,
        &d3d.wabsurf, NULL);
#endif
...
#ifdef SUPPORT_WAB
    // if(!np2wabwnd.multiwindow && (np2wab.relay&0x3)!=0 &&
    // np2wab.realWidth>=640 && np2wab.realHeight>=400){
    if (!np2wabwnd.multiwindow && (np2wab.relay & 0x3) != 0 &&
        scrnstat.width >= 640 && scrnstat.height >= 400) {
      // 実サイズに
...
void scrnmngD3D_setwidth(int posx, int width) {
  ...
  if (scrnstat.multiple < 1)
    scrnstat.multiple = 8;
#if defined(SUPPORT_WAB)
  if (width > WAB_MAX_WIDTH)
    width = WAB_MAX_WIDTH;
#else
  if (width > 640)
    width = 640;
  ...
  if (scrnstat.width != width) {
    ...
    if (d3d.d3dbacksurf) {
      if (d3d.scrnmode & SCRNMODE_FULLSCREEN) {
        ...
        DEVMODE devmode;
        if (EnumDisplaySettings(NULL, ENUM_REGISTRY_SETTINGS, &devmode)) {
          while (((width * scrnstat.multiple) >> 3) >=
                 (int)devmode.dmPelsWidth - 32) {
            scrnstat.multiple--;
            if (scrnstat.multiple == 1)
              break;
          }
...
void scrnmngD3D_setheight(int posy, int height) {
  ...
  if (scrnstat.multiple < 1)
    scrnstat.multiple = 8;
#if defined(SUPPORT_WAB)
  if (height > WAB_MAX_HEIGHT)
    height = WAB_MAX_HEIGHT;
#else
  if (height > 1024)
    height = 1024;
#endif
  if (scrnstat.height != height) {
    ...
    if (d3d.d3dbacksurf) {
      if (d3d.scrnmode & SCRNMODE_FULLSCREEN) {
        ...
        DEVMODE devmode;
        if (EnumDisplaySettings(NULL, ENUM_REGISTRY_SETTINGS, &devmode)) {
          while (((height * scrnstat.multiple) >> 3) >=
                 (int)devmode.dmPelsHeight - 32) {
            scrnstat.multiple--;
            if (scrnstat.multiple == 1)
              break;
          }
...
void scrnmngD3D_update(void) {
  ...
  // d3d_enter_criticalsection();
  if (d3d.backsurf != NULL) {
    if (d3d.backsurf2 != NULL && (current_d3d_imode == D3D_IMODE_PIXEL ||
                                  current_d3d_imode == D3D_IMODE_PIXEL2 ||
                                  current_d3d_imode == D3D_IMODE_PIXEL3)) {
      RECT rectbuf = {0};
      RECT rectsrcbuf = {0};
      ...
      if (d3d.scrnmode & SCRNMODE_FULLSCREEN) {
        ...
        if (d3d.backsurf2mul > 1) {
          ...
          if (nvidia_fixflag) {
            ...
            r = d3d.d3ddev->StretchRect(d3d.backsurf, &rectsrcbuf,
                                        d3d.backsurf2, &rectdstbuf,
                                        D3DTEXF_POINT); // 右端
            rectsrcbuf = *rect;
            rectdstbuf = rectbuf;
          ...
          r = d3d.d3ddev->StretchRect(d3d.backsurf, &rectsrcbuf, d3d.backsurf2,
                                      &rectdstbuf, D3DTEXF_POINT);
          r = d3d.d3ddev->StretchRect(d3d.backsurf2, &rectbuf, d3d.d3dbacksurf,
                                      scrn, D3DTEXF_LINEAR);
        } else {
          r = d3d.d3ddev->StretchRect(d3d.backsurf, rect, d3d.d3dbacksurf, scrn,
                                      D3DTEXF_LINEAR);
        }
        *rect = lastrect;
        *scrn = lastscrn;
        ...
        if (d3d.backsurf2mul > 1) {
          ...
          r = d3d.d3ddev->StretchRect(d3d.backsurf, &d3d.rect, d3d.d3dbacksurf,
                                      &dst, D3DTEXF_LINEAR);
        }
      }
  ...
  if (scrnstat.multiple != multiple) {
    ...
    if (d3d.d3dbacksurf) {
      if (d3d.scrnmode & SCRNMODE_FULLSCREEN) {
        ...
        DEVMODE devmode;
        if (EnumDisplaySettings(NULL, ENUM_REGISTRY_SETTINGS, &devmode)) {
          while (((scrnstat.width * scrnstat.multiple) >> 3) >=
                     (int)devmode.dmPelsWidth - 32 ||
                 ((scrnstat.height * scrnstat.multiple) >> 3) >=
                     (int)devmode.dmPelsHeight - 32) {
            scrnstat.multiple--;
            if (scrnstat.multiple == 1)
              break;
...
int scrnmngD3D_getmultiple(void) { return scrnstat.multiple; }
...
void scrnmngD3D_dispclock(void) {
  ...
  d3d.d3ddev->StretchRect(d3d.clocksurf, &rectclk, d3d.d3dbacksurf, &dstrect,
                          D3DTEXF_LINEAR);
  d3d_leave_criticalsection();
...
// ウィンドウアクセラレータ画面転送 GDI Device Independent Bitmap -> Direct3D
// WAB surface
void scrnmngD3D_blthdc(HDC hdc) {
  ...
  if (np2wabwnd.multiwindow)
    return;
  if (mt_wabpausedrawing)
    return;
  if (np2wab.wndWidth < 32 || np2wab.wndHeight < 32)
    return;
  if (d3d.wabsurf != NULL) {
    ...
    if (r == D3D_OK) {
      ...
      case SCRNMODE_ROTATELEFT:
        ...
        r = PlgBlt(hDCDD, pt, hdc, 0, 0, np2wab.realWidth, np2wab.realHeight,
                   NULL, 0, 0);
        break;
      case SCRNMODE_ROTATERIGHT:
        ...
        break;
      default:
        r = BitBlt(hDCDD, 0, 0, scrnstat.width, scrnstat.height, hdc, 0, 0,
                   SRCCOPY);
      }
...
// ウィンドウアクセラレータ画面転送 Direct3D WAB surface -> Direct3D back
// surface
void scrnmngD3D_bltwab() {
  ...
  if (d3d.backsurf != NULL) {
    ...
    d3d_enter_criticalsection();
    d3d.d3ddev->StretchRect(d3d.wabsurf, &src, d3d.backsurf, &dstmp,
                            D3DTEXF_POINT);
    d3d_leave_criticalsection();
  }
#endif
