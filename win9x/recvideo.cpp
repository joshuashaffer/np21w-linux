 ...
 * ヘッダー定義
 */
static const BITMAPINFOHEADER s_bmih = {sizeof(BITMAPINFOHEADER),
                                        VIDEO_WIDTH,
                                        VIDEO_HEIGHT,
                                        1,
                                        24,
...
RecodeVideo::RecodeVideo()
    : m_bEnabled(false), m_bDirty(false), m_nStep(0), m_pWork8(NULL),
      m_pWork24(NULL), m_pAvi(NULL), m_pStm(NULL), m_pStmTmp(NULL), m_nFrame(0)
#if defined(AVI_SPLIT_SIZE)
      ,
      m_nNumber(0), m_dwSize(0)
...
bool RecodeVideo::OpenFile(LPCTSTR lpFilename) {
  ...
  if (::AVIFileOpen(&m_pAvi, lpFilename,
...
void RecodeVideo::Write() {
  ...
  while (m_nStep >= 0) {
    ...
    LONG lSize = 0;
    if (::AVIStreamWrite(m_pStmTmp, m_nFrame, 1, pBuffer, nBufferSize, dwFlags,
                         NULL, &lSize) != 0) {
      break;
    }
    m_bDirty = false;
