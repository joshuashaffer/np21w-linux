...
HHOOK hHook = NULL;
LRESULT CALLBACK np2_ThreadFuncKeyHook_WndProc(HWND hWnd, UINT msg,
                                               WPARAM wParam, LPARAM lParam) {
  switch (msg) {
  case WM_CLOSE:
    if (!np2_hThreadKeyHookexit)
      return 0;
    break;
  case WM_DESTROY:
    PostQuitMessage(0);
...
static unsigned int __stdcall np2_ThreadFuncKeyHook(LPVOID vdParam) {
  ...
  if (!(np2_hThreadKeyHookhWnd = CreateWindow(
            wndclassname, _T("NP2 Key Hook"), WS_POPUPWINDOW, CW_USEDEFAULT, 0,
            CW_USEDEFAULT, 0, NULL, NULL, g_hInstance, NULL)))
    return 0;
  ...
  if (!hHook) {
    hHook =
        SetWindowsHookEx(WH_KEYBOARD_LL, LowLevelKeyboardProc, g_hInstance, 0);
  }
  // メイン メッセージ ループ
...
static void start_hook_systemkey() {
  ...
  if (!np2_hThreadKeyHook) {
    np2_hThreadKeyHook =
        (HANDLE)_beginthreadex(NULL, 0, np2_ThreadFuncKeyHook, NULL, 0, &dwID);
  }
  // if(!hHook){
...
VOID CALLBACK SysMngHideTimerProc(HWND hwnd, UINT uMsg, UINT_PTR idEvent,
                                  DWORD dwTime) {
  ...
  if (renewal) {
    ...
    if ((newmode & SCRNMODE_FULLSCREEN) == 0) {
      ...
      if (EnumDisplaySettings(NULL, ENUM_REGISTRY_SETTINGS, &devmode)) {
        while ((DWORD)((scrnstat.width * scrnstat.multiple) >> 3) >=
                   devmode.dmPelsWidth - 64 ||
               (DWORD)((scrnstat.height * scrnstat.multiple) >> 3) >=
                   devmode.dmPelsHeight - 64) {
          scrnstat.multiple--;
...
static void OpenSoundDevice(HWND hWnd) {
  ...
  if (pSoundMng->Open(
          static_cast<CSoundMng::DeviceType>(np2oscfg.cSoundDeviceType),
          np2oscfg.szSoundDeviceName, hWnd)) {
...
static void np2updatemenu() {
  ...
  for (drv = 0x00; drv < 0x04; drv++) {
    ...
    if (menu_searchmenu(hMenu, IDM_IDE0OPEN + drv, &hMenuTgt, &hMenuTgtPos)) {
      ...
      for (nDrive = 0; nDrive < 26; nDrive++) {
        if (drvMenuVisible[drv][nDrive]) {
          DeleteMenu(hMenuTgt, IDM_IDE0PHYSICALDRV_ID0 + 26 * drv + nDrive,
                     MF_BYCOMMAND);
      ...
      if (np2cfg.idetype[drv] == SXSIDEV_CDROM) {
        ...
        for (nDrive = 0; nDrive < 26; nDrive++) {
          if (drvAvailable[nDrive]) {
            TCHAR mnuText[200] = {0};
            szBuff2[0] = nDrive + 'A';
            if (!LoadString(g_hInstance, IDS_PHYSICALDRIVE, mnuText,
                            sizeof(mnuText) / sizeof(mnuText[0]) -
                                sizeof(szBuff2) / sizeof(szBuff2[0]))) {
              _tcscpy(mnuText, OEMTEXT("&Physical Drive "));
            }
            _tcscat(mnuText, szBuff2);
            InsertMenu(hMenuTgt, mnupos++, MF_BYPOSITION,
                       IDM_IDE0PHYSICALDRV_ID0 + 26 * drv + nDrive, mnuText);
    ...
    // if(np2cfg.idetype[drv]==SXSIDEV_CDROM)
    //{
    //	EnableMenuItem(hMenu, IDM_IDE0PHYSICALDRV+drv,
...
static void OnCommand(HWND hWnd, WPARAM wParam) {
...
#if defined(SUPPORT_IDEIO)
#if defined(SUPPORT_PHYSICAL_CDDRV)
  if (IDM_IDE0PHYSICALDRV_ID0 <= uID &&
      uID < IDM_IDE0PHYSICALDRV_ID0 + 26 * 4) {
  ...
  case IDM_IDE3STATE: {
    ...
    if (fname && *fname) {
      ...
      ShellExecute(NULL, NULL, OEMTEXT("explorer.exe"), seltmp, NULL,
                   SW_SHOWNORMAL);
    }
  } break;
  ...
  case IDM_SWAPPAGEUPDOWN:
    ...
    winkbd_roll(np2oscfg.KEYBOARD == KEY_PC98 ? np2oscfg.xrollkey
                                              : !np2oscfg.xrollkey);
    update |= SYS_UPDATEOSCFG;
    break;
  ...
  case IDM_MOUSENC:
    ...
    if (np2oscfg.mouse_nc) {
      SetClassLong(g_hWndMain, GCL_STYLE,
                   GetClassLong(g_hWndMain, GCL_STYLE) & ~CS_DBLCLKS);
  ...
  case IDM_COPYPASTE_COPYTVRAM: {
    ...
    OEMCHAR *lpMem;
    hMem =
        GlobalAlloc(GHND, 0x4000); // アトリビュート分小さくなるので0x4000で十分
...
#ifdef SUPPORT_WAB
  {
    ...
    if (LOADINTELWORD(bi.biBitCount) <= 8) {
      ...
      hBmp = CreateDIBitmap(hDC, &(lpbinfo->bmiHeader), CBM_INIT, lppixels,
                            lpbinfo, DIB_RGB_COLORS);
    } else {
      hBmp = CreateDIBitmap(hDC, &(lpbinfo->bmiHeader), CBM_INIT, lppixels,
                            lpbinfo, DIB_RGB_COLORS);
    }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
...
#if defined(SUPPORT_IDEIO)
      while (hddrv_IDE <= 0x03 && np2cfg.idetype[hddrv_IDE] != 0x01)
        hddrv_IDE++;
      while (hddrv_IDECD <= 0x03 && np2cfg.idetype[hddrv_IDECD] != 0x02)
        hddrv_IDECD++;
#endif //	SUPPORT_IDEIO
      for (i = 0; i < files; i++) {
...
#endif //	SUPPORT_IDEIO
       //	HDイメージ？
        if ((!file_cmpname(ext, str_hdi)) || (!file_cmpname(ext, str_thd)) ||
  ...
  case WM_PAINT:
    ...
    if (np2opening) {
      ...
      HINSTANCE hInstance =
          CWndProc::FindResourceHandle(TEXT("NP2BMP"), RT_BITMAP);
      ...
      BitBlt(hdc, (width - bmp.bmWidth) / 2, (height - bmp.bmHeight) / 2,
  ...
  case WM_KEYDOWN:
    autokey_sendbufferpos =
        autokey_sendbufferlen; // コピペ強制終了 np21w ver0.86 rev22
    if (wParam == VK_F11) {
  ...
  case WM_KEYUP:
    ...
    if ((wParam == VK_F12) && (np2oscfg.F12COPY == 7)) {
      ...
      update |= SYS_UPDATECFG;
               ...
               (np2oscfg.F12COPY && np2oscfg.F12COPY != 7)) {
  ...
  case WM_SYSKEYDOWN:
    ...
    if (GetAsyncKeyState(VK_RMENU) >>
        ((sizeof(SHORT) * 8) - 1)) { // np21w ver0.86 rev6
#else
    ...
    } /*else*/
    {
      if (np2oscfg.mouse_nc /* && !scrnmng_isfullscreen()*/) {
        ...
        if ((x - lastmx) || (y - lastmy)) {
          ...
          if (mousebufX >= np2oscfg.mousediv ||
              mousebufX <= -np2oscfg.mousediv) {
            dx += (SINT16)(mousebufX / np2oscfg.mousediv);
            mousebufX = mousebufX % np2oscfg.mousediv;
          }
          if (mousebufY >= np2oscfg.mousediv ||
              mousebufY <= -np2oscfg.mousediv) {
...
#define MOUSE_EDGE_ACM 4
          if (x < mouse_edge_sh_x && dx < 0) {
            dxmul =
                1 + (mouse_edge_sh_x - x) * MOUSE_EDGE_ACM / mouse_edge_sh_x;
          } else if (r.right - mouse_edge_sh_x <= x && dx > 0) {
            dxmul = 1 + (mouse_edge_sh_x - (r.right - x)) * MOUSE_EDGE_ACM /
                            mouse_edge_sh_x;
          } else {
            dxmul = 1;
          }
          if (y < mouse_edge_sh_y && dy < 0) {
            dymul =
                1 + (mouse_edge_sh_y - y) * MOUSE_EDGE_ACM / mouse_edge_sh_y;
          } else if (r.bottom - mouse_edge_sh_y <= y && dy > 0) {
            dymul = 1 + (mouse_edge_sh_y - (r.bottom - y)) * MOUSE_EDGE_ACM /
                            mouse_edge_sh_y;
          } else {
            dymul = 1;
          }
  ...
  case WM_LBUTTONDOWN:
    ...
    if (!mousemng_buttonevent(MOUSEMNG_LEFTDOWN)) {
      if (!scrnmng_isfullscreen()) {
        ...
        POINT p;
        if ((GetCursorPos(&p)) && (scrnmng_isdispclockclick(&p))) {
          np2oscfg.clk_x++;
          sysmng_update(SYS_UPDATEOSCFG);
    ...
    if (np2oscfg.usewheel) {
      if ((wParam & (MK_CONTROL | MK_SHIFT)) == (MK_CONTROL | MK_SHIFT)) {
        ...
        if (np2oscfg.usemastervolume) {
          ...
          sysmng_updatecaption(SYS_UPDATECAPTION_MISC);
          tmrSysMngHide =
              SetTimer(hWnd, TMRSYSMNG_ID, 5000, SysMngHideTimerProc);
        }
      }
    }
    break;
  ...
  case WM_SETFOCUS:
    if (scrnmng_isfullscreen() &&
        scrnmng_current_drawtype == DRAWTYPE_DIRECT3D &&
        !np2oscfg.d3d_exclusive && !winui_en) {
  ...
  case MM_MIM_DATA:
    CComMidiIn32::RecvData(reinterpret_cast<HMIDIIN>(wParam),
                           static_cast<UINT>(lParam));
    break;

  case MM_MIM_LONGDATA:
    CComMidiIn32::RecvExcv(reinterpret_cast<HMIDIIN>(wParam),
                           reinterpret_cast<MIDIHDR *>(lParam));
    break;
  ...
  default:
    ...
    if (msg == WM_QueryCancelAutoPlay) {
      ...
      for (i = 0; i < 4; i++) {
        if (sxsi_getdevtype(i) == SXSIDEV_CDROM) {
          if ((np2cfg.idecd[i][0] == _T('\0') ||
               np2cfg.idecd[i][0] == _T('\\')) &&
              np2cfg.idecd[i][1] == _T('\\') && np2cfg.idecd[i][2] == _T('.') &&
              np2cfg.idecd[i][3] == _T('\\')) {
            return 1;
          }
        }
...
void createAsciiTo98KeyCodeList() {
  ...
  char asckeycode[] = {0x1d, 0x2d, 0x2b, 0x1f, 0x12, 0x20, 0x21, 0x22, 0x17,
  ...
  char spshascii[] = {'=', '`', '|', '~', '{', '+',
                      '*', '}', '<', '>', '?', '_'};
  char spkeycode[] = {0x0B, 0x0C, 0x0D, 0x1A, 0x1B, 0x26,
                      0x27, 0x28, 0x30, 0x31, 0x32, 0x33};
  vkeylist[' '] = 0x34;
  vkeylist['\t'] = 0x0f;
  vkeylist['\n'] = 0x1c;
  ...
  if (np2oscfg.syskhook) {
    ...
    case HC_ACTION: {
      if (GetForegroundWindow() == g_hWndMain) {
        ...
        bControlKeyDown =
            GetAsyncKeyState(VK_LCONTROL) >> ((sizeof(SHORT) * 8) - 1);
        bShiftKeyDown =
            GetAsyncKeyState(VK_LSHIFT) >> ((sizeof(SHORT) * 8) - 1);
        ...
        if (pkbhs->vkCode == VK_ESCAPE && bControlKeyDown ||
            ...
            pkbhs->vkCode == VK_LWIN || pkbhs->vkCode == VK_APPS) {

          switch ((int)wParam) {
          case WM_KEYDOWN:
          case WM_SYSKEYDOWN:
            winkbd_keydown(kbstruct->vkCode, ((kbstruct->flags) << 24) |
                                                 (kbstruct->scanCode << 16));
            break;
          case WM_KEYUP:
          case WM_SYSKEYUP:
            winkbd_keyup(kbstruct->vkCode, ((kbstruct->flags) << 24) |
                                               (kbstruct->scanCode << 16));
            break;
          }
          return 1;
...
#endif
    if (lateframecount > np2oscfg.cpustabf)
      lateframecount = np2oscfg.cpustabf;
    timing_setcount(0);
    framereset(cnt);
  } else {
    if (lateframecount) {
...
#define WINVER2 0x0500
  ...
  winx = np2oscfg.winx;
  winy = np2oscfg.winy;
  hWnd = CreateWindowEx(0, szClassName, np2oscfg.titles, style, winx, winy, 640,
  ...
  SetWindowPos(
      hWnd, NULL, winx, winy, 0, 0,
      SWP_NOSIZE | SWP_NOZORDER | SWP_NOZORDER | SWP_NOACTIVATE |
          SWP_FRAMECHANGED); // Win10環境でウィンドウ位置がずれる問題の対策
