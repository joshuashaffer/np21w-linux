...
UINT oemtext_mbtoucs2(UINT cp, wchar_t *dst, UINT dcnt, const char *src,
                      UINT scnt) {
...
UINT oemtext_ucs2tomb(UINT cp, char *dst, UINT dcnt, const wchar_t *src,
                      UINT scnt) {
...
UINT oemtext_mbtoutf8(UINT cp, char *dst, UINT dcnt, const char *src,
                      UINT scnt) {
...
UINT oemtext_utf8tomb(UINT cp, char *dst, UINT dcnt, const char *src,
                      UINT scnt) {
...
UINT textcnv_getinfo(TCINF *inf, const UINT8 *hdr, UINT hdrsize) {
  ...
  if ((hdrsize >= 3) && (hdr[0] == 0xef) && (hdr[1] == 0xbb) &&
      (hdr[2] == 0xbf)) {
    // UTF-8
