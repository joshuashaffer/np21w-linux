 ...
 * コンストラクタ
 */
CMpu98Page::CMpu98Page() : CPropPageProc(IDD_MPUPC98), m_mpu(0) {}
...
BOOL CMpu98Page::OnInitDialog() {
  ...
  CheckDlgButton(IDC_MPU98DEFE,
                 (np2oscfg.mpu.def_en) ? BST_CHECKED : BST_UNCHECKED);
...
void CMpu98Page::OnOK() {
  ...
  np2oscfg.mpu.def_en =
      (IsDlgButtonChecked(IDC_MPU98DEFE) != BST_UNCHECKED) ? 1 : 0;
  ...
  if (milstr_cmp(np2oscfg.mpu.def, mdef)) {
    ...
    if (cm_mpu98) {
      cm_mpu98->msg(cm_mpu98, COMMSG_MIMPIDEFFILE,
                    reinterpret_cast<INTPTR>(mdef));
    }
 ...
 * @param[in] cValue 設定
 */
void CMpu98Page::SetPort(UINT8 cValue) { m_port.SetCurItemData(cValue & 0xf0); }

/**
 * I/O を取得
 * @return I/O
 */
UINT8 CMpu98Page::GetPort() const { return m_port.GetCurItemData(0x00); }

/**
 * INT を設定
 * @param[in] cValue 設定
 */
void CMpu98Page::SetInt(UINT8 cValue) { m_int.SetCurItemData(cValue & 0x03); }

/**
 * INT を取得
 * @return INT
 */
UINT8 CMpu98Page::GetInt() const { return m_int.GetCurItemData(0x00); }
 ...
 * コンストラクタ
 */
CSMpu98Page::CSMpu98Page() : CPropPageProc(IDD_SMPU98), m_smpu(0) {}
...
BOOL CSMpu98Page::OnInitDialog() {
  ...
  CheckDlgButton(IDC_MPU98DEFE,
                 (np2oscfg.smpuA.def_en) ? BST_CHECKED : BST_UNCHECKED);
  ...
  CheckDlgButton(IDC_MPU98DEFE_B,
                 (np2oscfg.smpuB.def_en) ? BST_CHECKED : BST_UNCHECKED);
...
void CSMpu98Page::OnOK() {
  ...
  // Port A
  {
    ...
    if (milstr_cmp(np2oscfg.smpuA.def, mdef)) {
      ...
      if (cm_smpu98[0]) {
        cm_smpu98[0]->msg(cm_smpu98[0], COMMSG_MIMPIDEFFILE,
                          reinterpret_cast<INTPTR>(mdef));
      }
      update |= SYS_UPDATEOSCFG;
    }
  ...
  // Port B
  {
    ...
    np2oscfg.smpuB.def_en =
        (IsDlgButtonChecked(IDC_MPU98DEFE_B) != BST_UNCHECKED) ? 1 : 0;
    ...
    if (milstr_cmp(np2oscfg.smpuB.def, mdef)) {
      ...
      if (cm_smpu98[1]) {
        cm_smpu98[1]->msg(cm_smpu98[1], COMMSG_MIMPIDEFFILE,
                          reinterpret_cast<INTPTR>(mdef));
      }
      update |= SYS_UPDATEOSCFG;
    }
 ...
 * @return I/O
 */
UINT8 CSMpu98Page::GetPort() const { return m_port.GetCurItemData(0x00); }

/**
 * INT を設定
 * @param[in] cValue 設定
 */
void CSMpu98Page::SetInt(UINT8 cValue) { m_int.SetCurItemData(cValue & 0x03); }

/**
 * INT を取得
 * @return INT
 */
UINT8 CSMpu98Page::GetInt() const { return m_int.GetCurItemData(0x00); }
