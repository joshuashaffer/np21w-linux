...
void dialog_changefdd(HWND hWnd, REG8 drv) {
  if (drv < 4) {
    ...
    CFileDlg dlg(TRUE, rExt.c_str(), lpPath,
                 OFN_FILEMUSTEXIST | OFN_HIDEREADONLY, rFilter.c_str(), hWnd);
...
void dialog_changehdd(HWND hWnd, REG8 drv) {
  ...
  CFileDlg dlg(TRUE, rExt.c_str(), lpPath,
               OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_SHAREAWARE,
               rFilter.c_str(), hWnd);
  dlg.m_ofn.lpstrTitle = rTitle.c_str();
...
static const UINT32 s_hddsizetbl[] = {20,  41,   65,   80,   127, 255,
                                      511, 1023, 2047, 4095, 8191};
#else
static const UINT32 s_hddsizetbl[] = {20,  41,  65,   80,  127,
                                      255, 511, 1023, 2047};
#endif

/** HDD サイズ */
static const UINT32 s_hddCtbl[] = {16,   32,   64,   128,   256,   512,  1024,
...
/** SASI HDD */
static const UINT16 s_sasires[6] = {IDC_NEWSASI5MB,  IDC_NEWSASI10MB,
...
public:
  ...
  CNewHddDlg(HWND hwndParent, UINT32 nHddMinSize, UINT32 nHddMaxSize)
      : CDlgProc(IDD_NEWHDDDISK, hwndParent), m_nHddSize(0),
        m_nHddMinSize(nHddMinSize), m_nHddMaxSize(nHddMaxSize), m_advanced(0),
        m_usedynsize(0), m_HddC(0), m_HddH(0), m_HddS(0), m_HddSS(0),
        m_dynsize(0), m_blank(0) {}
   ...
   * @return サイズ
   */
  UINT32 GetSize() const { return m_nHddSize; }

  /**
   * シリンダ数を返す
   * @return サイズ
   */
  UINT GetC() const { return m_HddC; }

  /**
   * ヘッド数を返す
   * @return サイズ
   */
  UINT GetH() const { return m_HddH; }

  /**
   * セクタ数を返す
   * @return サイズ
   */
  UINT GetS() const { return m_HddS; }

  /**
   * セクタサイズを返す
   * @return サイズ
   */
  UINT GetSS() const { return m_HddSS; }

  /**
   * 詳細設定モードならtrue
   * @return 詳細設定モード
   */
  bool IsAdvancedMode() const { return (m_advanced != 0); }

  /**
   * 容量可変モードならtrue
   * @return 詳細設定モード
   */
  bool IsDynamicDisk() const { return (m_dynsize != 0); }

  /**
   * 空ディスク作成ならtrue
   * @return 詳細設定モード
   */
  bool IsBlankDisk() const { return (m_blank != 0); }
  ...
  void SetCHSfromSize() {
    ...
    m_HddC =
        (UINT32)((FILELEN)m_nHddSize * 1024 * 1024 / m_HddH / m_HddS / m_HddSS);
  ...
  void SetSizefromCHS() {
    ...
    m_nHddSize =
        (UINT32)((FILELEN)m_HddC * m_HddH * m_HddS * m_HddSS / 1024 / 1024);
  ...
  void GetDlgItemRect(UINT nID, RECT &rect) {
    ...
    ::MapWindowPoints(HWND_DESKTOP, m_hWnd, reinterpret_cast<LPPOINT>(&rect),
                      2);
  }
...
protected:
  ...
  virtual BOOL OnInitDialog() {
    ...
    while (hddsizetblcount < _countof(s_hddsizetbl) &&
           s_hddsizetbl[hddsizetblcount] <= m_nHddMaxSize) {
      hddsizetblcount++;
    }
    ...
    TCHAR work[32];
    ::wsprintf(work, TEXT("(%u-%uMB)"), m_nHddMinSize,
               m_nHddMaxSize > NHD_MAXSIZE28
                   ? NHD_MAXSIZE28
                   : m_nHddMaxSize); // 表向き28bit LBA制限
  ...
  BOOL OnCommand(WPARAM wParam, LPARAM lParam) {
    ...
    case IDC_HDDADVANCED:
      RECT rect;
      GetWindowRect(&rect);
      np2class_move(m_hWnd, rect.left, rect.top, m_szNewDisk.cx,
                    m_szNewDisk.cy);
  ...
  /**
   * CWndProc オブジェクトの Windows プロシージャ (WindowProc)
   * が用意されています
...
public:
  ...
  CNewSasiDlg(HWND hwndParent)
      : CDlgProc(IDD_NEWSASI, hwndParent), m_nType(0) {}
...
public:
   ...
   * @return タイプ
   */
  UINT8 GetType() const { return m_nFdType; }

  /**
   * ラベルを得る
   * @return ラベル
   */
  LPCTSTR GetLabel() const { return m_szDiskLabel; }
...
public:
  ...
  void SetProgressMax(UINT32 value) const {
    ::SendMessage(GetDlgItem(IDC_HDDCREATE_PROGRESS), PBM_SETRANGE, (WPARAM)0,
                  MAKELPARAM(0, value));
  }
  ...
  void SetProgressValue(UINT32 value) const {
    ::SendMessage(GetDlgItem(IDC_HDDCREATE_PROGRESS), PBM_SETPOS, (WPARAM)value,
                  0);
  }
...
static unsigned int __stdcall newdisk_ThreadFunc(LPVOID vdParam) {
  ...
  if (!file_cmpname(ext, str_thd)) {
    ...
    if (_mt_diskSize) {
      // 全容量指定モード
      newdisk_nhd_ex(lpPath, _mt_diskSize, _mt_blank, &mt_progressvalue,
                     &_mt_cancel);
    } else {
      // CHS指定モード
      newdisk_nhd_ex_CHS(lpPath, _mt_diskC, _mt_diskH, _mt_diskS, _mt_diskSS,
                         _mt_blank, &mt_progressvalue, &_mt_cancel);
    }
    ...
    if (_mt_diskSize) {
      // 全容量指定モード
      newdisk_vpcvhd_ex(lpPath, _mt_diskSize, _mt_dyndisk, _mt_blank,
                        &mt_progressvalue, &_mt_cancel);
    } else {
      // CHS指定モード
      newdisk_vpcvhd_ex_CHS(lpPath, _mt_diskC, _mt_diskH, _mt_diskS, _mt_diskSS,
                            _mt_dyndisk, _mt_blank, &mt_progressvalue,
                            &_mt_cancel);
    }
  }
#endif
...
void dialog_newdisk_ex(HWND hWnd, int mode) {
  ...
  CFileDlg fileDlg(FALSE, rDefExt.c_str(), szPath,
                   OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY, rFilter.c_str(),
                   hWnd);
  ...
  if (!file_cmpname(ext, str_thd)) {
    ...
    if (dlg.DoModal() == IDOK) {
      if (dlg.IsAdvancedMode()) {
        ...
        if ((_mt_diskC > 65535 || _mt_diskH > 16 || _mt_diskS > 255) &&
            MessageBoxA(hWnd,
                        "Specified parameters are incompatible with CHS "
                        "access.\nDo you want to continue anyway?",
                        "New Disk", MB_YESNO | MB_ICONQUESTION) != IDYES) {
          return;
        }
      } else {
        _mt_diskSize = dlg.GetSize();
        if (((FILELEN)_mt_diskSize * _mt_diskH * _mt_diskS >
             65535 * 16 * 255) &&
            MessageBoxA(hWnd,
                        "Specified parameters are incompatible with CHS "
                        "access.\nDo you want to continue anyway?",
      ...
      newdisk_hThread =
          (HANDLE)_beginthreadex(NULL, 0, newdisk_ThreadFunc, NULL, 0, &dwID);
...
#endif
  else if ((!file_cmpname(ext, str_d88)) || (!file_cmpname(ext, str_d98)) ||
           (!file_cmpname(ext, str_88d)) || (!file_cmpname(ext, str_98d))) {
    CNewFddDlg dlg(hWnd);
    if (dlg.DoModal() == IDOK) {
 ...
 * @param[in] hWnd 親ウィンドウ
 */
void dialog_newdisk(HWND hWnd) { dialog_newdisk_ex(hWnd, NEWDISKMODE_ALL); }
