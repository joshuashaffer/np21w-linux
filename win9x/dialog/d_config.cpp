...
//! 倍率リスト
static const UINT32 s_mulval[] = {1,  2,  4,  5,  6,  8,  10, 12, 16, 20, 24,
                                  30, 32, 34, 36, 40, 42, 44, 46, 48, 50, 52,
...
//! サンプリング レート
static const UINT32 s_nSamplingRate[] = {11025, 22050, 44100,
                                         48000, 88200, 96000};
...
BOOL CConfigureDlg::OnInitDialog() {
  ...
  const UINT32 nBaseClock =
      (np2cfg.baseclock == PCBASECLOCK20) ? PCBASECLOCK20 : PCBASECLOCK25;
...
#if defined(CPUCORE_IA32)
  // const UINT32 cpufeaturelist[] = {0, CPU_FEATURES_I486SX,
  // CPU_FEATURES_I486DX, CPU_FEATURES_PENTIUM, CPU_FEATURES_MMX_PENTIUM,
  // CPU_FEATURES_PENTIUM_PRO, CPU_FEATURES_PENTIUM_II}; i = 0;
  ...
  const CSoundMng::DeviceType nType =
  ...
  if (np2oscfg.szSoundDeviceName[0] != '\0') {
    ...
    if (pDevices) {
      ...
      while ((it != pDevices->end()) &&
             (::lstrcmpi(np2oscfg.szSoundDeviceName, *it) != 0)) {
        ++it;
      }
      if (it == pDevices->end()) {
  ...
  CheckDlgButton(IDC_ALLOWRESIZE,
                 (np2oscfg.thickframe) ? BST_CHECKED : BST_UNCHECKED);
  ...
  if (mmxflag & MMXFLAG_NOTSUPPORT) {
    ...
    CheckDlgButton(IDC_DISABLEMMX,
                   (np2oscfg.disablemmx) ? BST_CHECKED : BST_UNCHECKED);
  }
#else  // !defined(_WIN64)
  GetDlgItem(IDC_DISABLEMMX).EnableWindow(FALSE);
  ...
  CheckDlgButton(IDC_SAVEWINDOWSIZE,
                 (np2oscfg.svscrmul) ? BST_CHECKED : BST_UNCHECKED);
  SetClock();
...
void CConfigureDlg::UpdateDeviceList() {
  const CSoundMng::DeviceType nType = static_cast<CSoundMng::DeviceType>(
      m_type.GetCurItemData(np2oscfg.cSoundDeviceType));
  ...
  if (pDevices) {
    for (std::vector<LPCTSTR>::const_iterator it = pDevices->begin();
         it != pDevices->end(); ++it) {
      m_name.Add(*it, TRUE);
    }
  }
...
NekoCPUCheck:
  if (np2cfg.cpu_family == 0 && np2cfg.cpu_model == 0 &&
      ...
      np2cfg.cpu_feature_ecx == 0 && np2cfg.cpu_feature_ex == 0) {
    return 255;
  }
#endif
  return 0;
...
int CConfigureDlg::SetCpuTypeIndex(UINT index) {
  ...
  //#ifdef UNICODE
  //	MultiByteToWideChar(CP_ACP, 0, np2cfg.cpu_vendor, -1,
  //np2cfg.cpu_vendor_o, sizeof(np2cfg.cpu_vendor_o));
  //	MultiByteToWideChar(CP_ACP, 0, np2cfg.cpu_brandstring, -1,
  //np2cfg.cpu_brandstring_o, sizeof(np2cfg.cpu_brandstring_o)); #else
...
void CConfigureDlg::OnOK() {
  ...
  const CSoundMng::DeviceType nOldType =
      static_cast<CSoundMng::DeviceType>(np2oscfg.cSoundDeviceType);
  const CSoundMng::DeviceType nType =
      static_cast<CSoundMng::DeviceType>(m_type.GetCurItemData(nOldType));
  ...
  if ((nType != nOldType) ||
      (::lstrcmpi(szName, np2oscfg.szSoundDeviceName) != 0)) {
    np2oscfg.cSoundDeviceType = static_cast<UINT8>(nType);
    ::lstrcpyn(np2oscfg.szSoundDeviceName, szName,
               _countof(np2oscfg.szSoundDeviceName));
  ...
  const UINT8 bAllowResize =
      (IsDlgButtonChecked(IDC_ALLOWRESIZE) != BST_UNCHECKED) ? 1 : 0;
...
#if !defined(_WIN64)
  if (!(mmxflag & MMXFLAG_NOTSUPPORT)) {
    const UINT8 bDisableMMX =
        (IsDlgButtonChecked(IDC_DISABLEMMX) != BST_UNCHECKED) ? 1 : 0;
  ...
  const UINT8 bConfirm =
      (IsDlgButtonChecked(IDC_COMFIRM) != BST_UNCHECKED) ? 1 : 0;
  ...
  const UINT8 bResume =
      (IsDlgButtonChecked(IDC_RESUME) != BST_UNCHECKED) ? 1 : 0;
  if (np2oscfg.resume != bResume) {
    np2oscfg.resume = bResume;
    nUpdated |= SYS_UPDATEOSCFG;
  }

  const UINT8 bSaveScrnMul =
      (IsDlgButtonChecked(IDC_SAVEWINDOWSIZE) != BST_UNCHECKED) ? 1 : 0;
