...
void CStaticDipSw::Draw(HDC hdc, const void *lpBitmap) {
  ...
  void *pImage;
  HBITMAP hBitmap =
      ::CreateDIBSection(hdc, reinterpret_cast<const BITMAPINFO *>(lpBmpInfo),
                         DIB_RGB_COLORS, &pImage, NULL, 0);
  if (hBitmap == NULL) {
    return;
  }
  CopyMemory(pImage,
             static_cast<const UINT8 *>(lpBitmap) +
                 (LOADINTELDWORD(lpBmpFile->bfOffBits)),
             ::bmpdata_getdatasize(lpBmpInfo));
