...
SerialOptComPage::SerialOptComPage(UINT nCaption, COMMNG cm, COMCFG &cfg)
    : CPropPageProc(IDD_SERIAL1, nCaption), m_cm(cm), m_cfg(cfg) {}
...
BOOL SerialOptComPage::OnInitDialog() {
  ...
  if (m_cm != cm_rs232c) {
    ...
    m_chkfixedspeed.SendMessage(
        BM_SETCHECK, m_cfg.fixedspeed ? BST_CHECKED : BST_UNCHECKED, 0);
  }
...
void SerialOptComPage::OnOK() {
  ...
  const UINT8 cFSpeedEnable =
      (IsDlgButtonChecked(IDC_COM1FSPEED) != BST_UNCHECKED) ? 1 : 0;
  ...
  const UINT8 cDefEnable =
      (IsDlgButtonChecked(IDC_COM1DEFE) != BST_UNCHECKED) ? 1 : 0;
...
void SerialOptComPage::UpdateControls() {
  ...
  static const UINT midi[] = {IDC_COM1MMAP,  IDC_COM1MMDL, IDC_COM1DEFE,
                              IDC_COM1DEFF,  IDC_COM1DEFB, IDC_COM1STR10,
                              IDC_COM1STR11, IDC_COM1STR12};
  ...
  // Named-pipe
  static const UINT pipe[] = {IDC_COM1PIPENAME, IDC_COM1PIPESERV, IDC_COM1STR30,
                              IDC_COM1STR31, IDC_COM1STR32};
 ...
 * コンストラクタ
 */
SerialOpt61Page::SerialOpt61Page() : CPropPageProc(IDD_PC9861A) {
...
BOOL SerialOpt61Page::OnInitDialog() {
  CheckDlgButton(IDC_PC9861E,
                 (np2cfg.pc9861enable) ? BST_CHECKED : BST_UNCHECKED);
...
void SerialOpt61Page::OnOK() {
  ...
  const UINT8 cEnabled =
      (IsDlgButtonChecked(IDC_PC9861E) != BST_UNCHECKED) ? 1 : 0;
...
void SerialOpt61Page::OnDipSw() {
  ...
  if ((memcmp(m_sw, sw, sizeof(sw)) != 0) ||
      (memcmp(m_jmp, jmp, sizeof(jmp)) != 0)) {
    Set(sw, jmp);
    m_dipsw.Invalidate();
  }
}
