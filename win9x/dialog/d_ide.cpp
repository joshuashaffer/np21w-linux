 ...
 * @param[in] hwndParent 親ウィンドウ
 */
CIdeDlg::CIdeDlg(HWND hwndParent) : CDlgProc(IDD_IDE, hwndParent) {}
...
void CIdeDlg::OnOK() {
  ...
  if (np2cfg.useasynccd !=
      (m_chkasynccd.SendMessage(BM_GETCHECK, 0, 0) ? 1 : 0)) {
  ...
  if (np2cfg.allowcdtraycmd !=
      (m_chkallowtraycmd.SendMessage(BM_GETCHECK, 0, 0) ? 1 : 0)) {
    np2cfg.allowcdtraycmd =
        (m_chkallowtraycmd.SendMessage(BM_GETCHECK, 0, 0) ? 1 : 0);
    update |= SYS_UPDATECFG;
  }
  ...
  if (np2cfg.autoidebios !=
      (m_chkautoidebios.SendMessage(BM_GETCHECK, 0, 0) ? 1 : 0)) {
    np2cfg.autoidebios =
        (m_chkautoidebios.SendMessage(BM_GETCHECK, 0, 0) ? 1 : 0);
    update |= SYS_UPDATECFG;
  }
...
BOOL CIdeDlg::OnCommand(WPARAM wParam, LPARAM lParam) {
  ...
  case IDC_IDE4TYPE:
    return TRUE;
  case IDC_USEIDEBIOS:
    m_chkautoidebios.EnableWindow(
        (m_chkidebios.SendMessage(BM_GETCHECK, 0, 0) ? TRUE : FALSE));
    return TRUE;
  }
  return FALSE;
}
