 ...
 * コンストラクタ
 */
ScrOptVideoPage::ScrOptVideoPage() : CPropPageProc(IDD_SCROPT1) {}
...
void ScrOptVideoPage::OnOK() {
  ...
  const UINT8 cSkipLine =
      (IsDlgButtonChecked(IDC_SKIPLINE) != BST_UNCHECKED) ? 1 : 0;
...
BOOL ScrOptVideoPage::OnCommand(WPARAM wParam, LPARAM lParam) {
  if (LOWORD(wParam) == IDC_LCD) {
    const BOOL bEnable =
        (IsDlgButtonChecked(IDC_LCD) != BST_UNCHECKED) ? TRUE : FALSE;
 ...
 * コンストラクタ
 */
ScrOptChipPage::ScrOptChipPage() : CPropPageProc(IDD_SCROPT2) {}
...
BOOL ScrOptChipPage::OnInitDialog() {
  ...
  static const UINT s_disabled[] = {IDC_GCBOX, IDC_GRCGNON, IDC_GRCG,
                                    IDC_GRCG2, IDC_EGC,     IDC_PC980124};
...
void ScrOptChipPage::OnOK() {
  ...
  const UINT8 cMode =
      (IsDlgButtonChecked(IDC_GDC72020) != BST_UNCHECKED) ? 1 : 0;
  ...
  const UINT8 cColor16 =
      (IsDlgButtonChecked(IDC_PC980124) != BST_UNCHECKED) ? 1 : 0;
 ...
 * コンストラクタ
 */
ScrOptTimingPage::ScrOptTimingPage() : CPropPageProc(IDD_SCROPT3) {}
...
BOOL ScrOptFullscreenPage::OnInitDialog() {
  ...
  CheckDlgButton(IDC_FULLSCREEN_SAMEBPP,
                 (c & FSCRNMOD_SAMEBPP) ? BST_CHECKED : BST_UNCHECKED);
  CheckDlgButton(IDC_FULLSCREEN_SAMERES,
                 (c & FSCRNMOD_SAMERES) ? BST_CHECKED : BST_UNCHECKED);
...
void ScrOptFullscreenPage::OnOK() {
  ...
  if ((np2oscfg.fsrescfg &&
       (!scrnrescfg.hasfscfg || scrnrescfg.fscrnmod != c)) ||
      np2oscfg.fscrnmod != c2) {
    if ((np2oscfg.fsrescfg &&
         (!scrnrescfg.hasfscfg || scrnrescfg.fscrnmod != c)) ||
        (!np2oscfg.fsrescfg && np2oscfg.fscrnmod != c2)) {
      resetScreen = 1;
    }
    if (np2oscfg.fsrescfg) {
...
BOOL ScrOptFullscreenPage::OnCommand(WPARAM wParam, LPARAM lParam) {
  if (LOWORD(wParam) == IDC_FULLSCREEN_SAMERES) {
    m_zoom.EnableWindow(
        (IsDlgButtonChecked(IDC_FULLSCREEN_SAMERES) != BST_UNCHECKED) ? TRUE
                                                                      : FALSE);
    return TRUE;
  }
  return FALSE;
 ...
 * コンストラクタ
 */
ScrOptRendererPage::ScrOptRendererPage() : CPropPageProc(IDD_SCROPT_RENDERER) {}
...
BOOL ScrOptRendererPage::OnInitDialog() {
  ...
  CheckDlgButton(IDC_SOFTWARERENDERING,
                 (np2oscfg.emuddraw) ? BST_CHECKED : BST_UNCHECKED);

  m_chksoftrender.SubclassDlgItem(IDC_SOFTWARERENDERING, this);
  m_chksoftrender.SendMessage(
      BM_SETCHECK, (np2oscfg.emuddraw) ? BST_CHECKED : BST_UNCHECKED, 0);

  m_chkexclusive.SubclassDlgItem(IDC_RENDERER_EXCLUSIVE, this);
  m_chkexclusive.SendMessage(
      BM_SETCHECK, (np2oscfg.d3d_exclusive) ? BST_CHECKED : BST_UNCHECKED, 0);
...
void ScrOptRendererPage::OnOK() {
  ...
  if (tmp != np2oscfg.drawtype) {
    if (tmp == DRAWTYPE_DIRECT3D) {
      if (scrnmngD3D_check() != SUCCESS) {
        MessageBox(g_hWndMain, _T("Failed to initialize Direct3D."),
                   _T("Direct3D Error"), MB_OK | MB_ICONEXCLAMATION);
  ...
  if ((np2oscfg.fsrescfg &&
       (!scrnrescfg.hasfscfg || scrnrescfg.d3d_imode != tmp)) ||
      np2oscfg.d3d_imode != tmp2) {
    if ((np2oscfg.fsrescfg &&
         (!scrnrescfg.hasfscfg || scrnrescfg.d3d_imode != tmp)) ||
        (!np2oscfg.fsrescfg && np2oscfg.d3d_imode != tmp2)) {
      resetScreen = 1;
    }
    if (np2oscfg.fsrescfg) {
...
BOOL ScrOptRendererPage::OnCommand(WPARAM wParam, LPARAM lParam) {
  if (LOWORD(wParam) == IDC_RENDERER_TYPE) {
    ...
    m_chksoftrender.EnableWindow((drawtype == DRAWTYPE_DIRECTDRAW_HW ||
                                  drawtype == DRAWTYPE_DIRECTDRAW_SW)
                                     ? TRUE
                                     : FALSE);
    return TRUE;
  }
