...
//! タイトル
static const TCHAR s_np2title[] =
    TEXT(PROJECTNAME) TEXT(PROJECTSUBNAME) TEXT("  ");

//! 情報
static const TCHAR s_np2infostr[] =
    TEXT("CPU: %CPU% %CLOCK%\nMEM: %MEM1%\nFPU:%FPU%\nSIMD:%SIMD%\nGDC: "
         "%GDC%\n     %GDC2%\nTEXT: %TEXT%\nGRPH: %GRPH%\nSOUND: "
         "%EXSND%\n\nBIOS: %BIOS%\nRHYTHM: %RHYTHM%\n\nSCREEN: %DISP%");
 ...
 * @param[in] hwndParent 親ウィンドウ
 */
CAboutDlg::CAboutDlg(HWND hwndParent) : CDlgProc(IDD_ABOUT, hwndParent) {
