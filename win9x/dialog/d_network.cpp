 ...
 * コンストラクタ
 */
CNetworkPage::CNetworkPage() : CPropPageProc(IDD_NETWORK) {}
/**
 * デストラクタ
 */
...
#define BUFSZ 256
  ...
  while (bDone != TRUE) { // {id1} を列挙するループ
    ...
    while (1) { // {id2} を列挙するループ
      ...
      nResult = RegQueryValueEx(hKey3, _T("PnpInstanceID"), 0, &dwType,
      ...
      if (nResult == ERROR_SUCCESS) {
        if (_tcslen(szData) > 0) {
          ...
          dwSize = sizeof(szData);
          nResult = RegQueryValueEx(hKey3, _T("Name"), 0, &dwType,
                                    (LPBYTE)szData, &dwSize);
...
static const CComboData::Entry s_port[] = {
    {MAKEINTRESOURCE(IDS_00D0), 0x00}, {MAKEINTRESOURCE(IDS_10D0), 0x10},
    {MAKEINTRESOURCE(IDS_20D0), 0x20}, {MAKEINTRESOURCE(IDS_30D0), 0x30},
    {MAKEINTRESOURCE(IDS_40D0), 0x40}, {MAKEINTRESOURCE(IDS_50D0), 0x50},
    {MAKEINTRESOURCE(IDS_60D0), 0x60}, {MAKEINTRESOURCE(IDS_70D0), 0x70},
};
...
CLgy98Page::CLgy98Page()
    : CPropPageProc(IDD_LGY98), m_port((UINT8)IDS_10D0),
      m_int((UINT8)IDS_INT1) {}
/**
 * デストラクタ
 */
CLgy98Page::~CLgy98Page() {}
...
void CLgy98Page::OnOK() {
  ...
  if (np2cfg.uselgy98 != m_enabled || (np2cfg.lgy98io >> 8) != m_port ||
 ...
 * @param[in] cValue 設定
 */
void CLgy98Page::SetPort(UINT8 cValue) { m_cmbport.SetCurItemData(cValue); }

/**
 * I/O を取得
 * @return I/O
 */
UINT8 CLgy98Page::GetPort() const { return m_cmbport.GetCurItemData(0x01); }

/**
 * INT を設定
 * @param[in] cValue 設定
 */
void CLgy98Page::SetInt(UINT8 cValue) { m_cmbint.SetCurItemData(cValue); }

/**
 * INT を取得
 * @return INT
 */
UINT8 CLgy98Page::GetInt() const { return m_cmbint.GetCurItemData(0x01); }
