 ...
 * @param[in] hwndParent 親ウィンドウ
 */
CHostdrvDlg::CHostdrvDlg(HWND hwndParent) : CDlgProc(IDD_HOSTDRV, hwndParent) {}
...
void CHostdrvDlg::OnOK() {
  ...
  hostdrv_setcurrentpath(m_hdrvroot);
  if (m_hdrvenable != np2cfg.hdrvenable ||
      _tcscmp(np2cfg.hdrvroot, m_hdrvroot) != 0 ||
      m_hdrvacc != np2cfg.hdrvacc) {
...
BOOL CHostdrvDlg::OnCommand(WPARAM wParam, LPARAM lParam) {
  ...
  case IDC_HOSTDRVWRITE:
    ...
    m_hdrvacc |=
        (m_chkwrite.SendMessage(BM_GETCHECK, 0, 0) ? HDFMODE_WRITE : 0);
    return TRUE;

  case IDC_HOSTDRVDELETE:
    m_hdrvacc = (m_hdrvacc & ~HDFMODE_DELETE);
    m_hdrvacc |=
        (m_chkdelete.SendMessage(BM_GETCHECK, 0, 0) ? HDFMODE_DELETE : 0);
    return TRUE;
  }
  return FALSE;
}
