...
/** フィルター */
static const UINT s_nFilter[1] = {IDS_CFGFILTER};
 ...
 */
static void GetDefaultFilename(LPCTSTR lpExt, LPTSTR lpFilename,
                               UINT cchFilename) {
...
void dialog_writenpcfg(HWND hWnd) {
  ...
  CFileDlg dlg(FALSE, rExt.c_str(), szPath,
               OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY, rFilter.c_str(), hWnd);
  ...
  if (dlg.DoModal()) {
    ...
    LPTSTR lpFilenameBuf =
        (LPTSTR)malloc((_tcslen(lpFilename) + 1) * sizeof(TCHAR));
