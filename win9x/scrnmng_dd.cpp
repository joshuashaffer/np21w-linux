...
// プライマリサーフェイスのDirectDraw
// DDBLT_COLORFILLで例外が出る環境があるようなので代替
//#define DDBLT_COLORFILL_FIX

#ifdef DDBLT_COLORFILL_FIX
static void DDBlt_ColorFill(LPDIRECTDRAWSURFACE lpDst, LPRECT lpDstRect,
                            LPDDBLTFX lpDDBltFx,
                            LPDIRECTDRAWSURFACE lpOffScrBuf) {
  HDC hDC = NULL;
  RECT src;
...
#else
static void DDBlt_ColorFill(LPDIRECTDRAWSURFACE lpDst, LPRECT lpRect,
                            LPDDBLTFX lpDDBltFx,
                            LPDIRECTDRAWSURFACE lpOffScrBuf) {
...
static void paletteset() {
  ...
  if (ddraw.palette != NULL) {
    ...
    ddraw.palette->SetEntries(0, START_PAL, PALLETES_8BPP,
                              &ddraw.pal[START_PAL]);
  }
  dd_leave_criticalsection();
}
...
BOOL WINAPI DDEnumDisplayCallbackA(GUID FAR *lpGUID, LPSTR lpDriverDescription,
...
BRESULT scrnmngDD_create(UINT8 scrnmode) {
  ...
  if (scrnmode & SCRNMODE_FULLSCREEN) {
    ...
    if (lastscrnmode & SCRNMODE_FULLSCREEN) {
      ...
      GetModuleFileNameA(NULL, szModulePath,
      ...
      if (PathFileExistsA(szModulePath) && !PathIsDirectoryA(szModulePath)) {
        ...
        ShowWindow(
            g_hWndMain,
            SW_HIDE); // Aeroな環境でフルスクリーン→ウィンドウの時にシステムアイコンが消える対策のためのおまじない（その1）
        SetWindowLong(g_hWndMain, GWL_STYLE, winstyle);
        SetWindowLong(g_hWndMain, GWL_EXSTYLE, winstyleex);
  ...
  if (np2oscfg.emuddraw) {
    ...
    devlpguid = NULL;
    if (scrnmode & SCRNMODE_FULLSCREEN) {
      r = DirectDrawEnumerateExA(DDEnumDisplayCallbackA, &devguid,
  ...
  if ((r = DirectDrawCreate(devlpguid, &ddraw.ddraw1, NULL)) != DD_OK) {
    // プライマリで再挑戦
    if (DirectDrawCreate(np2oscfg.emuddraw ? (LPGUID)DDCREATE_EMULATIONONLY
                                           : NULL,
                         &ddraw.ddraw1, NULL) != DD_OK) {
      goto scre_err;
    }
  }
  ...
  if (scrnmode & SCRNMODE_FULLSCREEN) {
...
#endif
    ddraw2->SetCooperativeLevel(g_hWndMain, DDSCL_EXCLUSIVE | DDSCL_FULLSCREEN |
                                                DDSCL_MULTITHREADED);
    ...
    if ((fscrnmod & (FSCRNMOD_SAMERES | FSCRNMOD_SAMEBPP)) &&
        (dd_displayName[0]
             ? EnumDisplaySettings(dd_displayName, ENUM_REGISTRY_SETTINGS,
                                   &devmode)
    ...
    if (!np2wabwnd.multiwindow) {
      if (FSCRNCFG_fscrnmod & FSCRNMOD_SAMERES) {
        ...
        if ((np2wab.relay & 0x3) != 0 && np2wab.realWidth >= 640 &&
            np2wab.realHeight >= 400) {
          // 実サイズに
...
RGB16 scrnmngDD_makepal16(RGB32 pal32) {
  ...
  return ((RGB16)((pal.p.g << ddraw.l16g) + (pal.p.r << ddraw.l16r) +
                  (pal.p.b >> ddraw.r16b)));
}
...
void scrnmngDD_update(void) {
  ...
  if (ddraw.backsurf != NULL) {
    if (ddraw.scrnmode & SCRNMODE_FULLSCREEN) {
      ...
      r = ddraw.primsurf->Blt(scrn, ddraw.backsurf, rect, DDBLT_WAIT, NULL);
      if (r == DDERR_SURFACELOST) {
        restoresurfaces();
        dd_leave_criticalsection();
        return;
      ...
      r = ddraw.primsurf->Blt(&dst, ddraw.backsurf, &ddraw.rect, DDBLT_WAIT,
                              NULL);
      if (r == DDERR_SURFACELOST) {
        restoresurfaces();
        dd_leave_criticalsection();
...
int scrnmngDD_getmultiple(void) { return scrnstat.multiple; }
...
void scrnmngDD_dispclock(void) {
  ...
  if (ddraw.primsurf->BltFast(ddraw.width - DCLOCK_WIDTH - 4,
                              ddraw.height - DCLOCK_HEIGHT, ddraw.clocksurf,
                              (RECT *)&rectclk,
...
void scrnmngDD_updatefsres(void) {
  ...
  if (np2wab.lastWidth != width || np2wab.lastHeight != height) {
    ...
    if ((g_scrnmode & SCRNMODE_FULLSCREEN) != 0) {
      ...
      scrnmngDD_destroy();
      if (scrnmngDD_create(g_scrnmode) != SUCCESS) {
        if (scrnmngDD_create(g_scrnmode | SCRNMODE_FULLSCREEN) !=
            SUCCESS) { // フルスクリーンでリトライ
          PostQuitMessage(0);
          return;
        }
        g_scrnmode = g_scrnmode | SCRNMODE_FULLSCREEN;
