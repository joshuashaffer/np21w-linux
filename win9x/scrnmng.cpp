 ...
 * 設定読み込み
 */
void scrnres_readini() { scrnres_readini_res(scrnstat.width, scrnstat.height); }
...
void scrnres_writeini() {
  if (!np2oscfg.readonly) {
    ...
    OEMSPRINTF(szSectionName, OEMTEXT("Resolution%dx%d"), scrnstat.width,
               scrnstat.height);
...
void scrnmng_setwindowsize(HWND hWnd, int width, int height) {
  ...
  do {
    ...
    int winx =
        (np2oscfg.winx != CW_USEDEFAULT) ? np2oscfg.winx : rectwindow.left;
    int winy =
        (np2oscfg.winy != CW_USEDEFAULT) ? np2oscfg.winy : rectwindow.top;
    int cx = width;
