 ...
 * コンストラクタ
 */
CScciIf::CScciIf() : m_hModule(NULL), m_pManager(NULL) {}

/**
 * デストラクタ
 */
CScciIf::~CScciIf() { Deinitialize(); }
...
bool CScciIf::Initialize() {
  ...
  do {
    ...
    SCCIFUNC fnGetSoundInterfaceManager = reinterpret_cast<SCCIFUNC>(
        ::GetProcAddress(m_hModule, "getSoundInterfaceManager"));
 ...
 * @return インスタンス
 */
IExternalChip *CScciIf::GetInterface(IExternalChip::ChipType nChipType,
                                     UINT nClock) {
...
CScciIf::Chip::Chip(CScciIf *pScciIf, SoundChip *pSoundChip)
    : m_pScciIf(pScciIf), m_pSoundChip(pSoundChip) {}

/**
 * デストラクタ
 */
CScciIf::Chip::~Chip() { m_pScciIf->Detach(this); }

/**
 * オペレータ
 */
CScciIf::Chip::operator SoundChip *() { return m_pSoundChip; }
 ...
 * @return リザルト
 */
INTPTR CScciIf::Chip::Message(UINT nMessage, INTPTR nParameter) { return 0; }
