...
// dB = 20 log10( (音量0〜1) * (pow(10, 最大dB値/20) - pow(10, 最小dB値/20)) +
// pow(10, 最小dB値/20) )
//#define LINEAR2DB(a)	(20 * log10((a) * (pow(10.0, 20/20) - pow(10.0,
//-192/20)) + pow(10.0, -192/20)))
#define LINEAR2DB(a)                                                           \
  (pow(a, 0.12) * (20 + 192) -                                                 \
   192) // XXX: fmgen音量と猫音源音量を一致させるための実験式･･･
...
void opnalist_clear() { opnalistconunt = 0; }
...
void opna_fmgen_setallvolumeRhythmTotal_linear(int lvol) {
  for (int i = 0; i < opnalistconunt; i++) {
    OPNA_SetVolumeRhythmTotal(opnalist[i]->fmgen,
                              (int)LINEAR2DB((double)lvol / 128));
  }
}
#endif /* SUPPORT_FMGEN */
...
void opna_reset(POPNA opna, REG8 cCaps) {
  ...
  if (np2cfg.usefmgen) {
    ...
    OPNA_SetVolumePSG(opna->fmgen,
                      (int)LINEAR2DB((double)np2cfg.vol_ssg / 128));
    OPNA_SetVolumeADPCM(opna->fmgen,
                        (int)LINEAR2DB((double)np2cfg.vol_adpcm / 128));
    OPNA_SetVolumeRhythmTotal(opna->fmgen,
                              (int)LINEAR2DB((double)np2cfg.vol_rhythm / 128));
...
static void writeRegister(POPNA opna, UINT nAddress, REG8 cData) {
  ...
  if (nAddress < 0x10) {
    ...
    if (nAddress == 0x28) {
      ...
      if (cChannel < 3) {
      } else if ((cCaps & OPNA_HAS_EXTENDEDFM) && (cChannel >= 4) &&
                 (cChannel < 7)) {
        cChannel--;
      } else {
        return;
...
#if defined(SUPPORT_FMGEN)
  if (statflag_read(sfh, &opna->usefmgen, sizeof(opna->usefmgen)) ==
      STATFLAG_SUCCESS) {
    if (opna->usefmgen) {
      OEMCHAR path[MAX_PATH];
