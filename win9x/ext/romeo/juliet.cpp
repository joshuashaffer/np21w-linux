 ...
 */
CJuliet::CJuliet()
    : m_hModule(NULL), m_fnRead32(NULL), m_fnOut8(NULL), m_fnOut32(NULL),
      m_fnIn8(NULL), m_ulAddress(0), m_ucIrq(0), m_nQueIndex(0), m_nQueCount(0),
      m_pChip288(NULL) {}
...
bool CJuliet::Initialize() {
  ...
  for (size_t i = 0; i < _countof(s_dllProc); i++) {
    ...
    *(reinterpret_cast<FARPROC *>(reinterpret_cast<INT_PTR>(this) +
                                  s_dllProc[i].nOffset)) = proc;
  }
  ...
  for (UINT bus = 0; bus < 0x100; bus++) {
    for (UINT dev = 0; dev < 0x20; dev++) {
      for (UINT func = 0; func < 0x08; func++) {
        ...
        if ((ulDeviceVendor == DEVVEND(ROMEO_VENDORID, ROMEO_DEVICEID)) ||
            (ulDeviceVendor == DEVVEND(ROMEO_VENDORID, ROMEO_DEVICEID2))) {
          return ulPciAddress;
        }
      }
    }
 ...
 * @return インスタンス
 */
IExternalChip *CJuliet::GetInterface(IExternalChip::ChipType nChipType,
                                     UINT nClock) {
...
void CJuliet::Write288(UINT nAddr, UINT8 cData) {
  ...
  m_que[(m_nQueIndex + m_nQueCount) % _countof(m_que)] =
      ((nAddr & 0x1ff) << 8) | cData;
  m_nQueCount++;
...
bool CJuliet::Task() {
  ...
  if (m_nQueCount == 0) {
    ...
    while (m_nQueCount) {
      ...
      (*m_fnOut8)(m_ulAddress + ((nData & 0x10000) ? ROMEO_YMF288ADDR2
                                                   : ROMEO_YMF288ADDR1),
                  static_cast<UINT8>(nData >> 8));

      while (((*m_fnIn8)(m_ulAddress + ROMEO_YMF288ADDR1) & 0x80) != 0) {
        ::Sleep(0);
      }
      (*m_fnOut8)(m_ulAddress + ((nData & 0x10000) ? ROMEO_YMF288DATA2
                                                   : ROMEO_YMF288DATA1),
                  static_cast<UINT8>(nData));
      m_pciGuard.Leave();
 ...
 */
CJuliet::Chip288::Chip288(CJuliet *pJuliet) : m_pJuliet(pJuliet) {}

/**
 * デストラクタ
 */
CJuliet::Chip288::~Chip288() { m_pJuliet->Detach(this); }
 ...
 * @return リザルト
 */
INTPTR CJuliet::Chip288::Message(UINT nMessage, INTPTR nParameter) { return 0; }
