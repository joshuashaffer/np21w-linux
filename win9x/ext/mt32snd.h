...
private:
  ...
  // 定義
  typedef int (*FnOpen)(int rate, int reverb, int def, int revtype, int revtime,
                        int revlvl);                    /*!< オープン */
 ...
 * @return インスタンス
 */
inline MT32Sound *MT32Sound::GetInstance() { return &sm_instance; }
 ...
 * @retval false 無効
 */
inline bool MT32Sound::IsEnabled() const { return (m_hModule != NULL); }

/**
 * サンプリング レートを設定
 * @param[in] nRate レート
 */
inline void MT32Sound::SetRate(UINT nRate) { m_nRate = nRate; }
