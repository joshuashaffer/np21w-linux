 ...
 * コンストラクタ
 */
C86CtlIf::C86CtlIf() : m_hModule(NULL), m_pChipBase(NULL) {}

/**
 * デストラクタ
 */
C86CtlIf::~C86CtlIf() { Deinitialize(); }
...
bool C86CtlIf::Initialize() {
  ...
  do {
    ...
    FnCreateInstance CreateInstance = reinterpret_cast<FnCreateInstance>(
        ::GetProcAddress(m_hModule, "CreateInstance"));
    if (CreateInstance == NULL) {
      break;
    }

    /* インスタンス作成 */
    (*CreateInstance)(IID_IRealChipBase,
                      reinterpret_cast<LPVOID *>(&m_pChipBase));
    if (m_pChipBase == NULL) {
      break;
    }
 ...
 * @return インスタンス
 */
IExternalChip *C86CtlIf::GetInterface(IExternalChip::ChipType nChipType,
                                      UINT nClock) {
  ...
  do {
    ...
    for (int i = 0; i < nDeviceCount; i++) {
      ...
      m_pChipBase->getChipInterface(i, IID_IRealChip,
                                    reinterpret_cast<LPVOID *>(&pRealChip));
      if (pRealChip == NULL) {
        continue;
      }
      ...
      /* G.I.M.I.C 判定 */
      IGimic *pGimic = NULL;
      m_pChipBase->getChipInterface(i, IID_IGimic,
                                    reinterpret_cast<LPVOID *>(&pGimic));
      if (pGimic) {
        Devinfo info;
        if (pGimic->getModuleInfo(&info) == C86CTL_ERR_NONE) {
          ...
          if (nChipType == nRealChipType) {
            /* サウンドチップ取得できた */
            Chip *pChip =
                new Chip(this, pRealChip, pGimic, nRealChipType, nClock);
            m_chips[i] = pChip;
            return pChip;
          }
        }
      ...
      m_pChipBase->getChipInterface(i, IID_IRealChip3,
                                    reinterpret_cast<LPVOID *>(&pChip3));
      if (pChip3 != NULL) {
 ...
 * @param[in] nClock クロック
 */
C86CtlIf::Chip::Chip(C86CtlIf *pC86CtlIf, c86ctl::IRealChip *pRealChip,
                     c86ctl::IGimic *pGimic, ChipType nChipType, UINT nClock)
    : m_pC86CtlIf(pC86CtlIf), m_pRealChip(pRealChip), m_pGimic(pGimic),
      m_nChipType(nChipType), m_nClock(nClock) {}

/**
 * デストラクタ
 */
C86CtlIf::Chip::~Chip() { m_pC86CtlIf->Detach(this); }

/**
 * Get chip type
 * @return The type of the chip
 */
IExternalChip::ChipType C86CtlIf::Chip::GetChipType() { return m_nChipType; }
 ...
 * @return リザルト
 */
INTPTR C86CtlIf::Chip::Message(UINT nMessage, INTPTR nParameter) { return 0; }
