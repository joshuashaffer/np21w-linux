...
static void inirdkb(LPCTSTR lpString, const PFTBL *ini) {
  if ((!milstr_extendcmp(lpString, TEXT("PC98"))) ||
      (!milstr_cmp(lpString, TEXT("98")))) {
    *(static_cast<UINT8 *>(ini->value)) = KEY_PC98;
  } else if ((!milstr_extendcmp(lpString, TEXT("DOS"))) ||
             (!milstr_cmp(lpString, TEXT("PCAT"))) ||
             (!milstr_cmp(lpString, TEXT("AT")))) {
    *(static_cast<UINT8 *>(ini->value)) = KEY_KEY106;
  } else if ((!milstr_extendcmp(lpString, TEXT("KEY101"))) ||
             (!milstr_cmp(lpString, TEXT("101")))) {
...
static BOOL bitmapget(const void *lpBuffer, UINT nPos) {
  const int nIndex = (nPos >> 3);
  const UINT8 cBit = 1 << (nPos & 7);
  return (((static_cast<const UINT8 *>(lpBuffer))[nIndex]) & cBit) ? TRUE
                                                                   : FALSE;
}
 ...
 * @param[in] cbBin バイナリのサイズ
 */
static void binget(LPTSTR lpString, int cchString, const void *lpBin,
                   UINT cbBin) {
 ...
 */
void ini_read(LPCTSTR lpPath, LPCTSTR lpTitle, const PFTBL *lpTable,
              UINT nCount) {
  ...
  while (p < pTerminate) {
    ...
    case PFTYPE_STR:
      GetPrivateProfileString(lpTitle, p->item, static_cast<LPCTSTR>(p->value),
                              static_cast<LPTSTR>(p->value), p->arg, lpPath);
      break;
    ...
    case PFTYPE_BOOL:
      ...
      *(static_cast<UINT8 *>(p->value)) =
          (!milstr_cmp(szWork, str_true)) ? 1 : 0;
      break;

    case PFTYPE_BITMAP:
      GetPrivateProfileString(lpTitle, p->item,
                              (bitmapget(p->value, p->arg)) ? str_true
                                                            : str_false,
    ...
    case PFTYPE_BIN:
      GetPrivateProfileString(lpTitle, p->item, str_null, szWork,
                              _countof(szWork), lpPath);
    ...
    case PFTYPE_UINT8:
      val = GetPrivateProfileInt(
          lpTitle, p->item, *(static_cast<const UINT8 *>(p->value)), lpPath);
    ...
    case PFTYPE_UINT16:
      val = GetPrivateProfileInt(
          lpTitle, p->item, *(static_cast<const UINT16 *>(p->value)), lpPath);
    ...
    case PFTYPE_UINT32:
      val = GetPrivateProfileInt(
          lpTitle, p->item, *(static_cast<const UINT32 *>(p->value)), lpPath);
      *(static_cast<UINT32 *>(p->value)) = static_cast<UINT32>(val);
      break;

    case PFTYPE_HEX8:
      wsprintf(szWork, str_x, *(static_cast<const UINT8 *>(p->value)));
      GetPrivateProfileString(lpTitle, p->item, szWork, szWork,
                              _countof(szWork), lpPath);
      *(static_cast<UINT8 *>(p->value)) =
          static_cast<UINT8>(milstr_solveHEX(szWork));
      break;
    ...
    case PFTYPE_HEX16:
      ...
      GetPrivateProfileString(lpTitle, p->item, szWork, szWork,
                              _countof(szWork), lpPath);
      *(static_cast<UINT16 *>(p->value)) =
          static_cast<UINT16>(milstr_solveHEX(szWork));
      break;
    ...
    case PFTYPE_HEX32:
      ...
      GetPrivateProfileString(lpTitle, p->item, szWork, szWork,
                              _countof(szWork), lpPath);
      *(static_cast<UINT32 *>(p->value)) =
          static_cast<UINT32>(milstr_solveHEX(szWork));
      break;
 ...
 * @param[in] nCount 設定テーブル アイテム数
 */
void ini_write(LPCTSTR lpPath, LPCTSTR lpTitle, const PFTBL *lpTable,
               UINT nCount) {
  const PFTBL *p = lpTable;
  const PFTBL *pTerminate = p + nCount;
  while (p < pTerminate) {
...
static const PFTBL s_IniItems[] = {
    ...
    PFVAL("AIDEBIOS", PFTYPE_BOOL,
          &np2cfg.autoidebios), // 実機IDE BIOS使用を自動設定する
    ...
    //	PFEXT("opt14BRD", PFTYPE_BIN,		np2cfg.snd14opt,
    //3),
    ...
    PFMAX(
        "MEMCHKMX", PFTYPE_UINT8, &np2cfg.memchkmx,
        0), // メモリチェックする最大サイズ（最小は15MB・0は制限無し・メモリチェックが長いのが嫌だけど見かけ上カウントだけはしておきたい人向け）
    PFMAX("SBEEPLEN", PFTYPE_UINT8, &np2cfg.sbeeplen,
          0), // ピポ音の長さ（0でデフォルト・4がNP2標準）
    PFVAL("SBEEPADJ", PFTYPE_BOOL, &np2cfg.sbeepadj), // ピポ音の長さ自動調整

    PFVAL("BIOSIOEM", PFTYPE_BOOL,
          &np2cfg.biosioemu), // np21w ver0.86 rev46 BIOS I/O emulation
    ...
    PFVAL("USERAM_D", PFTYPE_BOOL,
          &np2cfg.useram_d), // EPSONでなくてもD0000h-DFFFFhをRAMに（ただしIDE
                             // BIOS D8000h-DBFFFhは駄目）
    PFVAL("USEPEGCP", PFTYPE_BOOL,
          &np2cfg.usepegcplane), // PEGC プレーンモードサポート

    PFVAL("USECDECC", PFTYPE_BOOL,
          &np2cfg.usecdecc), // CD-ROM EDC/ECC エミュレーションサポート
    PFVAL(
        "CDDTSKIP", PFTYPE_BOOL,
        &np2cfg.cddtskip), // CD-ROM オーディオ再生時にデータトラックをスキップ

#if defined(SUPPORT_ASYNC_CPU)
    PFVAL("ASYNCCPU", PFTYPE_BOOL, &np2cfg.asynccpu), // 非同期CPUモード有効
#endif
    PFVAL("CONSTTSC", PFTYPE_BOOL,
          &np2cfg.consttsc), // RDTSCをクロック変更によらず一定間隔にする
#if defined(SUPPORT_IDEIO)
    PFVAL("IDEBADDR", PFRO_HEX8,
          &np2cfg.idebaddr), // IDE BIOS アドレス（デフォルト：D8h(D8000h)）
#endif
    ...
    PFVAL("fsrescfg", PFTYPE_BOOL,
          &np2oscfg.fsrescfg), // 解像度毎に設定保存する

#if defined(SUPPORT_SCRN_DIRECT3D)
    PFVAL("D3D_IMODE", PFTYPE_UINT8,
          &np2oscfg.d3d_imode), // Direct3D 拡大縮小補間モード
    PFVAL("D3D_EXCLU", PFTYPE_BOOL,
          &np2oscfg.d3d_exclusive), // Direct3D 排他モード使用
#endif
    ...
    PFVAL(
        "EMUDDRAW", PFTYPE_BOOL,
        &np2oscfg
             .emuddraw), // 最近はEMULATIONONLYにした方速かったりする（特にピクセル操作する場合とか）
    PFVAL("DRAWTYPE", PFTYPE_UINT8,
          &np2oscfg.drawtype), // 画面レンダラ (0: DirectDraw, 1:
                               // reserved(DirecrDraw), 2: Direct3D)

    PFVAL("DRAGDROP", PFRO_BOOL,
          &np2oscfg.dragdrop), // ドラッグアンドドロップサポート
    PFVAL("MAKELHDD", PFRO_BOOL,
          &np2oscfg.makelhdd), // 巨大HDDイメージ作成サポート
    PFVAL("SYSKHOOK", PFTYPE_BOOL,
          &np2oscfg.syskhook), // システムキーフックサポート
    PFVAL("RAWMOUSE", PFTYPE_BOOL,
          &np2oscfg.rawmouse), // 直接マウスデータ読み取り
    PFVAL("MOUSEMUL", PFTYPE_SINT16,
          &np2oscfg.mousemul), // マウススピード倍率（分子）
    PFVAL("MOUSEDIV", PFTYPE_SINT16,
          &np2oscfg.mousediv), // マウススピード倍率（分母）

    PFVAL("SCRNMODE", PFTYPE_UINT8, &np2oscfg.scrnmode), // フルスクリーン設定
    PFVAL("SAVESCRN", PFTYPE_BOOL,
          &np2oscfg.savescrn), // フルスクリーン設定を保存・復元する

    PFVAL("SVSCRMUL", PFTYPE_BOOL,
          &np2oscfg.svscrmul), // 画面表示倍率を保存するか
    PFVAL("SCRN_MUL", PFTYPE_UINT8,
          &np2oscfg.scrn_mul), // 画面表示倍率（8が等倍）

    PFVAL("MOUSE_NC", PFTYPE_BOOL,
          &np2oscfg.mouse_nc), // マウスキャプチャ無しコントロール
    PFVAL("CPUSTABF", PFTYPE_UINT16,
          &np2oscfg.cpustabf), // クロック安定器適用限界時間（フレーム）
    PFVAL("READONLY", PFRO_BOOL,
          &np2oscfg.readonly), // 変更を設定ファイルに書き込まない
    PFVAL("TICKMODE", PFRO_UINT8,
          &np2oscfg.tickmode), // Tickカウンタのモードを強制的に設定する
    PFVAL("USEWHEEL", PFRO_BOOL,
          &np2oscfg
               .usewheel), // マウスホイールによる音量・マウス速度設定を使用する
    PFVAL("USE_MVOL", PFRO_BOOL,
          &np2oscfg.usemastervolume), // マスタボリューム設定を使用する

    PFVAL("TWNDHIST", PFRO_UINT8,
          &np2oscfg.toolwndhistory), // ツールウィンドウのFDファイル履歴の記憶数

#if defined(SUPPORT_WACOM_TABLET)
    PFVAL("PENTABFA", PFTYPE_BOOL,
          &np2oscfg.pentabfa), // ペンタブレット アスペクト比固定モード
#endif
};
