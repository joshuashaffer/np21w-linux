...
void sysmng_updatecaption(UINT8 flag) {
      ...
      if (sxsi_getdevtype(i) == SXSIDEV_CDROM) {
        ...
        if (sxsi_getdevtype(i) == SXSIDEV_CDROM) {
          if (*(np2cfg.idecd[i])) {
            ...
            milstr_ncat(title, sysmng_file_getname(np2cfg.idecd[i]),
                        NELEMENTS(title));
          } else if (i == cdchange_drv &&
      ...
      if (g_hWndMain) {
        ...
        if (hMenu) {
          ...
          if (!hddimgmenustrorg[i][0]) {
            GetMenuString(hMenuTgt, IDM_IDE0STATE + i, hddimgmenustrorg[i],
                          NELEMENTS(hddimgmenustrorg[0]), MF_BYCOMMAND);
          }
          if (np2cfg.idetype[i] == SXSIDEV_NC) {
            ...
            if (fname && *fname && fnamenext && *fnamenext &&
                (fnametmp = sysmng_file_getname(fname)) != NULL &&
                (fnamenexttmp = sysmng_file_getname(fnamenext)) != NULL) {
              _tcscpy(newtext, hddimgmenustrorg[i]);
            ...
            } else if (fnamenext && *fnamenext &&
                       (fnamenexttmp = sysmng_file_getname(fnamenext)) !=
                           NULL) {
              _tcscpy(newtext, hddimgmenustrorg[i]);
      ...
      if (g_hWndMain) {
        ...
        if (hMenu) {
          ...
          if (!scsiimgmenustrorg[i][0]) {
            GetMenuString(hMenuTgt, IDM_SCSI0STATE + i, scsiimgmenustrorg[i],
                          NELEMENTS(scsiimgmenustrorg[0]), MF_BYCOMMAND);
          }
  ...
  if (flag & 2) {
    ...
    if (np2oscfg.DISPCLK & 2) {
      if (workclock.fps) {
        OEMSPRINTF(clock, OEMTEXT(" - %u.%1uFPS"), workclock.fps / 10,
                   workclock.fps % 10);
      } else {
    ...
    if (np2oscfg.DISPCLK & 1) {
      OEMSPRINTF(work, OEMTEXT(" %2u.%03uMHz"), workclock.khz / 1000,
                 workclock.khz % 1000);
      if (clock[0] == '\0') {
  ...
  if (flag & 4) {
    ...
    if (sys_miscinfo.showvolume && sys_miscinfo.showmousespeed) {
      OEMSPRINTF(misc, OEMTEXT(" (Volume: %d%%, Mouse speed: %d%%)"),
                 np2cfg.vol_master,
                 100 * np2oscfg.mousemul / np2oscfg.mousediv);
    } else if (sys_miscinfo.showvolume) {
      OEMSPRINTF(misc, OEMTEXT(" (Volume: %d%%)"), np2cfg.vol_master);
    } else if (sys_miscinfo.showmousespeed) {
      OEMSPRINTF(misc, OEMTEXT(" (Mouse speed: %d%%)"),
                 100 * np2oscfg.mousemul / np2oscfg.mousediv);
    }
  }
