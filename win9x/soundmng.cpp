...
void CSoundMng::Initialize() {
  ...
  CSoundDeviceDSound3::s_mastervol_available =
      np2oscfg.usemastervolume ? true : false;
 ...
 * コンストラクタ
 */
CSoundMng::CSoundMng() : m_pSoundDevice(NULL), m_nMute(0) { SetReverse(false); }
 ...
 * ストリーム破棄
 */
void soundmng_destroy(void) { CSoundMng::GetInstance()->DestroyStream(); }

/**
 * ストリーム リセット
 */
void soundmng_reset(void) { CSoundMng::GetInstance()->ResetStream(); }

/**
 * ストリーム開始
 */
void soundmng_play(void) { CSoundMng::GetInstance()->PlayStream(); }

/**
 * ストリーム停止
 */
void soundmng_stop(void) { CSoundMng::GetInstance()->StopStream(); }
