 ...
 * コンストラクタ
 */
Np2Arg::Np2Arg() { ZeroMemory(this, sizeof(*this)); }
  ...
  for (int i = ARG_BASE; i < argc; i++) {
    ...
    if ((lpArg[0] == TEXT('/')) || (lpArg[0] == TEXT('-'))) {
      ...
      if (::file_cmpname(lpExt, TEXT("ini")) == 0 ||
          ::file_cmpname(lpExt, TEXT("npc")) == 0 ||
          ::file_cmpname(lpExt, TEXT("npcfg")) == 0 ||
          ::file_cmpname(lpExt, TEXT("np2cfg")) == 0 ||
          ::file_cmpname(lpExt, TEXT("np21cfg")) == 0 ||
          ::file_cmpname(lpExt, TEXT("np21wcfg")) == 0) {
        lpIniFile = lpArg;
  ...
  if (lpIniFile) { // np21w ver0.86 rev8
    ...
    if (!(_tcsstr(lpIniFile, _T(":")) != NULL || (lpIniFile[0] == '\\'))) {
      ...
      if (strbuf[_tcslen(strbuf) - 1] != '\\') {
        _tcscat(
            strbuf,
            _T("\\")); // XXX: Linuxとかだったらスラッシュじゃないと駄目だよね？
                       // -> Win専用だから問題ない
      }
    }
 ...
 * ディスク情報をクリア
 */
void Np2Arg::ClearDisk() { ZeroMemory(m_lpDisk, sizeof(m_lpDisk)); }
