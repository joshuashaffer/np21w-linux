...
int WINAPI FindActCtxSectionStringA_stub(int flg1, int flg2, int flg3,
                                         char *pszFilename, void *data) {
...
int WINAPI FindActCtxSectionStringW_stub(int flg1, int flg2, int flg3,
                                         wchar_t *pszFilename, void *data) {
...
BOOL WINAPI InitializeCriticalSectionAndSpinCount_stub(HANDLE flg1,
                                                       DWORD flg2) {
  ICSAC PICSAC = NULL;
  HINSTANCE hDll;
  hDll = GetModuleHandleA(strKERNEL32DLL);
  if (hDll) {
    PICSAC =
        (ICSAC)GetProcAddress(hDll, "InitializeCriticalSectionAndSpinCount");
  }
...
BOOL WINAPI HeapSetInformation_stub(HANDLE flg1, int flg2, void *flg3,
                                    DWORD flg4) {
  HSIS PHSIS;
  HINSTANCE hDll;
  hDll = GetModuleHandleA(strKERNEL32DLL);
...
typedef BOOL(WINAPI *HQIS)(HANDLE flg1, int flg2, void *flg3, DWORD flg4,
                           DWORD *flg5);
BOOL WINAPI HeapQueryInformation_stub(HANDLE flg1, int flg2, void *flg3,
                                      DWORD flg4, DWORD *flg5) {
  HQIS PHQIS;
  HINSTANCE hDll;
  hDll = GetModuleHandleA(strKERNEL32DLL);
  if (hDll) {
...
int _declspec(naked) WINAPI
    _imp__InitializeCriticalSectionAndSpinCount(void *flg1, DWORD flg2) {
  _asm {
		_emit 0x90
		_emit 0x90
		_emit 0xc3
...
int _declspec(naked) WINAPI
    _imp__HeapSetInformation(HANDLE flg1, int flg2, void *flg3, DWORD flg4) {
  _asm {
		_emit 0x33
		_emit 0xc0
		_emit 0xc3
...
int _declspec(naked) WINAPI
    _imp__HeapQueryInformation(HANDLE flg1, int flg2, void *flg3, DWORD flg4,
                               DWORD *flg5) {
  _asm {
		_emit 0x33
		_emit 0xc0
...
#if _MSC_VER >= 1600 // VC2010
int _declspec(naked) WINAPI
    _imp__FindActCtxSectionStringA(int flg1, int flg2, int flg3,
                                   wchar_t *pszFilename, void *data) {
  _asm {
		_emit 0x33
		_emit 0xc0
...
int _declspec(naked) WINAPI
    _imp__FindActCtxSectionStringW(int flg1, int flg2, int flg3,
                                   wchar_t *pszFilename, void *data) {
  _asm {
		_emit 0xc3
		_emit 0x33
...
void initialize_findacx() {
  ...
  *(FARPROC *)i = (FARPROC)&IsProcessorFeaturePresent_stub;
  ...
  VirtualProtect(i, sizeof(FARPROC), PAGE_EXECUTE_READWRITE, &dw);
  *(FARPROC *)i = (FARPROC)&FindActCtxSectionStringW_stub;
  VirtualProtect(i, sizeof(FARPROC), dw, &dw);
