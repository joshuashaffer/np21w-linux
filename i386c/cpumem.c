...
static REG8 MEMCALL memmain_rd8(UINT32 address) { return (mem[address]); }
...
static const MMAPTBL mmaptbl[2] = {
    {memmain_rd8, memf800_rd8, memnc_wr8, memmain_rd16, memf800_rd16,
     memnc_wr16, memmain_rd32, memf800_rd32, memnc_wr32},
    {memf800_rd8, memf800_rd8, memepson_wr8, memf800_rd16, memf800_rd16,
     memepson_wr16, memf800_rd32, memf800_rd32, memepson_wr32}};
...
static const MEMFNF memfnf = {
    {memvgaf_rd8, memvgaf_rd8, memvgaf_rd8, memvgaf_rd8, memnc_rd8, memsys_rd8,
     memsys_rd8, memsys_rd8},
    {memvgaf_wr8, memvgaf_wr8, memvgaf_wr8, memvgaf_wr8, memnc_wr8, memsys_wr8,
     memsys_wr8, memsys_wr8},

    {memvgaf_rd16, memvgaf_rd16, memvgaf_rd16, memvgaf_rd16, memnc_rd16,
     memsys_rd16, memsys_rd16, memsys_rd16},
    {memvgaf_wr16, memvgaf_wr16, memvgaf_wr16, memvgaf_wr16, memnc_wr16,
     memsys_wr16, memsys_wr16, memsys_wr16},

    {memvgaf_rd32, memvgaf_rd32, memvgaf_rd32, memvgaf_rd32, memnc_rd32,
     memsys_rd32, memsys_rd32, memsys_rd32},
    {memvgaf_wr32, memvgaf_wr32, memvgaf_wr32, memvgaf_wr32, memnc_wr32,
     memsys_wr32, memsys_wr32, memsys_wr32}};
#else
static const MEMFNF memfnf = {
    {memnc_rd8, memnc_rd8, memnc_rd8, memnc_rd8, memnc_rd8, memsys_rd8,
     memsys_rd8, memsys_rd8},
    {memnc_wr8, memnc_wr8, memnc_wr8, memnc_wr8, memnc_wr8, memsys_wr8,
     memsys_wr8, memsys_wr8},

    {memnc_rd16, memnc_rd16, memnc_rd16, memnc_rd16, memnc_rd16, memsys_rd16,
     memsys_rd16, memsys_rd16},
    {memnc_wr16, memnc_wr16, memnc_wr16, memnc_wr16, memnc_wr16, memsys_wr16,
     memsys_wr16, memsys_wr16},
  ...
  if (address < I286_MEMREADMAX) {
    ...
    if (np2clvga.enabled && cirrusvga_opaque && (cirrusvga_wab_46e8 & 0x08)) {
      ...
      if (np2clvga.pciLFB_Addr &&
          (address & np2clvga.pciLFB_Mask) == np2clvga.pciLFB_Addr) {
      ...
      } else if (np2clvga.pciMMIO_Addr &&
                 (address & np2clvga.pciMMIO_Mask) == np2clvga.pciMMIO_Addr) {
        if (np2clvga.gd54xxtype == CIRRUS_98ID_PCI ||
            ((np2clvga.pciMMIO_Addr & 0xfff00000) != 0xf00000 ||
             !(gdc.analog & ((1 << GDCANALOG_256) | (1 << GDCANALOG_256E)))))
      ...
      if (np2clvga.gd54xxtype != CIRRUS_98ID_PCI) {
          ...
          if (vramWndAddr <= address) {
            if (address < vramWndAddr + VRAMWINDOW_SIZE) {
              return cirrus_linear_readb(cirrusvga_opaque, address);
                       ...
                       vramWndAddr + VRAMWINDOW_SIZE + EXT_WINDOW_SIZE) {
        ...
        if (vramWndAddr3) {
          ...
          if (vramWndAddr3 <= addr3 &&
              addr3 < vramWndAddr3 + VRA3WINDOW_SIZEX &&
              !(gdc.analog & ((1 << GDCANALOG_256) | (1 << GDCANALOG_256E)))) {
            return CIRRUS_VRAMWND3_FUNC_rb(cirrusvga_opaque, addr3);
          }
        }
        if (vramWndAddr2 &&
            (vramWndAddr2 != 0xE0000 ||
             !(gdc.analog & ((1 << GDCANALOG_16) | (1 << GDCANALOG_256) |
                             (1 << GDCANALOG_256E))))) {
...
REG16 MEMCALL memp_read16(UINT32 address) {
  ...
  if (0x400 <= address && address < 0x600) {
    ...
    default:
      TRACEOUT(
          ("BDA (read16): %x ret %x", address, *((UINT16 *)(mem + address))));
    }
  }
#endif
#ifdef MEM_D8_TRACEOUT
  if (0xD8000 <= address && address < 0xDC000) {
    TRACEOUT(
        ("D8000h (read16): %x ret %x", address, *((UINT16 *)(mem + address))));
  }
#endif
  // if (address == 0xf0000) {
  //	printf("SYS (read16): %x ret %x", address, *((UINT16*)(mem+address)));
  //}
  // if (0x400 <= address && address <= 0x5ff && address != 0x58a && address !=
  // 0x58b) { 	printf("BDA (read16): %x ret %x", address,
  //*((UINT16*)(mem+address)));
  //}
        ...
        if (np2clvga.gd54xxtype != CIRRUS_98ID_PCI) {
          ...
          if (vramWndAddr3) {
            ...
            if (vramWndAddr3 <= addr3 &&
                addr3 < vramWndAddr3 + VRA3WINDOW_SIZEX &&
                !(gdc.analog &
                  ((1 << GDCANALOG_256) | (1 << GDCANALOG_256E)))) {
...
UINT32 MEMCALL memp_read32(UINT32 address) {
  ...
  if (0x400 <= address && address < 0x600) {
    ...
    default:
      TRACEOUT(
          ("BDA (read32): %x ret %x", address, *((UINT32 *)(mem + address))));
    }
  }
#endif
#ifdef MEM_D8_TRACEOUT
  if (0xD8000 <= address && address < 0xDC000) {
    TRACEOUT(
        ("D8000h (read32): %x ret %x", address, *((UINT32 *)(mem + address))));
  }
#endif
  // if (0x400 <= address && address <= 0x5ff) {
      ...
      if (np2clvga.gd54xxtype != CIRRUS_98ID_PCI) {
        ...
        if (vramWndAddr3) {
          ...
          if (vramWndAddr3 <= addr3 &&
            ...
            // if(!(gdc.analog & ((1 << GDCANALOG_256) | (1 <<
            // GDCANALOG_256E)))) 	return;
          }
        }
        if (vramWndAddr2 &&
            (vramWndAddr2 != 0xE0000 ||
...
void MEMCALL memp_write16(UINT32 address, REG16 value) {
  ...
  if (0x400 <= address && address < 0x600) {
    ...
    case 0x4fd:
      break;
    default:
      TRACEOUT(("BDA (write16): %x val %x -> %x", address,
                *((UINT16 *)(mem + address)), value));
    }
  }
#endif
            ...
            if (vramWndAddr <= address) {
              if (address < vramWndAddr + VRAMWINDOW_SIZE) {
                ...
                return;
                         ...
                         vramWndAddr + VRAMWINDOW_SIZE + EXT_WINDOW_SIZE) {
                if (address >=
                    vramWndAddr + VRAMWINDOW_SIZE + EXT_WINDOW_SHFT) {
                  g_cirrus_linear_write[1](cirrusvga_opaque, address, value);
                  return;
                }
              }
...
void MEMCALL memp_write32(UINT32 address, UINT32 value) {
  ...
  if (0x400 <= address && address < 0x600) {
    ...
    case 0x4fd:
      break;
    default:
      TRACEOUT(("BDA (write32): %x val %x -> %x", address,
                *((UINT32 *)(mem + address)), value));
    }
  }
#endif
