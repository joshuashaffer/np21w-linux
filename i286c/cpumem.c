...
static const MMAPTBL mmaptbl[2] = {{memmain_rd8, memf800_rd8, memnc_wr8,
...
void MEMCALL memr_reads(UINT seg, UINT off, void *dat, UINT leng) {
  ...
  adrs = seg << 4;
  off = LOW16(off);
  if ((I286_MEMREADMAX >= 0x10000) && (adrs < (I286_MEMREADMAX - 0x10000))) {
    if (leng) {
      size = 0x10000 - off;
      if (size >= leng) {
...
void MEMCALL memr_writes(UINT seg, UINT off, const void *dat, UINT leng) {
  ...
  adrs = seg << 4;
  off = LOW16(off);
  if ((I286_MEMWRITEMAX >= 0x10000) && (adrs < (I286_MEMWRITEMAX - 0x10000))) {
    if (leng) {
      size = 0x10000 - off;
      if (size >= leng) {
